...
function DymoManager(a,b,c,d,e,h,k){function l(a,b,c,e){p=b;g=c[0];q=a.getMappings();for(var u in c[1])a=c[1][u],DYMO_STORE.isSubclassOf(a.getType(),UI_CONTROL)&&(n[u]=new UIControl(a,h)),DYMO_STORE.isSubclassOf(a.getType(),SENSOR_CONTROL)&&(m[u]=a);d||(d="bower_components/dymo-core/audio/impulse_rev.wav");f.init(d,b,e)}DYMO_STORE=new DymoStore(e);var f=new Scheduler(a,k);c&&(OPTIMIZED_MODE=!0);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var p,g,n={},m={},q={};this.loadDymoAndRenderingFromStore=function(a,
...
"audio/achachbach10/01-AchGottundHerr-clarinet.wav"):3==n[3]?r=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-saxphone.wav"):4==n[3]&&(r=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-bassoon.wav"));a.setDymoFeature(r,d,m);a.setDymoFeature(r,l,p);a.setDymoFeature(r,c,p);a.setDymoFeature(r,e,v);a.setDymoFeature(r,h,u);a.setDymoFeature(r,k,w);r.getParameter(ONSET).update(p)}DYMO_STORE.updatePartOrder(f,c.name)});$http.get("getsourcefilesindir/",{params:{directory:"audio/achachbach10/"}}).success(function(a){a=
...
b,a)})};this.toJsonGraph=function(a,b,c){var d={nodes:[],edges:[]},g={},f=this.findAllSubjects(TYPE,a),h=this.find(null,b,null);async.map(f,e,function(e,n){d.nodes=n;for(var m=0;m<f.length;m++)g[f[m]]=d.nodes[m];d.edges=[];for(m=0;m<h.length;m++)0==k.find(h[m].object,TYPE,a).length?0<k.find(h[m].object,FIRST).length&&(e=k.findObjectOrList(h[m].subject,b),e=e.map(function(a){return{source:g[h[m].subject],target:g[a],value:1}}),d.edges=d.edges.concat(e)):d.edges.push({source:g[h[m].subject],target:g[h[m].object],
...
FunctionTools.invertFunction=function(a){var b=FunctionTools.toReturnValueString(a);if(!FunctionTools.inverted[a]){var c=$jscomp.makeIterator(FunctionTools.invertReturnValue(b)),b=c.next().value,c=c.next().value;null!=c&&(FunctionTools.inverted[a]=FunctionTools.toJavaScriptFunction(b,c))}return FunctionTools.inverted[a]};
...
FunctionTools.getInvertedOperator=function(a){if("add"==a)return"subtract";if("subtract"==a)return"add";if("multiply"==a)return"divide";if("divide"==a)return"multiply";if("unaryMinus"==a)return"unaryMinus"};FunctionTools.getOp=function(a){if("add"==a)return"+";if("subtract"==a||"unaryMinus"==a)return"-";if("multiply"==a)return"*";if("divide"==a)return"/"};function LogicTools(){}LogicTools.created={};LogicTools.varCount=0;LogicTools.createConstraint=function(a){console.log(a);LogicTools.created[a]||(LogicTools.created[a]=LogicTools.createGoal(LogicTools.toEquation(a)));return LogicTools.created[a]};LogicTools.createGoal=function(a){console.log(a);var b;try{b=math.parse(a)}catch(c){c instanceof SyntaxError||console.log(c);return}return LogicTools.createGoalFunction(b)};
LogicTools.createGoalFunction=function(a){var b=[],c=[];if(a=LogicTools.recursiveCreateGoalString(a,b,c)){a=a.slice(0,-1);a="return logic.and("+a+");";for(var d="",e=0;e<c.length;e++)d+="var "+c[e]+" = logic.lvar('"+c[e]+"');\n";console.log(b,d+a);return FunctionTools.createFunction(b,d+a)}};
