...
new Float32Array(2048),q=e.extract(h,1024),n=0;q;){for(l=0;l<q;l++)for(var p=0;p<g.length;p++)g[p][l+n]=h[l*g.length+p%g.length];n+=q;q=e.extract(h,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var g=0;g<e.length;g++)a[f*e.length+g%e.length]=e[g][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
...
c=u(c,0,p(x+FADE_LENGTH,c));r(c,c.length);A.buffer=c});m[AMPLITUDE]=D.gain;m[TIME_STRETCH_RATIO]={value:0};m[REVERB]=G.gain;m[DELAY]=d.gain;m[FILTER]=e.frequency;m[PAN]={value:0};m[HEIGHT]={value:0};m[DISTANCE]={value:0};m[LOOP]={value:0};h(AMPLITUDE,a.getParameter(AMPLITUDE));h(TIME_STRETCH_RATIO,a.getParameter(TIME_STRETCH_RATIO));h(REVERB,a.getParameter(REVERB));h(DELAY,a.getParameter(DELAY));h(FILTER,a.getParameter(FILTER));h(PAN,a.getParameter(PAN));h(HEIGHT,a.getParameter(HEIGHT));h(DISTANCE,
...
0,d,c=$jscomp.makeIterator(k.values()),e=c.next();!e.done;e=c.next()){var e=e.value,f;f=e;var g=f.getDymo().getParameter(PLAYBACK_RATE).getValue();f=f.getDuration()/g;f>b&&(b=f,d=e)}return[a+b,d]}function r(){var a=b.getNextParts();if(a){console.log(a.map(function(a){return isNaN(a.getIndex())?"top":a.getIndex()}));for(var m=new Map,g=0;g<a.length;g++)if(a[g].getSourcePath()){var h=d[a[g].getSourcePath()];m.set(a[g],new Source(a[g],c,h,e,f,p))}return m}}var v=new Map,w,y,k=[],t,m;b||(b=new DymoNavigator(a,
...
a=k.findFirstObjectValue(a,HAS_BODY);return[b,a]}function q(b,c){var d=k.findFirstObjectUri(b,CDT),d=new DynamicMusicObject(b,d,a);d.setBasePath(t);c[b]=d;d.setSourcePath(k.findFirstObjectValue(b,HAS_SOURCE));for(var e=k.findAllObjectUris(b,HAS_FEATURE),f=0;f<e.length;f++)d.setFeature(e[f],k.findFirstObjectUri(e[f],VALUE));e=k.findAllObjectUris(b,HAS_PARAMETER);for(f=0;f<e.length;f++)r(d,k.findFirstObjectUri(e[f],TYPE),k.findFirstObjectValue(e[f],VALUE));e=k.findAllObjectUris(b,HAS_PART);for(f=0;f<
e.length;f++)d.addPart(q(e[f],c));return d}function n(a,b){for(var c=b[a],d=k.findAllObjectUris(a,HAS_SIMILAR),e=0;e<d.length;e++)c.addSimilar(b[d[e]]);d=k.findAllObjectUris(a,HAS_MAPPING);for(e=0;e<d.length;e++)c.addMapping(p(d[e],b,c));c=k.findAllObjectUris(a,HAS_PART);for(e=0;e<c.length;e++)n(c[e],b)}function p(b,c,d,e){var f=k.findAllObjectUris(b,TO_TARGET);if(0<f.length){var g=[],h=l(f[0]);if(h)f=Object.keys(c).map(function(a){return c[a]}),Array.prototype.push.apply(g,f.filter(h));else for(var q=
0;q<f.length;q++)k.findFirstObjectUri(f[q],TYPE)==DYMO?g.push(c[f[q]]):g.push(e[f[q]]);return u(b,c,d,g,e,h)}return u(b,c,d,[a],e)}function u(a,b,c,d,e,f){b=k.findFirstObjectUri(a,IS_RELATIVE);for(var g=[],q=k.findAllObjectUris(a,HAS_DOMAIN_DIMENSION),l=0;l<q.length;l++){var p=k.findFirstObjectUri(q[l],TYPE),n=k.findFirstObjectValue(q[l],NAME);n||(n=q[l]);p==FEATURE_TYPE||k.isSubclassOf(p,FEATURE_TYPE)?g.push(n):p==PARAMETER_TYPE||k.isSubclassOf(p,PARAMETER_TYPE)?(p=c?r(c,n,0):new Parameter(n,0),
g.push(p)):g.push(e[q[l]])}e=$jscomp.makeIterator(h(k.findFirstObjectUri(a,HAS_FUNCTION)));c=e.next().value;e=e.next().value;a=k.findFirstObjectUri(a,HAS_RANGE);return new Mapping(g,b,{args:c,body:e},d,a,f)}function r(a,b,c){var d=a.getParameter(b);d?d.update(c):(d=new Parameter(b,c),a.addParameter(d));return d}function v(a){return a==SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):new SequentialNavigator(void 0)}function w(a,b){var c=new XMLHttpRequest;c.open("GET",a,!0);c.onload=function(){b(this.responseText)};
...
b,d){b||(b={});c(a,function(){for(var a=b,c=k.findFirstSubjectUri(TYPE,RENDERING),e=new Rendering(a[k.findFirstObjectUri(c,HAS_DYMO)]),f=k.findAllObjectUris(c,HAS_MAPPING),m={},g=0;g<f.length;g++)for(var h=k.findAllObjectUris(f[g],HAS_DOMAIN_DIMENSION),q=0;q<h.length;q++){var n=k.findFirstObjectUri(h[q],TYPE),u=k.findFirstObjectValue(h[q],NAME);u||(u=h[q]);if(k.isSubclassOf(n,MOBILE_CONTROL)&&!m[h[q]]){var t;t=h[q];var r=n,n=void 0;r==ACCELEROMETER_X||r==ACCELEROMETER_Y||r==ACCELEROMETER_Z?n=new AccelerometerControl(r):
r==TILT_X||r==TILT_Y?n=new TiltControl(r):r==GEOLOCATION_LATITUDE||r==GEOLOCATION_LONGITUDE?n=new GeolocationControl(r):r==GEOLOCATION_DISTANCE?n=new DistanceControl:r==COMPASS_HEADING?n=new CompassControl:r==BEACON?(u=k.findFirstObjectValue(t,HAS_UUID),n=k.findFirstObjectValue(t,HAS_MAJOR),r=k.findFirstObjectValue(t,HAS_MINOR),n=new BeaconControl(u,n,r)):r==SLIDER||r==TOGGLE||r==BUTTON||r==CUSTOM_CONTROL?n=new Control(u,r):r==RANDOM?n=new RandomControl:r==BROWNIAN?(u=k.findFirstObjectValue(t,HAS_INITIAL_VALUE),
n=new BrownianControl(u)):r==RAMP&&(n=Math.round(1E3*k.findFirstObjectUri(t,HAS_DURATION)),u=k.findFirstObjectValue(t,HAS_INITIAL_VALUE),n=new RampControl(n,u));k.findFirstObjectUri(t,IS_SMOOTH)&&n.setSmooth&&n.setSmooth(!0);t=Number(k.findFirstObjectUri(t,IS_AVERAGE_OF));!isNaN(t)&&n.setAverageOf&&n.setAverageOf(t);t=n;u=Number(k.findFirstObjectUri(h[q],VALUE));isNaN(u)||t.update(u);m[h[q]]=t}}for(g=0;g<f.length;g++)e.addMapping(p(f[g],a,void 0,m));if(a=k.findFirstObjectUri(c,HAS_NAVIGATOR))c=k.findFirstObjectUri(a,
...
b.getParts()[c],d)}function l(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var f=b.getSimilars()[e];c[f.getUri()]||(c[f.getUri()]=f.toFlatJson());a.links.push(h(d,c[f.getUri()]))}for(e=0;e<b.getParts().length;e++)l(a,b.getParts()[e],c)}function h(a,b){return{source:a,target:b,value:1}}var q=this,n=null,p=[],u=[],r={},v={},w=[],y=[],k="",t;v[PLAY]=new Parameter(PLAY,0,!0);v[LOOP]=new Parameter(LOOP,0,!0);
