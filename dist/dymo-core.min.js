...
RANDOM="Random",BROWNIAN="Brownian",RAMP="Ramp",CONTROLS=[SLIDER,TOGGLE,ACCELEROMETER_X,ACCELEROMETER_Y,ACCELEROMETER_Z,TILT_X,TILT_Y,GEOLOCATION_LATITUDE,GEOLOCATION_LONGITUDE,GEOLOCATION_DISTANCE,COMPASS_HEADING,RANDOM,BROWNIAN,RAMP],UI_CONTROLS=[SLIDER,TOGGLE,BUTTON];function DymoManager(a,b,c){var d=new Scheduler(a,function(){});c||(c="bower_components/dymo-core/audio/impulse_rev.wav");d.setReverbFile(c);SCHEDULE_AHEAD_TIME=b;var e,f={};this.loadDymoAndRendering=function(a,b,c){var l=new DymoLoader(d);l.loadDymoFromJson(a,function(a){l.loadRenderingFromJson(b,a[1],function(b){e=b[0];e.dymo=a[0];for(var g in b[1]){var h=b[1][g];0<=UI_CONTROLS.indexOf(h.getType())&&(f[g]=new UIControl(h))}d.loadBuffers(e.dymo);c&&c()})})};this.loadDymoFromJson=function(a,b){(new DymoLoader(d)).loadDymoFromJson(a,
function(a){b&&b(a[0])})};this.parseDymoFromJson=function(a,b){(new DymoLoader(d)).parseDymoFromJson(a,function(a){b(a[0])})};this.replacePartOfTopDymo=function(a,b){var c=e.dymo.getPart(a);e.dymo.replacePart(a,b);d.stop(c)};this.syncNavigators=function(a,b,c){};this.startPlaying=function(){d.play(e.dymo)};this.stopPlaying=function(){d.stop(e.dymo)};this.getTopDymo=function(){return e.dymo};this.getUIControl=function(a){return f[a]}};function AudioProcessor(a){function b(a){return{extract:function(b,e,f){for(var g=[],h=0;h<a.numberOfChannels;h++)g.push(a.getChannelData(h));for(h=0;h<e;h++)for(var k=0;k<g.length;k++)b[h*g.length+k%g.length]=g[k][h+f];return Math.min(e,g[0].length-f)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var f=b(c),e=new SimpleFilter(f,e),f=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),g=[],h=0;h<c.numberOfChannels;h++)g.push(f.getChannelData(h));for(var k=
new Float32Array(2048),l=e.extract(k,1024),n=0;l;){for(h=0;h<l;h++)for(var p=0;p<g.length;p++)g[p][h+n]=k[h*g.length+p%g.length];n+=l;l=e.extract(k,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var g=0;g<e.length;g++)a[f*e.length+g%e.length]=e[g][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),h=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(h,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=h[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(){0<p&&p--;0==p&&b&&b(p)}function e(a){k.splice(k.indexOf(a),1)}function f(){for(var a=[],b=0;b<k.length;b++)for(var d=$jscomp.makeIterator(k[b].getSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();l=a;c&&c()}function g(b,c){console.log(b);p++;var e=new XMLHttpRequest;e.open("GET",b,!0);e.responseType="arraybuffer";e.onload=function(){a.decodeAudioData(e.response,function(a){c?
c(a):(h[b]=a,d())})};e.error=function(){};e.send()}var h={},k=[],l=[];this.listenerOrientation=new Parameter(LISTENER_ORIENTATION,0);this.listenerOrientation.addObserver(this);var n=a.createConvolver();n.connect(a.destination);var p=0;this.setReverbFile=function(a){g(a,function(a){n.buffer=a;d()})};this.loadBuffers=function(a){a=a.getAllSourcePaths();for(var b=0,c=a.length;b<c;b++)h[a[b]]||g(a[b])};this.getBuffer=function(a){return h[a.getSourcePath()]};this.getParameter=function(a){if(a==LISTENER_ORIENTATION)return this.listenerOrientation};
this.play=function(b){b.updatePartOrder(ONSET);b=new SchedulerThread(b,void 0,a,h,n,f,e);k.push(b)};this.pause=function(a){if(a)for(var b=0;b<k.length;b++)k[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<k.length;b++)k[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<k.length;c++){var d=k[c].getSource(a);d&&b.push(d)}return b};this.getUrisOfPlayingDymos=function(){return l};this.observedParameterChanged=function(b){b=this.listenerOrientation.getValue()/180*Math.PI;a.listener.setOrientation(Math.sin(b),
0,-Math.cos(b),0,1,0)}};function Source(a,b,c,d,e){function f(){var d=a.getParameter(TIME_STRETCH_RATIO).getValue();if(1!=d){if(0!=y||w<c.duration)c=n(c,l(y,c),l(w+.3,c));c=(new AudioProcessor(b)).timeStretch(c,d);d=w/d;c=n(c,0,l(d+.02,c));w=d}else if(0!=y||w<c.duration)c=n(c,l(y,c),l(w+.02,c));p(c,c.length);x.buffer=c;m[PLAYBACK_RATE]=x.playbackRate;g(PLAYBACK_RATE,a.getParameter(PLAYBACK_RATE))}function g(a,b){h(a,b.getValue());b.addObserver(r)}function h(a,b,c){c&&(b+=r.getParameterValue(a));0>b&&0<=z.indexOf(a)&&(b=
0);c=b;m[a]&&(m[a].value||0==m[a].value?m[a].value=c:m[a].setValue(c));0<=F.indexOf(a)?(0==m[DISTANCE].value&&(m[DISTANCE].value=-.01),D.setPosition(m[PAN].value,m[HEIGHT].value,m[DISTANCE].value)):a==LOOP&&(x.loop=1==b)}function k(){v=!1;var a=b.currentTime;m[AMPLITUDE].setValueAtTime(m[AMPLITUDE].value,a);m[AMPLITUDE].linearRampToValueAtTime(0,a+.02);x.stop(a+.04)}function l(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function n(a,c,d){d=Math.min(a.length-c,d);for(var m=b.createBuffer(a.numberOfChannels,
d,a.sampleRate),e=0;e<a.numberOfChannels;e++)for(var C=m.getChannelData(e),f=a.getChannelData(e),z=0;z<d;z++)C[z]=f[c+z];return m}function p(a,b){for(var c=.02*a.sampleRate,d=0;d<a.numberOfChannels;d++)for(var m=a.getChannelData(d),e=0;e<c;e++)m[e]*=e/c,m[b-e-1]*=e/c}function q(a,b,c,d){var m=a.lastIndexOf("/");m&&(a=a.substring(m+1));u("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function u(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType=
"arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}var r=this,v,t,m={},C=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,FILTER,PAN,HEIGHT,DISTANCE,LOOP],z=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,FILTER],F=[PAN,HEIGHT,DISTANCE],x=b.createBufferSource(),A=b.createGain();A.connect(b.destination);var B=b.createGain();B.connect(d);B.gain.value=0;A.connect(B);var D=b.createPanner();D.connect(A);
d=b.createBiquadFilter();d.type="lowpass";d.frequency.value=2E4;d.connect(D);x.connect(d);var E=a.getSegment(),y=E[0],w=E[1];y||(y=0);!w&&c&&(w=c.duration-y);c?f():q(a.getSourcePath(),y,y+w+.3,function(d){c=d;d=a.getParameter(TIME_STRETCH_RATIO).getValue();1!=d?(c=(new AudioProcessor(b)).timeStretch(c,d),d=w/d,c=n(c,0,l(d+.02,c)),w=d):c=n(c,0,l(w+.02,c));p(c,c.length);x.buffer=c});m[AMPLITUDE]=A.gain;m[TIME_STRETCH_RATIO]={value:0};m[REVERB]=B.gain;m[FILTER]=d.frequency;m[PAN]={value:0};m[HEIGHT]=
{value:0};m[DISTANCE]={value:0};m[LOOP]={value:0};g(AMPLITUDE,a.getParameter(AMPLITUDE));g(TIME_STRETCH_RATIO,a.getParameter(TIME_STRETCH_RATIO));g(REVERB,a.getParameter(REVERB));g(PAN,a.getParameter(PAN));g(HEIGHT,a.getParameter(HEIGHT));g(DISTANCE,a.getParameter(DISTANCE));g(LOOP,a.getParameter(LOOP));this.observedParameterChanged=function(a){h(a.getName(),a.getChange(),!0)};this.getDymo=function(){return a};this.getDuration=function(){return w};this.isLoopingAndPlaying=function(){return x.loop&&
v};this.getParameterValue=function(a){if(m[a])return m[a].value||0==m[a].value?m[a].value:m[a].getValue()};this.play=function(b){x.onended=function(){for(var b=0;b<C.length;b++){var c=a.getParameter(C[b]);c&&c.removeObserver(r)}x.disconnect();A.disconnect();B.disconnect();e&&e(r)};x.start(b);v=!0};this.pause=function(){v?(k(),t=!0):t&&(t=!1,this.play())};this.stop=function(){v&&k()}};function SchedulerThread(a,b,c,d,e,f,g){function h(){r=t?t:p();k(r);m||(m=r[0].getDymo().getParameter(ONSET).getValue());var a;a=v?v-c.currentTime:SCHEDULE_AHEAD_TIME;for(var d=c.currentTime+a,e=0;e<r.length;e++)r[e].play(d);setTimeout(function(){f()},a);(t=p())?(v=n(d),u=setTimeout(function(){h()},1E3*(v-c.currentTime-SCHEDULE_AHEAD_TIME))):(b.reset(),g&&g())}function k(a){for(var b=0;b<a.length;b++)q.set(a[b].getDymo(),a[b])}function l(a){q.delete(a.getDymo());0==q.size&&window.clearTimeout(u);
f()}function n(a){if(t){var b=t[0].getDymo().getParameter(ONSET).getValue(),c=b-m;m=b;if(!isNaN(b))return a+c}b=r[0];c=b.getDymo().getParameter(PLAYBACK_RATE).getValue();b=b.getDuration()/c;return a+b}function p(){var a=b.getNextParts();if(a){for(var m=[],f=0;f<a.length;f++)if(a[f].getSourcePath()){var g=d[a[f].getSourcePath()];m.push(new Source(a[f],c,g,e,l))}return m}}var q=new Map,u,r=[],v,t,m;b||(b=new DymoNavigator(a,new SequentialNavigator(a)));h();this.pause=function(a){a=a.getAllDymosInHierarchy();
for(var b=0,c=a.length;b<c;b++){var d=q.get(a[b]);d&&d.pause()}};this.stop=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=q.get(a[b]);d&&d.stop()}};this.getSources=function(){return q};this.getSource=function(a){return q.get(a)}};function AutoControl(a,b,c){function d(a){var b=a.getName();f[b]=a;f[b].addObserver(e)}var e=this,f={};d(new Parameter(AUTO_CONTROL_FREQUENCY,100));d(new Parameter(AUTO_CONTROL_TRIGGER,0));Control.call(this,a,AUTO_CONTROL,f);var g;this.addParameter=d;this.getParameter=function(a){return f[a]};this.startUpdate=function(){g=setInterval(b,f[AUTO_CONTROL_FREQUENCY].getValue())};this.reset=function(){clearInterval(g);g=null;c&&c()};this.observedParameterChanged=function(a){a==f[AUTO_CONTROL_FREQUENCY]?
g&&(this.reset(),this.startUpdate()):a==f[AUTO_CONTROL_TRIGGER]&&(g?this.reset():this.startUpdate())}}inheritPrototype(AutoControl,Control);function BrownianControl(a){var b=this;a||(a=.5);AutoControl.call(this,BROWNIAN,function(){var a=b.getParameter(BROWNIAN_MAX_STEP_SIZE).getValue(),a=2*a*Math.random()-a,a=b.getValue()+a,a=Math.min(Math.max(a,0),1);b.update(a)});this.addParameter(new Parameter(BROWNIAN_MAX_STEP_SIZE,.1));this.update(a);this.startUpdate()}inheritPrototype(BrownianControl,AutoControl);function RampControl(a,b){var c=this;a||(a=1E4);var d=0;b&&(d=b);var e=1!=d;AutoControl.call(this,RAMP,function(){var b=1/a*c.getParameter(AUTO_CONTROL_FREQUENCY).getValue();e||(b*=-1);d+=b;0<d&&1>d?c.update(d):1<=d?(c.update(1),c.reset()):0>=d&&(c.update(0),c.reset())},function(){e=!e});this.update(d)}inheritPrototype(RampControl,AutoControl);function RandomControl(){var a=this;AutoControl.call(this,RANDOM,function(){a.update(Math.random())});this.startUpdate()}inheritPrototype(RandomControl,AutoControl);function UIControl(a,b){var c=this;this.value=a.getValue();a.setUpdateFunction(function(a){c.value=a;b&&setTimeout(function(){b.$apply()},10)});this.getName=function(){return a.getName()};this.getType=function(){return a.getType()};this.update=function(){a.getType()==BUTTON&&(isNaN(this.value)&&(this.value=0),this.value=1-this.value);1==this.value?a.update(1):0==this.value?a.update(0):a.update(this.value)}};function OntologyLoader2(a,b,c){};function DymoLoader(a,b){function c(a,b,e,f,g){var h=new XMLHttpRequest;h.open("GET",t+a,!0);h.onload=function(){var h;a:{try{JSON.parse(this.responseText)}catch(k){h=!1;break a}h=!0}h&&(b=b?b.replace('"'+a+'"',this.responseText):this.responseText);(h=d(b))?c(h,b,e,f,g):b&&(h=g(JSON.parse(b),e),f(h))};h.error=function(a){console.log(a)};h.send()}function d(a){var b=a.indexOf(".json");if(0<=b){if(b!=a.indexOf("context.json")+7){var c=a.substring(0,b).lastIndexOf('"');return a.substring(c+1,b+5)}return d(a.substring(b+
1))}}function e(a,b){var c=f(a,b);g(a,b);return[c,b]}function f(a,b){var c=new DynamicMusicObject(a["@id"],a.ct);c.setBasePath(t);b[a["@id"]]=c;a.source&&c.setSourcePath(a.source);for(var d in a)0>v.indexOf(d)&&(a[d].type==FEATURE?c.setFeature(d,a[d].value):a[d].type==PARAMETER&&n(c,d,a[d].value));if(a.parts)for(d=0;d<a.parts.length;d++)c.addPart(f(a.parts[d],b));return c}function g(a,b){var c=b[a["@id"]];if(a.similars)for(var d=0;d<a.similars.length;d++)c.addSimilar(b[a.similars[d]]);if(a.mappings)for(d=
0;d<a.mappings.length;d++)c.addMapping(k(a.mappings[d],b,c));if(a.parts)for(d=0;d<a.parts.length;d++)g(a.parts[d],b)}function h(a,b){for(var c=new Rendering(b[a.topDymo]),d={},e=0;e<a.mappings.length;e++)c.addMapping(k(a.mappings[e],b,void 0,d));a.navigator&&(e=Function.apply(null,a.navigator.dymos.args.concat(a.navigator.dymos.body)),c.addNavigator(a.navigator.type==SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):new SequentialNavigator(void 0),e));return[c,d]}function k(b,c,d,e){if(b.controls){for(var f=
[],g=0;g<b.controls.length;g++)f.push(e[b.controls[g]]);return l(b,c,d,f,e)}if(b.dymos){var f=[],h;if(b.dymos instanceof Array)for(g=0;g<b.dymos.length;g++)f.push(c[b.dymos[g]]);else h=Function.apply(null,b.dymos.args.concat(b.dymos.body)),g=Object.keys(c).map(function(a){return c[a]}),Array.prototype.push.apply(f,g.filter(h));return l(b,c,d,f,e,h)}return l(b,c,d,[a],e)}function l(a,b,c,d,e,f){b=a.relative;for(var g=[],h=0;h<a.domainDims.length;h++){var k=a.domainDims[h],l=k.name,p=k.type;p==FEATURE?
g.push(l):p==PARAMETER?(l=c?n(c,l,0):new Parameter(l,0),g.push(l)):(k=e&&e[l]?e[l]:q(k),e&&!e[l]&&(e[l]=k),g.push(k))}return new Mapping(g,b,a["function"],d,a.parameter,f)}function n(a,b,c){var d=a.getParameter(b);d?d.update(c):(d=new Parameter(b,c),a.addParameter(d));return d}function p(a,b){for(var c=0;c<a.length;c++)if(a[c])for(var d=0;d<a[c].length;d++){var e=b["dymo"+c],f=b["dymo"+a[c][d]];e&&f&&e.addSimilar(f)}}function q(a){var c=a.type,d=a.name;if(c==ACCELEROMETER_X)return u(0);if(c==ACCELEROMETER_Y)return u(1);
if(c==ACCELEROMETER_Z)return u(2);if(c==TILT_X)return u(3);if(c==TILT_Y)return u(4);if(c==GEOLOCATION_LATITUDE)return r(0);if(c==GEOLOCATION_LONGITUDE)return r(1);if(c==GEOLOCATION_DISTANCE)return r(2);if(c==COMPASS_HEADING)return b.compassWatcher||(b.compassWatcher=new CompassWatcher),a=b.compassWatcher.headingControl,a;if(c==SLIDER||c==TOGGLE||c==BUTTON)return new Control(d,c);if(c==RANDOM)return new RandomControl;if(c==BROWNIAN)return new BrownianControl(parseFloat(a.value));if(c==RAMP)return c=
Math.round(1E3*parseFloat(a.duration)),new RampControl(c,parseFloat(a.value))}function u(a){b.accelerometerWatcher||(b.accelerometerWatcher=new AccelerometerWatcher);if(0==a)return b.accelerometerWatcher.xControl;if(1==a)return b.accelerometerWatcher.yControl;if(2==a)return b.accelerometerWatcher.zControl;if(3==a)return b.accelerometerWatcher.tiltXControl;if(4==a)return b.accelerometerWatcher.tiltYControl}function r(a){b.geolocationWatcher||(b.geolocationWatcher=new GeolocationWatcher);return 0==
a?b.geolocationWatcher.latitudeControl:1==a?b.geolocationWatcher.longitudeControl:b.geolocationWatcher.distanceControl}var v="@id @type ct source navigator similars mappings parts".split(" "),t="";this.loadDymoFromJson=function(a,b){var d=a.lastIndexOf("/")+1;t=a.substring(0,d);a=a.substring(d);c(a,"",{},b,e)};this.parseDymoFromJson=function(a,b){b(e(a,{}))};this.loadRenderingFromJson=function(a,b,d){c(a,"",b,d,h)};this.loadGraphFromJson=function(a,b,d){c(a,"",b,d,p)}};function DymoWriter(a){function b(a){a.addTriple("_:","http://www.w3.org/1999/02/22-rdf-syntax-ns#type","dmo:DMO");a.addTriple("_:","ch:hasPart","ch:test2")}function c(a,b){var c=new XMLHttpRequest;c.send(b);c.addEventListener("save",function(){console.log("saved "+a)});c.addEventListener("error",function(){console.log("saving "+a+" failed")});c.open("POST",a);c.send()}this.writeDymoToJson=function(a,b,f){a["@context"]="http://purl.org/ontology/dymo/context.json";f||(f="dymo.json");c(b+f,a)};this.writeRenderingToJson=
function(a,b){c(b+"rendering.json",a)};this.writeDmoToRdf=function(a){a=N3.Writer({prefixes:{ch:"rdf/charm.n3#",dmo:"rdf/dmo.n3#",mb:"rdf/mobile.n3#"}});b(a);a.end(function(a,b){console.log(b)})}};function Control(a,b,c){function d(a,b){if(void 0==f||1E-6<Math.abs(a-f))f=a,e(b)}function e(a){for(var b=0;b<k.length;b++)k[b]!=a&&k[b].updateParameter(f,this)}var f,g,h,k=[],l,n,p;this.getName=function(){return a};this.getParameter=function(a){if(c)return c[a]};this.getValue=function(){return f};this.getType=function(){return b};this.getReferenceValue=function(){return g};this.setReferenceAverageCount=function(a){h=a;this.resetReferenceValue()};this.resetReferenceValue=function(){g=f=void 0;n=l=
0};this.setUpdateFunction=function(a){p=a};this.addMapping=function(a){k.push(a);a.updateParameter(f,this)};this.removeMapping=function(a){a=k.indexOf(a);-1<a&&k.splice(a,1)};this.backpropagate=function(a,b){isFinite(a)&&(d(a,b),p&&p(f))};this.update=function(a){h&&l<h?(n+=a,l++,l==h&&(g=n/=h)):isNaN(a)||(g&&(a-=g),d(a))}};function DynamicMusicObject(a,b){function c(a,b){b[a.getUri()]=a;for(var d=0;d<a.getParts().length;d++)c(a.getParts()[d],b)}function d(a,b,c,e){var f=a-b,g=e.getParts();if(e.getLevel()==c&&0==f)return e;if(e.getLevel()>=c-1)return f<g.length?g[f]:b+g.length;for(e=0;e<g.length;e++)if(b=d(a,b,c,g[e]),isNaN(b))return b}function e(a,b,c){c(a,b);a=a.getParts();for(var d=0,f=a.length;d<f;d++)e(a[d],b,c)}function f(a,b,c){var d=b.toFlatJson();a.nodes.push(d);c&&a.links.push(h(c,d));for(c=0;c<b.getParts().length;c++)f(a,
b.getParts()[c],d)}function g(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var f=b.getSimilars()[e];c[f.getUri()]||(c[f.getUri()]=f.toFlatJson());a.links.push(h(d,c[f.getUri()]))}for(e=0;e<b.getParts().length;e++)g(a,b.getParts()[e],c)}function h(a,b){return{source:a,target:b,value:1}}var k=null,l=[],n=[],p={},q={},u=[],r=[],v="",t;q[PLAY]=new Parameter(PLAY,0,!0);q[LOOP]=new Parameter(LOOP,0,!0);q[ONSET]=
new Parameter(ONSET,void 0);q[DURATION_RATIO]=new Parameter(DURATION_RATIO,1);q[AMPLITUDE]=new Parameter(AMPLITUDE,1);q[PLAYBACK_RATE]=new Parameter(PLAYBACK_RATE,1);q[TIME_STRETCH_RATIO]=new Parameter(TIME_STRETCH_RATIO,1);q[PAN]=new Parameter(PAN,0);q[DISTANCE]=new Parameter(DISTANCE,0);q[HEIGHT]=new Parameter(HEIGHT,0);q[REVERB]=new Parameter(REVERB,0);q[FILTER]=new Parameter(FILTER,0);q[PART_INDEX]=new Parameter(PART_INDEX,0,!0);q[PART_COUNT]=new Parameter(PART_COUNT,Number.POSITIVE_INFINITY,
!0);q[PLAY].addObserver(this);q[PART_INDEX].addObserver(this);q[PART_COUNT].addObserver(this);this.setType=function(a){b=a};this.getType=function(){return b};this.getUri=function(){return a};this.setParent=function(a){k=a;for(var b in q)b!=PLAY&&b!=PART_COUNT&&b!=PART_INDEX&&r.push(new Mapping([a.getParameter(b)],!0,void 0,[this],b));a=k.getMappings();var c=this.getDymoMap(),c=Object.keys(c).map(function(a){return c[a]});for(b=0;b<a.length;b++){var d=a[b].getDymoConstraint();d&&(d=a[b].getTargets().concat(c.filter(d)),
a[b].setTargets(d))}};this.removeParent=function(){for(var a=0;a<r.length;a++)r[a].disconnect();r=[];for(var b=k.getMappings(),c=this,a=0;a<b.length;a++){var d=b[a].getTargets(),d=d.filter(function(a){return!a.isSubDymoOf(c)});b[a].setTargets(d)}k=null};this.getParent=function(){return k};this.isSubDymoOf=function(a){for(var b=this;b;){if(b==a)return!0;b=b.getParent()}return!1};this.getDymoMap=function(){var a={};c(this,a);return a};this.getLevel=function(){return k?k.getLevel()+1:0};this.getIndex=
function(){if(k)return k.getParts().indexOf(this)};this.addPart=function(a){l.push(a);a.setParent(this)};this.removePart=function(a){l.splice(l.indexOf(a));a.removeParent(this)};this.replacePart=function(a,b){l[a]&&l[a].removeParent(this);l[a]=b;b.setParent(this)};this.getPart=function(a){return l[a]};this.getParts=function(){return l};this.hasParts=function(){return 0<l.length};this.getNthPart=function(a,b){return d(a,0,b,this)};this.setBasePath=function(a){v=a};this.getBasePath=function(){return k&&
!v?k.getBasePath():v};this.setSourcePath=function(a){t=a};this.getSourcePath=function(){if(k&&!t)return k.getSourcePath();if(t)return this.getBasePath()+t};this.getAllSourcePaths=function(){var a=[];e(this,a,function(b,c){var d=b.getSourcePath();d&&0>a.indexOf(d)&&c.push(d)});return a};this.getAllDymosInHierarchy=function(){var a=[];e(this,a,function(a,b){b.push(a)});return a};this.addSimilar=function(a){n.push(a)};this.getSimilars=function(a){return n};this.getFeatures=function(){return p};this.setFeature=
function(a,b){p[a]=b};this.getFeature=function(a){if(a===LEVEL)return this.getLevel();if(a===INDEX)return this.getIndex();if(p.hasOwnProperty(a))return p[a];if(k)return k.getFeature(a);for(var b=[],c=0;c<l.length;c++)if(l[c]){var d=l[c].getFeatureWithoutLookingFurther(a);isNaN(d)||b.push(d)}if(0<b.length)return b};this.getFeatureWithoutLookingFurther=function(a){if(p[a])return p[a]};this.getMappings=function(){return u};this.getSegment=function(){return[this.getFeature("time"),q[DURATION_RATIO].getValue()*
this.getFeature("duration")]};this.addParameter=function(a){q[a.getName()]=a};this.getParameter=function(a){return q[a]};this.addMapping=function(a){u.push(a)};this.observedParameterChanged=function(a){a.getName()==PLAY&&a.getChange()};this.updatePartOrder=function(a){l&&l[0]&&!isNaN(l[0].getFeature(a))?l.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):l&&l.sort(function(b,c){return b.getParameter(a).getValue()-c.getParameter(a).getValue()})};this.toJsonHierarchy=function(){for(var a=
this.toFlatJson(),b=0;b<l.length;b++)a.parts.push(l[b].toJsonHierarchy());return a};this.toJsonHierarchyGraph=function(){var a={nodes:[],links:[]};f(a,this);return a};this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};g(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=b);t&&(c.source=t);for(var d in p)c[d]=this.getFeatureJson(d);if(0<u.length)for(c.mappings=[],d=0;d<u.length;d++)c.mappings.push(u[d].toJson());0<l.length&&(c.parts=[]);if(0<n.length)for(c.similars=
[],d=0;d<n.length;d++)c.similars.push(n[d].getUri());return c};this.getFeatureJson=function(a){return{value:p[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+a.slice(1)}}};function Mapping(a,b,c,d,e,f){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=f;this.init()}
...
PolygonTools.getInterpolatedPolygonFunctionArgs=function(a){var b=getPolygonOrPointString(a);a=getPolygonOrPointString(PolygonTools.getCentroid(a));return["a","b","return PolygonTools.howFarIsPointInPolygon("+b+","+a+", {0:a, 1:b});"]};function getPolygonOrPointString(a){return JSON.stringify(a).replace(/"/g,"'")};function DymoNavigator(a,b){function c(a){var c;if(!f.has(a)){c=0;for(var k=e.length;c<k;c++)e[c][0](a)&&a.hasParts()&&f.set(a,e[c][1].getCopy(a));f.has(a)||(b&&a.hasParts()?f.set(a,b.getCopy(a)):f.set(a,new OneShotNavigator(a)))}if(c=f.get(a)){a.getLevel();if(c.getType()==ONE_SHOT_NAVIGATOR)return c.getCurrentParts();a=c.getCurrentParts();return(a=d(a))?a:d(c.getNextParts())}return[a]}function d(a){if(a&&0<a.length){for(var b=[],d=0,e=a.length;d<e;d++){var f=c(a[d]);f&&(f.length?b=b.concat(f):b.push(f))}if(0<
b.length)return b}}var e=[],f;f=new Map;this.reset=function(){f=new Map};this.addSubsetNavigator=function(a,b){e.push([a,b])};this.getNextParts=function(){return c(a)}};function OneShotNavigator(a){var b=!1;this.getType=function(){return ONE_SHOT_NAVIGATOR};this.getCopy=function(a){return new OneShotNavigator(a)};this.getCurrentParts=function(){if(!b)return b=!0,[a]};this.getNextParts=function(){return this.getCurrentParts()}};function SequentialNavigator(a,b){var c=0;this.resetPartsNavigated=function(){c=0};this.setPartsNavigated=function(a){c=a};this.getPartsNavigated=function(a){return c[a]};this.getType=function(){return SEQUENTIAL_NAVIGATOR};this.getCopy=function(a){return new SequentialNavigator(a,b)};this.getCurrentParts=function(){if(0<a.getParts().length){if(a.getType()==PARALLEL){var d;0>=c?(c++,d=a.getParts()):d=void 0;return d}b?(d=a.getParts().length,d=a.getPart(d-c)):d=a.getPart(c);d?(d.hasParts()||c++,d=
[d]):d=void 0;return d}};this.getNextParts=function(){c++;return this.getCurrentParts()}};function SimilarityNavigator(a){function b(b){if(b&&0<b.length)for(var f=0;f<b.length;f++)if(0<b[f].getSimilars().length&&Math.random()<c.leapingProbability.getValue()){var g=b[f].getSimilars(),h=Math.floor(Math.random()*g.length);if(c.continueAfterLeaping.getValue()){var k=a.getParts().indexOf(h);k&&(d=k+1)}b[f]=g[h]}return b}var c=this,d=0;this.leapingProbability=new Parameter(LEAPING_PROBABILITY,.5);this.continueAfterLeaping=new Parameter(CONTINUE_AFTER_LEAPING,0,!0);this.resetPartsPlayed=function(){d=
...
e,f,g,h,k,l,n;h=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*h,k=e*h,f=2*e,l=f+a,n=f+d,c[n+0]=b[l+0]*k+this.pMidBuffer[f+0]*g,c[n+1]=b[l+1]*k+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
