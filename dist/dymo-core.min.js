...
b===INDEX_FEATURE?this.findPartIndex(a):this.findObjectValuesOfType(a,HAS_FEATURE,b,VALUE)};this.findAllFeatureValues=function(a){return this.findAllObjectValuesOfType(a,HAS_FEATURE,VALUE)};this.findAllNumericFeatureValues=function(a){return this.findAllFeatureValues(a).filter(function(a){return!isNaN(a)})};this.findParameterValue=function(a,b){return a?this.findObjectValuesOfType(a,HAS_PARAMETER,b,VALUE):this.findValueOfType(b,VALUE)};this.findParameterUri=function(a,b){return a?this.findObjectOfType(a,
HAS_PARAMETER,b):this.findSubject(null,b)};this.findPartIndex=function(a){var b=this.findParents(a)[0];return this.findObjectIndexInList(b,HAS_PART,a)};this.findLevel=function(a){var b=0;for(a=this.findParents(a)[0];a;)b++,a=this.findParents(a)[0];return b};this.findMaxLevel=function(){for(var a=this.findAllSubjectUris(TYPE,DYMO),b=0,c=0;c<a.length;c++)b=Math.max(b,this.findLevel(a[c]));return b};this.toJsonld=function(a){var b=this.findTopDymos()[0];this.toRdf(function(c){d(c,b,a)})};this.toJsonGraph=
function(a,b,c){var d={nodes:[],edges:[]},g={},l=this.findAllSubjectUris(TYPE,a),u=this.find(null,b,null);async.map(l,e,function(e,r){d.nodes=r;for(var q=0;q<l.length;q++)g[l[q]]=d.nodes[q];d.edges=[];for(q=0;q<u.length;q++)if(0==h.find(u[q].object,TYPE,a).length){if(0<h.find(u[q].object,FIRST).length){var x=h.findObjectListUris(u[q].subject,b),x=x.map(function(a){return{source:g[u[q].subject],target:g[a],value:1}});d.edges=d.edges.concat(x)}}else d.edges.push({source:g[u[q].subject],target:g[u[q].object],
value:1});c(d)})}}inheritPrototype(DymoStore,EasyStore);function DymoWriter(a,b){function c(a,b){var c=new XMLHttpRequest;c.send(b);c.addEventListener("save",function(){console.log("saved "+a)});c.addEventListener("error",function(){console.log("saving "+a+" failed")});c.open("POST",a);c.send()}this.writeToJsonld=function(b,e){e||(e="dymo.json");a.toJsonld(function(a){c(b+e,a)})}};function EasyStore(){function a(a,b,c){if(f[a]&&f[a][b]){b=f[a][b];var d=h.findObject(a,TYPE),e;for(e in b)b[e].observedValueChanged(a,d,c)}}function b(a,b){if(a){var c=h.findAllObjectValues(a,b);if(0<c.length)return 1==c.length&&(c=c[0]),c}}function c(a){if(a){var b=h.findObject(a,FIRST);if(b){var c=[];c.push(b);for(a=h.findObject(a,REST);a!=NIL;)c.push(h.findObject(a,FIRST)),a=h.findObject(a,REST);return c}return a}}function d(a,b){if(a){for(var c=0,d=a,e=h.findObject(d,REST);e!=NIL&&(null==b||
c<b);)d=e,e=h.findObject(d,REST),c++;if(null==b||c==b)return d}}function e(a){var b=N3.Util.getLiteralValue(a);"http://www.w3.org/2001/XMLSchema#string"!=N3.Util.getLiteralType(a)&&(b=Number(b));return b}function g(a,b,c){b?jsonld.toRDF(a,{format:"application/nquads"},function(a,b){c(b)}):c(a)}var h=this,l=N3.Store(),f={};this.addValueObserver=function(a,b,c){f[a]||(f[a]={});f[a][b]||(f[a][b]=[]);f[a][b].push(c)};this.removeValueObserver=function(a,b,c){f[a]&&f[a][b]&&(c=f[a][b].indexOf(c),-1<c&&
(f[a][b].splice(c,1),0==f[a][b].length&&(delete f[a][b],0===Object.keys(f[a]).length&&delete f[a])))};this.getValueObservers=function(a,b){return f[a]&&f[a][b]?f[a][b]:[]};this.addTriple=function(a,b,c){null!=a&&null!=b&&null!=c&&l.addTriple(a,b,c)};this.removeTriple=function(a,b,c){c||(c=this.findObject(a,b));return l.removeTriple(a,b,c)};this.createBlankNode=function(){return l.createBlankNode()};this.setTriple=function(a,b,c){this.removeTriple(a,b);this.addTriple(a,b,c)};this.setValue=function(b,
c,d){b&&c&&null!=d&&!Number.isNaN(d)&&(Array.isArray(d)?(d=d.map(function(a){return N3.Util.createLiteral(a)}),this.removeTriple(b,c),this.addObjectsToList(b,c,d)):this.setTriple(b,c,N3.Util.createLiteral(d)),a(b,c,d))};this.setObjectValue=function(a,b,c,d,e){var f;a&&b?f=this.findObjectOfType(a,b,c):a||(f=this.findSubject(TYPE,c));f||(f=this.createBlankNode(),a&&b&&this.addTriple(a,b,f),this.addTriple(f,TYPE,c));this.setValue(f,d,e);return f};this.addObjectToList=function(a,b,c){var e=this.findObject(a,
...
c-1);a=h.findObject(c,REST);b=h.findObject(a,REST);this.setTriple(c,REST,b);this.removeTriple(a,FIRST);this.removeTriple(a,REST)};this.deleteList=function(a,b){var c=this.findObject(a,b);if(c){for(;c!=NIL;){this.removeTriple(c,FIRST);var d=h.findObject(c,REST);this.removeTriple(c,REST);c=d}this.removeTriple(a,b)}};this.find=function(a,b,c){return l.find(a,b,c)};this.findObject=function(a,b){var c=l.find(a,b);if(0<c.length)return c[0].object};this.findObjectOrList=function(a,b){return c(this.findObject(a,
b))};this.findObjectOfType=function(a,b,c){a=l.find(a,b).map(function(a){return a.object});a=a.filter(function(a){return 0<l.find(a,TYPE,c).length});if(0<a.length)return a[0]};this.findSubject=function(a,b){"string"!=typeof b&&(b=N3.Util.createLiteral(b));var c=l.find(null,a,b);if(0<c.length)return c[0].subject;b=N3.Util.createLiteral(b);c=l.find(null,a,b);if(0<c.length)return c[0].subject};this.findObjectValue=function(a,b){var c=this.findObjectOrList(a,b);if(c)return Array.isArray(c)?c.map(e):e(c)};
this.findObjectValuesOfType=function(a,c,d,e){a=this.findObjectOfType(a,c,d);return b(a,e)};this.findValueOfType=function(a,c){var d=this.findSubject(TYPE,a);return b(d,c)};this.findAllSubjectUris=function(a,b){return l.find(null,a,b).map(function(a){return a.subject})};this.findAllObjects=function(a,b){for(var d=l.find(a,b).map(function(a){return a.object}),e=0;e<d.length;e++){var f=c(d[e]);f&&(d[e]=f)}return flattenArray(d)};this.findAllObjectValues=function(a,b){return this.findAllObjects(a,b).map(e)};
this.findAllObjectValuesOfType=function(a,b,c){var d=[];a=this.findAllObjects(a,b);for(b=0;b<a.length;b++)d.push(this.findObjectValue(a[b],c));return d};this.findObjectListUris=function(a,b){if(a&&b){var d=this.findObject(a,b);return c(d)}};this.findObjectIndexInList=function(a,b,c){if(a&&b){a:{var d=0;for(a=this.findObject(a,b);a!=NIL;){if(h.findObject(a,FIRST)==c){c=d;break a}a=h.findObject(a,REST);d++}c=-1}return c}};this.findObjectInListAt=function(a,b,c){if(a&&b&&(a=d(this.findObject(a,b),c)))return h.findObject(a,
FIRST)};this.findContainingLists=function(a){var b=[],c=[];a=this.findAllSubjectUris(FIRST,a);for(var d=0;d<a.length;d++){for(var e=a[d],f=this.findSubject(REST,e);f;)e=f,f=this.findSubject(REST,e);e=this.find(null,null,e)[0];b[d]=e.subject;c[d]=e.predicate}return[b,c]};this.recursiveFindAllTriplesExcept=function(a,b){for(var c=l.find(a,null,null),d=[],e=c.length-1;0<=e;e--)0<l.find(c[e].object,TYPE,b).length?c.splice(e,1):d=d.concat(this.recursiveFindAllTriplesExcept(c[e].object,b));return c.concat(d)};
this.findAllSubClasses=function(a){return this.findAllSubjectUris(RDFS_URI+"subClassOf",a)};this.isSubclassOf=function(a,b){for(var c=this.findObject(a,RDFS_URI+"subClassOf");c;){if(c==b)return!0;c=this.findObject(c,RDFS_URI+"subClassOf")}return!1};this.isSubtypeOf=function(a,b){for(var c=this.findObject(a,TYPE);c;){if(c==b)return!0;c=this.findObject(c,TYPE)}return!1};this.loadData=function(a,b,c){g(a,b,function(a){N3.Parser().parse(a,function(a,b,d){b?l.addTriple(b):c&&c()})})};this.loadFileIntoStore=
function(a,b,c){loadFile(a,function(a){h.loadData(a,b,c)})};this.toRdf=function(a){var b=l.find(null,null,null);this.triplesToRdf(b,a)};this.triplesToRdf=function(a,b){for(var c=N3.Writer({format:"application/nquads"}),d=0;d<a.length;d++)c.addTriple(a[d]);c.end(function(a,c){b(c)})};this.logData=function(){var a=l.find(null).map(function(a){return a.subject+"\n"+a.predicate+"\n"+a.object}),b;for(b in a)console.log(a[b])}};function FunctionTools(){}FunctionTools.IDENTITY_JSON={args:["a"],body:"return a;"};FunctionTools.inverted={};FunctionTools.created={};FunctionTools.createFunction=function(a,b){FunctionTools.created[b]||(FunctionTools.created[b]=Function.apply(null,a.concat(b)));return FunctionTools.created[b]};
