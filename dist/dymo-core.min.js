...
$jscomp.math.hypot=function(a,b,c){for(var d=[],e=2;e<arguments.length;++e)d[e-2]=arguments[e];a=Number(a);b=Number(b);for(var g=Math.max(Math.abs(a),Math.abs(b)),k=$jscomp.makeIterator(d),e=k.next();!e.done;e=k.next())g=Math.max(g,Math.abs(e.value));if(1E100<g||1E-100>g){a/=g;b/=g;k=a*a+b*b;d=$jscomp.makeIterator(d);for(e=d.next();!e.done;e=d.next())e=e.value,e=Number(e)/g,k+=e*e;return Math.sqrt(k)*g}g=a*a+b*b;d=$jscomp.makeIterator(d);for(e=d.next();!e.done;e=d.next())e=e.value,g+=e*e;return Math.sqrt(g)};
...
function DymoManager(a,b,c,d,e,g){function k(a,b,d){f=a;n=b[0];for(var g in b[1]){var v=b[1][g];DYMO_STORE.isSubclassOf(v.getType(),UI_CONTROL)&&(h[g]=new UIControl(v,e));DYMO_STORE.isSubclassOf(v.getType(),SENSOR_CONTROL)&&(l[g]=v)}c||(c="bower_components/dymo-core/audio/impulse_rev.wav");m.init(c,a,d)}DYMO_STORE=new DymoStore(d);var m=new Scheduler(a,g);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var f,n,h={},l={};this.loadDymoAndRenderingFromStore=function(a,b){var c=new DymoLoader(a);k(c.createDymoFromStore(),
c.createRenderingFromStore(),b)};this.loadDymoAndRendering=function(a,b,c,d){var h=new DymoLoader(DYMO_STORE);h.loadDymoFromJson(a,function(a){h.loadRenderingFromJson(b,function(b){k(a,b,d);c&&c()})})};this.loadDymoFromJson=function(a,b){(new DymoLoader(DYMO_STORE)).loadDymoFromJson(a,b)};this.parseDymoFromJson=function(a,b){(new DymoLoader(DYMO_STORE)).parseDymoFromJson(a,b)};this.replacePartOfTopDymo=function(a,b){var c=DYMO_STORE.replacePartAt(f[0],b,a);m.stop(c)};this.updateNavigatorPosition=
...
DymoNode.prototype.removeAndDisconnect=function(){for(var a in this.parameters)DYMO_STORE.removeParameterObserver(this.dymoUri,a,this);this.dryGain.disconnect();this.reverbGain&&this.reverbGain.disconnect();this.delayGain&&this.delayGain.disconnect()};function AudioProcessor(a){function b(a){return{extract:function(b,e,g){for(var k=[],m=0;m<a.numberOfChannels;m++)k.push(a.getChannelData(m));for(m=0;m<e;m++)for(var f=0;f<k.length;f++)b[m*k.length+f%k.length]=k[f][m+g];return Math.min(e,k[0].length-g)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var g=b(c),e=new SimpleFilter(g,e),g=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),k=[],m=0;m<c.numberOfChannels;m++)k.push(g.getChannelData(m));for(var f=
new Float32Array(2048),n=e.extract(f,1024),h=0;n;){for(m=0;m<n;m++)for(var l=0;l<k.length;l++)k[l][m+h]=f[m*k.length+l%k.length];h+=n;n=e.extract(f,1024)}return g}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var g=new SimpleFilter({extract:function(a,c,d){for(var l=[],p=0;p<b.numberOfChannels;p++)l.push(b.getChannelData(p));for(p=0;p<c;p++)for(var e=0;e<l.length;e++)a[p*l.length+e%l.length]=l[e][p+d];return Math.min(c,l[0].length-d)}},e),k=a.createScriptProcessor(1024,
2,b.numberOfChannels),m=new Float32Array(2048);k.onaudioprocess=function(a){for(var c=[],h=0;h<b.numberOfChannels;h++)c.push(a.outputBuffer.getChannelData(h));a=g.extract(m,1024);0==a&&d.stop();for(h=0;h<a;h++)for(var l=0;l<c.length;l++)c[l][h]=m[h*c.length+l%c.length]};this.start=function(a){setTimeout(function(){k.connect(c)},a)};this.stop=function(){k.disconnect()}};function Scheduler(a,b){function c(a,b){for(var c=[],d=0,h=a.length;d<h;d++)var e=DYMO_STORE.findAllObjectsInHierarchy(a[d]),c=c.concat(e.map(function(a){return DYMO_STORE.getSourcePath(a)}));d=0;for(h=c.length;d<h;d++)c[d]&&!m[c[d]]&&g(c[d],function(a,c){m[c]=a;0<l&&l--;0==l&&b&&b()});0==l&&b&&b()}function d(a){f.splice(f.indexOf(a),1)}function e(){for(var a=[],c=0;c<f.length;c++)for(var d=$jscomp.makeIterator(f[c].getAllSources().keys()),h=d.next();!h.done;h=d.next())for(h=h.value;null!=h;)0>a.indexOf(h)&&
a.push(h),h=DYMO_STORE.findParents(h)[0];a.sort();n=a;b&&b(n)}function g(b,c){l++;var d=new XMLHttpRequest;d.open("GET",b,!0);d.responseType="arraybuffer";d.onload=function(){a.decodeAudioData(d.response,function(a){c(a,b)})};d.error=function(){};d.send()}var k=this,m={},f=[],n=[],h,l=0;this.init=function(b,d,e){DYMO_STORE.addParameter(null,LISTENER_ORIENTATION,0,k);c(d,e);b&&0<DYMO_STORE.find(null,null,REVERB).length&&(h=a.createConvolver(),h.connect(a.destination),g(b,function(a){h.buffer=a;0<l&&
l--;0==l&&e&&e()}));0<DYMO_STORE.find(null,null,DELAY).length&&(b=a.createDelay(),b.delayTime.value=.5,b.connect(a.destination),d=a.createGain(),d.gain.value=.6,b.connect(d),d.connect(b));DYMO_STORE.addTypeObserver(PLAY,VALUE,k)};this.getBuffer=function(a){return m[DYMO_STORE.getSourcePath(a)]};this.updateNavigatorPosition=function(a,b,c){for(var d=0,h=f.length;d<h;d++)f[d].hasDymo(a)&&f[d].getNavigator().setPosition(c,b,a)};this.getNavigatorPosition=function(a,b){for(var c=0,d=f.length;c<d;c++)if(f[c].hasDymo(a))return f[c].getNavigator().getPosition(b,
...
a};this.setAudioFileChanged=function(){x=!0;if(p){var b=d();a.addDymo(b,null,p);p=b}};this.addDymo=function(a,b){return c(a,b)};this.addFeature=function(b,c){Benchmarker.startTask("addFeature");g();for(var d=f(b),h=a.findAllObjectsInHierarchy(p),l=0;l<h.length;l++){var n=a.findFeatureValue(h[l],TIME_FEATURE),r=a.findFeatureValue(h[l],DURATION_FEATURE),k=c;isNaN(n)||(k=k.filter(function(a){return n<=a.time&&(isNaN(r)||a.time<n+r)}));if(1>k.length)var v=c.filter(function(a){return a.time.value<=n}),
k=0<v.length?[v[k.length-1]]:[c[0]];Benchmarker.startTask("summarize");k=e(k);this.setDymoFeature(h[l],d.uri,k)}};this.addSegmentation=function(b){g();for(var d=a.findMaxLevel(),l=0;l<b.length;l++){var e=k(b[l].time,d),f=b[l].time,p;if(b[l].duration)p=b[l].duration;else if(b[l+1])p=b[l+1].time-f;else{var n=a.findFeatureValue(e,TIME_FEATURE),r=a.findFeatureValue(e,DURATION_FEATURE);n&&r&&(p=n+r-f)}if(0<p){n=c(e);this.setDymoFeature(n,TIME_FEATURE,f);this.setDymoFeature(n,DURATION_FEATURE,p);var v=
...
"audio/achachbach10/01-AchGottundHerr-clarinet.wav"):3==l[3]?q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-saxphone.wav"):4==l[3]&&(q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-bassoon.wav"));a.setDymoFeature(q,d,p);a.setDymoFeature(q,m,r);a.setDymoFeature(q,c,r);a.setDymoFeature(q,e,u);a.setDymoFeature(q,g,x);a.setDymoFeature(q,k,v);q.getParameter(ONSET).update(r)}DYMO_STORE.updatePartOrder(f,c.name)});$http.get("getsourcefilesindir/",{params:{directory:"audio/achachbach10/"}}).success(function(a){a=
...
f||(f=e(a,d[h].object,"http://purl.org/NET/c4dm/timeline.owl#beginsAt"));var p=e(a,d[h].object,"http://purl.org/NET/c4dm/timeline.owl#duration"),f={time:k(f),label:g(e(a,d[h].subject,"http://www.w3.org/2000/01/rdf-schema#label"))};p&&(f.duration=k(p));c.push(f)}b(c)}function d(a,b){for(var c=g(e(a,null,"http://purl.org/dc/elements/1.1/title")),d=g(e(a,null,"http://purl.org/ontology/af/value")),d=d.split(" ").map(function(a){return parseFloat(a)}),h=g(e(a,null,"http://purl.org/ontology/af/dimensions")),
h=h.split(" ").map(function(a){return Number.parseInt(a,10)}),l=e(a,null,"http://purl.org/ontology/vamp/computed_by"),f=k(e(a,l,"http://purl.org/ontology/vamp/step_size")),l=k(e(a,l,"http://purl.org/ontology/vamp/sample_rate")),p=[],n=0;n<d.length;){for(var r=[],x=0;x<h[0];x++)r[x]=d[n+x];p.push({time:{value:n*f/l},value:r});n+=h[0]}b({name:c,values:p})}function e(a,b,c){a=a.find(b,c,null);if(0<a.length)return a[0].object}function g(a){if(a)return N3.Util.getLiteralValue(a)}function k(a){a=N3.Util.getLiteralValue(a);
...
Similarity.toVectors=function(a,b,c){for(var d=[],e=0,g=a.length;e<g;e++){for(var k=[],m=b.findAllFeatureValues(a[e]).filter(function(a){return"string"!=typeof a}),f=0,n=m.length;f<n;f++){var h=m[f];c&&1<h.length&&(h=Similarity.reduce(h));1<h.length?k=k.concat(h):(h=Number(h),k.push(h))}d[e]=k}for(var l=[],p=[],e=0;e<d[0].length;e++){b=[];for(f=0;f<a.length;f++)isNaN(d[f][e])||b.push(d[f][e]);l[e]=math.mean(b);p[e]=math.var(b)}d=d.map(function(a){return a.map(function(a,b){return(a-l[b])/p[b]})});
...
"http://tiny.cc/mobile-audio-ontology#isSmooth",IS_AVERAGE_OF="http://tiny.cc/mobile-audio-ontology#isAverageOf",HAS_UUID="http://tiny.cc/mobile-audio-ontology#hasUuid",HAS_MAJOR="http://tiny.cc/mobile-audio-ontology#hasMajor",HAS_MINOR="http://tiny.cc/mobile-audio-ontology#hasMinor",HAS_DURATION="http://tiny.cc/mobile-audio-ontology#hasDuration",HAS_NAVIGATOR="http://tiny.cc/mobile-audio-ontology#hasNavigator",NAV_DYMOS="http://tiny.cc/mobile-audio-ontology#navDymos";function OntologyLoader2(a,b,c){};function DymoLoader(a){function b(b,d){c(b,"",function(b){a.loadData(b,!0,d)})}function c(a,b,h){loadFile(a,function(e){if(0>e.indexOf("Cannot GET")){var n;a:{try{JSON.parse(e)}catch(g){n=!1;break a}n=!0}n&&(b?(0<=a.indexOf(f)&&(a=a.replace(f,"")),b=b.replace('"'+a+'"',e)):b=e);(e=d(b))?(0>e.indexOf(f)&&(e=f+e),c(e,b,h)):b&&h(JSON.parse(b))}})}function d(a){var b=a.indexOf(".json");if(0<=b){if(b!=a.indexOf("context.json")+7){var c=a.substring(0,b).lastIndexOf('"');return a.substring(c+1,b+5)}return d(a.substring(b+
1))}}function e(b){var c;c=b?a.findAllObjects(b,HAS_MAPPING):a.findAllSubjects(TYPE,MAPPING);for(var d=a.recursiveFindAllSubClasses(MOBILE_CONTROL),e=0;e<d.length;e++){var f=a.findAllSubjects(TYPE,d[e]);for(b=0;b<f.length;b++){var k=a.findObjectValue(f[b],NAME);k||(k=f[b]);if(!n[f[b]]){var m=f[b],w=f[b],y=k,t=d[e],k=void 0;t==ACCELEROMETER_X||t==ACCELEROMETER_Y||t==ACCELEROMETER_Z?k=new AccelerometerControl(t):t==TILT_X||t==TILT_Y?k=new TiltControl(t):t==GEOLOCATION_LATITUDE||t==GEOLOCATION_LONGITUDE?
k=new GeolocationControl(t):t==GEOLOCATION_DISTANCE?k=new DistanceControl:t==COMPASS_HEADING?k=new CompassControl:t==BEACON?(k=a.findObjectValue(w,HAS_UUID),y=a.findObjectValue(w,HAS_MAJOR),t=a.findObjectValue(w,HAS_MINOR),k=new BeaconControl(k,y,t)):t==SLIDER||t==TOGGLE||t==BUTTON||t==CUSTOM_CONTROL?(k=new Control(w,y,t),y=a.findObjectValue(w,HAS_INITIAL_VALUE),k.update(y)):t==RANDOM?k=new RandomControl(w):t==BROWNIAN?(y=a.findObjectValue(w,HAS_INITIAL_VALUE),k=new BrownianControl(w,y)):t==RAMP&&
(k=Math.round(1E3*a.findObject(w,HAS_DURATION)),y=a.findObjectValue(w,HAS_INITIAL_VALUE),k=new RampControl(w,k,y));a.findObjectValue(w,IS_SMOOTH)&&k.setSmooth&&k.setSmooth(!0);w=a.findObjectValue(w,IS_AVERAGE_OF);!isNaN(w)&&k.setAverageOf&&k.setAverageOf(w);n[m]=k}}}for(d=0;d<c.length;d++)h[c[d]]||(b=DYMO_STORE.findSubject(HAS_MAPPING,c[d]),e=c[d],f=c[d],m=a.findObject(f,HAS_FUNCTION),b=g(m,b),m=a.findAllObjects(f,TO_TARGET),m=0<m.length?(w=g(m[0]))?w:m:void 0,f=a.findObject(f,HAS_RANGE),f=new Mapping(b,
m,f),h[e]=f)}function g(b,c){var d=$jscomp.makeIterator(a.findFunction(b)),h=d.next().value,e=d.next().value,d=d.next().value;if(h&&e&&d){for(var f=[],k=0;k<e.length;k++){var g=a.findObject(e[k],TYPE);g==FEATURE_TYPE||a.isSubtypeOf(g,FEATURE_TYPE)?(g==FEATURE_TYPE&&(g=e[k]),f.push(g)):g==PARAMETER_TYPE||a.isSubclassOf(g,PARAMETER_TYPE)?(g==CUSTOM_PARAMETER&&(g=e[k]),g=c?a.setParameter(c,g):e[k],f.push(g)):f.push(n[e[k]])}return new DymoFunction(h,f,d)}}function k(a){return a==SIMILARITY_NAVIGATOR?
...
function(a,c){b(a,function(){c(m.createRenderingFromStore())})};this.parseDymoFromTurtle=function(b,c){a.loadData(b,!1,function(){c(m.createDymoFromStore())})};this.createDymoFromStore=function(){for(var b=a.findTopDymos(),c=0;c<b.length;c++)a.addBasePath(b[c],f),e();return b};this.createRenderingFromStore=function(){var b=a.findSubject(TYPE,RENDERING),c=new Rendering(a.findObject(b,HAS_DYMO));e(b);for(var b=a.findAllObjects(b,HAS_NAVIGATOR),d=0;d<b.length;d++){var h=g(a.findObject(b[d],NAV_DYMOS));
c.addSubsetNavigator(h,k(a.findObject(b[d],TYPE)))}return[c,n]}};function DymoStore(a){function b(a,c){for(var d=0;d<a.length;d++){var e=c(a[d]);if(e)return e}for(d=0;d<a.length;d++)if(e=b(k.findParents(a[d]),c))return e}function c(a,b,c){k.triplesToRdf(a,function(a){d(a,b,c)})}function d(a,b,c){a=a.split("_b").join("b");jsonld.fromRDF(a,{format:"application/nquads"},function(a,d){jsonld.frame(d,{"@id":b},function(a,b){jsonld.compact(b,DYMO_CONTEXT,function(a,b){jsonld.compact(b,DYMO_SIMPLE_CONTEXT,function(a,b){g(b);b=JSON.stringify(b,null,2);b=b.replace(/http:\/\/tiny.cc\/dymo-context\//g,
"");b=b.replace("http://tiny.cc/dymo-context-simple","http://tiny.cc/dymo-context");c(b)})})})})}function e(a,b){var d=k.findObject(a,TYPE,null),d=k.recursiveFindAllTriples(a,d);c(d,a,function(a){b(null,JSON.parse(a))})}function g(a){if(a&&a instanceof Object)for(var b in a)"@id"==b&&a[b].includes("_:b")?delete a[b]:g(a[b])}var k=this;EasyStore.call(this);var m={};(function(){k.loadFileIntoStore("http://tiny.cc/dymo-ontology",!1,function(){k.loadFileIntoStore("http://tiny.cc/mobile-audio-ontology",
...
e=0;e<d.length;e++){var g=this.createBlankNode();this.addTriple(c,HAS_ARGUMENT,g);this.addTriple(g,HAS_VARIABLE,N3.Util.createLiteral(d[e]));this.addTriple(g,HAS_VALUE,a[d[e]])}this.addTriple(c,HAS_BODY,N3.Util.createLiteral(b));return c};this.updatePartOrder=function(a,b){var c=this.findParts(a);0<c.length&&(c.sort(function(a,c){return k.findAttributeValue(a,b)-k.findAttributeValue(c,b)}),this.setParts(a,c))};this.findTopDymos=function(){var a=this.findAllSubjects(TYPE,DYMO),b=this.findAllSubjects(HAS_PART),
...
b){for(var c=this.findObject(a,TYPE);c;){if(c==b)return!0;c=this.findObject(c,TYPE)}return!1};this.loadData=function(a,b,c){g(a,b,function(a){N3.Parser().parse(a,function(a,b,d){b?m.addTriple(b):c&&c()})})};this.loadFileIntoStore=function(a,b,c){loadFile(a,function(a){k.loadData(a,b,c)})};this.toRdf=function(a){var b=m.find(null,null,null);this.triplesToRdf(b,a)};this.triplesToRdf=function(a,b){for(var c=N3.Writer({format:"application/nquads"}),d=0;d<a.length;d++)c.addTriple(a[d]);c.end(function(a,
...
Control.prototype.updateMappings=function(a){for(var b=0;b<this.observers.length;b++)this.observers[b]!=a&&this.observers[b].observedControlChanged(this.value,this)};function DymoFunction(a,b,c){this.vars=a;this.args=b;this.body=c;this.argCache={};this.isDymoSpecificParam=[];this.observers=[];this.init()}
DymoFunction.prototype.init=function(){this.vars||(this.vars=FunctionTools.IDENTITY_JSON.args,this.body=FunctionTools.IDENTITY_JSON.body);this.directFunction=FunctionTools.createFunction(this.vars,this.body);this.inverseFunction=FunctionTools.invertFunction(this.body);for(var a=0;a<this.args.length;a++)if(this.args[a].addObserver)this.args[a].addObserver(this);else if("string"===typeof this.args[a]||this.args[a]instanceof String){var b=DYMO_STORE.findObject(this.args[a],TYPE);b==PARAMETER_TYPE||DYMO_STORE.isSubclassOf(b,
PARAMETER_TYPE)?(DYMO_STORE.addTypeObserver(this.args[a],VALUE,this),this.isDymoSpecificParam[a]=!0):("string"===typeof this.args[a]||this.args[a]instanceof String)&&DYMO_STORE.addSpecificParameterObserver(this.args[a],this)}};
...
DymoFunction.prototype.notifyObservers=function(){var a=this;this.observers.forEach(function(b){b.observedFunctionChanged(a)})};DymoFunction.prototype.applyDirect=function(a,b,c){return this.directFunction.apply(this,this.getArgValues(a,b,c))};DymoFunction.prototype.applyInverse=function(a,b){this.inverseFunction&&this.args&&1==this.args.length&&this.args[0].backpropagate&&(a=this.inverseFunction(a),null!=a&&this.args[0].backpropagate(a,this))};
DymoFunction.prototype.observedControlChanged=function(a,b){var c=this.args.indexOf(b);this.getArgValues(c,a,null);this.notifyObservers()};DymoFunction.prototype.observedValueChanged=function(a,b,c){var d=this.args.indexOf(a);0>d&&(d=this.args.indexOf(b));-1<d&&(a=DYMO_STORE.findSubject(HAS_PARAMETER,a),this.getArgValues(d,c,a),this.notifyObservers())};
DymoFunction.prototype.getArgValues=function(a,b,c){var d=c?c:"";this.argCache[d]||(this.argCache[d]=[]);if(null!=a)if(this.isDymoSpecificParam[a])this.argCache[d][a]=b;else{var e=this.argCache;Object.keys(e).map(function(c){e[c][a]=b})}for(var g=0;g<this.args.length;g++)null==this.argCache[d][g]&&(this.argCache[d][g]=this.getArgValue(g,c));return this.argCache[d]};
...
Mapping.prototype.getTargets=function(){if(this.targetUris&&0<this.targetUris.length)return this.targetUris;if(this.targetFunction){var a=DYMO_STORE.findAllSubjects(TYPE,DYMO),b=this.targetFunction;return a.filter(function(a){return b.applyDirect(null,null,a)})}};Mapping.prototype.disconnect=function(){this.mappingFunction.disconnect();this.targetFunction.disconnect();for(var a=0,b=this.targetUris.length;a<b;a++)DYMO_STORE.removeParameterObserver(this.targetUris[a],this.parameterName,this)};
Mapping.prototype.updateAll=function(a,b){var c=this.getTargets();if(c)for(var d=0,e=c.length;d<e;d++)this.update(a,b,c[d]);else this.update(a,b)};Mapping.prototype.update=function(a,b,c){a=this.mappingFunction.applyDirect(a,b,c);DYMO_STORE.setParameter(c,this.parameterName,a)};Mapping.prototype.observedValueChanged=function(a,b,c){b==this.parameterName&&(a=DYMO_STORE.findSubject(HAS_PARAMETER,a),this.mappingFunction.applyInverse(c,a))};Mapping.prototype.observedFunctionChanged=function(a){this.updateAll()};
Mapping.prototype.requestValue=function(a){return this.mappingFunction.requestValue(a)};function Rendering(a){var b;this.play=function(){a&&DYMO_STORE.setParameter(a,PLAY,1)};this.stop=function(){a&&DYMO_STORE.setParameter(a,PLAY,0)};this.addSubsetNavigator=function(c,d){b||(b=new DymoNavigator(a));b.addSubsetNavigator(c,d)};this.getNavigator=function(){return b}};function FunctionTools(){}FunctionTools.IDENTITY_JSON={args:["a"],body:"return a;"};FunctionTools.inverted={};FunctionTools.created={};FunctionTools.createFunction=function(a,b){FunctionTools.created[b]||(FunctionTools.created[b]=Function.apply(null,a.concat(b)));return FunctionTools.created[b]};
...
e=this._outputBuffer.endIndex,g=0,k=0;1>this.slopeCount;)d[e+2*k]=(1-this.slopeCount)*this.prevSampleL+this.slopeCount*b[c],d[e+2*k+1]=(1-this.slopeCount)*this.prevSampleR+this.slopeCount*b[c+1],k++,this.slopeCount+=this._rate;--this.slopeCount;if(1!=a)a:for(;;){for(;1<this.slopeCount;)if(--this.slopeCount,g++,g>=a-1)break a;var m=c+2*g;d[e+2*k]=(1-this.slopeCount)*b[m]+this.slopeCount*b[m+2];d[e+2*k+1]=(1-this.slopeCount)*b[m+1]+this.slopeCount*b[m+3];k++;this.slopeCount+=this._rate}this.prevSampleL=
...
e,g,k,m,f,n,h;m=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)k=(this.overlapLength-e)*m,f=e*m,g=2*e,n=g+a,h=g+d,c[h+0]=b[n+0]*f+this.pMidBuffer[g+0]*k,c[h+1]=b[n+1]*f+this.pMidBuffer[g+1]*k},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
