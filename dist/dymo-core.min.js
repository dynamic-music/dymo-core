...
"GeolocationDistance",COMPASS_HEADING="CompassHeading",RANDOM="Random",BROWNIAN="Brownian",RAMP="Ramp",CONTROLS=[SLIDER,TOGGLE,BUTTON,CUSTOM,ACCELEROMETER_X,ACCELEROMETER_Y,ACCELEROMETER_Z,TILT_X,TILT_Y,GEOLOCATION_LATITUDE,GEOLOCATION_LONGITUDE,GEOLOCATION_DISTANCE,COMPASS_HEADING,RANDOM,BROWNIAN,RAMP],UI_CONTROLS=[SLIDER,TOGGLE,BUTTON,CUSTOM];function DymoManager(a,b,c,d){var e=new Scheduler(a,function(){});c||(c="bower_components/dymo-core/audio/impulse_rev.wav");e.setReverbFile(c);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var f,g={};this.loadDymoAndRendering=function(a,b,c){var n=new DymoLoader(e);n.loadDymoFromJson(a,function(a){n.loadRenderingFromJson(b,a[1],function(b){f=b[0];f.dymo=a[0];for(var h in b[1]){var q=b[1][h];0<=UI_CONTROLS.indexOf(q.getType())&&(g[h]=new UIControl(q,d))}e.loadBuffers(f.dymo);c&&c()})})};this.loadDymoFromJson=
function(a,b){(new DymoLoader(e)).loadDymoFromJson(a,function(a){b&&b(a[0])})};this.parseDymoFromJson=function(a,b){(new DymoLoader(e)).parseDymoFromJson(a,function(a){b(a[0])})};this.replacePartOfTopDymo=function(a,b){var c=f.dymo.getPart(a);f.dymo.replacePart(a,b);e.stop(c)};this.updateNavigatorPosition=function(a,b,c){e.updateNavigatorPosition(a,b,c)};this.getNavigatorPosition=function(a,b){return e.getNavigatorPosition(a,b)};this.syncNavigators=function(a,b,c){e.syncNavigators(a,b,c)};this.startPlaying=
function(){f.dymo.updatePartOrder(ONSET);e.play(f.dymo)};this.stopPlaying=function(){e.stop(f.dymo)};this.getTopDymo=function(){return f.dymo};this.getRendering=function(){return f};this.getUIControls=function(){return g};this.getUIControl=function(a){return g[a]}};function AudioProcessor(a){function b(a){return{extract:function(b,e,f){for(var g=[],k=0;k<a.numberOfChannels;k++)g.push(a.getChannelData(k));for(k=0;k<e;k++)for(var h=0;h<g.length;h++)b[k*g.length+h%g.length]=g[h][k+f];return Math.min(e,g[0].length-f)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var f=b(c),e=new SimpleFilter(f,e),f=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),g=[],k=0;k<c.numberOfChannels;k++)g.push(f.getChannelData(k));for(var h=
new Float32Array(2048),p=e.extract(h,1024),n=0;p;){for(k=0;k<p;k++)for(var m=0;m<g.length;m++)g[m][k+n]=h[k*g.length+m%g.length];n+=p;p=e.extract(h,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var g=0;g<e.length;g++)a[f*e.length+g%e.length]=e[g][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),k=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(k,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=k[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(){0<t&&t--;0==t&&b&&b(t)}function e(a){h.splice(h.indexOf(a),1)}function f(){for(var a=[],b=0;b<h.length;b++)for(var d=$jscomp.makeIterator(h[b].getSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();p=a;c&&c()}function g(b,c){t++;var e=new XMLHttpRequest;e.open("GET",b,!0);e.responseType="arraybuffer";e.onload=function(){a.decodeAudioData(e.response,function(a){c?c(a):(k[b]=
a,d())})};e.error=function(){};e.send()}var k={},h=[],p=[];this.listenerOrientation=new Parameter(LISTENER_ORIENTATION,0);this.listenerOrientation.addObserver(this);var n=a.createConvolver();n.connect(a.destination);var m=a.createDelay();m.delayTime.value=.5;m.connect(a.destination);var u=a.createGain();u.gain.value=.6;m.connect(u);u.connect(m);var t=0;this.setReverbFile=function(a){g(a,function(a){n.buffer=a;d()})};this.loadBuffers=function(a){a=a.getAllSourcePaths();for(var b=0,c=a.length;b<c;b++)k[a[b]]||
...
[],g=0;g<b.controls.length;g++)f.push(e[b.controls[g]]);return p(b,c,d,f,e)}if(b.dymos){var f=[],h;if(b.dymos instanceof Array)for(g=0;g<b.dymos.length;g++)f.push(c[b.dymos[g]]);else h=Function.apply(null,b.dymos.args.concat(b.dymos.body)),g=Object.keys(c).map(function(a){return c[a]}),Array.prototype.push.apply(f,g.filter(h));return p(b,c,d,f,e,h)}return p(b,c,d,[a],e)}function p(a,b,c,d,e,f){b=a.relative;for(var g=[],h=0;h<a.domainDims.length;h++){var p=a.domainDims[h],k=p.name,m=p.type;m==FEATURE?
g.push(k):m==PARAMETER?(p=c?n(c,k,0):new Parameter(k,0),g.push(p)):(m=e&&e[k]?e[k]:u(p),"value"in p&&m.update(p.value),e&&!e[k]&&(e[k]=m),g.push(m))}return new Mapping(g,b,a["function"],d,a.parameter,f)}function n(a,b,c){var d=a.getParameter(b);d?d.update(c):(d=new Parameter(b,c),a.addParameter(d));return d}function m(a,b){for(var c=0;c<a.length;c++)if(a[c])for(var d=0;d<a[c].length;d++){var e=b["dymo"+c],f=b["dymo"+a[c][d]];e&&f&&e.addSimilar(f)}}function u(a){var c=a.type,d=a.name;if(c==ACCELEROMETER_X)return t(0);
...
Number.POSITIVE_INFINITY,!0);q[PLAY].addObserver(p);q[PART_INDEX].addObserver(p);q[PART_COUNT].addObserver(p);this.setType=function(a){b=a};this.getType=function(){return b};this.getUri=function(){return a};this.setParent=function(a){n=a;for(var b in q)b!=PLAY&&b!=PART_COUNT&&b!=PART_INDEX&&v.push(new Mapping([a.getParameter(b)],!0,void 0,[this],b));a=n.getMappings();var c=this.getDymoMap(),c=Object.keys(c).map(function(a){return c[a]});for(b=0;b<a.length;b++){var d=a[b].getDymoConstraint();d&&(d=
a[b].getTargets().concat(c.filter(d)),a[b].setTargets(d))}};this.removeParent=function(){for(var a=0;a<v.length;a++)v[a].disconnect();v=[];for(var b=n.getMappings(),c=this,a=0;a<b.length;a++){var d=b[a].getTargets(),d=d.filter(function(a){return!a.isSubDymoOf(c)});b[a].setTargets(d)}n=null};this.getParent=function(){return n};this.isSubDymoOf=function(a){for(var b=this;b;){if(b==a)return!0;b=b.getParent()}return!1};this.getDymoMap=function(){var a={};d(this,a);return a};this.getLevel=function(){return n?
...
;function FunctionTools(){}FunctionTools.IDENTITY_JSON={args:["a"],body:"return a;"};FunctionTools.inverted={};FunctionTools.created={};FunctionTools.createFunction=function(a,b){FunctionTools.created[b]||(FunctionTools.created[b]=Function.apply(null,a.concat(b)));return FunctionTools.created[b]};
...
e,f,g,k,h,p,n;k=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*k,h=e*k,f=2*e,p=f+a,n=f+d,c[n+0]=b[p+0]*h+this.pMidBuffer[f+0]*g,c[n+1]=b[p+1]*h+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
