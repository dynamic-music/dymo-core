...
b.currentTime;k.parameters[AMPLITUDE].setValueAtTime(k.parameters[AMPLITUDE].value,a);k.parameters[AMPLITUDE].linearRampToValueAtTime(0,a+FADE_LENGTH);t.stop(a+2*FADE_LENGTH)}function m(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function n(a,c,d){d=Math.min(a.length-c,d);for(var f=b.createBuffer(a.numberOfChannels,d,a.sampleRate),e=0;e<a.numberOfChannels;e++)for(var g=f.getChannelData(e),h=a.getChannelData(e),l=0;l<d;l++)g[l]=h[c+l];return f}function p(a,b,c,d){var f=a.lastIndexOf("/");f&&
(a=a.substring(f+1));u("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function u(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType="arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}DymoNode.call(this,a,b,d,e);var k=this,v,q,r,t=b.createBufferSource();t.connect(this.getInput());(function(){var b=a.getSegment();b[0]||(b[0]=0);!b[1]&&c&&
...
1))}}function g(b){var c=k.findFirstObjectUri(b,CDT),c=new DynamicMusicObject(b,c,a);c.setBasePath(v);r[b]=c;c.setSourcePath(k.findFirstObjectValue(b,HAS_SOURCE));for(var d=k.findAllObjectUris(b,HAS_FEATURE),f=0;f<d.length;f++)c.setFeature(k.findFirstObjectUri(d[f],TYPE),k.findFeatureValue(d[f]));d=k.findAllObjectUris(b,HAS_PARAMETER);for(f=0;f<d.length;f++)n(c,k.findFirstObjectUri(d[f],TYPE),k.findFirstObjectValue(d[f],VALUE));b=k.findParts(b);for(f=0;f<b.length;f++)c.addPart(g(b[f]));return c}function f(a){for(var b=
r[a],c=k.findAllObjectUris(a,HAS_SIMILAR),d=0;d<c.length;d++)b.addSimilar(r[c[d]]);c=k.findAllObjectUris(a,HAS_MAPPING);h(c);for(d=0;d<c.length;d++)b.addMapping(l(c[d],b));a=k.findParts(a);for(d=0;d<a.length;d++)f(a[d])}function h(a){for(var b=0;b<a.length;b++)for(var c=k.findAllObjectUris(a[b],HAS_DOMAIN_DIMENSION),d=0;d<c.length;d++){var f=k.findFirstObjectUri(c[d],TYPE),e=k.findFirstObjectValue(c[d],NAME);e||(e=c[d]);if(k.isSubclassOf(f,MOBILE_CONTROL)&&!q[c[d]]){var g=c[d],h=f,f=void 0;h==ACCELEROMETER_X||
h==ACCELEROMETER_Y||h==ACCELEROMETER_Z?f=new AccelerometerControl(h):h==TILT_X||h==TILT_Y?f=new TiltControl(h):h==GEOLOCATION_LATITUDE||h==GEOLOCATION_LONGITUDE?f=new GeolocationControl(h):h==GEOLOCATION_DISTANCE?f=new DistanceControl:h==COMPASS_HEADING?f=new CompassControl:h==BEACON?(f=k.findFirstObjectValue(g,HAS_UUID),e=k.findFirstObjectValue(g,HAS_MAJOR),h=k.findFirstObjectValue(g,HAS_MINOR),f=new BeaconControl(f,e,h)):h==SLIDER||h==TOGGLE||h==BUTTON||h==CUSTOM_CONTROL?(f=new Control(e,h),e=k.findFirstObjectValue(g,
HAS_INITIAL_VALUE),f.update(e)):h==RANDOM?f=new RandomControl:h==BROWNIAN?(e=k.findFirstObjectValue(g,HAS_INITIAL_VALUE),f=new BrownianControl(e)):h==RAMP&&(f=Math.round(1E3*k.findFirstObjectUri(g,HAS_DURATION)),e=k.findFirstObjectValue(g,HAS_INITIAL_VALUE),f=new RampControl(f,e));k.findFirstObjectValue(g,IS_SMOOTH)&&f.setSmooth&&f.setSmooth(!0);g=k.findFirstObjectValue(g,IS_AVERAGE_OF);!isNaN(g)&&f.setAverageOf&&f.setAverageOf(g);q[c[d]]=f}}}function l(b,c){var d=k.findAllObjectUris(b,TO_TARGET);
if(0<d.length){var f=[],e=k.findFunction(d[0]);if(e)d=Object.keys(r).map(function(a){return r[a]}),Array.prototype.push.apply(f,d.filter(e));else for(var g=0;g<d.length;g++)k.findFirstObjectUri(d[g],TYPE)==DYMO?f.push(r[d[g]]):f.push(q[d[g]]);return m(b,c,f,e)}return m(b,c,[a])}function m(a,b,c,d){for(var f=k.findFirstObjectUri(a,IS_RELATIVE),e=[],g=k.findAllObjectUris(a,HAS_DOMAIN_DIMENSION),h=0;h<g.length;h++){var l=k.findFirstObjectUri(g[h],TYPE),m=k.findFirstObjectValue(g[h],NAME);m||(m=g[h]);
l==FEATURE_TYPE||k.isSubclassOf(l,FEATURE_TYPE)?e.push(m):l==PARAMETER_TYPE||k.isSubclassOf(l,PARAMETER_TYPE)?(l=b?n(b,m):new Parameter(m,0),e.push(l)):e.push(q[g[h]])}g=$jscomp.makeIterator(k.findArgsAndBody(k.findFirstObjectUri(a,HAS_FUNCTION)));b=g.next().value;g=g.next().value;a=k.findFirstObjectUri(a,HAS_RANGE);for(h=0;h<c.length;h++)c[h]instanceof DynamicMusicObject&&!c[h].getParameter(a)&&n(c[h],a);return new Mapping(e,f,{args:b,body:g},c,a,d)}function n(a,b,c){var d=a.getParameter(b);d?d.update(c):
(isNaN(c)&&(c=k.findFirstObjectValue(b,HAS_STANDARD_VALUE,null),isNaN(c)&&(c=0)),d=k.findFirstObjectValue(b,IS_INTEGER,null),d=new Parameter(b,c,d),a.addParameter(d));return d}function p(a){return a==SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):a==ONE_SHOT_NAVIGATOR?new OneShotNavigator(void 0):a==REPEATED_NAVIGATOR?new RepeatedNavigator(void 0):new SequentialNavigator(void 0)}var u=this,k=new DymoStore(b),v="",q={},r={};this.setStore=function(a){k=a};this.getStore=function(){return k};this.loadDymoFromJson=
function(a,b){var d=a.lastIndexOf("/")+1;v=a.substring(0,d);c(a,function(){b(u.createDymoFromStore())})};this.parseDymoFromJson=function(a,b){k.loadData(a,!0,function(){b(u.createDymoFromStore())})};this.loadRenderingFromJson=function(a,b){c(a,function(){b(u.createRenderingFromStore())})};this.loadGraphFromJson=function(a,b){d(a,"",function(a){for(var c=0;c<a.length;c++)if(a[c])for(var d=0;d<a[c].length;d++){var f=r[CONTEXT_URI+"dymo"+c],e=r[CONTEXT_URI+"dymo"+a[c][d]];f&&e&&f.addSimilar(e)}b(void 0)})};
this.createDymoFromStore=function(){for(var a=[],b=k.findTopDymos(),c=0;c<b.length;c++)a.push(g(b[c])),f(b[c]);return[a,r]};this.createRenderingFromStore=function(){var a=k.findFirstSubjectUri(TYPE,RENDERING),b=new Rendering(r[k.findFirstObjectUri(a,HAS_DYMO)]),c=k.findAllObjectUris(a,HAS_MAPPING);h(c);for(var d=0;d<c.length;d++)b.addMapping(l(c[d]));a=k.findAllObjectUris(a,HAS_NAVIGATOR);for(d=0;d<a.length;d++)c=k.findFirstObjectUri(a[d],NAV_DYMOS),c=k.findFunction(c),b.addSubsetNavigator(c,p(k.findFirstObjectUri(a[d],
TYPE)));return[b,q]}};function DymoStore(a){function b(a,b,d){g.triplesToRdf(a,function(a){c(a,b,d)})}function c(a,b,c){a=a.split("_b").join("b");jsonld.fromRDF(a,{format:"application/nquads"},function(a,d){jsonld.frame(d,{"@id":b},function(a,b){jsonld.compact(b,"http://tiny.cc/dymo-context",function(a,b){jsonld.compact(b,"http://tiny.cc/dymo-context-simple",function(a,b){e(b);b=JSON.stringify(b,null,2);b=b.replace(/http:\/\/tiny.cc\/dymo-context\//g,"");b=b.replace("http://tiny.cc/dymo-context-simple","http://tiny.cc/dymo-context");
...
b.getParts()[c],d)}function h(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var f=b.getSimilars()[e];c[f.getUri()]||(c[f.getUri()]=f.toFlatJson());a.links.push(l(d,c[f.getUri()]))}for(e=0;e<b.getParts().length;e++)h(a,b.getParts()[e],c)}function l(a,b){return{source:a,target:b,value:1}}var m=this,n=null,p=[],u=[],k={},v={},q=[],r=[],t="",y;this.setType=function(a){b=a};this.getType=function(){return b};
...
this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};h(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=b);y&&(c.source=y);for(var d in k)c[d]=this.getFeatureJson(d);if(0<q.length)for(c.mappings=[],d=0;d<q.length;d++)c.mappings.push(q[d].toJson());0<p.length&&(c.parts=[]);if(0<u.length)for(c.similars=[],d=0;d<u.length;d++)c.similars.push(u[d].getUri());return c};this.getFeatureJson=function(a){return{value:k[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+
