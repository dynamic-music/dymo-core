...
TO_DYMO="http://tiny.cc/mobile-audio-ontology#toDymo";function DymoManager(a,b,c,d,e){function g(a,b){m=b[0];m.dymos=a[0];l=a[1];for(var e in b[1]){var g=b[1][e];h.isSubclassOf(g.getType(),UI_CONTROL)&&(n[e]=new UIControl(g,d));h.isSubclassOf(g.getType(),SENSOR_CONTROL)&&(p[e]=g)}c||(c="bower_components/dymo-core/audio/impulse_rev.wav");f.initEffectSends(h,c)}var f=new Scheduler(a,e);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var h,l,m,n={},p={};this.loadDymoAndRenderingFromStore=function(a,b){h=a;var c=new DymoLoader(f);c.setStore(h);g(c.createDymoFromStore(),
c.createRenderingFromStore());f.loadBuffers(m.dymos,b)};this.loadDymoAndRendering=function(a,b,c,d){var e=new DymoLoader(f,function(){e.loadDymoFromJson(a,function(a){e.loadRenderingFromJson(b,function(b){h=e.getStore();g(a,b);f.loadBuffers(m.dymos,d);c&&c()})})})};this.loadDymoFromJson=function(a,b){var c=new DymoLoader(f,function(){c.loadDymoFromJson(a,function(a){b&&b(a[0])})})};this.parseDymoFromJson=function(a,b){var c=new DymoLoader(f,function(){c.parseDymoFromJson(a,function(a){b(a[0])})})};
this.replacePartOfTopDymo=function(a,b){var c=m.dymos[0].getPart(a);m.dymos[0].replacePart(a,b);f.stop(c)};this.updateNavigatorPosition=function(a,b,c){f.updateNavigatorPosition(a,b,c)};this.getNavigatorPosition=function(a,b){return f.getNavigatorPosition(a,b)};this.syncNavigators=function(a,b,c){f.syncNavigators(a,b,c)};this.startPlayingUri=function(a){f.play(l[a])};this.stopPlayingUri=function(a){f.stop(l[a])};this.startPlaying=function(){for(var a=0;a<m.dymos.length;a++)m.dymos[a].updatePartOrder(ONSET),
f.play(m.dymos[a])};this.stopPlaying=function(){for(var a=0;a<m.dymos.length;a++)f.stop(m.dymos[a])};this.getTopDymo=function(){return m.dymos[0]};this.getRendering=function(){return m};this.getUIControls=function(){return n};this.getUIControl=function(a){return n[a]};this.getSensorControls=function(){return p}};var URI_TO_TERM={"http://tiny.cc/dymo-ontology#LevelFeature":"level","http://tiny.cc/dymo-ontology#IndexFeature":"index","http://tiny.cc/dymo-ontology#OnsetFeature":"onset","http://tiny.cc/dymo-ontology#DurationFeature":"duration","http://tiny.cc/dymo-ontology#TimeFeature":"time","http://tiny.cc/dymo-ontology#SegmentLabelFeature":"segmentLabel","http://tiny.cc/dymo-ontology#hasSource":"source","http://tiny.cc/dymo-ontology#hasParameter":"parameters","http://tiny.cc/dymo-ontology#hasFeature":"features",
...
e}}}function l(b,c){var d=k.findAllObjectUris(b,TO_TARGET);if(0<d.length){var e=[],f=k.findFunction(d[0]);if(f)d=Object.keys(q).map(function(a){return q[a]}),Array.prototype.push.apply(e,d.filter(f));else for(var h=0;h<d.length;h++)k.findFirstObjectUri(d[h],TYPE)==DYMO?e.push(q[d[h]]):e.push(u[d[h]]);return m(b,c,e,f)}return m(b,c,[a])}function m(a,b,c,d){for(var e=k.findFirstObjectUri(a,IS_RELATIVE),f=[],h=k.findAllObjectUris(a,HAS_DOMAIN_DIMENSION),g=0;g<h.length;g++){var l=k.findFirstObjectUri(h[g],
TYPE),m=k.findFirstObjectValue(h[g],NAME);m||(m=h[g]);l==FEATURE_TYPE||k.isSubclassOf(l,FEATURE_TYPE)?f.push(m):l==PARAMETER_TYPE||k.isSubclassOf(l,PARAMETER_TYPE)?(l=b?n(b,m):new Parameter(m,0),f.push(l)):f.push(u[h[g]])}h=$jscomp.makeIterator(k.findArgsAndBody(k.findFirstObjectUri(a,HAS_FUNCTION)));b=h.next().value;h=h.next().value;a=k.findFirstObjectUri(a,HAS_RANGE);for(g=0;g<c.length;g++)c[g]instanceof DynamicMusicObject&&!c[g].getParameter(a)&&n(c[g],a);return new Mapping(f,e,{args:b,body:h},
c,a,d)}function n(a,b,c){var d=a.getParameter(b);d?d.update(c):(isNaN(c)&&(c=k.findFirstObjectValue(b,HAS_STANDARD_VALUE,null),isNaN(c)&&(c=0)),d=k.findFirstObjectValue(b,IS_INTEGER,null),d=new Parameter(b,c,d),a.addParameter(d));return d}function p(a){return a==SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):new SequentialNavigator(void 0)}var k=new DymoStore(b),t="",u={},q={};this.setStore=function(a){k=a};this.getStore=function(){return k};this.loadDymoFromJson=function(a,b){var d=a.lastIndexOf("/")+
...
TYPE,DYMO);b&&this.addPart(b,a);c&&this.addPart(a,c);d&&this.addTriple(a,HAS_SOURCE,N3.Util.createLiteral(d))};this.addPart=function(a,b){this.addObjectToList(a,HAS_PART,b)};this.addSimilar=function(a,b){this.addTriple(a,HAS_SIMILAR,b)};this.setFeature=function(a,b,c){var d=this.findFirstObjectUriOfType(a,HAS_FEATURE,b);d?this.removeTriple(d,VALUE):(d=this.createBlankNode(),this.addTriple(a,HAS_FEATURE,d),this.addTriple(d,TYPE,b));if(Array.isArray(c))for(a=0;a<c.length;a++)this.addObjectToList(d,
VALUE,N3.Util.createLiteral(c[a]));else this.addTriple(d,VALUE,N3.Util.createLiteral(c))};this.findTopDymos=function(){var a=this.findAllSubjectUris(TYPE,DYMO),b=this.findAllSubjectUris(HAS_PART),c=[].concat.apply([],b.map(function(a){return g.findParts(a)}));return a.filter(function(a){return 0>c.indexOf(a)})};this.findParts=function(a){return this.findObjectListUris(a,HAS_PART)};this.findParents=function(a){var b=this.findContainingLists(a);return b[0].filter(function(a,c){return b[1][c]==HAS_PART})};
this.findAllObjectsInHierarchy=function(a){var b=[a];a=this.findParts(a);for(var c=0;c<a.length;c++)b=b.concat(this.findAllObjectsInHierarchy(a[c]));return b};this.findFunction=function(a){var b=$jscomp.makeIterator(this.findArgsAndBody(a));a=b.next().value;b=b.next().value;if(a&&b)return Function.apply(null,a.concat(b))};this.findArgsAndBody=function(a){var b=this.findAllObjectValues(a,HAS_ARGUMENT);a=this.findFirstObjectValue(a,HAS_BODY);return[b,a]};this.findFeature=function(a,b){var c=this.findFirstObjectUriOfType(a,
HAS_FEATURE,b);if(c)return this.findFirstObjectValue(c,VALUE)};this.findAllFeatureValues=function(a){var b=[];a=this.findAllObjectUris(a,HAS_FEATURE);for(var c=0;c<a.length;c++)b.push(this.findFirstObjectValue(a[c],VALUE));return b};this.getLevel=function(a){var b=0;for(a=this.findParents(a)[0];a;)b++,a=this.findParents(a)[0];return b};this.getMaxLevel=function(){for(var a=this.findAllSubjectUris(TYPE,DYMO),b=0,c=0;c<a.length;c++)b=Math.max(b,this.getLevel(a[c]));return b};this.toJsonld=function(a){var b=
this.findTopDymos()[0];this.toRdf(function(d){c(d,b,a)})};this.toJsonGraph=function(a,b,c){var e={nodes:[],links:[]},n={},p=this.findAllSubjectUris(TYPE,a),k=this.find(null,b,null);async.map(p,d,function(d,u){e.nodes=u;for(var q=0;q<p.length;q++)n[p[q]]=e.nodes[q];e.links=[];for(q=0;q<k.length;q++)if(0==g.find(k[q].object,TYPE,a).length){if(0<g.find(k[q].object,FIRST).length){var v=g.findObjectListUris(k[q].subject,b),v=v.map(function(a){return{source:n[k[q].subject],target:n[a],value:1}});e.links=
e.links.concat(v)}}else e.links.push({source:n[k[q].subject],target:n[k[q].object],value:1});c(e)})}}inheritPrototype(DymoStore,EasyStore);function DymoWriter(a,b){function c(a,b){var c=new XMLHttpRequest;c.send(b);c.addEventListener("save",function(){console.log("saved "+a)});c.addEventListener("error",function(){console.log("saving "+a+" failed")});c.open("POST",a);c.send()}this.writeToJsonld=function(b,e){e||(e="dymo.json");a.toJsonld(function(a){c(b+e,a)})}};function EasyStore(){function a(a,b,c){b?jsonld.toRDF(a,{format:"application/nquads"},function(a,b){c(b)}):c(a)}function b(a){var b=N3.Util.getLiteralValue(a);"http://www.w3.org/2001/XMLSchema#string"!=N3.Util.getLiteralType(a)&&(b=Number(b));return b}var c=this,d=N3.Store();this.addTriple=function(a,b,c){return d.addTriple(a,b,c)};this.removeTriple=function(a,b,c){c||(c=this.findFirstObjectUri(a,b));return d.removeTriple(a,b,c)};this.createBlankNode=function(){return d.createBlankNode()};this.addObjectToList=
function(a,b,c){var d=this.findFirstObjectUri(a,b),l=this.createBlankNode();if(d){a=d;for(b=this.findFirstObjectUri(a,REST);b!=NIL;)a=b,b=this.findFirstObjectUri(a,REST);this.removeTriple(a,REST,NIL);this.addTriple(a,REST,l)}else this.addTriple(a,b,l);this.addTriple(l,FIRST,c);this.addTriple(l,REST,NIL)};this.loadData=function(b,c,f){a(b,c,function(a){N3.Parser().parse(a,function(a,b,c){b?d.addTriple(b):f&&f()})})};this.loadFileIntoStore=function(a,b,d){loadFile(a,function(a){c.loadData(a,b,d)})};
this.find=function(a,b,c){return d.find(a,b,c)};this.findFirstObjectUri=function(a,b){var c=d.find(a,b);if(0<c.length)return c[0].object};this.findFirstObjectUriOfType=function(a,b,c){a=d.find(a,b).map(function(a){return a.object});a=a.filter(function(a){return 0<d.find(a,TYPE,c).length});if(0<a.length)return a[0]};this.findFirstSubjectUri=function(a,b){var c=d.find(null,a,b);if(0<c.length)return c[0].subject};this.findFirstObjectValue=function(a,c){var d=this.findFirstObjectUri(a,c);if(d)return this.isList(d)?
this.findObjectListUris(a,c).map(function(a){return b(a)}):b(d)};this.findAllSubjectUris=function(a,b){return d.find(null,a,b).map(function(a){return a.subject})};this.findAllObjectUris=function(a,b){return d.find(a,b,null).map(function(a){return a.object})};this.findAllObjectValues=function(a,b){return this.findAllObjectUris(a,b).map(function(a){return N3.Util.getLiteralValue(a)})};this.findObjectListUris=function(a,b){var c=[],d=this.findFirstObjectUri(a,b);if(d)for(c.push(this.findFirstObjectUri(d,
FIRST)),d=this.findFirstObjectUri(d,REST);d!=NIL;)c.push(this.findFirstObjectUri(d,FIRST)),d=this.findFirstObjectUri(d,REST);return c};this.findContainingLists=function(a){var b=[],c=[];a=this.findAllSubjectUris(FIRST,a);for(var d=0;d<a.length;d++){for(var l=a[d],m=this.findFirstSubjectUri(REST,l);m;)l=m,m=this.findFirstSubjectUri(REST,l);l=this.find(null,null,l)[0];b[d]=l.subject;c[d]=l.predicate}return[b,c]};this.recursiveFindAllTriplesExcept=function(a,b){for(var c=d.find(a,null,null),h=[],l=c.length-
1;0<=l;l--)0<d.find(c[l].object,TYPE,b).length?c.splice(l,1):h=h.concat(this.recursiveFindAllTriplesExcept(c[l].object,b));return c.concat(h)};this.isList=function(a){return void 0!=this.findFirstObjectUri(a,FIRST)};this.isSubclassOf=function(a,b){for(var c=this.findFirstObjectUri(a,RDFS_URI+"subClassOf");c;){if(c==b)return!0;c=this.findFirstObjectUri(c,RDFS_URI+"subClassOf")}return!1};this.toRdf=function(a){var b=d.find(null,null,null);this.triplesToRdf(b,a)};this.triplesToRdf=function(a,b){for(var c=
N3.Writer({format:"application/nquads"}),d=0;d<a.length;d++)c.addTriple(a[d]);c.end(function(a,c){b(c)})};this.logData=function(){var a=d.find(null).map(function(a){return a.subject+"\n"+a.predicate+"\n"+a.object}),b;for(b in a)console.log(a[b])}};function Control(a,b,c){this.name=a;this.type=b;this.parameters=c;this.mappings=[]}Control.prototype.getName=function(){return this.name};Control.prototype.getParameter=function(a){if(this.parameters)return this.parameters[a]};Control.prototype.getValue=function(){return this.value};Control.prototype.getType=function(){return this.type};Control.prototype.setUpdateFunction=function(a){this.updateFunction=a};
...
e,g,f,h,l,m,n;h=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)f=(this.overlapLength-e)*h,l=e*h,g=2*e,m=g+a,n=g+d,c[n+0]=b[m+0]*l+this.pMidBuffer[g+0]*f,c[n+1]=b[m+1]*l+this.pMidBuffer[g+1]*f},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
