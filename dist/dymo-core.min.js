...
"http://tiny.cc/mobile-audio-ontology#hasUuid",HAS_MAJOR="http://tiny.cc/mobile-audio-ontology#hasMajor",HAS_MINOR="http://tiny.cc/mobile-audio-ontology#hasMinor",HAS_DURATION="http://tiny.cc/mobile-audio-ontology#hasDuration",HAS_NAVIGATOR="http://tiny.cc/mobile-audio-ontology#hasNavigator",TO_DYMO="http://tiny.cc/mobile-audio-ontology#toDymo";function DymoManager(a,b,c,d){function e(a,b){l=b[0];l.dymos=a[0];h=a[1];for(var e in b[1]){var u=b[1][e];g.isSubclassOf(u.getType(),UI_CONTROL)&&(p[e]=new UIControl(u,d));g.isSubclassOf(u.getType(),SENSOR_CONTROL)&&(n[e]=u)}c||(c="bower_components/dymo-core/audio/impulse_rev.wav");f.initEffectSends(g,c)}var f=new Scheduler(a);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var g,h,l,p={},n={};this.loadDymoAndRenderingFromStore=function(a,b){g=a;var c=new DymoLoader(f);c.setStore(g);e(c.createDymoFromStore(),c.createRenderingFromStore());
f.loadBuffers(l.dymos,b)};this.loadDymoAndRendering=function(a,b,c,d){var h=new DymoLoader(f,function(){h.loadDymoFromJson(a,function(a){h.loadRenderingFromJson(b,function(b){g=h.getStore();e(a,b);f.loadBuffers(l.dymos,d);c&&c()})})})};this.loadDymoFromJson=function(a,b){var c=new DymoLoader(f,function(){c.loadDymoFromJson(a,function(a){b&&b(a[0])})})};this.parseDymoFromJson=function(a,b){var c=new DymoLoader(f,function(){c.parseDymoFromJson(a,function(a){b(a[0])})})};this.replacePartOfTopDymo=function(a,
b){var c=l.dymos[0].getPart(a);l.dymos[0].replacePart(a,b);f.stop(c)};this.updateNavigatorPosition=function(a,b,c){f.updateNavigatorPosition(a,b,c)};this.getNavigatorPosition=function(a,b){return f.getNavigatorPosition(a,b)};this.syncNavigators=function(a,b,c){f.syncNavigators(a,b,c)};this.startPlayingUri=function(a){f.play(h[a])};this.stopPlayingUri=function(a){f.stop(h[a])};this.startPlaying=function(){l.dymos[0].updatePartOrder(ONSET);f.play(l.dymos[0])};this.stopPlaying=function(){f.stop(l.dymos[0])};
this.getTopDymo=function(){return l.dymos[0]};this.getRendering=function(){return l};this.getUIControls=function(){return p};this.getUIControl=function(a){return p[a]};this.getSensorControls=function(){return n}};var URI_TO_TERM={"http://tiny.cc/dymo-ontology#LevelFeature":"level","http://tiny.cc/dymo-ontology#IndexFeature":"index","http://tiny.cc/dymo-ontology#OnsetFeature":"onset","http://tiny.cc/dymo-ontology#DurationFeature":"duration","http://tiny.cc/dymo-ontology#TimeFeature":"time","http://tiny.cc/dymo-ontology#SegmentLabelFeature":"segmentLabel","http://tiny.cc/dymo-ontology#hasSource":"source","http://tiny.cc/dymo-ontology#hasParameter":"parameters","http://tiny.cc/dymo-ontology#hasFeature":"features",
...
DymoNode.prototype.removeAndDisconnect=function(){for(var a in this.parameters){var b=this.dymo.getParameter(a);b&&b.removeObserver(this)}this.dryGain.disconnect();this.reverbGain&&this.reverbGain.disconnect();this.delayGain&&this.delayGain.disconnect()};function AudioProcessor(a){function b(a){return{extract:function(b,e,f){for(var g=[],h=0;h<a.numberOfChannels;h++)g.push(a.getChannelData(h));for(h=0;h<e;h++)for(var l=0;l<g.length;l++)b[h*g.length+l%g.length]=g[l][h+f];return Math.min(e,g[0].length-f)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var f=b(c),e=new SimpleFilter(f,e),f=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),g=[],h=0;h<c.numberOfChannels;h++)g.push(f.getChannelData(h));for(var l=
new Float32Array(2048),p=e.extract(l,1024),n=0;p;){for(h=0;h<p;h++)for(var m=0;m<g.length;m++)g[m][h+n]=l[h*g.length+m%g.length];n+=p;p=e.extract(l,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var g=0;g<e.length;g++)a[f*e.length+g%e.length]=e[g][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),h=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(h,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=h[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b){function c(){0<n&&n--;0==n&&m&&m()}function d(a){h.splice(h.indexOf(a),1)}function e(){for(var a=[],c=0;c<h.length;c++)for(var d=$jscomp.makeIterator(h[c].getAllSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();l=a;b&&b()}function f(b,c){n++;var d=new XMLHttpRequest;d.open("GET",b,!0);d.responseType="arraybuffer";d.onload=function(){a.decodeAudioData(d.response,function(a){c(a,b)})};d.error=
function(){};d.send()}var g={},h=[],l=[];this.listenerOrientation=new Parameter(LISTENER_ORIENTATION,0);this.listenerOrientation.addObserver(this);var p,n=0,m;this.initEffectSends=function(b,d){0<b.find(null,null,REVERB).length&&(p=a.createConvolver(),p.connect(a.destination),f(d,function(a){p.buffer=a;c()}));if(0<b.find(null,null,DELAY).length){var e=a.createDelay();e.delayTime.value=.5;e.connect(a.destination);var g=a.createGain();g.gain.value=.6;e.connect(g);g.connect(e)}};this.loadBuffers=function(a,
b){m=b;for(var d=[],e=0,h=a.length;e<h;e++)d=d.concat(a[e].getAllSourcePaths());e=0;for(h=d.length;e<h;e++)g[d[e]]||f(d[e],function(a,b){g[b]=a;c()})};this.getBuffer=function(a){return g[a.getSourcePath()]};this.getParameter=function(a){if(a==LISTENER_ORIENTATION)return this.listenerOrientation};this.updateNavigatorPosition=function(a,b,c){for(var d=0,e=h.length;d<e;d++)h[d].hasDymo(a)&&h[d].getNavigator().setPosition(c,b,a)};this.getNavigatorPosition=function(a,b){for(var c=0,d=h.length;c<d;c++)if(h[c].hasDymo(a))return h[c].getNavigator().getPosition(b,
...
(b[1]=c.duration-b[0]);if(a.hasParameter(DURATION_RATIO)){var d=a.getParameter(DURATION_RATIO).getValue();0<d&&1>d&&(b[1]*=d)}v=b[1];d=a.hasParameter(TIME_STRETCH_RATIO)?a.getParameter(TIME_STRETCH_RATIO).getValue():1;r.buffer=g(b,d)})();this.addParameter(PLAYBACK_RATE,r.playbackRate);this.addParameter(TIME_STRETCH_RATIO,{value:0});this.addParameter(LOOP,{value:0});this.getDuration=function(){return v};this.isLoopingAndPlaying=function(){return r.loop&&u};this.play=function(a){r.onended=function(){r.disconnect();
t.removeAndDisconnect();f&&f(t)};a||(a=0);r.start(a);u=!0};this.pause=function(){u?(l(),q=!0):q&&(q=!1,this.play())};this.stop=function(){u&&l()}}inheritPrototype(DymoSource,DymoNode);function SchedulerThread(a,b,c,d,e,f,g,h){function l(){y=r?r:t();p(y);if(!z){var a=y.keys().next().value;a.hasParameter(ONSET)&&(z=a.getParameter(ONSET).getValue())}a=w?w-c.currentTime:SCHEDULE_AHEAD_TIME;for(var b=c.currentTime+a,d=$jscomp.makeIterator(y.values()),e=d.next();!e.done;e=d.next())e.value.play(b);setTimeout(function(){g()},a);(r=t())&&0<r.size?(w=k(b),a=w[1],w=w[0],a.getDymo().hasParameter(LOOP)&&a.getDymo().getParameter(LOOP).getValue()&&(w-=FADE_LENGTH),a=1E3*(w-c.currentTime-SCHEDULE_AHEAD_TIME),
x=setTimeout(function(){l()},a)):(w=k(b),a=1E3*(w-c.currentTime),setTimeout(function(){m()},a+100))}function p(a){for(var b=$jscomp.makeIterator(a.keys()),c=b.next();!c.done;c=b.next())c=c.value,q.get(c)||q.set(c,[]),q.get(c).push(a.get(c))}function n(a){var b=q.get(a.getDymo());b.splice(b.indexOf(a),1);0>=b.length&&q.delete(a.getDymo());g();m()}function m(){if(0==q.size&&(!r||0==r.size)){window.clearTimeout(x);b.reset();for(var c=a.getAllDymosInHierarchy(),d=0,e=c.length;d<e;d++){var f=v.get(c[d]);
f&&f.removeAndDisconnect()}h&&h()}}function k(a){if(r){var b=r.keys().next().value,c;b.hasParameter(ONSET)&&(c=b.getParameter(ONSET).getValue());b=c-z;z=c;if(!isNaN(c))return a+b}c=0;for(var d,b=$jscomp.makeIterator(y.values()),e=b.next();!e.done;e=b.next()){var e=e.value,f=e.getDuration();f>c&&(c=f,d=e)}return[a+c,d]}function t(){var a=b.getNextParts();if(a){console.log(a.map(function(a){return isNaN(a.getIndex())?"top":a.getIndex()}));for(var y=new Map,g=0;g<a.length;g++)if(a[g].getSourcePath()){var h=
d[a[g].getSourcePath()],h=new DymoSource(a[g],c,h,e,f,n);u(h);y.set(a[g],h)}return y}}function u(a){var b=a;for(a=a.getDymo().getParent();a;){if(v.has(a)){b.connect(v.get(a).getInput());return}var d=new DymoNode(a,c,e,f);b.connect(d.getInput());v.set(a,d);a=a.getParent();b=d}b.connect(c.destination)}var q=new Map,v=new Map,r,x,y=[],w,z;b||(b=new DymoNavigator(a,new SequentialNavigator(a)));l();this.hasDymo=function(b){if(a.getAllDymosInHierarchy().indexOf(b))return!0};this.getNavigator=function(){return b};
this.stop=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=q.get(a[b]);if(d)for(var e=0;e<d.length;e++)d[e].stop();r&&(r.delete(a[b]),0>=r.size&&(r=void 0))}};this.getAllSources=function(){return q};this.getSources=function(a){return q.get(a)}};function AutoControl(a,b,c){function d(a){var b=a.getName();f[b]=a;f[b].addObserver(e)}var e=this,f={};d(new Parameter(AUTO_CONTROL_FREQUENCY,100));d(new Parameter(AUTO_CONTROL_TRIGGER,0));Control.call(this,a,AUTO_CONTROL,f);var g;this.addParameter=d;this.getParameter=function(a){return f[a]};this.startUpdate=function(){g=setInterval(b,f[AUTO_CONTROL_FREQUENCY].getValue())};this.reset=function(){clearInterval(g);g=null;c&&c()};this.observedParameterChanged=function(a){a==f[AUTO_CONTROL_FREQUENCY]?
...
1))}}function f(b){var c=k.findFirstObjectUri(b,CDT),c=new DynamicMusicObject(b,c,a);c.setBasePath(t);q[b]=c;c.setSourcePath(k.findFirstObjectValue(b,HAS_SOURCE));for(var d=k.findAllObjectUris(b,HAS_FEATURE),e=0;e<d.length;e++)c.setFeature(d[e],k.findFirstObjectValue(d[e],VALUE));d=k.findAllObjectUris(b,HAS_PARAMETER);for(e=0;e<d.length;e++)n(c,k.findFirstObjectUri(d[e],TYPE),k.findFirstObjectValue(d[e],VALUE));b=k.findAllObjectUris(b,HAS_PART);for(e=0;e<b.length;e++)c.addPart(f(b[e]));return c}function g(a){for(var b=
q[a],c=k.findAllObjectUris(a,HAS_SIMILAR),d=0;d<c.length;d++)b.addSimilar(q[c[d]]);c=k.findAllObjectUris(a,HAS_MAPPING);h(c);for(d=0;d<c.length;d++)b.addMapping(l(c[d],b));a=k.findAllObjectUris(a,HAS_PART);for(d=0;d<a.length;d++)g(a[d])}function h(a){for(var b=0;b<a.length;b++)for(var c=k.findAllObjectUris(a[b],HAS_DOMAIN_DIMENSION),d=0;d<c.length;d++){var e=k.findFirstObjectUri(c[d],TYPE),f=k.findFirstObjectValue(c[d],NAME);f||(f=c[d]);if(k.isSubclassOf(e,MOBILE_CONTROL)&&!u[c[d]]){var g=c[d],h=
e,e=void 0;h==ACCELEROMETER_X||h==ACCELEROMETER_Y||h==ACCELEROMETER_Z?e=new AccelerometerControl(h):h==TILT_X||h==TILT_Y?e=new TiltControl(h):h==GEOLOCATION_LATITUDE||h==GEOLOCATION_LONGITUDE?e=new GeolocationControl(h):h==GEOLOCATION_DISTANCE?e=new DistanceControl:h==COMPASS_HEADING?e=new CompassControl:h==BEACON?(e=k.findFirstObjectValue(g,HAS_UUID),f=k.findFirstObjectValue(g,HAS_MAJOR),h=k.findFirstObjectValue(g,HAS_MINOR),e=new BeaconControl(e,f,h)):h==SLIDER||h==TOGGLE||h==BUTTON||h==CUSTOM_CONTROL?
(e=new Control(f,h),f=k.findFirstObjectValue(g,HAS_INITIAL_VALUE),e.update(f)):h==RANDOM?e=new RandomControl:h==BROWNIAN?(f=k.findFirstObjectValue(g,HAS_INITIAL_VALUE),e=new BrownianControl(f)):h==RAMP&&(e=Math.round(1E3*k.findFirstObjectUri(g,HAS_DURATION)),f=k.findFirstObjectValue(g,HAS_INITIAL_VALUE),e=new RampControl(e,f));k.findFirstObjectValue(g,IS_SMOOTH)&&e.setSmooth&&e.setSmooth(!0);g=k.findFirstObjectValue(g,IS_AVERAGE_OF);!isNaN(g)&&e.setAverageOf&&e.setAverageOf(g);u[c[d]]=e}}}function l(b,
c){var d=k.findAllObjectUris(b,TO_TARGET);if(0<d.length){var e=[],f=k.findFunction(d[0]);if(f)d=Object.keys(q).map(function(a){return q[a]}),Array.prototype.push.apply(e,d.filter(f));else for(var g=0;g<d.length;g++)k.findFirstObjectUri(d[g],TYPE)==DYMO?e.push(q[d[g]]):e.push(u[d[g]]);return p(b,c,e,f)}return p(b,c,[a])}function p(a,b,c,d){for(var e=k.findFirstObjectUri(a,IS_RELATIVE),f=[],g=k.findAllObjectUris(a,HAS_DOMAIN_DIMENSION),h=0;h<g.length;h++){var l=k.findFirstObjectUri(g[h],TYPE),p=k.findFirstObjectValue(g[h],
NAME);p||(p=g[h]);l==FEATURE_TYPE||k.isSubclassOf(l,FEATURE_TYPE)?f.push(p):l==PARAMETER_TYPE||k.isSubclassOf(l,PARAMETER_TYPE)?(l=b?n(b,p):new Parameter(p,0),f.push(l)):f.push(u[g[h]])}g=$jscomp.makeIterator(k.findArgsAndBody(k.findFirstObjectUri(a,HAS_FUNCTION)));b=g.next().value;g=g.next().value;a=k.findFirstObjectUri(a,HAS_RANGE);for(h=0;h<c.length;h++)c[h]instanceof DynamicMusicObject&&!c[h].getParameter(a)&&n(c[h],a);return new Mapping(f,e,{args:b,body:g},c,a,d)}function n(a,b,c){var d=a.getParameter(b);
d?d.update(c):(isNaN(c)&&(c=k.findFirstObjectValue(b,HAS_STANDARD_VALUE,null),isNaN(c)&&(c=0)),d=k.findFirstObjectValue(b,IS_INTEGER,null),d=new Parameter(b,c,d),a.addParameter(d));return d}function m(a){return a==SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):new SequentialNavigator(void 0)}var k=new DymoStore(b),t="",u={},q={};this.setStore=function(a){k=a};this.getStore=function(){return k};this.loadDymoFromJson=function(a,b){var d=a.lastIndexOf("/")+1;t=a.substring(0,d);c(a,function(){b(this.createDymoFromStore())})};
this.parseDymoFromJson=function(a,b){k.loadData(a,!0,function(){b(this.createDymoFromStore())})};this.loadRenderingFromJson=function(a,b){c(a,function(){b(this.createRenderingFromStore())})};this.loadGraphFromJson=function(a,b){d(a,"",function(a){for(var c=0;c<a.length;c++)if(a[c])for(var d=0;d<a[c].length;d++){var e=q[CONTEXT_URI+"dymo"+c],f=q[CONTEXT_URI+"dymo"+a[c][d]];e&&f&&e.addSimilar(f)}b(void 0)})};this.createDymoFromStore=function(){for(var a=[],b=k.findTopDymos(),c=0;c<b.length;c++)a.push(f(b[c])),
g(b[c]);return[a,q]};this.createRenderingFromStore=function(){var a=k.findFirstSubjectUri(TYPE,RENDERING),b=new Rendering(q[k.findFirstObjectUri(a,HAS_DYMO)]),c=k.findAllObjectUris(a,HAS_MAPPING);h(c);for(var d=0;d<c.length;d++)b.addMapping(l(c[d]));if(a=k.findFirstObjectUri(a,HAS_NAVIGATOR))c=k.findFirstObjectUri(a,TO_DYMO),c=k.findFunction(c),b.addNavigator(m(k.findFirstObjectUri(a,TYPE)),c);return[b,u]}};function DymoStore(a){function b(a,b,d){f.triplesToRdf(a,function(a){c(a,b,d)})}function c(a,b,c){a=a.split("_b").join("b");jsonld.fromRDF(a,{format:"application/nquads"},function(a,d){jsonld.frame(d,{"@id":b},function(a,b){jsonld.compact(b,"http://tiny.cc/dymo-context",function(a,b){jsonld.compact(b,"http://tiny.cc/dymo-context-simple",function(a,b){e(b);b=JSON.stringify(b,null,2);b=b.replace(/http:\/\/tiny.cc\/dymo-context\//g,"");b=b.replace("http://tiny.cc/dymo-context-simple","http://tiny.cc/dymo-context");
c(b)})})})})}function d(a,c){var d=f.findFirstObjectUri(a,TYPE,null),d=f.recursiveFindAllTriplesExcept(a,d);b(d,a,function(a){c(null,JSON.parse(a))})}function e(a){if(a&&a instanceof Object)for(var b in a)"@id"==b&&a[b].includes("_:b")?delete a[b]:e(a[b])}var f=this;EasyStore.call(this);(function(){f.loadFileIntoStore("http://tiny.cc/dymo-ontology",!1,function(){f.loadFileIntoStore("http://tiny.cc/mobile-audio-ontology",!1,function(){a&&a()})})})();this.addDymo=function(a,b,c,d){this.addTriple(a,
TYPE,DYMO);b&&this.addTriple(b,HAS_PART,a);c&&this.addTriple(a,HAS_PART,c);d&&this.addTriple(a,HAS_SOURCE,N3.Util.createLiteral(d))};this.setFeature=function(a,b,c){var d=this.findFirstObjectUriOfType(a,HAS_FEATURE,b);d?this.removeTriple(d,VALUE):(d=this.createBlankNode(),this.addTriple(a,HAS_FEATURE,d),this.addTriple(d,TYPE,b));this.addTriple(d,VALUE,N3.Util.createLiteral(c))};this.findTopDymos=function(){var a=this.find(null,HAS_PART,null),b=this.findAllSubjectUris(TYPE,DYMO),c=Array.from(new Set(a.map(function(a){return a.object})),
function(a){return a});return b.filter(function(a){return 0>c.indexOf(a)})};this.findParts=function(a){return this.findAllObjectUris(a,HAS_PART)};this.findAllParts=function(a){var b=[a];a=this.findParts(a);for(var c=0;c<a.length;c++)b=b.concat(this.findAllParts(a[c]));return b};this.findFunction=function(a){var b=$jscomp.makeIterator(this.findArgsAndBody(a));a=b.next().value;b=b.next().value;if(a&&b)return Function.apply(null,a.concat(b))};this.findArgsAndBody=function(a){var b=this.findAllObjectValues(a,
HAS_ARGUMENT);a=this.findFirstObjectValue(a,HAS_BODY);return[b,a]};this.findFeature=function(a,b){var c=this.findFirstObjectUriOfType(a,HAS_FEATURE,b);if(c)return this.findFirstObjectValue(c,VALUE)};this.getLevel=function(a){for(var b=0,c=this.findFirstSubjectUri(HAS_PART,a);c;)b++,c=this.findFirstSubjectUri(HAS_PART,a);return b};this.toJsonld=function(a){var b=this.findTopDymos()[0];this.toRdf(function(d){c(d,b,a)})};this.toJsonGraph=function(a,b,c){var e={nodes:[],links:[]},f={},m=this.find(null,
TYPE,a).map(function(a){return a.subject}),k=this.find(null,b,null);async.map(m,d,function(a,b){e.nodes=b;for(var d=0;d<m.length;d++)f[m[d]]=e.nodes[d];e.links=k.map(function(a){return{source:f[a.subject],target:f[a.object],value:1}});c(e)})}}inheritPrototype(DymoStore,EasyStore);function DymoWriter(a,b){function c(a,b){var c=new XMLHttpRequest;c.send(b);c.addEventListener("save",function(){console.log("saved "+a)});c.addEventListener("error",function(){console.log("saving "+a+" failed")});c.open("POST",a);c.send()}this.writeToJsonld=function(b,e){e||(e="dymo.json");a.toJsonld(function(a){c(b+e,a)})}};function EasyStore(){function a(a,b,c){b?jsonld.toRDF(a,{format:"application/nquads"},function(a,b){c(b)}):c(a)}var b=this,c=N3.Store();this.addTriple=function(a,b,f){return c.addTriple(a,b,f)};this.removeTriple=function(a,b,f){f||(f=this.findFirstObjectUri(a,b));return c.removeTriple(a,b,f)};this.createBlankNode=function(){return c.createBlankNode()};this.loadData=function(b,e,f){a(b,e,function(a){N3.Parser().parse(a,function(a,b,d){b?c.addTriple(b):f&&f()})})};this.loadFileIntoStore=function(a,
c,f){loadFile(a,function(a){b.loadData(a,c,f)})};this.find=function(a,b,f){return c.find(a,b,f)};this.findFirstObjectUri=function(a,b){var f=c.find(a,b);if(0<f.length)return f[0].object};this.findFirstObjectUriOfType=function(a,b,f){a=c.find(a,b).map(function(a){return a.object});a=a.filter(function(a){return 0<c.find(a,TYPE,f).length});if(0<a.length)return a[0]};this.findFirstSubjectUri=function(a,b){var f=c.find(null,a,b);if(0<f.length)return f[0].subject};this.findFirstObjectValue=function(a,b){var c=
this.findFirstObjectUri(a,b);if(c){var g=N3.Util.getLiteralValue(c);"http://www.w3.org/2001/XMLSchema#string"!=N3.Util.getLiteralType(c)&&(g=Number(g));return g}};this.findAllSubjectUris=function(a,b){return c.find(null,a,b).map(function(a){return a.subject})};this.findAllObjectUris=function(a,b){return c.find(a,b,null).map(function(a){return a.object})};this.findAllObjectValues=function(a,b){return this.findAllObjectUris(a,b).map(function(a){return N3.Util.getLiteralValue(a)})};this.recursiveFindAllTriplesExcept=
function(a,b){for(var f=c.find(a,null,null),g=[],h=f.length-1;0<=h;h--)0<c.find(f[h].object,TYPE,b).length?f.splice(h,1):g=g.concat(this.recursiveFindAllTriplesExcept(f[h].object,b));return f.concat(g)};this.isSubclassOf=function(a,b){for(var c=this.findFirstObjectUri(a,RDFS_URI+"subClassOf");c;){if(c==b)return!0;c=this.findFirstObjectUri(c,RDFS_URI+"subClassOf")}return!1};this.toRdf=function(a){var b=c.find(null,null,null);this.triplesToRdf(b,a)};this.triplesToRdf=function(a,b){for(var c=N3.Writer({format:"application/nquads"}),
g=0;g<a.length;g++)c.addTriple(a[g]);c.end(function(a,c){b(c)})};this.logData=function(){var a=c.find(null).map(function(a){return a.subject+"\n"+a.predicate+"\n"+a.object}),b;for(b in a)console.log(a[b])}};function Control(a,b,c){this.name=a;this.type=b;this.parameters=c;this.mappings=[]}Control.prototype.getName=function(){return this.name};Control.prototype.getParameter=function(a){if(this.parameters)return this.parameters[a]};Control.prototype.getValue=function(){return this.value};Control.prototype.getType=function(){return this.type};Control.prototype.setUpdateFunction=function(a){this.updateFunction=a};
...
null};this.getParent=function(){return n};this.isSubDymoOf=function(a){for(var b=this;b;){if(b==a)return!0;b=b.getParent()}return!1};this.getMapOfDymosInSubtree=function(){var a={};d(this,a);return a};this.getLevel=function(){return n?n.getLevel()+1:0};this.getIndex=function(){if(n)return n.getParts().indexOf(this)};this.addPart=function(a){m.push(a);a.setParent(this)};this.removePart=function(a){m.splice(m.indexOf(a));a.removeParent(this)};this.replacePart=function(a,b){m[a]&&m[a].removeParent(this);
m[a]=b;b.setParent(this)};this.getPart=function(a){return m[a]};this.getParts=function(){return m};this.hasParts=function(){return 0<m.length};this.getNthPart=function(a,b){return e(a,0,b,this)};this.setBasePath=function(a){r=a};this.getBasePath=function(){return n&&!r?n.getBasePath():r};this.setSourcePath=function(a){x=a};this.getSourcePath=function(){if(n&&!x)return n.getSourcePath();if(x)return this.getBasePath()+x};this.getAllSourcePaths=function(){var a=[];f(this,a,function(b,c){var d=b.getSourcePath();
d&&0>a.indexOf(d)&&c.push(d)});return a};this.getAllDymosInHierarchy=function(){var a=[];f(this,a,function(a,b){b.push(a)});return a};this.addSimilar=function(a){k.push(a)};this.getSimilars=function(a){return k};this.getFeatures=function(){return t};this.setFeature=function(a,b){t[a]=b};this.getFeature=function(a){if(a===LEVEL_FEATURE)return this.getLevel();if(a===INDEX_FEATURE)return this.getIndex();if(t.hasOwnProperty(a))return t[a];if(n)return n.getFeature(a);for(var b=[],c=0;c<m.length;c++)if(m[c]){var d=
m[c].getFeatureWithoutLookingFurther(a);isNaN(d)||b.push(d)}if(0<b.length)return b};this.getFeatureWithoutLookingFurther=function(a){if(t[a])return t[a]};this.getMappings=function(){return q};this.getSegment=function(){return[this.getFeature(TIME_FEATURE),this.getFeature(DURATION_FEATURE)]};this.addParameter=function(a){u[a.getName()]=a};this.getParameter=function(a){return u[a]};this.hasParameter=function(a){return a in u};this.addMapping=function(a){q.push(a)};this.observedParameterChanged=function(a){a.getName()==
PLAY&&c&&(0<a.getChange()?c.play(p):c.stop(p))};this.updatePartOrder=function(a){m&&m[0]&&!isNaN(m[0].getFeature(a))?m.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):m&&m[0]&&m[0].hasParameter(a)&&m.sort(function(b,c){return b.getParameter(a).getValue()-c.getParameter(a).getValue()})};this.toJsonHierarchy=function(){for(var a=this.toFlatJson(),b=0;b<m.length;b++)a.parts.push(m[b].toJsonHierarchy());return a};this.toJsonHierarchyGraph=function(){var a={nodes:[],links:[]};g(a,this);return a};
this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};h(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=b);x&&(c.source=x);for(var d in t)c[d]=this.getFeatureJson(d);if(0<q.length)for(c.mappings=[],d=0;d<q.length;d++)c.mappings.push(q[d].toJson());0<m.length&&(c.parts=[]);if(0<k.length)for(c.similars=[],d=0;d<k.length;d++)c.similars.push(k[d].getUri());return c};this.getFeatureJson=function(a){return{value:t[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+
...
e,f,g,h,l,p,n;h=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*h,l=e*h,f=2*e,p=f+a,n=f+d,c[n+0]=b[p+0]*l+this.pMidBuffer[f+0]*g,c[n+1]=b[p+1]*l+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
