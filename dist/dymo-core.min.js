...
new Float32Array(2048),r=e.extract(k,1024),n=0;r;){for(l=0;l<r;l++)for(var m=0;m<g.length;m++)g[m][l+n]=k[l*g.length+m%g.length];n+=r;r=e.extract(k,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var q=0;q<e.length;q++)a[f*e.length+q%e.length]=e[q][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),l=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(l,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=l[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(){0<q&&q--;0==q&&b&&b(q)}function e(a){k.splice(k.indexOf(a),1)}function f(){for(var a=[],b=0;b<k.length;b++)for(var d=$jscomp.makeIterator(k[b].getAllSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();r=a;c&&c()}function g(b,c){q++;var e=new XMLHttpRequest;e.open("GET",b,!0);e.responseType="arraybuffer";e.onload=function(){a.decodeAudioData(e.response,function(a){c?c(a):(l[b]=
a,d())})};e.error=function(){};e.send()}var l={},k=[],r=[];this.listenerOrientation=new Parameter(LISTENER_ORIENTATION,0);this.listenerOrientation.addObserver(this);var n=a.createConvolver();n.connect(a.destination);var m=a.createDelay();m.delayTime.value=.5;m.connect(a.destination);var t=a.createGain();t.gain.value=.6;m.connect(t);t.connect(m);var q=0;this.setReverbFile=function(a){g(a,function(a){n.buffer=a;d()})};this.loadBuffers=function(a){a=a.getAllSourcePaths();for(var b=0,c=a.length;b<c;b++)l[a[b]]||
g(a[b])};this.getBuffer=function(a){return l[a.getSourcePath()]};this.getParameter=function(a){if(a==LISTENER_ORIENTATION)return this.listenerOrientation};this.updateNavigatorPosition=function(a,b,c){for(var d=0,e=k.length;d<e;d++)k[d].hasDymo(a)&&k[d].getNavigator().setPosition(c,b,a)};this.getNavigatorPosition=function(a,b){for(var c=0,d=k.length;c<d;c++)if(k[c].hasDymo(a))return k[c].getNavigator().getPosition(b,a)};this.syncNavigators=function(a,b,c){for(var d,e,h=0,f=k.length;h<f;h++)k[h].hasDymo(a)&&
(d=k[h].getNavigator()),k[h].hasDymo(b)&&(e=k[h].getNavigator());e&&(b=e.getPosition(c,b),d.setPosition(b+1,c,a))};this.play=function(b){b=new SchedulerThread(b,void 0,a,l,n,m,f,e);k.push(b)};this.pause=function(a){if(a)for(var b=0;b<k.length;b++)k[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<k.length;b++)k[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<k.length;c++){var d=k[c].getSources(a);if(d)for(var e=0;e<d.length;e++)b=b.concat(d)}return b};this.getUrisOfPlayingDymos=function(){return r};
this.observedParameterChanged=function(b){b=this.listenerOrientation.getValue()/180*Math.PI;a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0)}};function Source(a,b,c,d,e,f){function g(a,c){var d=b.createGain();d.connect(c);d.gain.value=0;a.connect(d);return d}function l(){var d=a.getParameter(TIME_STRETCH_RATIO).getValue();if(1!=d){if(0!=z||x<c.duration)c=t(c,m(z,c),m(x+.3,c));c=(new AudioProcessor(b)).timeStretch(c,d);d=x/d;c=t(c,0,m(d+.02,c));x=d}else if(0!=z||x<c.duration)c=t(c,m(z,c),m(x+.02,c));q(c,c.length);A.buffer=c;h[PLAYBACK_RATE]=A.playbackRate;k(PLAYBACK_RATE,a.getParameter(PLAYBACK_RATE))}function k(a,b){r(a,b.getValue());b.addObserver(u)}
function r(a,b,c){c&&(b+=u.getParameterValue(a));0>b&&0<=G.indexOf(a)&&(b=0);h[a]&&(h[a].value||0==h[a].value?h[a].value=b:h[a].setValue(b));0<=H.indexOf(a)&&(0==h[DISTANCE].value&&(h[DISTANCE].value=-.01),E.setPosition(h[PAN].value,h[HEIGHT].value,h[DISTANCE].value))}function n(){y=!1;var a=b.currentTime;h[AMPLITUDE].setValueAtTime(h[AMPLITUDE].value,a);h[AMPLITUDE].linearRampToValueAtTime(0,a+.02);A.stop(a+.04)}function m(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function t(a,c,d){d=Math.min(a.length-
c,d);for(var e=b.createBuffer(a.numberOfChannels,d,a.sampleRate),h=0;h<a.numberOfChannels;h++)for(var f=e.getChannelData(h),q=a.getChannelData(h),p=0;p<d;p++)f[p]=q[c+p];return e}function q(a,b){for(var c=.02*a.sampleRate,d=0;d<a.numberOfChannels;d++)for(var e=a.getChannelData(d),h=0;h<c;h++)e[h]*=h/c,e[b-h-1]*=h/c}function p(a,b,c,d){var e=a.lastIndexOf("/");e&&(a=a.substring(e+1));v("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function v(a,
c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType="arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}var u=this,y,w,h={},B=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER,PAN,HEIGHT,DISTANCE,LOOP],G=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER],H=[PAN,HEIGHT,DISTANCE],A=b.createBufferSource(),D=b.createGain();D.connect(b.destination);var F=g(D,d);d=g(D,e);
var E=b.createPanner();E.connect(D);e=b.createBiquadFilter();e.type="lowpass";e.frequency.value=2E4;e.connect(E);A.connect(e);var C=a.getSegment(),z=C[0],x=C[1];z||(z=0);!x&&c&&(x=c.duration-z);C=a.getParameter(DURATION_RATIO).getValue();0<C&&1>C&&(x*=C);console.log(C,x);c?l():p(a.getSourcePath(),z,z+x+.3,function(d){c=d;d=a.getParameter(TIME_STRETCH_RATIO).getValue();1!=d?(c=(new AudioProcessor(b)).timeStretch(c,d),d=x/d,c=t(c,0,m(d+.02,c)),x=d):c=t(c,0,m(x+.02,c));q(c,c.length);A.buffer=c});h[AMPLITUDE]=
D.gain;h[TIME_STRETCH_RATIO]={value:0};h[REVERB]=F.gain;h[DELAY]=d.gain;h[FILTER]=e.frequency;h[PAN]={value:0};h[HEIGHT]={value:0};h[DISTANCE]={value:0};h[LOOP]={value:0};k(AMPLITUDE,a.getParameter(AMPLITUDE));k(TIME_STRETCH_RATIO,a.getParameter(TIME_STRETCH_RATIO));k(REVERB,a.getParameter(REVERB));k(DELAY,a.getParameter(DELAY));k(FILTER,a.getParameter(FILTER));k(PAN,a.getParameter(PAN));k(HEIGHT,a.getParameter(HEIGHT));k(DISTANCE,a.getParameter(DISTANCE));k(LOOP,a.getParameter(LOOP));this.observedParameterChanged=
function(a){r(a.getName(),a.getChange(),!0)};this.getDymo=function(){return a};this.getDuration=function(){return x};this.isLoopingAndPlaying=function(){return A.loop&&y};this.getParameterValue=function(a){if(h[a])return h[a].value||0==h[a].value?h[a].value:h[a].getValue()};this.play=function(b){A.onended=function(){for(var b=0;b<B.length;b++){var c=a.getParameter(B[b]);c&&c.removeObserver(u)}A.disconnect();D.disconnect();F.disconnect();f&&f(u)};b||(b=0);A.start(b);y=!0};this.pause=function(){y?(n(),
w=!0):w&&(w=!1,this.play())};this.stop=function(){y&&n()}};function SchedulerThread(a,b,c,d,e,f,g,l){function k(){w=u?u:p();n(w);B||(B=w.keys().next().value.getParameter(ONSET).getValue());var a;a=h?h-c.currentTime:SCHEDULE_AHEAD_TIME;for(var b=c.currentTime+a,d=$jscomp.makeIterator(w.values()),e=d.next();!e.done;e=d.next())e.value.play(b);setTimeout(function(){g()},a);(u=p())&&0<u.size?(h=t(b),a=1E3*(h-c.currentTime-SCHEDULE_AHEAD_TIME),y=setTimeout(function(){k()},a)):(h=t(b),a=1E3*(h-c.currentTime),setTimeout(function(){r()},a+100))}function r(){0!=v.size||
u&&0!=u.size||(window.clearTimeout(y),b.reset(),l&&l())}function n(a){for(var b=$jscomp.makeIterator(a.keys()),c=b.next();!c.done;c=b.next())c=c.value,v.get(c)||v.set(c,[]),v.get(c).push(a.get(c))}function m(a){var b=v.get(a.getDymo());b.splice(b.indexOf(a),1);0>=b.length&&v.delete(a.getDymo());g();r()}function t(a){if(u){var b=u.keys().next().value.getParameter(ONSET).getValue(),c=b-B;B=b;if(!isNaN(b))return a+c}for(var b=0,c=$jscomp.makeIterator(w.values()),d=c.next();!d.done;d=c.next())b=Math.max(b,
q(d.value));return a+b}function q(a){var b=a.getDymo().getParameter(PLAYBACK_RATE).getValue();return a.getDuration()/b}function p(){var a=b.getNextParts();if(a){console.log(a.map(function(a){return isNaN(a.getIndex())?"top":a.getIndex()}));for(var h=new Map,q=0;q<a.length;q++)if(a[q].getSourcePath()){var p=d[a[q].getSourcePath()];h.set(a[q],new Source(a[q],c,p,e,f,m))}return h}}var v=new Map,u,y,w=[],h,B;b||(b=new DymoNavigator(a,new SequentialNavigator(a)));k();this.hasDymo=function(b){if(a.getAllDymosInHierarchy().indexOf(b))return!0};
...
return k(b,c,d,f,e,h)}return k(b,c,d,[a],e)}function k(a,b,c,d,e,f){b=a.relative;for(var h=[],g=0;g<a.domainDims.length;g++){var k=a.domainDims[g],l=k.name,n=k["@type"];n==FEATURE?h.push(l):n==PARAMETER?(k=c?r(c,l,0):new Parameter(l,0),h.push(k)):(n=e&&e[l]?e[l]:m(k),"value"in k&&n.update(k.value),e&&!e[l]&&(e[l]=n),h.push(n))}return new Mapping(h,b,a["function"],d,a.range,f)}function r(a,b,c){var d=a.getParameter(b);d?d.update(c):(d=new Parameter(b,c),a.addParameter(d));return d}function n(a,b){for(var c=
...
a.uuid,c=parseInt(a.major,10),a=parseInt(a.minor,10),new BeaconControl(b,c,a);if(b==SLIDER||b==TOGGLE||b==BUTTON||b==CUSTOM)return new Control(c,b);if(b==RANDOM)return new RandomControl;if(b==BROWNIAN)return new BrownianControl(parseFloat(a.value));if(b==RAMP)return b=Math.round(1E3*parseFloat(a.duration)),new RampControl(b,parseFloat(a.value))}var t="";this.loadDymoFromJson=function(a,c){var e=a.lastIndexOf("/")+1;t=a.substring(0,e);b(a,"",{},c,d)};this.parseDymoFromJson=function(a,b){b(d(a,{}))};
...
b.getParts()[c],d)}function l(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var f=b.getSimilars()[e];c[f.getUri()]||(c[f.getUri()]=f.toFlatJson());a.links.push(k(d,c[f.getUri()]))}for(e=0;e<b.getParts().length;e++)l(a,b.getParts()[e],c)}function k(a,b){return{source:a,target:b,value:1}}var r=this,n=null,m=[],t=[],q={},p={},v=[],u=[],y="",w;p[PLAY]=new Parameter(PLAY,0,!0);p[LOOP]=new Parameter(LOOP,0,!0);
...
this.addParameter=function(a){p[a.getName()]=a};this.getParameter=function(a){return p[a]};this.addMapping=function(a){v.push(a)};this.observedParameterChanged=function(a){a.getName()==PLAY&&c&&(0<a.getChange()?c.play(r):c.stop(r))};this.updatePartOrder=function(a){m&&m[0]&&!isNaN(m[0].getFeature(a))?m.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):m&&m.sort(function(b,c){return b.getParameter(a).getValue()-c.getParameter(a).getValue()})};this.toJsonHierarchy=function(){for(var a=this.toFlatJson(),
b=0;b<m.length;b++)a.parts.push(m[b].toJsonHierarchy());return a};this.toJsonHierarchyGraph=function(){var a={nodes:[],links:[]};g(a,this);return a};this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};l(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=b);w&&(c.source=w);for(var d in q)c[d]=this.getFeatureJson(d);if(0<v.length)for(c.mappings=[],d=0;d<v.length;d++)c.mappings.push(v[d].toJson());0<m.length&&(c.parts=[]);if(0<t.length)for(c.similars=[],d=
0;d<t.length;d++)c.similars.push(t[d].getUri());return c};this.getFeatureJson=function(a){return{value:q[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+a.slice(1)}}};function Mapping(a,b,c,d,e,f){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=f;this.init()}
...
e,f,g,l,k,r,n;l=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*l,k=e*l,f=2*e,r=f+a,n=f+d,c[n+0]=b[r+0]*k+this.pMidBuffer[f+0]*g,c[n+1]=b[r+1]*k+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
