...
new Float32Array(2048),m=e.extract(h,1024),n=0;m;){for(k=0;k<m;k++)for(var q=0;q<g.length;q++)g[q][k+n]=h[k*g.length+q%g.length];n+=m;m=e.extract(h,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var g=0;g<e.length;g++)a[f*e.length+g%e.length]=e[g][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),k=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(k,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=k[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(){0<l&&l--;0==l&&b&&b(l)}function e(b){var c=b.getUri(),d;if(u[c])for(d=u[c],l=0;l<d.length;l++)p.set(d[l].getDymo(),d[l]);else{d=k(b);for(var l=0;l<d.length;l++){var h=d[l].getDymo();p.set(h,d[l]);w[c]=h.getParameter(ONSET).getValue()}}var m;m=t[c]?t[c]-a.currentTime:n;h=a.currentTime+m;for(l=0;l<d.length;l++)d[l].play(h);setTimeout(function(){g()},m);(l=k(b))?(u[c]=l,l=l[0].getDymo().getParameter(ONSET).getValue(),m=l-w[c],-1!=l?(t[c]=h+m,w[c]=l):t[c]=h+d[0].getDuration()/
...
RAMP="Ramp",CONTROLS=[SLIDER,TOGGLE,ACCELEROMETER_X,ACCELEROMETER_Y,ACCELEROMETER_Z,TILT_X,TILT_Y,GEOLOCATION_LATITUDE,GEOLOCATION_LONGITUDE,GEOLOCATION_DISTANCE,COMPASS_HEADING,RANDOM,BROWNIAN,RAMP],UI_CONTROLS=[SLIDER,TOGGLE,BUTTON];function Control(a,b,c){function d(a,b){if(void 0==f||1E-6<Math.abs(a-f))f=a,e(b)}function e(a){for(var b=0;b<h.length;b++)h[b]!=a&&h[b].updateParameter(f,this)}var f,g,k,h=[],m,n,q;this.getName=function(){return a};this.getParameter=function(a){if(c)return c[a]};this.getValue=function(){return f};this.getType=function(){return b};this.getReferenceValue=function(){return g};this.setReferenceAverageCount=function(a){k=a;this.resetReferenceValue()};this.resetReferenceValue=function(){g=f=void 0;n=m=
0};this.setUpdateFunction=function(a){q=a};this.addMapping=function(a){h.push(a);a.updateParameter(f,this)};this.removeMapping=function(a){a=h.indexOf(a);-1<a&&h.splice(a,1)};this.backpropagate=function(a,b){isFinite(a)&&(d(a,b),q&&q(f))};this.update=function(a){k&&m<k?(n+=a,m++,m==k&&(g=n/=k)):isNaN(a)||(g&&(a-=g),d(a))}};function DynamicMusicObject(a,b){function c(a,b){b[a.getUri()]=a;for(var d=0;d<a.getParts().length;d++)c(a.getParts()[d],b)}function d(a,b,c,e){var f=a-b,g=e.getParts();if(e.getLevel()==c&&0==f)return e;if(e.getLevel()>=c-1)return f<g.length?g[f]:b+g.length;for(e=0;e<g.length;e++)if(b=d(a,b,c,g[e]),isNaN(b))return b}function e(a,b,c){c(a,b);a=a.getParts();for(var d=0,f=a.length;d<f;d++)e(a[d],b,c)}function f(a,b,c){var d=b.toFlatJson();a.nodes.push(d);c&&a.links.push(k(c,d));for(c=0;c<b.getParts().length;c++)f(a,
b.getParts()[c],d)}function g(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var f=b.getSimilars()[e];c[f.getUri()]||(c[f.getUri()]=f.toFlatJson());a.links.push(k(d,c[f.getUri()]))}for(e=0;e<b.getParts().length;e++)g(a,b.getParts()[e],c)}function k(a,b){return{source:a,target:b,value:1}}var h=null,m=[],n=[],q={},p={},u=[],t=[],w="",v;p[PLAY]=new Parameter(PLAY,0,!0);p[LOOP]=new Parameter(LOOP,0,!0);p[ONSET]=
...
this.addParameter=function(a){p[a.getName()]=a};this.getParameter=function(a){return p[a]};this.addMapping=function(a){u.push(a)};this.observedParameterChanged=function(a){a.getName()==PLAY&&a.getChange()};this.setNavigator=function(a){l=a};this.getNavigator=function(){return l};this.resetPartsPlayed=function(){l.resetPartsPlayed();for(var a=0;a<m.length;a++)m[a].resetPartsPlayed()};this.updatePartOrder=function(a){m&&m[0]&&!isNaN(m[0].getFeature(a))?m.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):
