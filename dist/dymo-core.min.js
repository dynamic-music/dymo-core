...
TO_DYMO="http://tiny.cc/mobile-audio-ontology#toDymo";function DymoManager(a,b,c,d){var e=new Scheduler(a,function(){});isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var f,g,l={},m={};this.loadDymoAndRendering=function(a,b,n,v){var u=new DymoLoader(e,function(){u.loadDymoFromJson(a,function(a){u.loadRenderingFromJson(b,function(b){f=u.getStore();g=b[0];g.dymo=a[0];for(var q in b[1]){var p=b[1][q];f.isSubclassOf(p.getType(),UI_CONTROL)&&(l[q]=new UIControl(p,d));f.isSubclassOf(p.getType(),SENSOR_CONTROL)&&(m[q]=p)}c||(c="bower_components/dymo-core/audio/impulse_rev.wav");
e.initEffectSends(f,c);e.loadBuffers(g.dymo,v);n&&n()})})})};this.loadDymoFromJson=function(a,b){var c=new DymoLoader(e,function(){c.loadDymoFromJson(a,function(a){b&&b(a[0])})})};this.parseDymoFromJson=function(a,b){var c=new DymoLoader(e,function(){c.parseDymoFromJson(a,function(a){b(a[0])})})};this.replacePartOfTopDymo=function(a,b){var c=g.dymo.getPart(a);g.dymo.replacePart(a,b);e.stop(c)};this.updateNavigatorPosition=function(a,b,c){e.updateNavigatorPosition(a,b,c)};this.getNavigatorPosition=
...
2,b.numberOfChannels),l=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(l,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=l[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(a){0<n&&n--;0==n&&(b&&b(n),a&&a())}function e(a){m.splice(m.indexOf(a),1)}function f(){for(var a=[],b=0;b<m.length;b++)for(var d=$jscomp.makeIterator(m[b].getAllSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();q=a;c&&c()}function g(b,c){n++;var d=new XMLHttpRequest;d.open("GET",b,!0);d.responseType="arraybuffer";d.onload=function(){a.decodeAudioData(d.response,function(a){c(a,
b)})};d.error=function(){};d.send()}var l={},m=[],q=[];this.listenerOrientation=new Parameter(LISTENER_ORIENTATION,0);this.listenerOrientation.addObserver(this);var p,n=0;this.initEffectSends=function(b,c){0<b.find(null,null,REVERB).length&&(p=a.createConvolver(),p.connect(a.destination),g(c,function(a){p.buffer=a;d()}));if(0<b.find(null,null,DELAY).length){var e=a.createDelay();e.delayTime.value=.5;e.connect(a.destination);var f=a.createGain();f.gain.value=.6;e.connect(f);f.connect(e)}};this.loadBuffers=
function(a,b){for(var c=a.getAllSourcePaths(),e=0,f=c.length;e<f;e++)l[c[e]]||g(c[e],function(a,c){l[c]=a;d(b)})};this.getBuffer=function(a){return l[a.getSourcePath()]};this.getParameter=function(a){if(a==LISTENER_ORIENTATION)return this.listenerOrientation};this.updateNavigatorPosition=function(a,b,c){for(var d=0,e=m.length;d<e;d++)m[d].hasDymo(a)&&m[d].getNavigator().setPosition(c,b,a)};this.getNavigatorPosition=function(a,b){for(var c=0,d=m.length;c<d;c++)if(m[c].hasDymo(a))return m[c].getNavigator().getPosition(b,
a)};this.syncNavigators=function(a,b,c){for(var d,e,f=0,g=m.length;f<g;f++)m[f].hasDymo(a)&&(d=m[f].getNavigator()),m[f].hasDymo(b)&&(e=m[f].getNavigator());e&&(b=e.getPosition(c,b),d.setPosition(b,c,a))};this.play=function(b){b=new SchedulerThread(b,void 0,a,l,p,void 0,f,e);m.push(b)};this.pause=function(a){if(a)for(var b=0;b<m.length;b++)m[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<m.length;b++)m[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<m.length;c++){var d=m[c].getSources(a);
if(d)for(var e=0;e<d.length;e++)b=b.concat(d)}return b};this.getUrisOfPlayingDymos=function(){return q};this.observedParameterChanged=function(b){b=this.listenerOrientation.getValue()/180*Math.PI;a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0)}};function Source(a,b,c,d,e,f){function g(a,c){var d=b.createGain();d.connect(c);d.gain.value=0;a.connect(d);return d}function l(){var d=a.getParameter(TIME_STRETCH_RATIO).getValue();if(1!=d){if(0!=B||x<c.duration)c=v(c,n(B,c),n(x+.3,c));c=(new AudioProcessor(b)).timeStretch(c,d);d=x/d;c=v(c,0,n(d+FADE_LENGTH,c));x=d}else if(0!=B||x<c.duration)c=v(c,n(B,c),n(x+FADE_LENGTH,c));u(c,c.length);C.buffer=c;h[PLAYBACK_RATE]=C.playbackRate;m(PLAYBACK_RATE,a.getParameter(PLAYBACK_RATE))}function m(a,b){q(a,
b.getValue());b.addObserver(y)}function q(a,b,c){c&&(b+=y.getParameterValue(a));0>b&&0<=A.indexOf(a)&&(b=0);h[a]&&(h[a].value||0==h[a].value?h[a].value=b:h[a].setValue(b));0<=D.indexOf(a)&&(0==h[DISTANCE].value&&(h[DISTANCE].value=-.01),F.setPosition(h[PAN].value,h[HEIGHT].value,h[DISTANCE].value))}function p(){w=!1;var a=b.currentTime;h[AMPLITUDE].setValueAtTime(h[AMPLITUDE].value,a);h[AMPLITUDE].linearRampToValueAtTime(0,a+FADE_LENGTH);C.stop(a+2*FADE_LENGTH)}function n(a,b){if(a||0==a)return Math.round(a*
b.sampleRate)}function v(a,c,d){d=Math.min(a.length-c,d);for(var e=b.createBuffer(a.numberOfChannels,d,a.sampleRate),h=0;h<a.numberOfChannels;h++)for(var f=e.getChannelData(h),D=a.getChannelData(h),z=0;z<d;z++)f[z]=D[c+z];return e}function u(a,b){for(var c=a.sampleRate*FADE_LENGTH,d=0;d<a.numberOfChannels;d++)for(var e=a.getChannelData(d),h=0;h<c;h++)e[h]*=h/c,e[b-h-1]*=h/c}function r(a,b,c,d){var e=a.lastIndexOf("/");e&&(a=a.substring(e+1));t("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+
b+"&toSecond="+c,function(a){d(a)})}function t(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType="arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}var y=this,w,k,h={},z=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER,PAN,HEIGHT,DISTANCE,LOOP],A=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER],D=[PAN,HEIGHT,DISTANCE],C=b.createBufferSource(),E=b.createGain();
E.connect(b.destination);h[AMPLITUDE]=E.gain;var H,G;d&&(H=g(E,d));e&&(G=g(E,e),h[DELAY]=G.gain);var F=b.createPanner();F.connect(E);a.getParameter(FILTER)?(d=b.createBiquadFilter(),d.type="lowpass",d.frequency.value=2E4,d.connect(F),C.connect(d),h[FILTER]=d.frequency):C.connect(F);d=a.getSegment();var B=d[0],x=d[1];B||(B=0);!x&&c&&(x=c.duration-B);d=a.getParameter(DURATION_RATIO).getValue();0<d&&1>d&&(x*=d);c?l():isNaN(B+x)||r(a.getSourcePath(),B,B+x+.3,function(d){c=d;d=a.getParameter(TIME_STRETCH_RATIO).getValue();
1!=d?(c=(new AudioProcessor(b)).timeStretch(c,d),d=x/d,c=v(c,0,n(d+FADE_LENGTH,c)),x=d):c=v(c,0,n(x+FADE_LENGTH,c));u(c,c.length);C.buffer=c});h[TIME_STRETCH_RATIO]={value:0};h[PAN]={value:0};h[HEIGHT]={value:0};h[DISTANCE]={value:0};h[LOOP]={value:0};m(AMPLITUDE,a.getParameter(AMPLITUDE));m(TIME_STRETCH_RATIO,a.getParameter(TIME_STRETCH_RATIO));m(REVERB,a.getParameter(REVERB));m(DELAY,a.getParameter(DELAY));m(FILTER,a.getParameter(FILTER));m(PAN,a.getParameter(PAN));m(HEIGHT,a.getParameter(HEIGHT));
m(DISTANCE,a.getParameter(DISTANCE));m(LOOP,a.getParameter(LOOP));this.observedParameterChanged=function(a){q(a.getName(),a.getChange(),!0)};this.getDymo=function(){return a};this.getDuration=function(){return x};this.isLoopingAndPlaying=function(){return C.loop&&w};this.getParameterValue=function(a){if(h[a])return h[a].value||0==h[a].value?h[a].value:h[a].getValue()};this.play=function(b){C.onended=function(){for(var b=0;b<z.length;b++){var c=a.getParameter(z[b]);c&&c.removeObserver(y)}C.disconnect();
E.disconnect();H&&H.disconnect();G&&G.disconnect();f&&f(y)};b||(b=0);C.start(b);w=!0};this.pause=function(){w?(p(),k=!0):k&&(k=!1,this.play())};this.stop=function(){w&&p()}};function SchedulerThread(a,b,c,d,e,f,g,l){function m(){w=t?t:u();p(w);h||(h=w.keys().next().value.getParameter(ONSET).getValue());var a;a=k?k-c.currentTime:SCHEDULE_AHEAD_TIME;for(var b=c.currentTime+a,d=$jscomp.makeIterator(w.values()),e=d.next();!e.done;e=d.next())e.value.play(b);setTimeout(function(){g()},a);(t=u())&&0<t.size?(k=v(b),a=k[1],k=k[0],a.getDymo().getParameter(LOOP).getValue()&&(k-=FADE_LENGTH),a=1E3*(k-c.currentTime-SCHEDULE_AHEAD_TIME),y=setTimeout(function(){m()},a)):(k=v(b),a=1E3*
(k-c.currentTime),setTimeout(function(){q()},a+100))}function q(){0!=r.size||t&&0!=t.size||(window.clearTimeout(y),b.reset(),l&&l())}function p(a){for(var b=$jscomp.makeIterator(a.keys()),c=b.next();!c.done;c=b.next())c=c.value,r.get(c)||r.set(c,[]),r.get(c).push(a.get(c))}function n(a){var b=r.get(a.getDymo());b.splice(b.indexOf(a),1);0>=b.length&&r.delete(a.getDymo());g();q()}function v(a){if(t){var b=t.keys().next().value.getParameter(ONSET).getValue(),c=b-h;h=b;if(!isNaN(b))return a+c}for(var b=
0,d,c=$jscomp.makeIterator(w.values()),e=c.next();!e.done;e=c.next()){var e=e.value,f;f=e;var g=f.getDymo().getParameter(PLAYBACK_RATE).getValue();f=f.getDuration()/g;f>b&&(b=f,d=e)}return[a+b,d]}function u(){var a=b.getNextParts();if(a){console.log(a.map(function(a){return isNaN(a.getIndex())?"top":a.getIndex()}));for(var h=new Map,D=0;D<a.length;D++)if(a[D].getSourcePath()){var g=d[a[D].getSourcePath()];h.set(a[D],new Source(a[D],c,g,e,f,n))}return h}}var r=new Map,t,y,w=[],k,h;b||(b=new DymoNavigator(a,
new SequentialNavigator(a)));m();this.hasDymo=function(b){if(a.getAllDymosInHierarchy().indexOf(b))return!0};this.getNavigator=function(){return b};this.pause=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=r.get(a[b]);if(d)for(var e=0;e<d.length;e++)d[e].pause()}};this.stop=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=r.get(a[b]);if(d)for(var e=0;e<d.length;e++)d[e].stop();t&&(t.delete(a[b]),0>=t.size&&(t=void 0))}};this.getAllSources=
function(){return r};this.getSources=function(a){return r.get(a)}};function AutoControl(a,b,c){function d(a){var b=a.getName();f[b]=a;f[b].addObserver(e)}var e=this,f={};d(new Parameter(AUTO_CONTROL_FREQUENCY,100));d(new Parameter(AUTO_CONTROL_TRIGGER,0));Control.call(this,a,AUTO_CONTROL,f);var g;this.addParameter=d;this.getParameter=function(a){return f[a]};this.startUpdate=function(){g=setInterval(b,f[AUTO_CONTROL_FREQUENCY].getValue())};this.reset=function(){clearInterval(g);g=null;c&&c()};this.observedParameterChanged=function(a){a==f[AUTO_CONTROL_FREQUENCY]?
...
VALUE));isNaN(h)||f.update(h);z[c[d]]=f}}}function v(b,c){var d=k.findAllObjectUris(b,TO_TARGET);if(0<d.length){var e=[],h=l(d[0]);if(h)d=Object.keys(A).map(function(a){return A[a]}),Array.prototype.push.apply(e,d.filter(h));else for(var f=0;f<d.length;f++)k.findFirstObjectUri(d[f],TYPE)==DYMO?e.push(A[d[f]]):e.push(z[d[f]]);return u(b,c,e,h)}return u(b,c,[a])}function u(a,b,c,d){for(var e=k.findFirstObjectUri(a,IS_RELATIVE),h=[],f=k.findAllObjectUris(a,HAS_DOMAIN_DIMENSION),g=0;g<f.length;g++){var q=
...
a};this.getSourcePath=function(){if(p&&!k)return p.getSourcePath();if(k)return this.getBasePath()+k};this.getAllSourcePaths=function(){var a=[];f(this,a,function(b,c){var d=b.getSourcePath();d&&0>a.indexOf(d)&&c.push(d)});return a};this.getAllDymosInHierarchy=function(){var a=[];f(this,a,function(a,b){b.push(a)});return a};this.addSimilar=function(a){v.push(a)};this.getSimilars=function(a){return v};this.getFeatures=function(){return u};this.setFeature=function(a,b){u[a]=b};this.getFeature=function(a){if(a===
LEVEL_FEATURE)return this.getLevel();if(a===INDEX_FEATURE)return this.getIndex();if(u.hasOwnProperty(a))return u[a];if(p)return p.getFeature(a);for(var b=[],c=0;c<n.length;c++)if(n[c]){var d=n[c].getFeatureWithoutLookingFurther(a);isNaN(d)||b.push(d)}if(0<b.length)return b};this.getFeatureWithoutLookingFurther=function(a){if(u[a])return u[a]};this.getMappings=function(){return t};this.getSegment=function(){return[this.getFeature("time"),this.getFeature("duration")]};this.addParameter=function(a){r[a.getName()]=
a};this.getParameter=function(a){return r[a]};this.addMapping=function(a){t.push(a)};this.observedParameterChanged=function(a){a.getName()==PLAY&&c&&(0<a.getChange()?c.play(q):c.stop(q))};this.updatePartOrder=function(a){n&&n[0]&&!isNaN(n[0].getFeature(a))?n.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):n&&n.sort(function(b,c){return b.getParameter(a).getValue()-c.getParameter(a).getValue()})};this.toJsonHierarchy=function(){for(var a=this.toFlatJson(),b=0;b<n.length;b++)a.parts.push(n[b].toJsonHierarchy());
return a};this.toJsonHierarchyGraph=function(){var a={nodes:[],links:[]};g(a,this);return a};this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};l(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=b);k&&(c.source=k);for(var d in u)c[d]=this.getFeatureJson(d);if(0<t.length)for(c.mappings=[],d=0;d<t.length;d++)c.mappings.push(t[d].toJson());0<n.length&&(c.parts=[]);if(0<v.length)for(c.similars=[],d=0;d<v.length;d++)c.similars.push(v[d].getUri());return c};
this.getFeatureJson=function(a){return{value:u[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+a.slice(1)}}};function Mapping(a,b,c,d,e,f){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=f;this.init()}
