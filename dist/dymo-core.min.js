...
function DymoManager(a,b,c){var d=new Scheduler(a,function(){});c||(c="bower_components/dymo-core/audio/impulse_rev.wav");d.setReverbFile(c);SCHEDULE_AHEAD_TIME=b;var e,f={};this.loadDymoAndRendering=function(a,b,c){var m=new DymoLoader;m.loadDymoFromJson(a,function(a){m.loadRenderingFromJson(b,a[1],function(b){e=b[0];e.dymo=a[0];for(var g in b[1]){var k=b[1][g];0<=UI_CONTROLS.indexOf(k.getType())&&(f[g]=new UIControl(k))}d.loadBuffers(e.dymo);c&&c()})})};this.loadDymoFromJson=function(a,b){(new DymoLoader(d)).loadDymoFromJson(a,
function(a){b&&b(a[0])})};this.parseDymoFromJson=function(a,b){(new DymoLoader(d)).parseDymoFromJson(a,function(a){b(a[0])})};this.replacePartOfTopDymo=function(a,b){var c=e.dymo.getPart(a);e.dymo.replacePart(a,b);d.stop(c)};this.startPlaying=function(){d.play(e.dymo)};this.stopPlaying=function(){d.stop(e.dymo)};this.getTopDymo=function(){return e.dymo};this.getUIControl=function(a){return f[a]}};function AudioProcessor(a){function b(a){return{extract:function(b,e,f){for(var g=[],k=0;k<a.numberOfChannels;k++)g.push(a.getChannelData(k));for(k=0;k<e;k++)for(var h=0;h<g.length;h++)b[k*g.length+h%g.length]=g[h][k+f];return Math.min(e,g[0].length-f)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var f=b(c),e=new SimpleFilter(f,e),f=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),g=[],k=0;k<c.numberOfChannels;k++)g.push(f.getChannelData(k));for(var h=
new Float32Array(2048),m=e.extract(h,1024),n=0;m;){for(k=0;k<m;k++)for(var p=0;p<g.length;p++)g[p][k+n]=h[k*g.length+p%g.length];n+=m;m=e.extract(h,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var g=0;g<e.length;g++)a[f*e.length+g%e.length]=e[g][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),k=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(k,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=k[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(){0<p&&p--;0==p&&b&&b(p)}function e(a){h.splice(h.indexOf(a),1)}function f(){for(var a=[],b=0;b<h.length;b++)for(var d=$jscomp.makeIterator(h[b].getSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();m=a;c&&c()}function g(b,c){p++;var e=new XMLHttpRequest;e.open("GET",b,!0);e.responseType="arraybuffer";e.onload=function(){a.decodeAudioData(e.response,function(a){c?c(a):(k[b]=
a,d())})};e.error=function(){};e.send()}var k={},h=[],m=[];this.listenerOrientation=new Parameter(this,0);var n=a.createConvolver();n.connect(a.destination);var p=0;this.setReverbFile=function(a){g(a,function(a){n.buffer=a;d()})};this.loadBuffers=function(a){a=a.getAllSourcePaths();for(var b=0,c=a.length;b<c;b++)k[a[b]]||g(a[b])};this.getBuffer=function(a){return k[a.getSourcePath()]};this.play=function(b){b.updatePartOrder(ONSET);b=new SchedulerThread(b,navigator,a,k,n,f,e);h.push(b)};this.pause=
function(a){if(a)for(var b=0;b<h.length;b++)h[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<h.length;b++)h[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<h.length;c++){var d=h[c].getSource(a);d&&b.push(d)}return b};this.getUrisOfPlayingDymos=function(){return m};this.updateListenerOrientation=function(){var b=this.listenerOrientation.getValue()/180*Math.PI;a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0)}};function Source(a,b,c,d,e){function f(){var d=a.getParameter(TIME_STRETCH_RATIO).getValue();if(1!=d){if(0!=z||w<c.duration)c=n(c,m(z,c),m(w+.3,c));c=(new AudioProcessor(b)).timeStretch(c,d);d=w/d;c=n(c,0,m(d+.02,c));w=d}else if(0!=z||w<c.duration)c=n(c,m(z,c),m(w+.02,c));p(c,c.length);y.buffer=c;l[PLAYBACK_RATE]=y.playbackRate;g(PLAYBACK_RATE,a.getParameter(PLAYBACK_RATE))}function g(a,b){k(a,b.getValue());b.addObserver(r)}function k(a,b,c){c&&(b+=r.getParameterValue(a));0>b&&0<=A.indexOf(a)&&(b=
0);c=b;l[a]&&(l[a].value||0==l[a].value?l[a].value=c:l[a].setValue(c));0<=F.indexOf(a)?(0==l[DISTANCE].value&&(l[DISTANCE].value=-.01),D.setPosition(l[PAN].value,l[HEIGHT].value,l[DISTANCE].value)):a==LOOP&&(y.loop=1==b)}function h(){v=!1;var a=b.currentTime;l[AMPLITUDE].setValueAtTime(l[AMPLITUDE].value,a);l[AMPLITUDE].linearRampToValueAtTime(0,a+.02);y.stop(a+.04)}function m(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function n(a,c,d){d=Math.min(a.length-c,d);for(var x=b.createBuffer(a.numberOfChannels,
d,a.sampleRate),e=0;e<a.numberOfChannels;e++)for(var A=x.getChannelData(e),f=a.getChannelData(e),l=0;l<d;l++)A[l]=f[c+l];return x}function p(a,b){for(var c=.02*a.sampleRate,d=0;d<a.numberOfChannels;d++)for(var x=a.getChannelData(d),e=0;e<c;e++)x[e]*=e/c,x[b-e-1]*=e/c}function q(a,b,c,d){var x=a.lastIndexOf("/");x&&(a=a.substring(x+1));u("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function u(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType=
"arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}var r=this,v,t,l={},x=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,FILTER,PAN,HEIGHT,DISTANCE,LOOP],A=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,FILTER],F=[PAN,HEIGHT,DISTANCE],y=b.createBufferSource(),B=b.createGain();B.connect(b.destination);var C=b.createGain();C.connect(d);C.gain.value=0;B.connect(C);var D=b.createPanner();D.connect(B);
d=b.createBiquadFilter();d.type="lowpass";d.frequency.value=2E4;d.connect(D);y.connect(d);var E=a.getSegment(),z=E[0],w=E[1];z||(z=0);!w&&c&&(w=c.duration-z);c?f():q(a.getSourcePath(),z,z+w+.3,function(d){c=d;d=a.getParameter(TIME_STRETCH_RATIO).getValue();1!=d?(c=(new AudioProcessor(b)).timeStretch(c,d),d=w/d,c=n(c,0,m(d+.02,c)),w=d):c=n(c,0,m(w+.02,c));p(c,c.length);y.buffer=c});l[AMPLITUDE]=B.gain;l[TIME_STRETCH_RATIO]={value:0};l[REVERB]=C.gain;l[FILTER]=d.frequency;l[PAN]={value:0};l[HEIGHT]=
...
v};this.getParameterValue=function(a){if(l[a])return l[a].value||0==l[a].value?l[a].value:l[a].getValue()};this.play=function(b){y.onended=function(){for(var b=0;b<x.length;b++){var c=a.getParameter(x[b]);c&&c.removeObserver(r)}y.disconnect();B.disconnect();C.disconnect();e&&e(r)};y.start(b);v=!0};this.pause=function(){v?(h(),t=!0):t&&(t=!1,this.play())};this.stop=function(){v&&h()}};function SchedulerThread(a,b,c,d,e,f,g){function k(){r=t?t:p();h(r);l||(l=r[0].getDymo().getParameter(ONSET).getValue());var b;b=v?v-c.currentTime:SCHEDULE_AHEAD_TIME;for(var d=c.currentTime+b,e=0;e<r.length;e++)r[e].play(d);setTimeout(function(){f()},b);(t=p())?(v=n(d),u=setTimeout(function(){k()},1E3*(v-c.currentTime-SCHEDULE_AHEAD_TIME))):(a.resetPartsPlayed(),g&&g())}function h(a){for(var b=0;b<a.length;b++)q.set(a[b].getDymo(),a[b])}function m(a){q.delete(a.getDymo());0==q.size&&window.clearTimeout(u);
f()}function n(a){if(t){var b=t[0].getDymo().getParameter(ONSET).getValue(),c=b-l;l=b;if(!isNaN(b))return a+c}b=r[0];c=b.getDymo().getParameter(PLAYBACK_RATE).getValue();b=b.getDuration()/c;return a+b}function p(){var b=a.getNextParts();if(b){for(var A=[],l=0;l<b.length;l++)if(b[l].getSourcePath()){var f=d[b[l].getSourcePath()];A.push(new Source(b[l],c,f,e,m))}return A}}var q=new Map,u,r=[],v,t,l;k();this.pause=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=q.get(a[b]);
d&&d.pause()}};this.stop=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=q.get(a[b]);d&&d.stop()}};this.getSources=function(){return q};this.getSource=function(a){return q.get(a)}};function AutoControl(a,b,c){function d(a){var b=a.getName();f[b]=a;f[b].addObserver(e)}var e=this,f={};d(new Parameter(AUTO_CONTROL_FREQUENCY,100));d(new Parameter(AUTO_CONTROL_TRIGGER,0));Control.call(this,a,AUTO_CONTROL,f);var g;this.addParameter=d;this.getParameter=function(a){return f[a]};this.startUpdate=function(){g=setInterval(b,f[AUTO_CONTROL_FREQUENCY].getValue())};this.reset=function(){clearInterval(g);g=null;c&&c()};this.observedParameterChanged=function(a){a==f[AUTO_CONTROL_FREQUENCY]?
g&&(this.reset(),this.startUpdate()):a==f[AUTO_CONTROL_TRIGGER]&&(g?this.reset():this.startUpdate())}}inheritPrototype(AutoControl,Control);function BrownianControl(a){var b=this;a||(a=.5);AutoControl.call(this,BROWNIAN,function(){var a=b.getParameter(BROWNIAN_MAX_STEP_SIZE).getValue(),a=2*a*Math.random()-a,a=b.getValue()+a,a=Math.min(Math.max(a,0),1);b.update(a)});this.addParameter(new Parameter(BROWNIAN_MAX_STEP_SIZE,.1));this.update(a);this.startUpdate()}inheritPrototype(BrownianControl,AutoControl);function RampControl(a,b){var c=this;a||(a=1E4);var d=0;b&&(d=b);var e=1!=d;AutoControl.call(this,RAMP,function(){var b=1/a*c.getParameter(AUTO_CONTROL_FREQUENCY).getValue();e||(b*=-1);d+=b;0<d&&1>d?c.update(d):1<=d?(c.update(1),c.reset()):0>=d&&(c.update(0),c.reset())},function(){e=!e});this.update(d)}inheritPrototype(RampControl,AutoControl);function RandomControl(){var a=this;AutoControl.call(this,RANDOM,function(){a.update(Math.random())});this.startUpdate()}inheritPrototype(RandomControl,AutoControl);function UIControl(a,b){var c=this;this.value=a.getValue();a.setUpdateFunction(function(a){c.value=a;b&&setTimeout(function(){b.$apply()},10)});this.getName=function(){return a.getName()};this.getType=function(){return a.getType()};this.update=function(){a.getType()==BUTTON&&(isNaN(this.value)&&(this.value=0),this.value=1-this.value);1==this.value?a.update(1):0==this.value?a.update(0):a.update(this.value)}};function Control(a,b,c){function d(a,b){if(void 0==f||1E-6<Math.abs(a-f))f=a,e(b)}function e(a){for(var b=0;b<h.length;b++)h[b]!=a&&h[b].updateParameter(f,this)}var f,g,k,h=[],m,n,p;this.getName=function(){return a};this.getParameter=function(a){if(c)return c[a]};this.getValue=function(){return f};this.getType=function(){return b};this.getReferenceValue=function(){return g};this.setReferenceAverageCount=function(a){k=a;this.resetReferenceValue()};this.resetReferenceValue=function(){g=f=void 0;n=m=
0};this.setUpdateFunction=function(a){p=a};this.addMapping=function(a){h.push(a);a.updateParameter(f,this)};this.removeMapping=function(a){a=h.indexOf(a);-1<a&&h.splice(a,1)};this.backpropagate=function(a,b){isFinite(a)&&(d(a,b),p&&p(f))};this.update=function(a){k&&m<k?(n+=a,m++,m==k&&(g=n/=k)):isNaN(a)||(g&&(a-=g),d(a))}};function DynamicMusicObject(a,b){function c(a,b){b[a.getUri()]=a;for(var d=0;d<a.getParts().length;d++)c(a.getParts()[d],b)}function d(a,b,c,e){var l=a-b,f=e.getParts();if(e.getLevel()==c&&0==l)return e;if(e.getLevel()>=c-1)return l<f.length?f[l]:b+f.length;for(e=0;e<f.length;e++)if(b=d(a,b,c,f[e]),isNaN(b))return b}function e(a,b,c){c(a,b);a=a.getParts();for(var d=0,l=a.length;d<l;d++)e(a[d],b,c)}function f(a,b,c){var d=b.toFlatJson();a.nodes.push(d);c&&a.links.push(k(c,d));for(c=0;c<b.getParts().length;c++)f(a,
b.getParts()[c],d)}function g(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var l=b.getSimilars()[e];c[l.getUri()]||(c[l.getUri()]=l.toFlatJson());a.links.push(k(d,c[l.getUri()]))}for(e=0;e<b.getParts().length;e++)g(a,b.getParts()[e],c)}function k(a,b){return{source:a,target:b,value:1}}var h=null,m=[],n=[],p={},q={},u=[],r=[],v="",t;q[PLAY]=new Parameter(PLAY,0,!0);q[LOOP]=new Parameter(LOOP,0,!0);q[ONSET]=
...
!0);q[PLAY].addObserver(this);q[PART_INDEX].addObserver(this);q[PART_COUNT].addObserver(this);var l=new SequentialNavigator(this);this.setType=function(a){b=a};this.getType=function(){return b};this.getUri=function(){return a};this.setParent=function(a){h=a;for(var b in q)b!=PLAY&&b!=PART_COUNT&&b!=PART_INDEX&&r.push(new Mapping([a.getParameter(b)],!0,void 0,[this],b));a=h.getMappings();var c=this.getDymoMap(),c=Object.keys(c).map(function(a){return c[a]});for(b=0;b<a.length;b++){var d=a[b].getDymoConstraint();
d&&(d=a[b].getTargets().concat(c.filter(d)),a[b].setTargets(d))}};this.removeParent=function(){for(var a=0;a<r.length;a++)r[a].disconnect();r=[];for(var b=h.getMappings(),c=this,a=0;a<b.length;a++){var d=b[a].getTargets(),d=d.filter(function(a){return!a.isSubDymoOf(c)});b[a].setTargets(d)}h=null};this.getParent=function(){return h};this.isSubDymoOf=function(a){for(var b=this;b;){if(b==a)return!0;b=b.getParent()}return!1};this.getDymoMap=function(){var a={};c(this,a);return a};this.getLevel=function(){return h?
h.getLevel()+1:0};this.getIndex=function(){if(h)return h.getParts().indexOf(this)};this.addPart=function(a){m.push(a);a.setParent(this)};this.removePart=function(a){m.splice(m.indexOf(a));a.removeParent(this)};this.replacePart=function(a,b){m[a]&&m[a].removeParent(this);m[a]=b;b.setParent(this)};this.getPart=function(a){return m[a]};this.getParts=function(){return m};this.getNthPart=function(a,b){return d(a,0,b,this)};this.setBasePath=function(a){v=a};this.getBasePath=function(){return h&&!v?h.getBasePath():
v};this.setSourcePath=function(a){t=a};this.getSourcePath=function(){if(h&&!t)return h.getSourcePath();if(t)return this.getBasePath()+t};this.getAllSourcePaths=function(){var a=[];e(this,a,function(b,c){var d=b.getSourcePath();d&&0>a.indexOf(d)&&c.push(d)});return a};this.getAllDymosInHierarchy=function(){var a=[];e(this,a,function(a,b){b.push(a)});return a};this.addSimilar=function(a){n.push(a)};this.getSimilars=function(a){return n};this.getFeatures=function(){return p};this.setFeature=function(a,
b){p[a]=b};this.getFeature=function(a){if(a===LEVEL)return this.getLevel();if(a===INDEX)return this.getIndex();if(p.hasOwnProperty(a))return p[a];if(h)return h.getFeature(a);for(var b=[],c=0;c<m.length;c++)if(m[c]){var d=m[c].getFeatureWithoutLookingFurther(a);isNaN(d)||b.push(d)}if(0<b.length)return b};this.getFeatureWithoutLookingFurther=function(a){if(p[a])return p[a]};this.getMappings=function(){return u};this.getSegment=function(){return[this.getFeature("time"),q[DURATION_RATIO].getValue()*this.getFeature("duration")]};
this.addParameter=function(a){q[a.getName()]=a};this.getParameter=function(a){return q[a]};this.addMapping=function(a){u.push(a)};this.observedParameterChanged=function(a){a.getName()==PLAY&&a.getChange()};this.setNavigator=function(a){l=a};this.getNavigator=function(){return l};this.resetPartsPlayed=function(){l.resetPartsPlayed();for(var a=0;a<m.length;a++)m[a].resetPartsPlayed()};this.updatePartOrder=function(a){m&&m[0]&&!isNaN(m[0].getFeature(a))?m.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):
m&&m.sort(function(b,c){return b.getParameter(a).getValue()-c.getParameter(a).getValue()})};this.getNextParts=function(){return l.getNextParts()};this.toJsonHierarchy=function(){for(var a=this.toFlatJson(),b=0;b<m.length;b++)a.parts.push(m[b].toJsonHierarchy());return a};this.toJsonHierarchyGraph=function(){var a={nodes:[],links:[]};f(a,this);return a};this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};g(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=
b);t&&(c.source=t);l instanceof SimilarityNavigator&&(c.navigator=SIMILARITY_NAVIGATOR);for(var d in p)c[d]=this.getFeatureJson(d);if(0<u.length)for(c.mappings=[],d=0;d<u.length;d++)c.mappings.push(u[d].toJson());0<m.length&&(c.parts=[]);if(0<n.length)for(c.similars=[],d=0;d<n.length;d++)c.similars.push(n[d].getUri());return c};this.getFeatureJson=function(a){return{value:p[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+a.slice(1)}}};function Mapping(a,b,c,d,e,f){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=f;this.init()}
...
Parameter.prototype.setValueAndNotify=function(a,b){if(!isNaN(b)&&(this.isInteger&&(b=Math.round(b)),1E-6<Math.abs(b-this.value))){this.change=b-this.value;this.value=b;for(var c=0;c<this.updaters.length;c++)this.updaters[c]!=a&&this.updaters[c].updatedParameterChanged(this.value);this.change&&this.notifyObservers()}};function Rendering(a){var b=[];this.play=function(){a&&a.getParameter(PLAY).update(1)};this.stop=function(){a&&a.getParameter(PLAY).update(0)};this.addMapping=function(a){b.push(a);a.updateParameter()};this.getMappings=function(){return b};this.toJson=function(){var c={mappings:[]};a&&(c.topDymo=a.getUri());for(var d=0;d<b.length;d++)c.mappings.push(b[d].toJson());return c}};function OntologyLoader2(a,b,c){};function DymoLoader(a){function b(a,d,e,f,g){var h=new XMLHttpRequest;h.open("GET",t+a,!0);h.onload=function(){var h;a:{try{JSON.parse(this.responseText)}catch(k){h=!1;break a}h=!0}h&&(d=d?d.replace('"'+a+'"',this.responseText):this.responseText);(h=c(d))?b(h,d,e,f,g):d&&(h=g(JSON.parse(d),e),f(h))};h.error=function(a){console.log(a)};h.send()}function c(a){var b=a.indexOf(".json");if(0<=b){if(b!=a.indexOf("context.json")+7){var d=a.substring(0,b).lastIndexOf('"');return a.substring(d+1,b+5)}return c(a.substring(b+
1))}}function d(a,b){var c=e(a,b);f(a,b);return[c,b]}function e(a,b){var c=new DynamicMusicObject(a["@id"],a.ct);c.setBasePath(t);b[a["@id"]]=c;a.source&&c.setSourcePath(a.source);a.navigator&&c.setNavigator(p(a.navigator,c));for(var d in a)0>v.indexOf(d)&&(a[d].type==FEATURE?c.setFeature(d,a[d].value):a[d].type==PARAMETER&&m(c,d,a[d].value));if(a.parts)for(d=0;d<a.parts.length;d++)c.addPart(e(a.parts[d],b));return c}function f(a,b){var c=b[a["@id"]];if(a.similars)for(var d=0;d<a.similars.length;d++)c.addSimilar(b[a.similars[d]]);
if(a.mappings)for(d=0;d<a.mappings.length;d++)c.addMapping(k(a.mappings[d],b,c));if(a.parts)for(d=0;d<a.parts.length;d++)f(a.parts[d],b)}function g(a,b){for(var c=new Rendering(b[a.topDymo]),d={},e=0;e<a.mappings.length;e++)c.addMapping(k(a.mappings[e],b,void 0,d));return[c,d]}function k(a,b,c,d){if(a.controls){for(var e=[],f=0;f<a.controls.length;f++)e.push(d[a.controls[f]]);return h(a,b,c,e,d)}if(a.dymos){var e=[],g;if(a.dymos instanceof Array)for(f=0;f<a.dymos.length;f++)e.push(b[a.dymos[f]]);
else g=Function.apply(null,a.dymos.args.concat(a.dymos.body)),f=Object.keys(b).map(function(a){return b[a]}),Array.prototype.push.apply(e,f.filter(g));return h(a,b,c,e,d,g)}}function h(a,b,c,d,e,f){b=a.relative;for(var g=[],h=0;h<a.domainDims.length;h++){var k=a.domainDims[h],n=k.name,p=k.type;p==FEATURE?g.push(n):p==PARAMETER?(n=c?m(c,n,0):new Parameter(n,0),g.push(n)):(k=e&&e[n]?e[n]:q(k),e&&!e[n]&&(e[n]=k),g.push(k))}return new Mapping(g,b,a["function"],d,a.parameter,f)}function m(a,b,c){var d=
a.getParameter(b);d?d.update(c):(d=new Parameter(b,c),a.addParameter(d));return d}function n(a,b){for(var c=0;c<a.length;c++)if(a[c])for(var d=0;d<a[c].length;d++){var e=b["dymo"+c],f=b["dymo"+a[c][d]];e&&f&&e.addSimilar(f)}}function p(a,b){return a==SIMILARITY_NAVIGATOR?new SimilarityNavigator(b):new SequentialNavigator(b)}function q(b){var c=b.type,d=b.name;if(c==ACCELEROMETER_X)return u(0);if(c==ACCELEROMETER_Y)return u(1);if(c==ACCELEROMETER_Z)return u(2);if(c==TILT_X)return u(3);if(c==TILT_Y)return u(4);
if(c==GEOLOCATION_LATITUDE)return r(0);if(c==GEOLOCATION_LONGITUDE)return r(1);if(c==GEOLOCATION_DISTANCE)return r(2);if(c==COMPASS_HEADING)return a.compassWatcher||(a.compassWatcher=new CompassWatcher),b=a.compassWatcher.headingControl,b;if(c==SLIDER||c==TOGGLE||c==BUTTON)return new Control(d,c);if(c==RANDOM)return new RandomControl;if(c==BROWNIAN)return new BrownianControl(parseFloat(b.value));if(c==RAMP)return c=Math.round(1E3*parseFloat(b.duration)),new RampControl(c,parseFloat(b.value))}function u(b){a.accelerometerWatcher||
(a.accelerometerWatcher=new AccelerometerWatcher);if(0==b)return a.accelerometerWatcher.xControl;if(1==b)return a.accelerometerWatcher.yControl;if(2==b)return a.accelerometerWatcher.zControl;if(3==b)return a.accelerometerWatcher.tiltXControl;if(4==b)return a.accelerometerWatcher.tiltYControl}function r(b){a.geolocationWatcher||(a.geolocationWatcher=new GeolocationWatcher);return 0==b?a.geolocationWatcher.latitudeControl:1==b?a.geolocationWatcher.longitudeControl:a.geolocationWatcher.distanceControl}
var v="@id @type ct source navigator similars mappings parts".split(" "),t="";this.loadDymoFromJson=function(a,c){var e=a.lastIndexOf("/")+1;t=a.substring(0,e);a=a.substring(e);b(a,"",{},c,d)};this.parseDymoFromJson=function(a,b){b(d(a,{}))};this.loadRenderingFromJson=function(a,c,d){b(a,"",c,d,g)};this.loadGraphFromJson=function(a,c,d){b(a,"",c,d,n)}};function DymoWriter(a){function b(a){a.addTriple("_:","http://www.w3.org/1999/02/22-rdf-syntax-ns#type","dmo:DMO");a.addTriple("_:","ch:hasPart","ch:test2")}function c(a,b){var c=new XMLHttpRequest;c.send(b);c.addEventListener("save",function(){console.log("saved "+a)});c.addEventListener("error",function(){console.log("saving "+a+" failed")});c.open("POST",a);c.send()}this.writeDymoToJson=function(a,b,f){a["@context"]="http://purl.org/ontology/dymo/context.json";f||(f="dymo.json");c(b+f,a)};this.writeRenderingToJson=
function(a,b){c(b+"rendering.json",a)};this.writeDmoToRdf=function(a){a=N3.Writer({prefixes:{ch:"rdf/charm.n3#",dmo:"rdf/dmo.n3#",mb:"rdf/mobile.n3#"}});b(a);a.end(function(a,b){console.log(b)})}};var SCHEDULE_AHEAD_TIME=.1,DYMO="Dymo",PARALLEL="parallel",SEQUENTIAL="sequential",DYMO_TYPES=[PARALLEL,SEQUENTIAL],FEATURE="Feature",PARAMETER="Parameter",SEQUENTIAL_NAVIGATOR="SequentialNavigator",SIMILARITY_NAVIGATOR="SimilarityNavigator",PLAY="Play",LOOP="Loop",ONSET="Onset",DURATION_RATIO="DurationRatio",AMPLITUDE="Amplitude",PLAYBACK_RATE="PlaybackRate",TIME_STRETCH_RATIO="TimeStretchRatio",PAN="Pan",DISTANCE="Distance",HEIGHT="Height",REVERB="Reverb",FILTER="Filter",PART_INDEX="PartIndex",
PART_COUNT="PartCount",PART_ORDER="PartOrder",LISTENER_ORIENTATION="ListenerOrientation",AUTO_CONTROL_FREQUENCY="AutoControlFrequency",AUTO_CONTROL_TRIGGER="AutoControlTrigger",BROWNIAN_MAX_STEP_SIZE="BrownianMaxStepSize",LEAPING_PROBABILITY="LeapingProbability",CONTINUE_AFTER_LEAPING="ContinueAfterLeaping",PARAMETERS=[PLAY,LOOP,ONSET,DURATION_RATIO,AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,PAN,DISTANCE,HEIGHT,REVERB,FILTER,PART_INDEX,PART_COUNT,PART_ORDER,LISTENER_ORIENTATION,AUTO_CONTROL_FREQUENCY,
AUTO_CONTROL_TRIGGER],LEVEL="level",INDEX="index",ONSET_FEATURE="onset",PITCH_FEATURE="pitch",DURATION_FEATURE="duration",AUTO_CONTROL="AutoControl",SLIDER="Slider",TOGGLE="Toggle",BUTTON="Button",ACCELEROMETER_X="AccelerometerX",ACCELEROMETER_Y="AccelerometerY",ACCELEROMETER_Z="AccelerometerZ",TILT_X="TiltX",TILT_Y="TiltY",GEOLOCATION_LATITUDE="GeolocationLatitude",GEOLOCATION_LONGITUDE="GeolocationLongitude",GEOLOCATION_DISTANCE="GeolocationDistance",COMPASS_HEADING="CompassHeading",RANDOM="Random",
BROWNIAN="Brownian",RAMP="Ramp",CONTROLS=[SLIDER,TOGGLE,ACCELEROMETER_X,ACCELEROMETER_Y,ACCELEROMETER_Z,TILT_X,TILT_Y,GEOLOCATION_LATITUDE,GEOLOCATION_LONGITUDE,GEOLOCATION_DISTANCE,COMPASS_HEADING,RANDOM,BROWNIAN,RAMP],UI_CONTROLS=[SLIDER,TOGGLE,BUTTON];function SequentialNavigator(a){var b=!1,c=0;this.resetPartsPlayed=function(){c=0};this.setPartsPlayed=function(a){c=a};this.getPartsPlayed=function(){return c};this.getNextParts=function(){var d=a.getParts();if(0<d.length){if(a.getType()==PARALLEL){for(var e=[],f=0;f<d.length;f++){var g=d[f].getNextParts();g&&(!g.length||0<g.length)&&(e=e.concat(g))}if(0<e.length)return console.log(e.map(function(a){return a.getIndex()})),e}else for(b=!0;c<d.length&&c<a.getParameter(PART_COUNT).getValue();){if((g=
d[c].getNextParts())&&(!g.length||0<g.length))return g instanceof Array||(g=[g]),g;c++}c=0;b=!1;return null}if(b)return b=!1,null;b=!0;return[a]}};function SimilarityNavigator(a){function b(b){for(var d=0;d<b.length;d++)if(0<b[d].getSimilars().length&&Math.random()<c.leapingProbability.getValue()){var k=b[d].getSimilars(),h=Math.floor(Math.random()*k.length);if(c.continueAfterLeaping.getValue()){var m=a.getParts().indexOf(h);m&&(e=m+1)}b[d]=k[h]}}var c=this,d=!1,e=0;this.leapingProbability=new Parameter(LEAPING_PROBABILITY,.5);this.continueAfterLeaping=new Parameter(CONTINUE_AFTER_LEAPING,0,!0);this.resetPartsPlayed=function(){e=0};this.getNextParts=
function(){var c=a.getParts();if(0<c.length){if(a.getType()==PARALLEL){for(var g=[],k=0;k<c.length;k++){var h=c[k].getNextParts();h&&(!h.length||0<h.length)&&(g=g.concat(h))}if(0<g.length)return b(g),g}else for(d=!0;e<c.length&&e<a.getParameter(PART_COUNT).getValue();){if((h=c[e].getNextParts())&&(!h.length||0<h.length))return h instanceof Array||(h=[h]),b(h),h;e++}e=0;d=!1;return null}if(d)return d=!1,null;d=!0;return[a]}};function FunctionTools(){}FunctionTools.IDENTITY_JSON={args:["a"],body:"return a;"};FunctionTools.inverted={};FunctionTools.created={};FunctionTools.createFunction=function(a,b){FunctionTools.created[b]||(FunctionTools.created[b]=Function.apply(null,a.concat(b)));return FunctionTools.created[b]};
...
e,f,g,k,h,m,n;k=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*k,h=e*k,f=2*e,m=f+a,n=f+d,c[n+0]=b[m+0]*h+this.pMidBuffer[f+0]*g,c[n+1]=b[m+1]*h+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
