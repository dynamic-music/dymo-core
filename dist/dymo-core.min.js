...
COMPASS_HEADING,BEACON];function DymoManager(a,b,c,d){var e=new Scheduler(a,function(){});c||(c="bower_components/dymo-core/audio/impulse_rev.wav");e.setReverbFile(c);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var f,g={},l={};this.loadDymoAndRendering=function(a,b,c,n){var q=new DymoLoader(e);q.loadDymoFromJson(a,function(a){q.loadRenderingFromJson(b,a[1],function(b){f=b[0];f.dymo=a[0];for(var k in b[1]){var u=b[1][k];0<=UI_CONTROLS.indexOf(u.getType())&&(g[k]=new UIControl(u,d));0<=SENSOR_CONTROLS.indexOf(u.getType())&&(l[k]=u)}e.loadBuffers(f.dymo,
n);c&&c()})})};this.loadDymoFromJson=function(a,b){(new DymoLoader(e)).loadDymoFromJson(a,function(a){b&&b(a[0])})};this.parseDymoFromJson=function(a,b){(new DymoLoader(e)).parseDymoFromJson(a,function(a){b(a[0])})};this.replacePartOfTopDymo=function(a,b){var c=f.dymo.getPart(a);f.dymo.replacePart(a,b);e.stop(c)};this.updateNavigatorPosition=function(a,b,c){e.updateNavigatorPosition(a,b,c)};this.getNavigatorPosition=function(a,b){return e.getNavigatorPosition(a,b)};this.syncNavigators=function(a,
b,c){e.syncNavigators(a,b,c)};this.startPlaying=function(){f.dymo.updatePartOrder(ONSET);e.play(f.dymo)};this.stopPlaying=function(){e.stop(f.dymo)};this.getTopDymo=function(){return f.dymo};this.getRendering=function(){return f};this.getUIControls=function(){return g};this.getUIControl=function(a){return g[a]};this.getSensorControls=function(){return l}};function AudioProcessor(a){function b(a){return{extract:function(b,e,f){for(var g=[],l=0;l<a.numberOfChannels;l++)g.push(a.getChannelData(l));for(l=0;l<e;l++)for(var k=0;k<g.length;k++)b[l*g.length+k%g.length]=g[k][l+f];return Math.min(e,g[0].length-f)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var f=b(c),e=new SimpleFilter(f,e),f=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),g=[],l=0;l<c.numberOfChannels;l++)g.push(f.getChannelData(l));for(var k=
new Float32Array(2048),r=e.extract(k,1024),m=0;r;){for(l=0;l<r;l++)for(var n=0;n<g.length;n++)g[n][l+m]=k[l*g.length+n%g.length];m+=r;r=e.extract(k,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var p=0;p<e.length;p++)a[f*e.length+p%e.length]=e[p][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),l=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(l,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=l[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(a){0<p&&p--;0==p&&(b&&b(p),a&&a())}function e(a){k.splice(k.indexOf(a),1)}function f(){for(var a=[],b=0;b<k.length;b++)for(var d=$jscomp.makeIterator(k[b].getAllSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();r=a;c&&c()}function g(b,c){p++;var d=new XMLHttpRequest;d.open("GET",b,!0);d.responseType="arraybuffer";d.onload=function(){a.decodeAudioData(d.response,function(a){c(a,
b)})};d.error=function(){};d.send()}var l={},k=[],r=[];this.listenerOrientation=new Parameter(LISTENER_ORIENTATION,0);this.listenerOrientation.addObserver(this);var m=a.createConvolver();m.connect(a.destination);var n=a.createDelay();n.delayTime.value=.5;n.connect(a.destination);var q=a.createGain();q.gain.value=.6;n.connect(q);q.connect(n);var p=0;this.setReverbFile=function(a){g(a,function(a){m.buffer=a;d()})};this.loadBuffers=function(a,b){for(var c=a.getAllSourcePaths(),e=0,f=c.length;e<f;e++)l[c[e]]||
g(c[e],function(a,c){l[c]=a;d(b)})};this.getBuffer=function(a){return l[a.getSourcePath()]};this.getParameter=function(a){if(a==LISTENER_ORIENTATION)return this.listenerOrientation};this.updateNavigatorPosition=function(a,b,c){for(var d=0,e=k.length;d<e;d++)k[d].hasDymo(a)&&k[d].getNavigator().setPosition(c,b,a)};this.getNavigatorPosition=function(a,b){for(var c=0,d=k.length;c<d;c++)if(k[c].hasDymo(a))return k[c].getNavigator().getPosition(b,a)};this.syncNavigators=function(a,b,c){for(var d,e,h=0,
f=k.length;h<f;h++)k[h].hasDymo(a)&&(d=k[h].getNavigator()),k[h].hasDymo(b)&&(e=k[h].getNavigator());e&&(b=e.getPosition(c,b),d.setPosition(b+1,c,a))};this.play=function(b){b=new SchedulerThread(b,void 0,a,l,m,n,f,e);k.push(b)};this.pause=function(a){if(a)for(var b=0;b<k.length;b++)k[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<k.length;b++)k[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<k.length;c++){var d=k[c].getSources(a);if(d)for(var e=0;e<d.length;e++)b=b.concat(d)}return b};
this.getUrisOfPlayingDymos=function(){return r};this.observedParameterChanged=function(b){b=this.listenerOrientation.getValue()/180*Math.PI;a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0)}};function Source(a,b,c,d,e,f){function g(a,c){var d=b.createGain();d.connect(c);d.gain.value=0;a.connect(d);return d}function l(){var d=a.getParameter(TIME_STRETCH_RATIO).getValue();if(1!=d){if(0!=z||x<c.duration)c=q(c,n(z,c),n(x+.3,c));c=(new AudioProcessor(b)).timeStretch(c,d);d=x/d;c=q(c,0,n(d+.02,c));x=d}else if(0!=z||x<c.duration)c=q(c,n(z,c),n(x+.02,c));p(c,c.length);A.buffer=c;h[PLAYBACK_RATE]=A.playbackRate;k(PLAYBACK_RATE,a.getParameter(PLAYBACK_RATE))}function k(a,b){r(a,b.getValue());b.addObserver(u)}
function r(a,b,c){c&&(b+=u.getParameterValue(a));0>b&&0<=G.indexOf(a)&&(b=0);h[a]&&(h[a].value||0==h[a].value?h[a].value=b:h[a].setValue(b));0<=H.indexOf(a)&&(0==h[DISTANCE].value&&(h[DISTANCE].value=-.01),E.setPosition(h[PAN].value,h[HEIGHT].value,h[DISTANCE].value))}function m(){y=!1;var a=b.currentTime;h[AMPLITUDE].setValueAtTime(h[AMPLITUDE].value,a);h[AMPLITUDE].linearRampToValueAtTime(0,a+.02);A.stop(a+.04)}function n(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function q(a,c,d){d=Math.min(a.length-
c,d);for(var e=b.createBuffer(a.numberOfChannels,d,a.sampleRate),h=0;h<a.numberOfChannels;h++)for(var f=e.getChannelData(h),p=a.getChannelData(h),k=0;k<d;k++)f[k]=p[c+k];return e}function p(a,b){for(var c=.02*a.sampleRate,d=0;d<a.numberOfChannels;d++)for(var e=a.getChannelData(d),h=0;h<c;h++)e[h]*=h/c,e[b-h-1]*=h/c}function t(a,b,c,d){var e=a.lastIndexOf("/");e&&(a=a.substring(e+1));v("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function v(a,
c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType="arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}var u=this,y,w,h={},B=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER,PAN,HEIGHT,DISTANCE,LOOP],G=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER],H=[PAN,HEIGHT,DISTANCE],A=b.createBufferSource(),C=b.createGain();C.connect(b.destination);var F=g(C,d);d=g(C,e);
var E=b.createPanner();E.connect(C);e=b.createBiquadFilter();e.type="lowpass";e.frequency.value=2E4;e.connect(E);A.connect(e);var D=a.getSegment(),z=D[0],x=D[1];z||(z=0);!x&&c&&(x=c.duration-z);D=a.getParameter(DURATION_RATIO).getValue();0<D&&1>D&&(x*=D);c?l():isNaN(z+x)||t(a.getSourcePath(),z,z+x+.3,function(d){c=d;d=a.getParameter(TIME_STRETCH_RATIO).getValue();1!=d?(c=(new AudioProcessor(b)).timeStretch(c,d),d=x/d,c=q(c,0,n(d+.02,c)),x=d):c=q(c,0,n(x+.02,c));p(c,c.length);A.buffer=c});h[AMPLITUDE]=
C.gain;h[TIME_STRETCH_RATIO]={value:0};h[REVERB]=F.gain;h[DELAY]=d.gain;h[FILTER]=e.frequency;h[PAN]={value:0};h[HEIGHT]={value:0};h[DISTANCE]={value:0};h[LOOP]={value:0};k(AMPLITUDE,a.getParameter(AMPLITUDE));k(TIME_STRETCH_RATIO,a.getParameter(TIME_STRETCH_RATIO));k(REVERB,a.getParameter(REVERB));k(DELAY,a.getParameter(DELAY));k(FILTER,a.getParameter(FILTER));k(PAN,a.getParameter(PAN));k(HEIGHT,a.getParameter(HEIGHT));k(DISTANCE,a.getParameter(DISTANCE));k(LOOP,a.getParameter(LOOP));this.observedParameterChanged=
function(a){r(a.getName(),a.getChange(),!0)};this.getDymo=function(){return a};this.getDuration=function(){return x};this.isLoopingAndPlaying=function(){return A.loop&&y};this.getParameterValue=function(a){if(h[a])return h[a].value||0==h[a].value?h[a].value:h[a].getValue()};this.play=function(b){A.onended=function(){for(var b=0;b<B.length;b++){var c=a.getParameter(B[b]);c&&c.removeObserver(u)}A.disconnect();C.disconnect();F.disconnect();f&&f(u)};b||(b=0);A.start(b);y=!0};this.pause=function(){y?(m(),
w=!0):w&&(w=!1,this.play())};this.stop=function(){y&&m()}};function SchedulerThread(a,b,c,d,e,f,g,l){function k(){w=u?u:t();m(w);B||(B=w.keys().next().value.getParameter(ONSET).getValue());var a;a=h?h-c.currentTime:SCHEDULE_AHEAD_TIME;for(var b=c.currentTime+a,d=$jscomp.makeIterator(w.values()),e=d.next();!e.done;e=d.next())e.value.play(b);setTimeout(function(){g()},a);(u=t())&&0<u.size?(h=q(b),a=1E3*(h-c.currentTime-SCHEDULE_AHEAD_TIME),y=setTimeout(function(){k()},a)):(h=q(b),a=1E3*(h-c.currentTime),setTimeout(function(){r()},a+100))}function r(){0!=v.size||
u&&0!=u.size||(window.clearTimeout(y),b.reset(),l&&l())}function m(a){for(var b=$jscomp.makeIterator(a.keys()),c=b.next();!c.done;c=b.next())c=c.value,v.get(c)||v.set(c,[]),v.get(c).push(a.get(c))}function n(a){var b=v.get(a.getDymo());b.splice(b.indexOf(a),1);0>=b.length&&v.delete(a.getDymo());g();r()}function q(a){if(u){var b=u.keys().next().value.getParameter(ONSET).getValue(),c=b-B;B=b;if(!isNaN(b))return a+c}for(var b=0,c=$jscomp.makeIterator(w.values()),d=c.next();!d.done;d=c.next())b=Math.max(b,
p(d.value));return a+b}function p(a){var b=a.getDymo().getParameter(PLAYBACK_RATE).getValue();return a.getDuration()/b}function t(){var a=b.getNextParts();if(a){console.log(a.map(function(a){return isNaN(a.getIndex())?"top":a.getIndex()}));for(var h=new Map,p=0;p<a.length;p++)if(a[p].getSourcePath()){var k=d[a[p].getSourcePath()];h.set(a[p],new Source(a[p],c,k,e,f,n))}return h}}var v=new Map,u,y,w=[],h,B;b||(b=new DymoNavigator(a,new SequentialNavigator(a)));k();this.hasDymo=function(b){if(a.getAllDymosInHierarchy().indexOf(b))return!0};
this.getNavigator=function(){return b};this.pause=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=v.get(a[b]);if(d)for(var e=0;e<d.length;e++)d[e].pause()}};this.stop=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=v.get(a[b]);if(d)for(var e=0;e<d.length;e++)d[e].stop();u&&u.delete(a[b])}};this.getAllSources=function(){return v};this.getSources=function(a){return v.get(a)}};function AutoControl(a,b,c){function d(a){var b=a.getName();f[b]=a;f[b].addObserver(e)}var e=this,f={};d(new Parameter(AUTO_CONTROL_FREQUENCY,100));d(new Parameter(AUTO_CONTROL_TRIGGER,0));Control.call(this,a,AUTO_CONTROL,f);var g;this.addParameter=d;this.getParameter=function(a){return f[a]};this.startUpdate=function(){g=setInterval(b,f[AUTO_CONTROL_FREQUENCY].getValue())};this.reset=function(){clearInterval(g);g=null;c&&c()};this.observedParameterChanged=function(a){a==f[AUTO_CONTROL_FREQUENCY]?
g&&(this.reset(),this.startUpdate()):a==f[AUTO_CONTROL_TRIGGER]&&(g?this.reset():this.startUpdate())}}inheritPrototype(AutoControl,Control);function BrownianControl(a){var b=this;a||(a=.5);AutoControl.call(this,BROWNIAN,function(){var a=b.getParameter(BROWNIAN_MAX_STEP_SIZE).getValue(),a=2*a*Math.random()-a,a=b.getValue()+a,a=Math.min(Math.max(a,0),1);b.update(a)});this.addParameter(new Parameter(BROWNIAN_MAX_STEP_SIZE,.1));this.update(a);this.startUpdate()}inheritPrototype(BrownianControl,AutoControl);function RampControl(a,b){var c=this;a||(a=1E4);var d=0;b&&(d=b);var e=1!=d;AutoControl.call(this,RAMP,function(){var b=1/a*c.getParameter(AUTO_CONTROL_FREQUENCY).getValue();e||(b*=-1);d+=b;0<d&&1>d?c.update(d):1<=d?(c.update(1),c.reset()):0>=d&&(c.update(0),c.reset())},function(){e=!e});this.update(d)}inheritPrototype(RampControl,AutoControl);function RandomControl(){var a=this;AutoControl.call(this,RANDOM,function(){a.update(Math.random())});this.startUpdate()}inheritPrototype(RandomControl,AutoControl);function UIControl(a,b){var c=this;this.value=a.getValue();a.setUpdateFunction(function(a){c.value=a;b&&setTimeout(function(){b.$apply()},10)});this.getName=function(){return a.getName()};this.getType=function(){return a.getType()};this.update=function(){a.getType()==BUTTON&&(isNaN(this.value)&&(this.value=0),this.value=1-this.value);1==this.value?a.update(1):0==this.value?a.update(0):a.update(this.value)}};function OntologyLoader2(a,b,c){};function DymoLoader(a){function b(a,d,e,f,k){var g=new XMLHttpRequest;g.open("GET",a,!0);g.onload=function(){if(0>this.responseText.indexOf("Cannot GET")){var h;a:{try{JSON.parse(this.responseText)}catch(g){h=!1;break a}h=!0}h&&(d?(0<=a.indexOf(q)&&(a=a.replace(q,"")),d=d.replace('"'+a+'"',this.responseText)):d=this.responseText);(h=c(d))?(0>h.indexOf(q)&&(h=q+h),b(h,d,e,f,k)):d&&(h=k(JSON.parse(d),e),f(h))}};g.error=function(a){console.log(a)};g.send()}function c(a){var b=a.indexOf(".json");if(0<=
b){if(b!=a.indexOf("context.json")+7){var d=a.substring(0,b).lastIndexOf('"');return a.substring(d+1,b+5)}return c(a.substring(b+1))}}function d(a,b){var c=e(a,b);f(a,b);return[c,b]}function e(b,c){var d=new DynamicMusicObject(b["@id"],b.cdt,a);d.setBasePath(q);c[b["@id"]]=d;b.source&&d.setSourcePath(b.source);if(b.features)for(var f=0;f<b.features.length;f++)d.setFeature(b.features[f]["@id"],b.features[f].value);if(b.parameters)for(f=0;f<b.parameters.length;f++)r(d,b.parameters[f]["@type"],b.parameters[f].value);
...
c.addNavigator(a.navigator["@type"]==SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):new SequentialNavigator(void 0),e));return[c,d]}function l(b,c,d,e){if(b.controls){for(var f=[],g=0;g<b.controls.length;g++)f.push(e[b.controls[g]]);return k(b,c,d,f,e)}if(b.dymos){var f=[],h;if(b.dymos instanceof Array)for(g=0;g<b.dymos.length;g++)f.push(c[b.dymos[g]]);else h=Function.apply(null,b.dymos.args.concat(b.dymos.body)),g=Object.keys(c).map(function(a){return c[a]}),Array.prototype.push.apply(f,g.filter(h));
return k(b,c,d,f,e,h)}return k(b,c,d,[a],e)}function k(a,b,c,d,e,f){b=a.relative;for(var h=[],k=0;k<a.domainDims.length;k++){var g=a.domainDims[k],l=g.name,m=g["@type"];m==FEATURE?h.push(l):m==PARAMETER?(g=c?r(c,l,0):new Parameter(l,0),h.push(g)):(e&&e[l]?m=e[l]:(m=n(g),g.smooth&&m.setSmooth&&m.setSmooth(!0),!isNaN(g.average)&&m.setAverageOf&&m.setAverageOf(g.average)),"value"in g&&m.update(g.value),e&&!e[l]&&(e[l]=m),h.push(m))}return new Mapping(h,b,a["function"],d,a.range,f)}function r(a,b,c){var d=
a.getParameter(b);d?d.update(c):(d=new Parameter(b,c),a.addParameter(d));return d}function m(a,b){for(var c=0;c<a.length;c++)if(a[c])for(var d=0;d<a[c].length;d++){var e=b["dymo"+c],f=b["dymo"+a[c][d]];e&&f&&e.addSimilar(f)}}function n(a){var b=a["@type"],c=a.name;if(b==ACCELEROMETER_X||b==ACCELEROMETER_Y||b==ACCELEROMETER_Z)return new AccelerometerControl(b);if(b==TILT_X||b==TILT_Y)return new TiltControl(b);if(b==GEOLOCATION_LATITUDE||b==GEOLOCATION_LONGITUDE)return new GeolocationControl(b);if(b==
...
Control.prototype.setValue=function(a,b){if(void 0==this.value||1E-6<Math.abs(a-this.value))this.value=a,this.updateMappings(b)};Control.prototype.updateMappings=function(a){for(var b=0;b<this.mappings.length;b++)this.mappings[b]!=a&&this.mappings[b].updateParameter(this.value,this)};function DynamicMusicObject(a,b,c){function d(a,b){b[a.getUri()]=a;for(var c=0;c<a.getParts().length;c++)d(a.getParts()[c],b)}function e(a,b,c,d){var f=a-b,g=d.getParts();if(d.getLevel()==c&&0==f)return d;if(d.getLevel()>=c-1)return f<g.length?g[f]:b+g.length;for(d=0;d<g.length;d++)if(b=e(a,b,c,g[d]),isNaN(b))return b}function f(a,b,c){c(a,b);a=a.getParts();for(var d=0,e=a.length;d<e;d++)f(a[d],b,c)}function g(a,b,c){var d=b.toFlatJson();a.nodes.push(d);c&&a.links.push(k(c,d));for(c=0;c<b.getParts().length;c++)g(a,
b.getParts()[c],d)}function l(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var f=b.getSimilars()[e];c[f.getUri()]||(c[f.getUri()]=f.toFlatJson());a.links.push(k(d,c[f.getUri()]))}for(e=0;e<b.getParts().length;e++)l(a,b.getParts()[e],c)}function k(a,b){return{source:a,target:b,value:1}}var r=this,m=null,n=[],q=[],p={},t={},v=[],u=[],y="",w;t[PLAY]=new Parameter(PLAY,0,!0);t[LOOP]=new Parameter(LOOP,0,!0);
t[ONSET]=new Parameter(ONSET,void 0);t[DURATION_RATIO]=new Parameter(DURATION_RATIO,1);t[AMPLITUDE]=new Parameter(AMPLITUDE,1);t[PLAYBACK_RATE]=new Parameter(PLAYBACK_RATE,1);t[TIME_STRETCH_RATIO]=new Parameter(TIME_STRETCH_RATIO,1);t[PAN]=new Parameter(PAN,0);t[DISTANCE]=new Parameter(DISTANCE,0);t[HEIGHT]=new Parameter(HEIGHT,0);t[REVERB]=new Parameter(REVERB,0);t[DELAY]=new Parameter(DELAY,0);t[FILTER]=new Parameter(FILTER,2E4);t[PART_INDEX]=new Parameter(PART_INDEX,0,!0);t[PART_COUNT]=new Parameter(PART_COUNT,
Number.POSITIVE_INFINITY,!0);t[PLAY].addObserver(r);t[PART_INDEX].addObserver(r);t[PART_COUNT].addObserver(r);this.setType=function(a){b=a};this.getType=function(){return b};this.getUri=function(){return a};this.setParent=function(a){m=a;for(var b in t)b!=PLAY&&b!=PART_COUNT&&b!=PART_INDEX&&u.push(new Mapping([a.getParameter(b)],!0,void 0,[this],b));a=m.getMappings();var c=this.getDymoMap(),c=Object.keys(c).map(function(a){return c[a]});for(b=0;b<a.length;b++){var d=a[b].getDymoConstraint();d&&(d=
a[b].getTargets().concat(c.filter(d)),a[b].setTargets(d))}};this.removeParent=function(){for(var a=0;a<u.length;a++)u[a].disconnect();u=[];for(var b=m.getMappings(),c=this,a=0;a<b.length;a++){var d=b[a].getTargets(),d=d.filter(function(a){return!a.isSubDymoOf(c)});b[a].setTargets(d)}m=null};this.getParent=function(){return m};this.isSubDymoOf=function(a){for(var b=this;b;){if(b==a)return!0;b=b.getParent()}return!1};this.getDymoMap=function(){var a={};d(this,a);return a};this.getLevel=function(){return m?
m.getLevel()+1:0};this.getIndex=function(){if(m)return m.getParts().indexOf(this)};this.addPart=function(a){n.push(a);a.setParent(this)};this.removePart=function(a){n.splice(n.indexOf(a));a.removeParent(this)};this.replacePart=function(a,b){n[a]&&n[a].removeParent(this);n[a]=b;b.setParent(this)};this.getPart=function(a){return n[a]};this.getParts=function(){return n};this.hasParts=function(){return 0<n.length};this.getNthPart=function(a,b){return e(a,0,b,this)};this.setBasePath=function(a){y=a};this.getBasePath=
function(){return m&&!y?m.getBasePath():y};this.setSourcePath=function(a){w=a};this.getSourcePath=function(){if(m&&!w)return m.getSourcePath();if(w)return this.getBasePath()+w};this.getAllSourcePaths=function(){var a=[];f(this,a,function(b,c){var d=b.getSourcePath();d&&0>a.indexOf(d)&&c.push(d)});return a};this.getAllDymosInHierarchy=function(){var a=[];f(this,a,function(a,b){b.push(a)});return a};this.addSimilar=function(a){q.push(a)};this.getSimilars=function(a){return q};this.getFeatures=function(){return p};
this.setFeature=function(a,b){p[a]=b};this.getFeature=function(a){if(a===LEVEL)return this.getLevel();if(a===INDEX)return this.getIndex();if(p.hasOwnProperty(a))return p[a];if(m)return m.getFeature(a);for(var b=[],c=0;c<n.length;c++)if(n[c]){var d=n[c].getFeatureWithoutLookingFurther(a);isNaN(d)||b.push(d)}if(0<b.length)return b};this.getFeatureWithoutLookingFurther=function(a){if(p[a])return p[a]};this.getMappings=function(){return v};this.getSegment=function(){return[this.getFeature("time"),this.getFeature("duration")]};
this.addParameter=function(a){t[a.getName()]=a};this.getParameter=function(a){return t[a]};this.addMapping=function(a){v.push(a)};this.observedParameterChanged=function(a){a.getName()==PLAY&&c&&(0<a.getChange()?c.play(r):c.stop(r))};this.updatePartOrder=function(a){n&&n[0]&&!isNaN(n[0].getFeature(a))?n.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):n&&n.sort(function(b,c){return b.getParameter(a).getValue()-c.getParameter(a).getValue()})};this.toJsonHierarchy=function(){for(var a=this.toFlatJson(),
b=0;b<n.length;b++)a.parts.push(n[b].toJsonHierarchy());return a};this.toJsonHierarchyGraph=function(){var a={nodes:[],links:[]};g(a,this);return a};this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};l(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=b);w&&(c.source=w);for(var d in p)c[d]=this.getFeatureJson(d);if(0<v.length)for(c.mappings=[],d=0;d<v.length;d++)c.mappings.push(v[d].toJson());0<n.length&&(c.parts=[]);if(0<q.length)for(c.similars=[],d=
0;d<q.length;d++)c.similars.push(q[d].getUri());return c};this.getFeatureJson=function(a){return{value:p[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+a.slice(1)}}};function Mapping(a,b,c,d,e,f){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=f;this.init()}
...
e,f,g,l,k,r,m;l=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*l,k=e*l,f=2*e,r=f+a,m=f+d,c[m+0]=b[r+0]*k+this.pMidBuffer[f+0]*g,c[m+1]=b[r+1]*k+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
