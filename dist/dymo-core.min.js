...
new Float32Array(2048),r=e.extract(h,1024),n=0;r;){for(l=0;l<r;l++)for(var m=0;m<g.length;m++)g[m][l+n]=h[l*g.length+m%g.length];n+=r;r=e.extract(h,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var p=0;p<e.length;p++)a[f*e.length+p%e.length]=e[p][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),l=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(l,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=l[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(){0<p&&p--;0==p&&b&&b(p)}function e(a){h.splice(h.indexOf(a),1)}function f(){for(var a=[],b=0;b<h.length;b++)for(var d=$jscomp.makeIterator(h[b].getAllSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();r=a;c&&c()}function g(b,c){p++;var e=new XMLHttpRequest;e.open("GET",b,!0);e.responseType="arraybuffer";e.onload=function(){a.decodeAudioData(e.response,function(a){c?c(a):(l[b]=
a,d())})};e.error=function(){};e.send()}var l={},h=[],r=[];this.listenerOrientation=new Parameter(LISTENER_ORIENTATION,0);this.listenerOrientation.addObserver(this);var n=a.createConvolver();n.connect(a.destination);var m=a.createDelay();m.delayTime.value=.5;m.connect(a.destination);var q=a.createGain();q.gain.value=.6;m.connect(q);q.connect(m);var p=0;this.setReverbFile=function(a){g(a,function(a){n.buffer=a;d()})};this.loadBuffers=function(a){a=a.getAllSourcePaths();for(var b=0,c=a.length;b<c;b++)l[a[b]]||
...
this.observedParameterChanged=function(b){b=this.listenerOrientation.getValue()/180*Math.PI;a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0)}};function Source(a,b,c,d,e,f){function g(a,c){var d=b.createGain();d.connect(c);d.gain.value=0;a.connect(d);return d}function l(){var d=a.getParameter(TIME_STRETCH_RATIO).getValue();if(1!=d){if(0!=z||x<c.duration)c=q(c,m(z,c),m(x+.3,c));c=(new AudioProcessor(b)).timeStretch(c,d);d=x/d;c=q(c,0,m(d+.02,c));x=d}else if(0!=z||x<c.duration)c=q(c,m(z,c),m(x+.02,c));p(c,c.length);A.buffer=c;k[PLAYBACK_RATE]=A.playbackRate;h(PLAYBACK_RATE,a.getParameter(PLAYBACK_RATE))}function h(a,b){r(a,b.getValue());b.addObserver(v)}
function r(a,b,c){c&&(b+=v.getParameterValue(a));0>b&&0<=G.indexOf(a)&&(b=0);k[a]&&(k[a].value||0==k[a].value?k[a].value=b:k[a].setValue(b));0<=H.indexOf(a)&&(0==k[DISTANCE].value&&(k[DISTANCE].value=-.01),D.setPosition(k[PAN].value,k[HEIGHT].value,k[DISTANCE].value))}function n(){y=!1;var a=b.currentTime;k[AMPLITUDE].setValueAtTime(k[AMPLITUDE].value,a);k[AMPLITUDE].linearRampToValueAtTime(0,a+.02);A.stop(a+.04)}function m(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function q(a,c,d){d=Math.min(a.length-
c,d);for(var e=b.createBuffer(a.numberOfChannels,d,a.sampleRate),k=0;k<a.numberOfChannels;k++)for(var f=e.getChannelData(k),p=a.getChannelData(k),h=0;h<d;h++)f[h]=p[c+h];return e}function p(a,b){for(var c=.02*a.sampleRate,d=0;d<a.numberOfChannels;d++)for(var e=a.getChannelData(d),k=0;k<c;k++)e[k]*=k/c,e[b-k-1]*=k/c}function t(a,b,c,d){var e=a.lastIndexOf("/");e&&(a=a.substring(e+1));u("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function u(a,
c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType="arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}var v=this,y,w,k={},B=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER,PAN,HEIGHT,DISTANCE,LOOP],G=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER],H=[PAN,HEIGHT,DISTANCE],A=b.createBufferSource(),C=b.createGain();C.connect(b.destination);var E=g(C,d);d=g(C,e);
var D=b.createPanner();D.connect(C);e=b.createBiquadFilter();e.type="lowpass";e.frequency.value=2E4;e.connect(D);A.connect(e);var F=a.getSegment(),z=F[0],x=F[1];z||(z=0);!x&&c&&(x=c.duration-z);c?l():t(a.getSourcePath(),z,z+x+.3,function(d){c=d;d=a.getParameter(TIME_STRETCH_RATIO).getValue();1!=d?(c=(new AudioProcessor(b)).timeStretch(c,d),d=x/d,c=q(c,0,m(d+.02,c)),x=d):c=q(c,0,m(x+.02,c));p(c,c.length);A.buffer=c});k[AMPLITUDE]=C.gain;k[TIME_STRETCH_RATIO]={value:0};k[REVERB]=E.gain;k[DELAY]=d.gain;
k[FILTER]=e.frequency;k[PAN]={value:0};k[HEIGHT]={value:0};k[DISTANCE]={value:0};k[LOOP]={value:0};h(AMPLITUDE,a.getParameter(AMPLITUDE));h(TIME_STRETCH_RATIO,a.getParameter(TIME_STRETCH_RATIO));h(REVERB,a.getParameter(REVERB));h(DELAY,a.getParameter(DELAY));h(FILTER,a.getParameter(FILTER));h(PAN,a.getParameter(PAN));h(HEIGHT,a.getParameter(HEIGHT));h(DISTANCE,a.getParameter(DISTANCE));h(LOOP,a.getParameter(LOOP));this.observedParameterChanged=function(a){r(a.getName(),a.getChange(),!0)};this.getDymo=
...
;function SchedulerThread(a,b,c,d,e,f,g,l){function h(){w=v?v:t();n(w);B||(B=w.keys().next().value.getParameter(ONSET).getValue());var a;a=k?k-c.currentTime:SCHEDULE_AHEAD_TIME;for(var b=c.currentTime+a,d=$jscomp.makeIterator(w.values()),e=d.next();!e.done;e=d.next())e.value.play(b);setTimeout(function(){g()},a);v=t();0<v.size?(k=q(b),a=1E3*(k-c.currentTime-SCHEDULE_AHEAD_TIME),y=setTimeout(function(){h()},a)):(k=q(b),a=1E3*(k-c.currentTime),setTimeout(function(){r()},a+100))}function r(){0==u.size&&
0==v.size&&(window.clearTimeout(y),b.reset(),l&&l())}function n(a){for(var b=$jscomp.makeIterator(a.keys()),c=b.next();!c.done;c=b.next())c=c.value,u.get(c)||u.set(c,[]),u.get(c).push(a.get(c))}function m(a){var b=u.get(a.getDymo());b.slice(b.indexOf(a),1);0>=b.length&&u.delete(a.getDymo());g();r()}function q(a){if(v){var b=v.keys().next().value.getParameter(ONSET).getValue(),c=b-B;B=b;if(!isNaN(b))return a+c}for(var b=0,c=$jscomp.makeIterator(w.values()),d=c.next();!d.done;d=c.next())b=Math.max(b,
p(d.value));return a+b}function p(a){var b=a.getDymo().getParameter(PLAYBACK_RATE).getValue();return a.getDuration()/b}function t(){var a=b.getNextParts();if(a){console.log(a.map(function(a){return a.getIndex()}));for(var k=new Map,p=0;p<a.length;p++)if(a[p].getSourcePath()){var h=d[a[p].getSourcePath()];k.set(a[p],new Source(a[p],c,h,e,f,m))}return k}}var u=new Map,v,y,w=[],k,B;b||(b=new DymoNavigator(a,new SequentialNavigator(a)));h();this.hasDymo=function(b){if(a.getAllDymosInHierarchy().indexOf(b))return!0};
...
return h(b,c,d,f,e,k)}return h(b,c,d,[a],e)}function h(a,b,c,d,e,f){b=a.relative;for(var k=[],h=0;h<a.domainDims.length;h++){var g=a.domainDims[h],l=g.name,q=g["@type"];q==FEATURE?k.push(l):q==PARAMETER?(g=c?r(c,l,0):new Parameter(l,0),k.push(g)):(q=e&&e[l]?e[l]:m(g),"value"in g&&q.update(g.value),e&&!e[l]&&(e[l]=q),k.push(q))}return new Mapping(k,b,a["function"],d,a.range,f)}function r(a,b,c){var d=a.getParameter(b);d?d.update(c):(d=new Parameter(b,c),a.addParameter(d));return d}function n(a,b){for(var c=
...
a.uuid,c=parseInt(a.major,10),a=parseInt(a.minor,10),new BeaconControl(b,c,a);if(b==SLIDER||b==TOGGLE||b==BUTTON||b==CUSTOM)return new Control(c,b);if(b==RANDOM)return new RandomControl;if(b==BROWNIAN)return new BrownianControl(parseFloat(a.value));if(b==RAMP)return b=Math.round(1E3*parseFloat(a.duration)),new RampControl(b,parseFloat(a.value))}var q="";this.loadDymoFromJson=function(a,c){var e=a.lastIndexOf("/")+1;q=a.substring(0,e);b(a,"",{},c,d)};this.parseDymoFromJson=function(a,b){b(d(a,{}))};
...
function(a,b){c(b+"rendering.json",a)};this.writeDmoToRdf=function(a){a=N3.Writer({prefixes:{ch:"rdf/charm.n3#",dmo:"rdf/dmo.n3#",mb:"rdf/mobile.n3#"}});b(a);a.end(function(a,b){console.log(b)})}};function Control(a,b,c){function d(a,b){if(void 0==f||1E-6<Math.abs(a-f))f=a,e(b)}function e(a){for(var b=0;b<h.length;b++)h[b]!=a&&h[b].updateParameter(f,this)}var f,g,l,h=[],r,n,m;this.getName=function(){return a};this.getParameter=function(a){if(c)return c[a]};this.getValue=function(){return f};this.getType=function(){return b};this.getReferenceValue=function(){return g};this.setReferenceAverageCount=function(a){l=a;this.resetReferenceValue()};this.resetReferenceValue=function(){g=f=void 0;n=r=
0};this.setUpdateFunction=function(a){m=a};this.addMapping=function(a){h.push(a);a.updateParameter(f,this)};this.removeMapping=function(a){a=h.indexOf(a);-1<a&&h.splice(a,1)};this.backpropagate=function(a,b){isFinite(a)&&(d(a,b),m&&m(f))};this.update=function(a){l&&r<l?(n+=a,r++,r==l&&(g=n/=l)):isNaN(a)||(g&&(a-=g),d(a))}};function DynamicMusicObject(a,b,c){function d(a,b){b[a.getUri()]=a;for(var c=0;c<a.getParts().length;c++)d(a.getParts()[c],b)}function e(a,b,c,d){var f=a-b,g=d.getParts();if(d.getLevel()==c&&0==f)return d;if(d.getLevel()>=c-1)return f<g.length?g[f]:b+g.length;for(d=0;d<g.length;d++)if(b=e(a,b,c,g[d]),isNaN(b))return b}function f(a,b,c){c(a,b);a=a.getParts();for(var d=0,e=a.length;d<e;d++)f(a[d],b,c)}function g(a,b,c){var d=b.toFlatJson();a.nodes.push(d);c&&a.links.push(h(c,d));for(c=0;c<b.getParts().length;c++)g(a,
b.getParts()[c],d)}function l(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var f=b.getSimilars()[e];c[f.getUri()]||(c[f.getUri()]=f.toFlatJson());a.links.push(h(d,c[f.getUri()]))}for(e=0;e<b.getParts().length;e++)l(a,b.getParts()[e],c)}function h(a,b){return{source:a,target:b,value:1}}var r=this,n=null,m=[],q=[],p={},t={},u=[],v=[],y="",w;t[PLAY]=new Parameter(PLAY,0,!0);t[LOOP]=new Parameter(LOOP,0,!0);
...
[]);if(0<q.length)for(c.similars=[],d=0;d<q.length;d++)c.similars.push(q[d].getUri());return c};this.getFeatureJson=function(a){return{value:p[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+a.slice(1)}}};function Mapping(a,b,c,d,e,f){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=f;this.init()}
...
e,f,g,l,h,r,n;l=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*l,h=e*l,f=2*e,r=f+a,n=f+d,c[n+0]=b[r+0]*h+this.pMidBuffer[f+0]*g,c[n+1]=b[r+1]*h+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
