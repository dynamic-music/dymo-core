...
function DymoManager(a,b,c){var d=new Scheduler(a,function(){});c||(c="bower_components/dymo-core/audio/impulse_rev.wav");d.setReverbFile(c);SCHEDULE_AHEAD_TIME=b;var e,f={};this.loadDymoAndRendering=function(a,b,c){var l=new DymoLoader;l.loadDymoFromJson(a,function(a){l.loadRenderingFromJson(b,a[1],function(b){e=b[0];e.dymo=a[0];for(var g in b[1]){var h=b[1][g];0<=UI_CONTROLS.indexOf(h.getType())&&(f[g]=new UIControl(h))}d.loadBuffers(e.dymo);c&&c()})})};this.loadDymoFromJson=function(a,b){(new DymoLoader(d)).loadDymoFromJson(a,
...
new Float32Array(2048),l=e.extract(k,1024),p=0;l;){for(h=0;h<l;h++)for(var q=0;q<g.length;q++)g[q][h+p]=k[h*g.length+q%g.length];p+=l;l=e.extract(k,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var g=0;g<e.length;g++)a[f*e.length+g%e.length]=e[g][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),h=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(h,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=h[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(){0<q&&q--;0==q&&b&&b(q)}function e(a){k.splice(k.indexOf(a),1)}function f(){for(var a=[],b=0;b<k.length;b++)for(var d=$jscomp.makeIterator(k[b].getSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();l=a;c&&c()}function g(b,c){console.log(b);q++;var e=new XMLHttpRequest;e.open("GET",b,!0);e.responseType="arraybuffer";e.onload=function(){a.decodeAudioData(e.response,function(a){c?
c(a):(h[b]=a,d())})};e.error=function(){};e.send()}var h={},k=[],l=[];this.listenerOrientation=new Parameter(this,0);var p=a.createConvolver();p.connect(a.destination);var q=0;this.setReverbFile=function(a){g(a,function(a){p.buffer=a;d()})};this.loadBuffers=function(a){a=a.getAllSourcePaths();for(var b=0,c=a.length;b<c;b++)h[a[b]]||g(a[b])};this.getBuffer=function(a){return h[a.getSourcePath()]};this.play=function(b){b.updatePartOrder(ONSET);b=new SchedulerThread(b,void 0,a,h,p,f,e);k.push(b)};this.pause=
function(a){if(a)for(var b=0;b<k.length;b++)k[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<k.length;b++)k[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<k.length;c++){var d=k[c].getSource(a);d&&b.push(d)}return b};this.getUrisOfPlayingDymos=function(){return l};this.updateListenerOrientation=function(){var b=this.listenerOrientation.getValue()/180*Math.PI;a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0)}};function Source(a,b,c,d,e){function f(){var d=a.getParameter(TIME_STRETCH_RATIO).getValue();if(1!=d){if(0!=y||w<c.duration)c=p(c,l(y,c),l(w+.3,c));c=(new AudioProcessor(b)).timeStretch(c,d);d=w/d;c=p(c,0,l(d+.02,c));w=d}else if(0!=y||w<c.duration)c=p(c,l(y,c),l(w+.02,c));q(c,c.length);x.buffer=c;m[PLAYBACK_RATE]=x.playbackRate;g(PLAYBACK_RATE,a.getParameter(PLAYBACK_RATE))}function g(a,b){h(a,b.getValue());b.addObserver(r)}function h(a,b,c){c&&(b+=r.getParameterValue(a));0>b&&0<=z.indexOf(a)&&(b=
0);c=b;m[a]&&(m[a].value||0==m[a].value?m[a].value=c:m[a].setValue(c));0<=F.indexOf(a)?(0==m[DISTANCE].value&&(m[DISTANCE].value=-.01),D.setPosition(m[PAN].value,m[HEIGHT].value,m[DISTANCE].value)):a==LOOP&&(x.loop=1==b)}function k(){t=!1;var a=b.currentTime;m[AMPLITUDE].setValueAtTime(m[AMPLITUDE].value,a);m[AMPLITUDE].linearRampToValueAtTime(0,a+.02);x.stop(a+.04)}function l(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function p(a,c,d){d=Math.min(a.length-c,d);for(var e=b.createBuffer(a.numberOfChannels,
d,a.sampleRate),m=0;m<a.numberOfChannels;m++)for(var C=e.getChannelData(m),f=a.getChannelData(m),z=0;z<d;z++)C[z]=f[c+z];return e}function q(a,b){for(var c=.02*a.sampleRate,d=0;d<a.numberOfChannels;d++)for(var e=a.getChannelData(d),m=0;m<c;m++)e[m]*=m/c,e[b-m-1]*=m/c}function n(a,b,c,d){var e=a.lastIndexOf("/");e&&(a=a.substring(e+1));v("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function v(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType=
"arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}var r=this,t,u,m={},C=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,FILTER,PAN,HEIGHT,DISTANCE,LOOP],z=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,FILTER],F=[PAN,HEIGHT,DISTANCE],x=b.createBufferSource(),A=b.createGain();A.connect(b.destination);var B=b.createGain();B.connect(d);B.gain.value=0;A.connect(B);var D=b.createPanner();D.connect(A);
d=b.createBiquadFilter();d.type="lowpass";d.frequency.value=2E4;d.connect(D);x.connect(d);var E=a.getSegment(),y=E[0],w=E[1];y||(y=0);!w&&c&&(w=c.duration-y);c?f():n(a.getSourcePath(),y,y+w+.3,function(d){c=d;d=a.getParameter(TIME_STRETCH_RATIO).getValue();1!=d?(c=(new AudioProcessor(b)).timeStretch(c,d),d=w/d,c=p(c,0,l(d+.02,c)),w=d):c=p(c,0,l(w+.02,c));q(c,c.length);x.buffer=c});m[AMPLITUDE]=A.gain;m[TIME_STRETCH_RATIO]={value:0};m[REVERB]=B.gain;m[FILTER]=d.frequency;m[PAN]={value:0};m[HEIGHT]=
...
t};this.getParameterValue=function(a){if(m[a])return m[a].value||0==m[a].value?m[a].value:m[a].getValue()};this.play=function(b){x.onended=function(){for(var b=0;b<C.length;b++){var c=a.getParameter(C[b]);c&&c.removeObserver(r)}x.disconnect();A.disconnect();B.disconnect();e&&e(r)};x.start(b);t=!0};this.pause=function(){t?(k(),u=!0):u&&(u=!1,this.play())};this.stop=function(){t&&k()}};function SchedulerThread(a,b,c,d,e,f,g){function h(){r=u?u:q();k(r);console.log(r);m||(m=r[0].getDymo().getParameter(ONSET).getValue());var a;a=t?t-c.currentTime:SCHEDULE_AHEAD_TIME;for(var d=c.currentTime+a,e=0;e<r.length;e++)r[e].play(d);setTimeout(function(){f()},a);(u=q())?(t=p(d),v=setTimeout(function(){h()},1E3*(t-c.currentTime-SCHEDULE_AHEAD_TIME))):(b.reset(),g&&g())}function k(a){for(var b=0;b<a.length;b++)n.set(a[b].getDymo(),a[b])}function l(a){n.delete(a.getDymo());0==n.size&&window.clearTimeout(v);
f()}function p(a){if(u){var b=u[0].getDymo().getParameter(ONSET).getValue(),c=b-m;m=b;if(!isNaN(b))return a+c}b=r[0];c=b.getDymo().getParameter(PLAYBACK_RATE).getValue();b=b.getDuration()/c;return a+b}function q(){var a=b.getNextParts();if(a){for(var m=[],f=0;f<a.length;f++)if(a[f].getSourcePath()){var g=d[a[f].getSourcePath()];m.push(new Source(a[f],c,g,e,l))}return m}}var n=new Map,v,r=[],t,u,m;b||(b=new DymoNavigator(a,new SequentialNavigator(a)));h();this.pause=function(a){a=a.getAllDymosInHierarchy();
for(var b=0,c=a.length;b<c;b++){var d=n.get(a[b]);d&&d.pause()}};this.stop=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=n.get(a[b]);d&&d.stop()}};this.getSources=function(){return n};this.getSource=function(a){return n.get(a)}};function AutoControl(a,b,c){function d(a){var b=a.getName();f[b]=a;f[b].addObserver(e)}var e=this,f={};d(new Parameter(AUTO_CONTROL_FREQUENCY,100));d(new Parameter(AUTO_CONTROL_TRIGGER,0));Control.call(this,a,AUTO_CONTROL,f);var g;this.addParameter=d;this.getParameter=function(a){return f[a]};this.startUpdate=function(){g=setInterval(b,f[AUTO_CONTROL_FREQUENCY].getValue())};this.reset=function(){clearInterval(g);g=null;c&&c()};this.observedParameterChanged=function(a){a==f[AUTO_CONTROL_FREQUENCY]?
g&&(this.reset(),this.startUpdate()):a==f[AUTO_CONTROL_TRIGGER]&&(g?this.reset():this.startUpdate())}}inheritPrototype(AutoControl,Control);function BrownianControl(a){var b=this;a||(a=.5);AutoControl.call(this,BROWNIAN,function(){var a=b.getParameter(BROWNIAN_MAX_STEP_SIZE).getValue(),a=2*a*Math.random()-a,a=b.getValue()+a,a=Math.min(Math.max(a,0),1);b.update(a)});this.addParameter(new Parameter(BROWNIAN_MAX_STEP_SIZE,.1));this.update(a);this.startUpdate()}inheritPrototype(BrownianControl,AutoControl);function RampControl(a,b){var c=this;a||(a=1E4);var d=0;b&&(d=b);var e=1!=d;AutoControl.call(this,RAMP,function(){var b=1/a*c.getParameter(AUTO_CONTROL_FREQUENCY).getValue();e||(b*=-1);d+=b;0<d&&1>d?c.update(d):1<=d?(c.update(1),c.reset()):0>=d&&(c.update(0),c.reset())},function(){e=!e});this.update(d)}inheritPrototype(RampControl,AutoControl);function RandomControl(){var a=this;AutoControl.call(this,RANDOM,function(){a.update(Math.random())});this.startUpdate()}inheritPrototype(RandomControl,AutoControl);function UIControl(a,b){var c=this;this.value=a.getValue();a.setUpdateFunction(function(a){c.value=a;b&&setTimeout(function(){b.$apply()},10)});this.getName=function(){return a.getName()};this.getType=function(){return a.getType()};this.update=function(){a.getType()==BUTTON&&(isNaN(this.value)&&(this.value=0),this.value=1-this.value);1==this.value?a.update(1):0==this.value?a.update(0):a.update(this.value)}};function OntologyLoader2(a,b,c){};function DymoLoader(a){function b(a,d,e,f,g){var h=new XMLHttpRequest;h.open("GET",t+a,!0);h.onload=function(){var h;a:{try{JSON.parse(this.responseText)}catch(n){h=!1;break a}h=!0}h&&(d=d?d.replace('"'+a+'"',this.responseText):this.responseText);(h=c(d))?b(h,d,e,f,g):d&&(h=g(JSON.parse(d),e),f(h))};h.error=function(a){console.log(a)};h.send()}function c(a){var b=a.indexOf(".json");if(0<=b){if(b!=a.indexOf("context.json")+7){var d=a.substring(0,b).lastIndexOf('"');return a.substring(d+1,b+5)}return c(a.substring(b+
1))}}function d(a,b){var c=e(a,b);f(a,b);return[c,b]}function e(a,b){var c=new DynamicMusicObject(a["@id"],a.ct);c.setBasePath(t);b[a["@id"]]=c;a.source&&c.setSourcePath(a.source);for(var d in a)0>r.indexOf(d)&&(a[d].type==FEATURE?c.setFeature(d,a[d].value):a[d].type==PARAMETER&&l(c,d,a[d].value));if(a.parts)for(d=0;d<a.parts.length;d++)c.addPart(e(a.parts[d],b));return c}function f(a,b){var c=b[a["@id"]];if(a.similars)for(var d=0;d<a.similars.length;d++)c.addSimilar(b[a.similars[d]]);if(a.mappings)for(d=
0;d<a.mappings.length;d++)c.addMapping(h(a.mappings[d],b,c));if(a.parts)for(d=0;d<a.parts.length;d++)f(a.parts[d],b)}function g(a,b){for(var c=new Rendering(b[a.topDymo]),d={},e=0;e<a.mappings.length;e++)c.addMapping(h(a.mappings[e],b,void 0,d));a.navigator&&(e=Function.apply(null,a.navigator.dymos.args.concat(a.navigator.dymos.body)),c.addNavigator(a.navigator.type==SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):new SequentialNavigator(void 0),e));return[c,d]}function h(a,b,c,d){if(a.controls){for(var e=
[],f=0;f<a.controls.length;f++)e.push(d[a.controls[f]]);return k(a,b,c,e,d)}if(a.dymos){var e=[],g;if(a.dymos instanceof Array)for(f=0;f<a.dymos.length;f++)e.push(b[a.dymos[f]]);else g=Function.apply(null,a.dymos.args.concat(a.dymos.body)),f=Object.keys(b).map(function(a){return b[a]}),Array.prototype.push.apply(e,f.filter(g));return k(a,b,c,e,d,g)}}function k(a,b,c,d,e,f){b=a.relative;for(var g=[],h=0;h<a.domainDims.length;h++){var n=a.domainDims[h],k=n.name,p=n.type;p==FEATURE?g.push(k):p==PARAMETER?
(k=c?l(c,k,0):new Parameter(k,0),g.push(k)):(n=e&&e[k]?e[k]:q(n),e&&!e[k]&&(e[k]=n),g.push(n))}return new Mapping(g,b,a["function"],d,a.parameter,f)}function l(a,b,c){var d=a.getParameter(b);d?d.update(c):(d=new Parameter(b,c),a.addParameter(d));return d}function p(a,b){for(var c=0;c<a.length;c++)if(a[c])for(var d=0;d<a[c].length;d++){var e=b["dymo"+c],f=b["dymo"+a[c][d]];e&&f&&e.addSimilar(f)}}function q(b){var c=b.type,d=b.name;if(c==ACCELEROMETER_X)return n(0);if(c==ACCELEROMETER_Y)return n(1);
if(c==ACCELEROMETER_Z)return n(2);if(c==TILT_X)return n(3);if(c==TILT_Y)return n(4);if(c==GEOLOCATION_LATITUDE)return v(0);if(c==GEOLOCATION_LONGITUDE)return v(1);if(c==GEOLOCATION_DISTANCE)return v(2);if(c==COMPASS_HEADING)return a.compassWatcher||(a.compassWatcher=new CompassWatcher),b=a.compassWatcher.headingControl,b;if(c==SLIDER||c==TOGGLE||c==BUTTON)return new Control(d,c);if(c==RANDOM)return new RandomControl;if(c==BROWNIAN)return new BrownianControl(parseFloat(b.value));if(c==RAMP)return c=
Math.round(1E3*parseFloat(b.duration)),new RampControl(c,parseFloat(b.value))}function n(b){a.accelerometerWatcher||(a.accelerometerWatcher=new AccelerometerWatcher);if(0==b)return a.accelerometerWatcher.xControl;if(1==b)return a.accelerometerWatcher.yControl;if(2==b)return a.accelerometerWatcher.zControl;if(3==b)return a.accelerometerWatcher.tiltXControl;if(4==b)return a.accelerometerWatcher.tiltYControl}function v(b){a.geolocationWatcher||(a.geolocationWatcher=new GeolocationWatcher);return 0==
b?a.geolocationWatcher.latitudeControl:1==b?a.geolocationWatcher.longitudeControl:a.geolocationWatcher.distanceControl}var r="@id @type ct source navigator similars mappings parts".split(" "),t="";this.loadDymoFromJson=function(a,c){var e=a.lastIndexOf("/")+1;t=a.substring(0,e);a=a.substring(e);b(a,"",{},c,d)};this.parseDymoFromJson=function(a,b){b(d(a,{}))};this.loadRenderingFromJson=function(a,c,d){b(a,"",c,d,g)};this.loadGraphFromJson=function(a,c,d){b(a,"",c,d,p)}};function DymoWriter(a){function b(a){a.addTriple("_:","http://www.w3.org/1999/02/22-rdf-syntax-ns#type","dmo:DMO");a.addTriple("_:","ch:hasPart","ch:test2")}function c(a,b){var c=new XMLHttpRequest;c.send(b);c.addEventListener("save",function(){console.log("saved "+a)});c.addEventListener("error",function(){console.log("saving "+a+" failed")});c.open("POST",a);c.send()}this.writeDymoToJson=function(a,b,f){a["@context"]="http://purl.org/ontology/dymo/context.json";f||(f="dymo.json");c(b+f,a)};this.writeRenderingToJson=
function(a,b){c(b+"rendering.json",a)};this.writeDmoToRdf=function(a){a=N3.Writer({prefixes:{ch:"rdf/charm.n3#",dmo:"rdf/dmo.n3#",mb:"rdf/mobile.n3#"}});b(a);a.end(function(a,b){console.log(b)})}};var SCHEDULE_AHEAD_TIME=.1,DYMO="Dymo",PARALLEL="parallel",SEQUENTIAL="sequential",DYMO_TYPES=[PARALLEL,SEQUENTIAL],FEATURE="Feature",PARAMETER="Parameter",ONE_SHOT_NAVIGATOR="OneShotNavigator",SEQUENTIAL_NAVIGATOR="SequentialNavigator",SIMILARITY_NAVIGATOR="SimilarityNavigator",PLAY="Play",LOOP="Loop",ONSET="Onset",DURATION_RATIO="DurationRatio",AMPLITUDE="Amplitude",PLAYBACK_RATE="PlaybackRate",TIME_STRETCH_RATIO="TimeStretchRatio",PAN="Pan",DISTANCE="Distance",HEIGHT="Height",REVERB="Reverb",FILTER=
...
RANDOM="Random",BROWNIAN="Brownian",RAMP="Ramp",CONTROLS=[SLIDER,TOGGLE,ACCELEROMETER_X,ACCELEROMETER_Y,ACCELEROMETER_Z,TILT_X,TILT_Y,GEOLOCATION_LATITUDE,GEOLOCATION_LONGITUDE,GEOLOCATION_DISTANCE,COMPASS_HEADING,RANDOM,BROWNIAN,RAMP],UI_CONTROLS=[SLIDER,TOGGLE,BUTTON];function Control(a,b,c){function d(a,b){if(void 0==f||1E-6<Math.abs(a-f))f=a,e(b)}function e(a){for(var b=0;b<k.length;b++)k[b]!=a&&k[b].updateParameter(f,this)}var f,g,h,k=[],l,p,q;this.getName=function(){return a};this.getParameter=function(a){if(c)return c[a]};this.getValue=function(){return f};this.getType=function(){return b};this.getReferenceValue=function(){return g};this.setReferenceAverageCount=function(a){h=a;this.resetReferenceValue()};this.resetReferenceValue=function(){g=f=void 0;p=l=
0};this.setUpdateFunction=function(a){q=a};this.addMapping=function(a){k.push(a);a.updateParameter(f,this)};this.removeMapping=function(a){a=k.indexOf(a);-1<a&&k.splice(a,1)};this.backpropagate=function(a,b){isFinite(a)&&(d(a,b),q&&q(f))};this.update=function(a){h&&l<h?(p+=a,l++,l==h&&(g=p/=h)):isNaN(a)||(g&&(a-=g),d(a))}};function DynamicMusicObject(a,b){function c(a,b){b[a.getUri()]=a;for(var d=0;d<a.getParts().length;d++)c(a.getParts()[d],b)}function d(a,b,c,e){var f=a-b,g=e.getParts();if(e.getLevel()==c&&0==f)return e;if(e.getLevel()>=c-1)return f<g.length?g[f]:b+g.length;for(e=0;e<g.length;e++)if(b=d(a,b,c,g[e]),isNaN(b))return b}function e(a,b,c){c(a,b);a=a.getParts();for(var d=0,f=a.length;d<f;d++)e(a[d],b,c)}function f(a,b,c){var d=b.toFlatJson();a.nodes.push(d);c&&a.links.push(h(c,d));for(c=0;c<b.getParts().length;c++)f(a,
b.getParts()[c],d)}function g(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var f=b.getSimilars()[e];c[f.getUri()]||(c[f.getUri()]=f.toFlatJson());a.links.push(h(d,c[f.getUri()]))}for(e=0;e<b.getParts().length;e++)g(a,b.getParts()[e],c)}function h(a,b){return{source:a,target:b,value:1}}var k=null,l=[],p=[],q={},n={},v=[],r=[],t="",u;n[PLAY]=new Parameter(PLAY,0,!0);n[LOOP]=new Parameter(LOOP,0,!0);n[ONSET]=
new Parameter(ONSET,void 0);n[DURATION_RATIO]=new Parameter(DURATION_RATIO,1);n[AMPLITUDE]=new Parameter(AMPLITUDE,1);n[PLAYBACK_RATE]=new Parameter(PLAYBACK_RATE,1);n[TIME_STRETCH_RATIO]=new Parameter(TIME_STRETCH_RATIO,1);n[PAN]=new Parameter(PAN,0);n[DISTANCE]=new Parameter(DISTANCE,0);n[HEIGHT]=new Parameter(HEIGHT,0);n[REVERB]=new Parameter(REVERB,0);n[FILTER]=new Parameter(FILTER,0);n[PART_INDEX]=new Parameter(PART_INDEX,0,!0);n[PART_COUNT]=new Parameter(PART_COUNT,Number.POSITIVE_INFINITY,
!0);n[PLAY].addObserver(this);n[PART_INDEX].addObserver(this);n[PART_COUNT].addObserver(this);this.setType=function(a){b=a};this.getType=function(){return b};this.getUri=function(){return a};this.setParent=function(a){k=a;for(var b in n)b!=PLAY&&b!=PART_COUNT&&b!=PART_INDEX&&r.push(new Mapping([a.getParameter(b)],!0,void 0,[this],b));a=k.getMappings();var c=this.getDymoMap(),c=Object.keys(c).map(function(a){return c[a]});for(b=0;b<a.length;b++){var d=a[b].getDymoConstraint();d&&(d=a[b].getTargets().concat(c.filter(d)),
...
function(){if(k)return k.getParts().indexOf(this)};this.addPart=function(a){l.push(a);a.setParent(this)};this.removePart=function(a){l.splice(l.indexOf(a));a.removeParent(this)};this.replacePart=function(a,b){l[a]&&l[a].removeParent(this);l[a]=b;b.setParent(this)};this.getPart=function(a){return l[a]};this.getParts=function(){return l};this.hasParts=function(){return 0<l.length};this.getNthPart=function(a,b){return d(a,0,b,this)};this.setBasePath=function(a){t=a};this.getBasePath=function(){return k&&
!t?k.getBasePath():t};this.setSourcePath=function(a){u=a};this.getSourcePath=function(){if(k&&!u)return k.getSourcePath();if(u)return this.getBasePath()+u};this.getAllSourcePaths=function(){var a=[];e(this,a,function(b,c){var d=b.getSourcePath();d&&0>a.indexOf(d)&&c.push(d)});return a};this.getAllDymosInHierarchy=function(){var a=[];e(this,a,function(a,b){b.push(a)});return a};this.addSimilar=function(a){p.push(a)};this.getSimilars=function(a){return p};this.getFeatures=function(){return q};this.setFeature=
function(a,b){q[a]=b};this.getFeature=function(a){if(a===LEVEL)return this.getLevel();if(a===INDEX)return this.getIndex();if(q.hasOwnProperty(a))return q[a];if(k)return k.getFeature(a);for(var b=[],c=0;c<l.length;c++)if(l[c]){var d=l[c].getFeatureWithoutLookingFurther(a);isNaN(d)||b.push(d)}if(0<b.length)return b};this.getFeatureWithoutLookingFurther=function(a){if(q[a])return q[a]};this.getMappings=function(){return v};this.getSegment=function(){return[this.getFeature("time"),n[DURATION_RATIO].getValue()*
this.getFeature("duration")]};this.addParameter=function(a){n[a.getName()]=a};this.getParameter=function(a){return n[a]};this.addMapping=function(a){v.push(a)};this.observedParameterChanged=function(a){a.getName()==PLAY&&a.getChange()};this.updatePartOrder=function(a){l&&l[0]&&!isNaN(l[0].getFeature(a))?l.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):l&&l.sort(function(b,c){return b.getParameter(a).getValue()-c.getParameter(a).getValue()})};this.toJsonHierarchy=function(){for(var a=
this.toFlatJson(),b=0;b<l.length;b++)a.parts.push(l[b].toJsonHierarchy());return a};this.toJsonHierarchyGraph=function(){var a={nodes:[],links:[]};f(a,this);return a};this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};g(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=b);u&&(c.source=u);for(var d in q)c[d]=this.getFeatureJson(d);if(0<v.length)for(c.mappings=[],d=0;d<v.length;d++)c.mappings.push(v[d].toJson());0<l.length&&(c.parts=[]);if(0<p.length)for(c.similars=
[],d=0;d<p.length;d++)c.similars.push(p[d].getUri());return c};this.getFeatureJson=function(a){return{value:q[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+a.slice(1)}}};function Mapping(a,b,c,d,e,f){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=f;this.init()}
...
PolygonTools.getInterpolatedPolygonFunctionArgs=function(a){var b=getPolygonOrPointString(a);a=getPolygonOrPointString(PolygonTools.getCentroid(a));return["a","b","return PolygonTools.howFarIsPointInPolygon("+b+","+a+", {0:a, 1:b});"]};function getPolygonOrPointString(a){return JSON.stringify(a).replace(/"/g,"'")};function DymoNavigator(a,b){function c(a){if(a.hasParts()){var c;if(!f.has(a)){c=0;for(var k=e.length;c<k;c++)e[c][0](a)&&f.set(a,e[c][1].getCopy(a));f.has(a)||(b?f.set(a,b.getCopy(a)):f.set(a,new OneShotNavigator(a)))}if(c=f.get(a)){a.getLevel();if(c.getType()==ONE_SHOT_NAVIGATOR)return c.getCurrentParts();a=c.getCurrentParts();return(a=d(a))?a:d(c.getNextParts())}}else return[a]}function d(a){if(a&&0<a.length){for(var b=[],d=0,e=a.length;d<e;d++){var f=c(a[d]);f&&(f.length?b=b.concat(f):b.push(f))}if(0<
...
e,f,g,h,k,l,p;h=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*h,k=e*h,f=2*e,l=f+a,p=f+d,c[p+0]=b[l+0]*k+this.pMidBuffer[f+0]*g,c[p+1]=b[l+1]*k+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
