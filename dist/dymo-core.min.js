...
g[a];if(c!==c&&f.key!==f.key||c===f.key)return{id:d,list:g,index:a,entry:f}}return{id:d,list:g,index:-1,entry:void 0}},e=function(a,b){var c=a.head_;return $jscomp.iteratorPrototype(function(){if(c){for(;c.head!=a.head_;)c=c.previous;for(;c.next!=c.head;)return c=c.next,{done:!1,value:b(c)};c=null}return{done:!0,value:void 0}})},h=function(){var a={};return a.previous=a.next=a.head=a},k=0;return c},"es6-impl","es3");
...
$jscomp.polyfill("Number.isNaN",function(a){return a?a:function(a){return"number"===typeof a&&isNaN(a)}},"es6-impl","es3");$jscomp.polyfill("Array.from",function(a){return a?a:function(a,c,d){$jscomp.initSymbolIterator();c=null!=c?c:function(a){return a};var b=[],h=a[Symbol.iterator];if("function"==typeof h)for(a=h.call(a);!(h=a.next()).done;)b.push(c.call(d,h.value));else for(var h=a.length,k=0;k<h;k++)b.push(c.call(d,a[k]));return b}},"es6-impl","es3");$jscomp.ASSUME_NO_NATIVE_SET=!1;
$jscomp.polyfill("Set",function(a){if(!$jscomp.ASSUME_NO_NATIVE_SET&&function(){if(!a||!a.prototype.entries||"function"!=typeof Object.seal)return!1;try{var b=Object.seal({x:4}),d=new a($jscomp.makeIterator([b]));if(!d.has(b)||1!=d.size||d.add(b)!=d||1!=d.size||d.add({x:4})!=d||2!=d.size)return!1;var e=d.entries(),h=e.next();if(h.done||h.value[0]!=b||h.value[1]!=b)return!1;h=e.next();return h.done||h.value[0]==b||4!=h.value[0].x||h.value[1]!=h.value[0]?!1:e.next().done}catch(k){return!1}}())return a;
...
b.prototype.values=function(){return this.map_.values()};b.prototype[Symbol.iterator]=b.prototype.values;b.prototype.forEach=function(a,b){var c=this;this.map_.forEach(function(d){return a.call(b,d,d,c)})};return b},"es6-impl","es3");
function DymoManager(a,b,c,d,e,h){function k(a,b,d,h){f=b;p=d[0];m=a.getMappings();for(var r in d[1])a=d[1][r],DYMO_STORE.isSubclassOf(a.getType(),UI_CONTROL)&&(g[r]=new UIControl(a,e)),DYMO_STORE.isSubclassOf(a.getType(),SENSOR_CONTROL)&&(n[r]=a);c||(c="bower_components/dymo-core/audio/impulse_rev.wav");l.init(c,b,h)}DYMO_STORE=new DymoStore(d);var l=new Scheduler(a,h);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var f,p,g={},n={},m={};this.loadDymoAndRenderingFromStore=function(a,b){DYMO_STORE=a;a=new DymoLoader(DYMO_STORE);
k(a,a.createDymoFromStore(),a.createRenderingFromStore(),b)};this.loadDymoAndRendering=function(a,b,c,d){var g=new DymoLoader(DYMO_STORE);g.loadDymoFromJson(a,function(a){g.loadRenderingFromJson(b,function(b){k(g,a,b,d);c&&c()})})};this.loadDymoFromJson=function(a,b){(new DymoLoader(DYMO_STORE)).loadDymoFromJson(a,b)};this.parseDymoFromJson=function(a,b){(new DymoLoader(DYMO_STORE)).parseDymoFromJson(a,b)};this.replacePartOfTopDymo=function(a,b){a=DYMO_STORE.replacePartAt(f[0],b,a);l.stop(a)};this.updateNavigatorPosition=
function(a,b,c){l.updateNavigatorPosition(a,b,c)};this.getNavigatorPosition=function(a,b){return l.getNavigatorPosition(a,b)};this.syncNavigators=function(a,b,c){l.syncNavigators(a,b,c)};this.startPlayingUri=function(a){l.play(a)};this.stopPlayingUri=function(a){l.stop(a)};this.startPlaying=function(){for(var a=0;a<f.length;a++)DYMO_STORE.updatePartOrder(f[a],ONSET),l.play(f[a],p.getNavigator())};this.stopPlaying=function(){for(var a=0;a<f.length;a++)l.stop(f[a])};this.getTopDymo=function(){return f[0]};
this.getRendering=function(){return p};this.getMappings=function(){return m};this.getUIControls=function(){return g};this.getUIControl=function(a){return g[a]};this.getSensorControls=function(){return n}};function DymoNode(a,b,c,d){this.POSITION_PARAMS=[PAN,HEIGHT,DISTANCE];this.dymoUri=a;this.parameters={};this.init(b,c,d)}
...
DymoNode.prototype.removeAndDisconnect=function(){for(var a in this.parameters)DYMO_STORE.removeParameterObserver(this.dymoUri,a,this);this.dryGain.disconnect();this.reverbGain&&this.reverbGain.disconnect();this.delayGain&&this.delayGain.disconnect()};function AudioProcessor(a){function b(a){return{extract:function(b,c,h){for(var d=[],e=0;e<a.numberOfChannels;e++)d.push(a.getChannelData(e));for(e=0;e<c;e++)for(var f=0;f<d.length;f++)b[e*d.length+f%d.length]=d[f][e+h];return Math.min(c,d[0].length-h)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;var h=b(c),e=new SimpleFilter(h,e);d=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate);for(var h=[],k=0;k<c.numberOfChannels;k++)h.push(d.getChannelData(k));c=
new Float32Array(2048);for(var l=e.extract(c,1024),f=0;l;){for(k=0;k<l;k++)for(var p=0;p<h.length;p++)h[p][k+f]=c[k*h.length+p%h.length];f+=l;l=e.extract(c,1024)}return d}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var h=new SimpleFilter({extract:function(a,c,d){for(var g=[],f=0;f<b.numberOfChannels;f++)g.push(b.getChannelData(f));for(f=0;f<c;f++)for(var e=0;e<g.length;e++)a[f*g.length+e%g.length]=g[e][f+d];return Math.min(c,g[0].length-d)}},e),k=a.createScriptProcessor(1024,
2,b.numberOfChannels),l=new Float32Array(2048);k.onaudioprocess=function(a){for(var c=[],g=0;g<b.numberOfChannels;g++)c.push(a.outputBuffer.getChannelData(g));a=h.extract(l,1024);0==a&&d.stop();for(g=0;g<a;g++)for(var f=0;f<c.length;f++)c[f][g]=l[g*c.length+f%c.length]};this.start=function(a){setTimeout(function(){k.connect(c)},a)};this.stop=function(){k.disconnect()}};function Scheduler(a,b){function c(a,b){for(var c=[],d=0,g=a.length;d<g;d++)var f=DYMO_STORE.findAllObjectsInHierarchy(a[d]),c=c.concat(f.map(function(a){return DYMO_STORE.getSourcePath(a)}));d=0;for(g=c.length;d<g;d++)c[d]&&!l[c[d]]&&h(c[d],function(a,c){l[c]=a;0<n&&n--;0==n&&b&&b()});0==n&&b&&b()}function d(a){f.splice(f.indexOf(a),1)}function e(){for(var a=[],c=0;c<f.length;c++)for(var d=$jscomp.makeIterator(f[c].getAllSources().keys()),g=d.next();!g.done;g=d.next())for(g=g.value;null!=g;)0>a.indexOf(g)&&
a.push(g),g=DYMO_STORE.findParents(g)[0];a.sort();p=a;b&&b(p)}function h(b,c){n++;var d=new XMLHttpRequest;d.open("GET",b,!0);d.responseType="arraybuffer";d.onload=function(){a.decodeAudioData(d.response,function(a){c(a,b)})};d.error=function(){};d.send()}var k=this,l={},f=[],p=[],g,n=0;this.init=function(b,d,f){DYMO_STORE.addParameter(null,LISTENER_ORIENTATION,0,k);c(d,f);b&&0<DYMO_STORE.find(null,null,REVERB).length&&(g=a.createConvolver(),g.connect(a.destination),h(b,function(a){g.buffer=a;0<n&&
n--;0==n&&f&&f()}));0<DYMO_STORE.find(null,null,DELAY).length&&(b=a.createDelay(),b.delayTime.value=.5,b.connect(a.destination),d=a.createGain(),d.gain.value=.6,b.connect(d),d.connect(b));DYMO_STORE.addTypeObserver(PLAY,VALUE,k)};this.getBuffer=function(a){return l[DYMO_STORE.getSourcePath(a)]};this.updateNavigatorPosition=function(a,b,c){for(var d=0,g=f.length;d<g;d++)f[d].hasDymo(a)&&f[d].getNavigator().setPosition(c,b,a)};this.getNavigatorPosition=function(a,b){for(var c=0,d=f.length;c<d;c++)if(f[c].hasDymo(a))return f[c].getNavigator().getPosition(b,
a)};this.syncNavigators=function(a,b,c){for(var d,g,m=0,n=f.length;m<n;m++)f[m].hasDymo(a)&&(d=f[m].getNavigator()),f[m].hasDymo(b)&&(g=f[m].getNavigator());g&&(b=g.getPosition(c,b),d.setPosition(b,c,a))};this.play=function(b,c){b=new SchedulerThread(b,c,a,l,g,void 0,e,d);f.push(b)};this.pause=function(a){if(a)for(var b=0;b<f.length;b++)f[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<f.length;b++)f[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<f.length;c++){var d=f[c].getSources(a);
if(d)for(var g=0;g<d.length;g++)b=b.concat(d)}return b};this.getUrisOfPlayingDymos=function(){return p};this.observedValueChanged=function(b,c,d){c==LISTENER_ORIENTATION?(d=d/180*Math.PI,a.listener.setOrientation(Math.sin(d),0,-Math.cos(d),0,1,0)):c==PLAY&&(b=DYMO_STORE.findSubject(HAS_PARAMETER,b),0<d?this.play(b):this.stop(b))}};function DymoSource(a,b,c,d,e,h){function k(b,d){var f=b[0],e=b[1],e=1!=d?e+.3:e+FADE_LENGTH;if(c||isNaN(f+e)){if(0!=f||e<c.duration)c=g(c,p(f,c),p(e,c));return l(c,e,d)}n(DYMO_STORE.getSourcePath(a),f,f+e,function(a){return l(a,e,d)})}function l(a,c,d){1!=d&&(a=(new AudioProcessor(b)).timeStretch(a,d),a=g(a,0,p(c/d+FADE_LENGTH,a)));c=a;d=a.length;for(var f=c.sampleRate*FADE_LENGTH,e=0;e<c.numberOfChannels;e++)for(var n=c.getChannelData(e),m=0;m<f;m++)n[m]*=m/f,n[d-m-1]*=m/f;return a}function f(){v=
!1;var a=b.currentTime;r.parameters[AMPLITUDE].setValueAtTime(r.parameters[AMPLITUDE].value,a);r.parameters[AMPLITUDE].linearRampToValueAtTime(0,a+FADE_LENGTH);q.stop(a+2*FADE_LENGTH)}function p(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function g(a,c,d){d=Math.min(a.length-c,d);for(var g=b.createBuffer(a.numberOfChannels,d,a.sampleRate),f=0;f<a.numberOfChannels;f++)for(var e=g.getChannelData(f),n=a.getChannelData(f),m=0;m<d;m++)e[m]=n[c+m];return g}function n(a,b,c,d){var g=a.lastIndexOf("/");
g&&(a=a.substring(g+1));m("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function m(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType="arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}DymoNode.call(this,a,b,d,e);var r=this,v,t,w,q=b.createBufferSource();q.connect(this.getInput());(function(){var b=[DYMO_STORE.findFeatureValue(a,TIME_FEATURE),
DYMO_STORE.findFeatureValue(a,DURATION_FEATURE)];b[0]||(b[0]=0);!b[1]&&c&&(b[1]=c.duration-b[0]);var d=DYMO_STORE.findParameterValue(a,DURATION_RATIO);d&&0<d&&1>d&&(b[1]*=d);w=b[1];d=DYMO_STORE.findParameterValue(a,TIME_STRETCH_RATIO);q.buffer=k(b,d?d:1)})();this.addParameter(PLAYBACK_RATE,q.playbackRate);this.addParameter(TIME_STRETCH_RATIO,{value:0});this.addParameter(LOOP,{value:0});this.getDuration=function(){return w};this.isLoopingAndPlaying=function(){return q.loop&&v};this.play=function(b){q.onended=
function(){q.disconnect();r.removeAndDisconnect();h&&h(r)};b||(b=0);DYMO_STORE.findParameterValue(a,LOOP)&&(q.loop=!0);q.start(b);v=!0};this.pause=function(){v?(f(),t=!0):t&&(t=!1,this.play())};this.stop=function(){v&&f()}}inheritPrototype(DymoSource,DymoNode);function SchedulerThread(a,b,c,d,e,h,k,l){function f(){var a=z;z=q?q:r();p(z);if(!u){var b=z.keys().next().value;u=DYMO_STORE.findParameterValue(b,ONSET)}b=y?y-c.currentTime:SCHEDULE_AHEAD_TIME;for(var d=c.currentTime+b,g=$jscomp.makeIterator(z.values()),e=g.next();!e.done;e=g.next())e=e.value,e.play(d);a=$jscomp.makeIterator(a.values());for(e=a.next();!e.done;e=a.next())e=e.value,DYMO_STORE.findParameterValue(e.getDymoUri(),LOOP)&&e.stop();setTimeout(function(){k()},b);(q=r())&&0<q.size?(y=m(d),
a=y[1],y=y[0],DYMO_STORE.findParameterValue(a.getDymoUri(),LOOP)&&(y-=FADE_LENGTH),a=1E3*(y-c.currentTime-SCHEDULE_AHEAD_TIME),x=setTimeout(function(){f()},a)):(y=m(d),a=1E3*(y-c.currentTime),setTimeout(function(){n()},a+100))}function p(a){for(var b=$jscomp.makeIterator(a.keys()),c=b.next();!c.done;c=b.next())c=c.value,t.get(c)||t.set(c,[]),t.get(c).push(a.get(c))}function g(a){var b=t.get(a.getDymoUri());b.splice(b.indexOf(a),1);0>=b.length&&t.delete(a.getDymoUri());k();n()}function n(){if(0==t.size&&
(!q||0==q.size)){window.clearTimeout(x);b.reset();for(var c=DYMO_STORE.findAllObjectsInHierarchy(a),d=0,g=c.length;d<g;d++){var f=w.get(c[d]);f&&f.removeAndDisconnect()}l&&l()}}function m(a){if(q){var b=q.keys().next().value,b=DYMO_STORE.findParameterValue(b,ONSET),c=b-u;u=b;if(!isNaN(b))return a+c}for(var b=0,d,c=$jscomp.makeIterator(z.values()),g=c.next();!g.done;g=c.next()){var g=g.value,f=g.getDuration();f>b&&(b=f,d=g)}return[a+b,d]}function r(){var a=b.getNextParts();if(a){v(a);for(var f=new Map,
n=0;n<a.length;n++){var m=DYMO_STORE.getSourcePath(a[n]);if(m){m=new DymoSource(a[n],c,d[m],e,h,g);a:{for(var p=m,k=DYMO_STORE.findParents(m.getDymoUri())[0];k;){if(w.has(k)){p.connect(w.get(k).getInput());break a}var r=new DymoNode(k,c,e,h);p.connect(r.getInput());w.set(k,r);k=DYMO_STORE.findParents(k)[0];p=r}p.connect(c.destination)}f.set(a[n],m)}}return f}}function v(a){console.log(a.map(function(a){a=DYMO_STORE.findPartIndex(a);return isNaN(a)?"top":a}))}var t=new Map,w=new Map,q,x,z=new Map,
y,u;b||(b=new DymoNavigator(a,new SequentialNavigator(a)));f();this.hasDymo=function(b){if(DYMO_STORE.findAllObjectsInHierarchy(a).indexOf(b))return!0};this.getNavigator=function(){return b};this.stop=function(a){a=DYMO_STORE.findAllObjectsInHierarchy(a);for(var b=0,c=a.length;b<c;b++){var d=t.get(a[b]);if(d)for(var g=0;g<d.length;g++)d[g].stop();q&&(q.delete(a[b]),0>=q.size&&(q=void 0))}};this.getAllSources=function(){return t};this.getSources=function(a){return t.get(a)}};var AutoControl=function(a,b,c){Control.call(this,a,b,AUTO_CONTROL);c||(c=100);DYMO_STORE.addParameter(a,AUTO_CONTROL_FREQUENCY,c,this);DYMO_STORE.addParameter(a,AUTO_CONTROL_TRIGGER,0,this);this.intervalID=null};$jscomp.inherits(AutoControl,Control);AutoControl.prototype.startUpdate=function(a){var b=this;a||(a=DYMO_STORE.findParameterValue(this.uri,AUTO_CONTROL_FREQUENCY));this.intervalID=setInterval(function(){return b.update()},a)};AutoControl.prototype.update=function(){};
AutoControl.prototype.reset=function(){clearInterval(this.intervalID);this.intervalID=null};AutoControl.prototype.observedValueChanged=function(a,b,c){b==AUTO_CONTROL_FREQUENCY?this.intervalID&&(this.reset(),this.startUpdate(c)):b==AUTO_CONTROL_TRIGGER&&(!this.intervalID&&0<c?this.startUpdate():this.intervalID&&0==c&&this.reset())};var DataControl=function(a,b,c,d){d||(d=1001);AutoControl.call(this,a,DATA_CONTROL,d);this.url=b;this.jsonMap=c};$jscomp.inherits(DataControl,AutoControl);DataControl.prototype.update=function(){var a=this;fetch(this.url).then(function(a){return a.json()}).then(function(b){return a.jsonMap(b)}).then(function(b){console.log("data received:",b);a.updateValue(b)})};function UIControl(a,b){var c=this;this.value=a.getValue();a.setUpdateFunction(function(a){c.value=a;b&&setTimeout(function(){b.$apply()},10)});this.getName=function(){return a.getName()};this.getType=function(){return a.getType()};this.update=function(){a.getType()==BUTTON&&(isNaN(this.value)&&(this.value=0),this.value=1-this.value);1==this.value?a.updateValue(1):0==this.value?a.updateValue(0):a.updateValue(this.value)}};function DymoGenerator(a,b){function c(b,c,g){var f=d();a.addDymo(f,b,null,c,g);b||(m=f);return f}function d(){var a=CONTEXT_URI+"dymo"+x;x++;return a}function e(a){var b=[];if(a&&0<a.length){for(var c=0;c<a.length;c++)a[c].value.constructor!==Array&&(a[c].value=[a[c].value]);for(var c=a[0].value.length,d=0;d<c;d++)if("string"==typeof a[0].value[d])b[d]=a[0].value[d];else if(w==SUMMARY.FIRST)b[d]=a[0].value[d];else if(w==SUMMARY.MEAN)b[d]=a.reduce(function(a,b){return a+b.value[d]},0)/a.length;else if(w==
SUMMARY.MEDIAN){a.sort(function(a,b){return a.value[d]-b.value[d]});var g=Math.floor(a.length/2);b[d]=a[g].value[d];0==a.length%2&&(b[d]+=a[g-1].value[d])}return 1==b.length?b[0]:b}return 0}function h(){0==x?m=c(void 0,q):v&&(m=c(n,q),v=!1)}function k(b,c,d){d||(d=m);var g=d;for(d=a.findLevel(d);d<c;){var f=a.findParts(g);if(0<f.length){f=f.map(function(b){return[a.findFeatureValue(b,TIME_FEATURE),b]});f.sort(function(a,b){return a[0]-b[0]});for(var e=0;e<f.length;e++)if(f[e][0]<=b)g=f[e][1];else if(0==
e)g=f[e][1];else break;d++}else break}return g}function l(a,b){void 0==a.max?(a.min=b,a.max=b):(a.min=Math.min(b,a.min),a.max=Math.max(b,a.max))}function f(a,b){for(var c=0;c<t.length;c++)if(t[c].name==a||t[c].uri==b)return t[c];return p(a,b)}function p(a,c,d,g){!a&&c&&(a=URI_TO_TERM[c]);a&&!c&&(c=CONTEXT_URI+a);a=void 0!=d&&void 0!=g?{name:a,uri:c,min:d,max:g}:{name:a,uri:c,min:1E3,max:0};2>t.length?t.push(a):t.splice(t.length-2,0,a);b(a);return a}var g=this,n,m,r,v,t,w=SUMMARY.MEAN,q,x=0;this.resetDymo=
function(){m=n=void 0;v=!1;t=[];p("level",LEVEL_FEATURE);p("random",null,0,1)};this.setStore=function(b){a=b};this.getStore=function(){return a};this.addRendering=function(){var b=CONTEXT_URI+"rendering0";b++;r=b;a.addRendering(r,m)};this.addMapping=function(b,c,d,g){a.addMapping(b,c,d,g)};this.addNavigator=function(b,c,d){a.addNavigator(r,b,c,d)};this.getCurrentTopDymo=function(){return m};this.getFeatures=function(){return t};this.setSummarizingMode=function(a){w=a};this.setCurrentSourcePath=function(a){q=
a};this.setAudioFileChanged=function(){v=!0;if(m){var b=d();a.addDymo(b,null,m);m=b}};this.addDymo=function(a,b,d){return c(a,b,d)};this.addFeature=function(b,c,d){d||(d=m);Benchmarker.startTask("addFeature");h();b=f(b);d=a.findAllObjectsInHierarchy(d);for(var g=0;g<d.length;g++){var n=a.findFeatureValue(d[g],TIME_FEATURE),p=a.findFeatureValue(d[g],DURATION_FEATURE),k=c;isNaN(n)||(k=k.filter(function(a){return n<=a.time&&(isNaN(p)||a.time<n+p)}));if(1>k.length)var r=c.filter(function(a){return a.time.value<=
n}),k=0<r.length?[r[k.length-1]]:[c[0]];Benchmarker.startTask("summarize");k=e(k);this.setDymoFeature(d[g],b.uri,k)}};this.addSegmentation=function(b,d){h();for(var f=a.findMaxLevel(),e=0;e<b.length;e++){var n=k(b[e].time,f,d),m=b[e].time,p;if(b[e].duration)p=b[e].duration;else if(b[e+1])p=b[e+1].time-m;else{var r=a.findFeatureValue(n,TIME_FEATURE),l=a.findFeatureValue(n,DURATION_FEATURE);r&&l&&(p=r+l-m)}if(0<p){r=c(n);this.setDymoFeature(r,TIME_FEATURE,m);this.setDymoFeature(r,DURATION_FEATURE,p);
var x=r,m=a.findFeatureValue(n,TIME_FEATURE),r=a.findFeatureValue(x,TIME_FEATURE);if(isNaN(m)||Array.isArray(m)||r<m)g.setDymoFeature(n,TIME_FEATURE,r),m=r;l=a.findFeatureValue(n,DURATION_FEATURE);x=a.findFeatureValue(x,DURATION_FEATURE);(isNaN(l)||Array.isArray(l)||m+l<r+x)&&g.setDymoFeature(n,DURATION_FEATURE,r+x-m)}}};this.setDymoFeature=function(b,c,d){a.setFeature(b,c,d);if(!isNaN(d))l(f(null,c),d);else if(d instanceof Array)for(b=0;b<d.length;b++)l(f(null,c),d[b])};this.resetDymo()};function DymoTemplates(){}DymoTemplates.createSingleSourceDymoFromFeatures=function(a,b,c,d,e){b=a.addDymo(void 0,b);DymoTemplates.loadMultipleFeatures(a,b,c,d,e)};DymoTemplates.createConjunctionDymo=function(a,b,c,d){var e=a.addDymo(null,null,CONJUNCTION);b=b.map(function(b,d){return new Promise(function(h){var f=a.addDymo(e,b);DymoTemplates.loadMultipleFeatures(a,f,c[d],null,h)})});Promise.all(b).then(function(a){return d()})};
DymoTemplates.createSimilarityDymoFromFeatures=function(a,b,c,d,e,h){b=a.addDymo(void 0,b);DymoTemplates.loadMultipleFeatures(a,b,c,d,function(){Similarity.addSimilaritiesTo(a.getCurrentTopDymo(),a.getStore(),e);a.addRendering();a.addNavigator(SIMILARITY_NAVIGATOR,"d","return d.getLevel() == 0");a.addNavigator(REPEATED_NAVIGATOR,"d","return d.getLevel() == 1");h()})};
DymoTemplates.createSimilaritySuccessorDymoFromFeatures=function(a,b,c,d,e,h){b=a.addDymo(void 0,b);DymoTemplates.loadMultipleFeatures(a,b,c,d,function(){Similarity.addSimilaritiesTo(a.getCurrentTopDymo(),a.getStore(),e);Similarity.addSuccessionGraphTo(a.getCurrentTopDymo(),a.getStore(),e);a.addRendering();a.addNavigator(GRAPH_NAVIGATOR,"d","return d.getLevel() == 0");a.addNavigator(REPEATED_NAVIGATOR,"d","return d.getLevel() == 1");h()})};
DymoTemplates.createAnnotatedBarAndBeatDymo=function(a,b,c){for(var d=[b[0],b[0]],e=["1",""],h=1;h<b.length;h++)d[h+1]=b[h],e[h+1]="";DymoTemplates.loadMultipleFeatures(a,null,d,e,c)};function getUris(a,b,c,d){for(var e=[],h=[],k=0,l=c.length;k<l;k++){var f=b.filter(function(a){return 0<a.indexOf(c[k])})[0];f&&(e.push(a+f),h.push(d[k]))}return[e,h]}
DymoTemplates.loadMultipleFeatures=function(a,b,c,d,e){Benchmarker.startTask("loadFeatures");var h=new FeatureLoader(a,b);a=c.map(function(a,b){return new Promise(function(c){return h.loadFeature(a,d?d[b]:null,c)})});Promise.all(a).then(function(a){return e()})};
DymoTemplates.createSebastianDymo=function(a,b,c){var d=a.getFeature("onset"),e=a.getFeature("pitch"),h=a.addDymo();a.setDymoFeature(h,d,0);a.setDymoFeature(h,e,0);c.get("getsourcefilesindir/",{params:{directory:"audio/Chopin_Op028-04_003_20100611-SMD/"}}).success(function(b){b=b.filter(function(a){return 4<a.split("_").length-1});for(var c=0;c<b.length;c++){var f=b[c].split("_"),p=f[f.length-1],p=Number.parseInt(p.substring(1,p.indexOf(".")),10)/1E3,f=Number.parseInt(f[f.length-3].substring(1),10),
g=a.addDymo(h,"audio/Chopin_Op028-04_003_20100611-SMD/"+b[c]);a.setDymoFeature(g,d,p);a.setDymoFeature(g,e,f)}DYMO_STORE.updatePartOrder(h,d.name)})};
...
b[h]);a.setDymoFeature(p,c,f);a.setDymoFeature(p,d,l)}DYMO_STORE.updatePartOrder(e,c.name)})};
DymoTemplates.createSebastianDymo3=function(a,b){var c=a.addDymo();a.setDymoFeature(c,ONSET_FEATURE,0);a.setDymoFeature(c,PITCH_FEATURE,0);a.setDymoFeature(c,DURATION_FEATURE,0);a.setDymoFeature(c,"velocity",0);a.setDymoFeature(c,"onsetS",0);a.setDymoFeature(c,"durationS",0);b.get("getsourcefilesindir/",{params:{directory:"audio/Chopin_Op028-11_003_20100611-SMD-cut/"}}).success(function(b){b=b.filter(function(a){return 4<a.split("_").length-1});for(var d=0;d<b.length;d++){var h=b[d].split("_"),k=
Number.parseInt(h[4].substring(1),10),l=Number.parseInt(h[6].substring(2),10)/1E3,f=Number.parseInt(h[7].substring(2),10)/1E3,p=Number.parseInt(h[8].substring(2),10),g=Number.parseInt(h[9].substring(2),10)/1E3,h=Number.parseInt(h[10].substring(2),10)/1E3,n=a.addDymo(c,"audio/Chopin_Op028-11_003_20100611-SMD-cut/"+b[d]);a.setDymoFeature(n,PITCH_FEATURE,k);a.setDymoFeature(n,ONSET_FEATURE,l);a.setDymoFeature(n,DURATION_FEATURE,f);a.setDymoFeature(n,"velocity",p);a.setDymoFeature(n,"onsetS",g);a.setDymoFeature(n,
"durationS",h);n.getParameter(ONSET).update(l)}DYMO_STORE.updatePartOrder(c,ONSET_FEATURE)})};
DymoTemplates.createAchBachDymo=function(a,b){var c=a.getFeature(ONSET_FEATURE),d=a.getFeature(PITCH_FEATURE),e=a.getFeature(DURATION_FEATURE),h=a.getFeature("onsetS"),k=a.getFeature("durationS"),l=a.getFeature("time"),f=a.addDymo();a.setDymoFeature(f,c,0);a.setDymoFeature(f,d,0);a.setDymoFeature(f,e,0);$http.get("audio/achachbach10/01-AchGottundHerr.txt").success(function(b){b=b.split("\n");for(var g=0;g<b.length;g++)b[g]=b[g].split("\t");b.sort(function(a,b){return a[0]-b[0]});for(var n=[],g=0;g<
b.length;g++){var m=b[g][0],p=b[g][1],v=Number.parseInt(b[g][3],10);if(n[v]){var t=n[v][0],w=p-n[v][2];b[t][4]=m-n[v][1];b[t][5]=w}n[v]=[g,m,p]}for(g=b.length-4;g<b.length;g++)b[g][4]=2700,b[g][5]=2700;for(g=0;g<b.length;g++)if(6==b[g].length){var n=b[g],m=Number.parseInt(n[2],10),p=Number.parseInt(n[0],10)/1E3,v=Number.parseInt(n[1],10)/1E3,t=Number.parseInt(n[4],10)/1E3,w=Number.parseInt(n[5],10)/1E3,q;1==n[3]?q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-violin.wav"):2==n[3]?q=a.addDymo(f,
"audio/achachbach10/01-AchGottundHerr-clarinet.wav"):3==n[3]?q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-saxphone.wav"):4==n[3]&&(q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-bassoon.wav"));a.setDymoFeature(q,d,m);a.setDymoFeature(q,l,p);a.setDymoFeature(q,c,p);a.setDymoFeature(q,e,t);a.setDymoFeature(q,h,v);a.setDymoFeature(q,k,w);q.getParameter(ONSET).update(p)}DYMO_STORE.updatePartOrder(f,c.name)});$http.get("getsourcefilesindir/",{params:{directory:"audio/achachbach10/"}}).success(function(a){a=
a.filter(function(a){return 0<=a.indexOf("wav")});for(var b=0;b<a.length;b++);})};function createRandomTriangle(){return[{0:Math.random(),1:Math.random()},{0:Math.random(),1:Math.random()},{0:Math.random(),1:Math.random()}]};function FeatureLoader(a,b){function c(c,g,f){w(c,function(n){d(n,function(d){e(d,function(e){0<e.length?(p(c,g,e),f&&f()):h(d,function(d){var g=d.name;g||(g=c.split("_"),g=g[g.length-1].split(".")[0]);a.addFeature(g,d.values,b);f&&f()})})})})}function d(a,b){var c=N3.Store();N3.Parser().parse(a,function(a,d,g){d?c.addTriple(d):b(c)})}function e(a,b){for(var c=[],d=a.find(null,"http://purl.org/NET/c4dm/event.owl#time",null),g=0,e=d.length;g<e;g++){var n=k(a,d[g].object,"http://purl.org/NET/c4dm/timeline.owl#at");
n||(n=k(a,d[g].object,"http://purl.org/NET/c4dm/timeline.owl#beginsAt"));var m=k(a,d[g].object,"http://purl.org/NET/c4dm/timeline.owl#duration"),n={time:f(n),label:l(k(a,d[g].subject,"http://www.w3.org/2000/01/rdf-schema#label"))};m&&(n.duration=f(m));c.push(n)}b(c)}function h(a,b){var c=l(k(a,null,"http://purl.org/dc/elements/1.1/title")),d=l(k(a,null,"http://purl.org/ontology/af/value")),d=d.split(" ").map(function(a){return parseFloat(a)}),g=l(k(a,null,"http://purl.org/ontology/af/dimensions")),
g=g.split(" ").map(function(a){return Number.parseInt(a,10)}),e=k(a,null,"http://purl.org/ontology/vamp/computed_by"),n=f(k(a,e,"http://purl.org/ontology/vamp/step_size"));a=f(k(a,e,"http://purl.org/ontology/vamp/sample_rate"));for(var e=[],m=0;m<d.length;){for(var p=[],h=0;h<g[0];h++)p[h]=d[m+h];e.push({time:{value:m*n/a},value:p});m+=g[0]}b({name:c,values:e})}function k(a,b,c){a=a.find(b,c,null);if(0<a.length)return a[0].object}function l(a){if(a)return N3.Util.getLiteralValue(a)}function f(a){a=
N3.Util.getLiteralValue(a);"P"==a.charAt(0)&&(a=a.substring(2,a.length-1));return Number(a)}function p(c,d,g){q[c]=g.sort(function(a,b){return a.time.value-b.time.value});g=q[c];d&&q[c][0].label&&(g=g.filter(function(a){return a.label.value==d}));a.addSegmentation(g,b)}function g(a,b,c){w(a,function(a){n(JSON.parse(a),b,c)})}function n(a,b,c){"file_metadata"==Object.keys(a)[0]?m(a,b,c):r(a,b,c)}function m(c,d,g){c=c[Object.keys(c)[1]][0];var f=c.annotation_metadata.annotator.output_id;"beats"==f||
"onsets"==f||"segmentation"==f?(c=c.data,d&&c[0].value&&(c=c.filter(function(a){return a.value==d})),a.addSegmentation(c,b)):a.addFeature(f,c.data,b);g&&g()}function r(c,d,g){var f=c["@type"];"afv:BarandBeatTracker"==f||"afv:Onsets"==f?(c=c["afo:values"],d&&c[0]["afo:value"]&&(c=c.filter(function(a){return a["afo:value"]==d})),c=v(c),a.addSegmentation(c,b)):(c=t(c["afo:values"]),a.addFeature(f,c,b));g&&g()}function v(a){for(var b=[],c=0;c<a.length;c++)b.push({time:{value:a[c]["tl:at"]},label:{value:a[c]["afo:value"]}});
return b}function t(a){for(var b=[],c=0;c<a.length;c++)b.push({time:{value:a[c]["tl:at"]},value:[a[c]["afo:value"]]});return b}function w(a,b){var c=new XMLHttpRequest;c.addEventListener("load",function(){b(this.responseText)});c.addEventListener("error",function(){b(this.responseText)});c.open("GET",a);c.send()}var q={};this.loadFeature=function(a,b,d){if(a.constructor==Object)n(a,b,d);else{var f=a.split("."),f=f[f.length-1];"n3"==f?c(a,b,d):"json"==f&&g(a,b,d)}}};var SUMMARY={MEAN:"mean",MEDIAN:"median",FIRST:"first"};function Similarity(){}Similarity.addSimilaritiesTo=function(a,b,c){for(a=[a];0<a.length;){if(1<a.length){var d=Similarity.toVectors(a,b),d=Similarity.getCosineSimilarities(d);Similarity.addSimilaritiesAbove(b,d,c)}a=Similarity.getAllParts(a,b)}};
Similarity.addSuccessionGraphTo=function(a,b,c){for(a=[a];0<a.length;){if(1<a.length){for(var d=0;d<a.length-1;d++)b.addSuccessor(a[d],a[d+1]);var d=Similarity.toVectors(a,b),d=Similarity.getCosineSimilarities(d),e;for(e in d)for(var h in d[e])d[e][h]>c&&(addSuccessorToPredecessorOf(e,h,a,b),addSuccessorToPredecessorOf(h,e,a,b))}a=Similarity.getAllParts(a,b)}};function addSuccessorToPredecessorOf(a,b,c,d){a=c.indexOf(a);0<a&&d.addSuccessor(c[a-1],b)}
Similarity.addSimilaritiesAbove=function(a,b,c){for(var d in b)for(var e in b[d])b[d][e]>c&&(a.addSimilar(d,e),a.addSimilar(e,d))};Similarity.addHighestSimilarities=function(a,b,c){var d=[],e;for(e in b)for(var h in b[e])d.push([b[e][h],e,h]);d=d.sort(function(a,b){return b[0]-a[0]});b=0;for(c=Math.min(d.length,c);b<c;b++)e=d[b],a.addSimilar(e[1],e[2]),a.addSimilar(e[2],e[1])};Similarity.getAllParts=function(a,b){for(var c=[],d=0,e=a.length;d<e;d++)c=c.concat(b.findParts(a[d]));return c};
Similarity.toVectors=function(a,b,c){for(var d=[],e=0,h=a.length;e<h;e++){for(var k=[],l=b.findAllFeatureValues(a[e]).filter(function(a){return"string"!=typeof a}),f=0,p=l.length;f<p;f++){var g=l[f];c&&1<g.length&&(g=Similarity.reduce(g));1<g.length?k=k.concat(g):(g=Number(g),k.push(g))}d[e]=k}for(var n=[],m=[],e=0;e<d[0].length;e++){b=[];for(f=0;f<a.length;f++)isNaN(d[f][e])||b.push(d[f][e]);n[e]=math.mean(b);m[e]=math.var(b)}d=d.map(function(a){return a.map(function(a,b){return(a-n[b])/m[b]})});
...
ListenerOrientation:"mb:ListenerOrientation",AutoControlFrequency:"mb:AutoControlFrequency",AutoControlTrigger:"mb:AutoControlTrigger",BrownianMaxStepSize:"mb:BrownianMaxStepSize",LeapingProbability:"mb:LeapingProbability",ContinueAfterLeaping:"mb:ContinueAfterLeaping",Navigator:"mb:Navigator",OneShotNavigator:"mb:OneShotNavigator",RepeatedNavigator:"mb:RepeatedNavigator",SequentialNavigator:"mb:SequentialNavigator",SimilarityNavigator:"mb:SimilarityNavigator",GraphNavigator:"mb:GraphNavigator",ArgumentValue:"mb:ArgumentValue",
MappingTarget:"mb:MappingTarget",MappingRange:"mb:MappingRange",MappingOwners:"mb:MappingOwners",dymo:"mb:hasDymo",mappings:"mb:hasMapping","function":"mb:hasFunction",args:"mb:hasArgument","var":"mb:hasVariable",val:"mb:hasValue",body:"mb:hasBody",targets:"mb:toTarget",range:"mb:hasRange",init:"mb:hasInitialValue",url:"mb:hasUrl",map:"mb:hasJsonMap",smooth:"mb:isSmooth",average:"mb:isAverageOf",uuid:"mb:hasUuid",major:"mb:hasMajor",minor:"mb:hasMinor",rampDuration:"mb:hasDuration",navigators:"mb:hasNavigator",
dymos:"mb:navDymos"}};var SCHEDULE_AHEAD_TIME=.1,FADE_LENGTH=.02,DYMO_ONTOLOGY_URI="http://tiny.cc/dymo-ontology#",DYMO_STORE;var URI_TO_TERM={"http://tiny.cc/dymo-ontology#LevelFeature":"level","http://tiny.cc/dymo-ontology#IndexFeature":"index","http://tiny.cc/dymo-ontology#OnsetFeature":"onset","http://tiny.cc/dymo-ontology#PitchFeature":"pitch","http://tiny.cc/dymo-ontology#DurationFeature":"duration","http://tiny.cc/dymo-ontology#TimeFeature":"time","http://tiny.cc/dymo-ontology#SegmentLabelFeature":"segmentLabel","http://tiny.cc/dymo-ontology#hasSource":"source","http://tiny.cc/dymo-ontology#hasParameter":"parameters",
...
HAS_NAVIGATOR="http://tiny.cc/mobile-audio-ontology#hasNavigator",NAV_DYMOS="http://tiny.cc/mobile-audio-ontology#navDymos";function OntologyLoader2(a,b,c){};function DymoLoader(a){function b(b,d){c(b,"",function(b){a.loadData(b,!0,d)})}function c(a,b,g){loadFile(a,function(f){if(0>f.indexOf("Cannot GET")){var e;a:{try{JSON.parse(f)}catch(q){e=!1;break a}e=!0}e&&(b?(0<=a.indexOf(p)&&(a=a.replace(p,"")),b=b.replace('"'+a+'"',f)):b=f);(f=d(b))?(0>f.indexOf(p)&&(f=p+f),c(f,b,g)):b&&g(JSON.parse(b))}})}function d(a){var b=a.indexOf(".json");if(0<=b){if(b!=a.indexOf("context.json")+7){var c=a.substring(0,b).lastIndexOf('"');return a.substring(c+1,b+5)}return d(a.substring(b+
1))}}function e(b){var c;c=b?a.findAllObjects(b,HAS_MAPPING):a.findAllSubjects(TYPE,MAPPING);for(var d=a.recursiveFindAllSubClasses(MOBILE_CONTROL),f=0;f<d.length;f++){var e=a.findAllSubjects(TYPE,d[f]);for(b=0;b<e.length;b++){var m=a.findObjectValue(e[b],NAME);m||(m=e[b]);if(!g[e[b]]){var p=e[b],k=e[b],l=m,u=d[f],m=void 0;u==ACCELEROMETER_X||u==ACCELEROMETER_Y||u==ACCELEROMETER_Z?m=new AccelerometerControl(u):u==TILT_X||u==TILT_Y?m=new TiltControl(u):u==GEOLOCATION_LATITUDE||u==GEOLOCATION_LONGITUDE?
m=new GeolocationControl(u):u==GEOLOCATION_DISTANCE?m=new DistanceControl:u==COMPASS_HEADING?m=new CompassControl:u==BEACON?(m=a.findObjectValue(k,HAS_UUID),l=a.findObjectValue(k,HAS_MAJOR),u=a.findObjectValue(k,HAS_MINOR),m=new BeaconControl(m,l,u)):u==SLIDER||u==TOGGLE||u==BUTTON||u==CUSTOM_CONTROL?(m=new Control(k,l,u),l=a.findObjectValue(k,HAS_INITIAL_VALUE),m.updateValue(l)):u==RANDOM?m=new RandomControl(k):u==BROWNIAN?(l=a.findObjectValue(k,HAS_INITIAL_VALUE),m=new BrownianControl(k,l)):u==
RAMP?(m=Math.round(1E3*a.findObject(k,HAS_DURATION)),l=a.findObjectValue(k,HAS_INITIAL_VALUE),m=new RampControl(k,m,l)):u==DATA_CONTROL&&(m=a.findObjectValue(k,HAS_URL),l=a.findObjectValue(k,HAS_JSON_MAP),l=new Function("json",l),m=new DataControl(k,m,l));a.findObjectValue(k,IS_SMOOTH)&&m.setSmooth&&m.setSmooth(!0);k=a.findObjectValue(k,IS_AVERAGE_OF);!isNaN(k)&&m.setAverageOf&&m.setAverageOf(k);g[p]=m}}}for(d=0;d<c.length;d++)n[c[d]]||(b=DYMO_STORE.findSubject(HAS_MAPPING,c[d]),f=c[d],e=c[d],p=a.findObject(e,
HAS_FUNCTION),b=h(p,b),p=a.findAllObjects(e,TO_TARGET),p=0<p.length?(k=h(p[0]))?k:p:void 0,e=a.findObject(e,HAS_RANGE),e=new Mapping(b,p,e),n[f]=e)}function h(b,c){var d=$jscomp.makeIterator(a.findFunction(b));b=d.next().value;var g=d.next().value,d=d.next().value;if(b&&g&&d)return c=$jscomp.makeIterator(k(g,c)),g=c.next().value,c=c.next().value,new DymoFunction(b,g,c,d)}function k(b,c){for(var d=[],f=[],e=0;e<b.length;e++){var n=a.findObject(b[e],TYPE);n==FEATURE_TYPE||a.isSubtypeOf(n,FEATURE_TYPE)?
(n==FEATURE_TYPE&&(n=b[e]),d.push(n),f.push(FEATURE_TYPE)):n==PARAMETER_TYPE||a.isSubclassOf(n,PARAMETER_TYPE)?(n==CUSTOM_PARAMETER&&(n=b[e]),n=c?a.setParameter(c,n):b[e],d.push(n),f.push(PARAMETER_TYPE)):(d.push(g[b[e]]),f.push(MOBILE_CONTROL))}return[d,f]}function l(a){return a==SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):a==GRAPH_NAVIGATOR?new GraphNavigator(void 0):a==ONE_SHOT_NAVIGATOR?new OneShotNavigator(void 0):a==REPEATED_NAVIGATOR?new RepeatedNavigator(void 0):new SequentialNavigator(void 0)}
var f=this,p="",g={},n={};this.getMappings=function(){return n};this.loadDymoFromJson=function(a,c){var d=a.lastIndexOf("/")+1;p=a.substring(0,d);b(a,function(){c(f.createDymoFromStore())})};this.parseDymoFromJson=function(b,c){a.loadData(b,!0,function(){c(f.createDymoFromStore())})};this.loadRenderingFromJson=function(a,c){b(a,function(){c(f.createRenderingFromStore())})};this.parseDymoFromTurtle=function(b,c){a.loadData(b,!1,function(){c(f.createDymoFromStore())})};this.createDymoFromStore=function(){for(var b=
a.findTopDymos(),c=0;c<b.length;c++)a.addBasePath(b[c],p),e();return b};this.createRenderingFromStore=function(){var b=a.findSubject(TYPE,RENDERING),c=new Rendering(a.findObject(b,HAS_DYMO));e(b);for(var b=a.findAllObjects(b,HAS_NAVIGATOR),d=0;d<b.length;d++){var f=h(a.findObject(b[d],NAV_DYMOS));c.addSubsetNavigator(f,l(a.findObject(b[d],TYPE)))}return[c,g]}};function DymoStore(a){function b(a,c){for(var d=0;d<a.length;d++){var f=c(a[d]);if(f)return f}for(d=0;d<a.length;d++)if(f=b(k.findParents(a[d]),c))return f}function c(a,b,c){k.triplesToRdf(a,function(a){d(a,b,c)})}function d(a,b,c){a=a.split("_b").join("b");jsonld.fromRDF(a,{format:"application/nquads"},function(a,d){jsonld.frame(d,{"@id":b},function(a,b){jsonld.compact(b,DYMO_CONTEXT,function(a,b){jsonld.compact(b,DYMO_SIMPLE_CONTEXT,function(a,b){h(b);b=JSON.stringify(b,null,2);b=b.replace(/http:\/\/tiny.cc\/dymo-context\//g,
"");b=b.replace("http://tiny.cc/dymo-context-simple","http://tiny.cc/dymo-context");c(b)})})})})}function e(a,b){var d=k.findObject(a,TYPE,null),d=k.recursiveFindAllTriples(a,d);c(d,a,function(a){b(null,JSON.parse(a))})}function h(a){if(a&&a instanceof Object)for(var b in a)"@id"==b&&a[b].includes("_:b")?delete a[b]:h(a[b])}var k=this;EasyStore.call(this);var l={};(function(){k.loadFileIntoStore("http://tiny.cc/dymo-ontology",!1,function(){k.loadFileIntoStore("http://tiny.cc/mobile-audio-ontology",
...
function(a){if(a){var b=k.findAllObjects(a,HAS_ARGUMENT),c=b.map(function(a){return k.findObjectValue(a,HAS_VARIABLE)}),b=b.map(function(a){return k.findObject(a,HAS_VALUE)});a=k.findObjectValue(a,HAS_BODY);return[c,b,a]}};this.findAttributeValue=function(a,b){var c=this.findParameterValue(a,b);null==c&&(c=this.findFeatureValue(a,b));return c};this.findFeatureValue=function(a,b){return b===LEVEL_FEATURE?this.findLevel(a):b===INDEX_FEATURE?this.findPartIndex(a):this.findObjectValueOfType(a,HAS_FEATURE,
...
TYPE);a;){if(a==b)return!0;a=this.findObject(a,TYPE)}return!1};this.loadData=function(a,b,c){h(a,b,function(a){N3.Parser().parse(a,function(a,b,d){b?l.addTriple(b):c&&c()})})};this.loadFileIntoStore=function(a,b,c){loadFile(a,function(a){k.loadData(a,b,c)})};this.toRdf=function(a){var b=l.find(null,null,null);this.triplesToRdf(b,a)};this.triplesToRdf=function(a,b){for(var c=N3.Writer({format:"application/nquads"}),d=0;d<a.length;d++)c.addTriple(a[d]);c.end(function(a,c){b(c)})};this.logData=function(){var a=
...
LogicTools.recursiveCreateGoalString=function(a,b,c,d,e){d||(d="");var h=LogicTools.recursiveCreateGoalString;if(a.isParenthesisNode)return h(a.content,b,c,d,e);if(a.isOperatorNode){for(var k=LogicTools.toLogicJsOperatorString(a),k=k+"(",l=0;l<a.args.length;l++){if(a.args[l].isSymbolNode){var f=a.args[l].name,k=k+f;if(0>b.indexOf(f))b.push(f);else return}else if(a.args[l].isConstantNode&&"number"==a.args[l].valueType)k+=a.args[l].value;else{f="v"+LogicTools.varCount++;c.push(f);k+=f;f=h(a.args[l],
b,c,d,f);if(!f)return;d+=f}l<a.args.length-1&&(k+=",")}e&&(k+=","+e);return d+(k+"),")}};LogicTools.toLogicJsOperatorString=function(a){if("=="==a.op)return"logic.eq";if("+"==a.op)return"logic.add";if("-"==a.op)return"logic.sub";if("*"==a.op)return"logic.mul";if("/"==a.op)return"logic.div"};LogicTools.toEquation=function(a){a=a.substring(a.indexOf("return ")+7,a.indexOf(";"));return"ret == "+a};function PolygonTools(){}PolygonTools.isPointInPolygon=function(a,b){for(var c=!1,d=-1,e=a.length,h=e-1;++d<e;h=d)(a[d][1]<=b[1]&&b[1]<a[h][1]||a[h][1]<=b[1]&&b[1]<a[d][1])&&b[0]<(a[h][0]-a[d][0])*(b[1]-a[d][1])/(a[h][1]-a[d][1])+a[d][0]&&(c=!c);return c};PolygonTools.getArea=function(a){var b=0,c,d,e=0;for(d=a.length-1;e<a.length;d=e,e++)c=a[e],d=a[d],b+=c[0]*d[1],b-=c[1]*d[0];return b/2};
PolygonTools.getCentroid=function(a){var b,c,d,e=0,h=0,k=0;for(c=a.length-1;k<a.length;c=k,k++)b=a[k],c=a[c],d=b[0]*c[1]-c[0]*b[1],e+=(b[0]+c[0])*d,h+=(b[1]+c[1])*d;d=6*PolygonTools.getArea(a);return{0:e/d,1:h/d}};function sqDist(a,b){return Math.pow(a[0]-b[0],2)+Math.pow(a[1]-b[1],2)}
...
Control.prototype.updateMappings=function(a){for(var b=0;b<this.observers.length;b++)this.observers[b]!=a&&this.observers[b].observedControlChanged(this.value,this)};function DymoFunction(a,b,c,d){this.vars=a;this.args=b;this.argTypes=c;this.body=d;this.argCache={};this.resultCache={};this.isDymoSpecificParam=[];this.observers=[];this.init()}
DymoFunction.prototype.init=function(){this.vars||(this.vars=FunctionTools.IDENTITY_JSON.args,this.body=FunctionTools.IDENTITY_JSON.body);1>=this.vars.length?(this.directFunction=FunctionTools.createFunction(this.vars,this.body),this.inverseFunction=FunctionTools.invertFunction(this.body)):(this.constraintFunction=null,this.constraintFunction||(this.directFunction=FunctionTools.createFunction(this.vars,this.body)));for(var a=0;a<this.args.length;a++)if(this.args[a].addObserver)this.args[a].addObserver(this);
...
DymoFunction.prototype.applyInverse=function(a,b){var c=this;if(!this.resultCache[b]||this.resultCache[b]!=a)if(this.constraintFunction){var d=this.args.filter(function(a,b){return a.backpropagate||c.argTypes[b]==PARAMETER_TYPE}),d=this.args.indexOf(d[Math.floor(Math.random()*d.length)]);b=this.getArgValues(null,null,b);var e=logic.lvar();b.splice(d,1,e);a=this.applyConstraint([a].concat(b),e);this.args[d].backpropagate?this.args[d].backpropagate(a,this):DYMO_STORE.setValue(this.args[d],VALUE,a)}else this.inverseFunction&&
this.args&&1==this.args.length&&this.args[0].backpropagate&&(a=this.inverseFunction(a),null!=a&&this.args[0].backpropagate(a,this))};DymoFunction.prototype.applyConstraint=function(a,b){a=logic.run(this.constraintFunction.apply(null,a),b,1);if(0<a.length&&!isNaN(a[0]))return a[0]};DymoFunction.prototype.observedControlChanged=function(a,b){b=this.args.indexOf(b);this.getArgValues(b,a,null);this.notifyObservers()};
...
e=this._outputBuffer.endIndex,h=0,k=0;1>this.slopeCount;)d[e+2*k]=(1-this.slopeCount)*this.prevSampleL+this.slopeCount*b[c],d[e+2*k+1]=(1-this.slopeCount)*this.prevSampleR+this.slopeCount*b[c+1],k++,this.slopeCount+=this._rate;--this.slopeCount;if(1!=a)a:for(;;){for(;1<this.slopeCount;)if(--this.slopeCount,h++,h>=a-1)break a;var l=c+2*h;d[e+2*k]=(1-this.slopeCount)*b[l]+this.slopeCount*b[l+2];d[e+2*k+1]=(1-this.slopeCount)*b[l+1]+this.slopeCount*b[l+3];k++;this.slopeCount+=this._rate}this.prevSampleL=
...
b=Number.MIN_VALUE;for(d=a=0;d<this.seekLength;d++)c=this.calcCrossCorrStereo(2*d,this.pRefMidBuffer),c>b&&(b=c,a=d);return a},seekBestOverlapPositionStereoQuick:function(){var a,b,c,d,e,h,k;this.precalcCorrReferenceStereo();c=Number.MIN_VALUE;for(e=h=b=0;4>e;e++){for(a=0;_SCAN_OFFSETS[e][a];){k=h+_SCAN_OFFSETS[e][a];if(k>=this.seekLength)break;d=this.calcCrossCorrStereo(2*k,this.pRefMidBuffer);d>c&&(c=d,b=k);a++}h=b}return b},precalcCorrReferenceStereo:function(){var a,b,c;for(a=0;a<this.overlapLength;a++)c=
a*(this.overlapLength-a),b=2*a,this.pRefMidBuffer[b]=this.pMidBuffer[b]*c,this.pRefMidBuffer[b+1]=this.pMidBuffer[b+1]*c},calcCrossCorrStereo:function(a,b){var c=this._inputBuffer.vector;a+=this._inputBuffer.startIndex;var d,e,h;d=0;for(e=2;e<2*this.overlapLength;e+=2)h=e+a,d+=c[h]*b[e]+c[h+1]*b[e+1];return d},overlap:function(a){this.overlapStereo(2*a)},overlapStereo:function(a){var b=this._inputBuffer.vector;a+=this._inputBuffer.startIndex;var c=this._outputBuffer.vector,d=this._outputBuffer.endIndex,
e,h,k,l,f,p,g;l=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)k=(this.overlapLength-e)*l,f=e*l,h=2*e,p=h+a,g=h+d,c[g+0]=b[p+0]*f+this.pMidBuffer[h+0]*k,c[g+1]=b[p+1]*f+this.pMidBuffer[h+1]*k},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
...
function less_equal_c(a,b){return function(c){var d=c.walk(a),e=c.walk(b),d=get_domain(c,d),h=get_domain(c,e);return(e=intersection(d,make_domain(minus_inf,h.max)))?(d=intersection(h,make_domain(d.min,inf)))?c.extend_domain(a,e).extend_domain(b,d):!1:!1}}
function add_c(a,b,c){return function(d){var e=d.walk(a),h=d.walk(b),k=d.walk(c),e=get_domain(d,e),h=get_domain(d,h),k=get_domain(d,k);if(k=intersection(k,e.add(h)))if(e=intersection(e,k.sub(h)))if(h=intersection(h,k.sub(e)))return d.extend_domain(a,e).extend_domain(b,h).extend_domain(c,k);return!1}}
function mul_c(a,b,c){return function(d){var e=d.walk(a),h=d.walk(b),k=d.walk(c),e=get_domain(d,e),h=get_domain(d,h),k=get_domain(d,k);if(k=intersection(k,e.mul(h)))if(e=intersection(e,k.div(h)))if(h=intersection(h,k.div(e)))return d.extend_domain(a,e).extend_domain(b,h).extend_domain(c,k);return!1}}
var clpr={dom:function(a,b,c){assert("number"===typeof b&&"number"===typeof c,"#2-3 arguments of dom must be number.");return function(d){var e=make_domain(b,c);d.walk(a);var h=get_domain(d,a);return(e=intersection(h,e))?logic.win(d.extend_domain(a,e)):logic.fail()}},add:function(a,b,c){return goal_construct(add_c,[a,b,c],"+")},sub:function(a,b,c){return clpr.add(c,b,a)},mul:function(a,b,c){return goal_construct(mul_c,[a,b,c],"*")},div:function(a,b,c){return clpr.mul(c,b,a)},less_equal:function(a,
...
logic.eq=function(a,b){return function(c){var d=c.frame,e=logic.unify(a,b,d);if(e){var h=c.set_frame(e);return d==e||c.store.is_empty()?logic.win(h):(c=run_constraints(h.store,h))?logic.win(c):logic.fail()}return logic.fail()}};logic.win=function(a){return logic.make_stream(a||logic.nil)};logic.fail=function(){return EMPTY_STREAM};logic.disj=function(a,b){return function(c){return a(c).append(b(c))}};
...
logic.run=function(a,b,c){assert(logic.is_lvar(b)||logic.is_array(b),"#2 of run must be variable/array");c="undefined"===typeof c?inf:c;a=a(logic.nil);for(var d=[],e=0;e<c&&!a.is_empty();++e){var h=a.first,k=h.frame;if(logic.is_lvar(b)){var l=k.walk(b),l=logic.is_lvar(l)?h.get_value(b):l;d.push(l)}else{for(var f=[],p=0;p<b.length;++p)l=k.walk(b[p]),l=logic.is_lvar(l)?h.get_value(l):l,f.push(l);d.push(f)}a=a.rest()}return d};
