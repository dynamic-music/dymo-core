...
"GeolocationDistance",COMPASS_HEADING="CompassHeading",RANDOM="Random",BROWNIAN="Brownian",RAMP="Ramp",CONTROLS=[SLIDER,TOGGLE,BUTTON,CUSTOM,ACCELEROMETER_X,ACCELEROMETER_Y,ACCELEROMETER_Z,TILT_X,TILT_Y,GEOLOCATION_LATITUDE,GEOLOCATION_LONGITUDE,GEOLOCATION_DISTANCE,COMPASS_HEADING,RANDOM,BROWNIAN,RAMP],UI_CONTROLS=[SLIDER,TOGGLE,BUTTON];function DymoManager(a,b,c){var d=new Scheduler(a,function(){});c||(c="bower_components/dymo-core/audio/impulse_rev.wav");d.setReverbFile(c);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var e,f={};this.loadDymoAndRendering=function(a,b,c){var n=new DymoLoader(d);n.loadDymoFromJson(a,function(a){n.loadRenderingFromJson(b,a[1],function(b){e=b[0];e.dymo=a[0];for(var g in b[1]){var k=b[1][g];0<=UI_CONTROLS.indexOf(k.getType())&&(f[g]=new UIControl(k))}d.loadBuffers(e.dymo);c&&c()})})};this.loadDymoFromJson=function(a,
b){(new DymoLoader(d)).loadDymoFromJson(a,function(a){b&&b(a[0])})};this.parseDymoFromJson=function(a,b){(new DymoLoader(d)).parseDymoFromJson(a,function(a){b(a[0])})};this.replacePartOfTopDymo=function(a,b){var c=e.dymo.getPart(a);e.dymo.replacePart(a,b);d.stop(c)};this.updateNavigatorPosition=function(a,b,c){d.updateNavigatorPosition(a,b,c)};this.getNavigatorPosition=function(a,b){return d.getNavigatorPosition(a,b)};this.syncNavigators=function(a,b,c){d.syncNavigators(a,b,c)};this.startPlaying=
function(){e.dymo.updatePartOrder(ONSET);d.play(e.dymo)};this.stopPlaying=function(){d.stop(e.dymo)};this.getTopDymo=function(){return e.dymo};this.getRendering=function(){return e};this.getUIControls=function(){return f};this.getUIControl=function(a){return f[a]}};function AudioProcessor(a){function b(a){return{extract:function(b,e,f){for(var g=[],k=0;k<a.numberOfChannels;k++)g.push(a.getChannelData(k));for(k=0;k<e;k++)for(var h=0;h<g.length;h++)b[k*g.length+h%g.length]=g[h][k+f];return Math.min(e,g[0].length-f)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var f=b(c),e=new SimpleFilter(f,e),f=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),g=[],k=0;k<c.numberOfChannels;k++)g.push(f.getChannelData(k));for(var h=
new Float32Array(2048),n=e.extract(h,1024),p=0;n;){for(k=0;k<n;k++)for(var l=0;l<g.length;l++)g[l][k+p]=h[k*g.length+l%g.length];p+=n;n=e.extract(h,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var g=0;g<e.length;g++)a[f*e.length+g%e.length]=e[g][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),k=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(k,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=k[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(){0<q&&q--;0==q&&b&&b(q)}function e(a){h.splice(h.indexOf(a),1)}function f(){for(var a=[],b=0;b<h.length;b++)for(var d=$jscomp.makeIterator(h[b].getSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();n=a;c&&c()}function g(b,c){q++;var e=new XMLHttpRequest;e.open("GET",b,!0);e.responseType="arraybuffer";e.onload=function(){a.decodeAudioData(e.response,function(a){c?c(a):(k[b]=
a,d())})};e.error=function(){};e.send()}var k={},h=[],n=[];this.listenerOrientation=new Parameter(LISTENER_ORIENTATION,0);this.listenerOrientation.addObserver(this);var p=a.createConvolver();p.connect(a.destination);var l=a.createDelay(.5);l.connect(a.destination);var u=a.createGain();u.gain.value=.8;l.connect(u);u.connect(l);var q=0;this.setReverbFile=function(a){g(a,function(a){p.buffer=a;d()})};this.loadBuffers=function(a){a=a.getAllSourcePaths();for(var b=0,c=a.length;b<c;b++)k[a[b]]||g(a[b])};
this.getBuffer=function(a){return k[a.getSourcePath()]};this.getParameter=function(a){if(a==LISTENER_ORIENTATION)return this.listenerOrientation};this.updateNavigatorPosition=function(a,b,c){for(var d=0,e=h.length;d<e;d++)h[d].hasDymo(a)&&h[d].getNavigator().setPosition(c,b,a)};this.getNavigatorPosition=function(a,b){for(var c=0,d=h.length;c<d;c++)if(h[c].hasDymo(a))return h[c].getNavigator().getPosition(b,a)};this.syncNavigators=function(a,b,c){for(var d,e,m=0,z=h.length;m<z;m++)h[m].hasDymo(a)&&
(d=h[m].getNavigator()),h[m].hasDymo(b)&&(e=h[m].getNavigator());e&&(b=e.getPosition(c,b),d.setPosition(b+1,c,a))};this.play=function(b){b=new SchedulerThread(b,void 0,a,k,p,l,f,e);h.push(b)};this.pause=function(a){if(a)for(var b=0;b<h.length;b++)h[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<h.length;b++)h[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<h.length;c++){var d=h[c].getSource(a);d&&b.push(d)}return b};this.getUrisOfPlayingDymos=function(){return n};this.observedParameterChanged=
function(b){b=this.listenerOrientation.getValue()/180*Math.PI;a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0)}};function Source(a,b,c,d,e,f){function g(a,c){var d=b.createGain();d.connect(c);d.gain.value=0;a.connect(d);return d}function k(){var d=a.getParameter(TIME_STRETCH_RATIO).getValue();if(1!=d){if(0!=B||y<c.duration)c=u(c,l(B,c),l(y+.3,c));c=(new AudioProcessor(b)).timeStretch(c,d);d=y/d;c=u(c,0,l(d+.02,c));y=d}else if(0!=B||y<c.duration)c=u(c,l(B,c),l(y+.02,c));q(c,c.length);A.buffer=c;m[PLAYBACK_RATE]=A.playbackRate;h(PLAYBACK_RATE,a.getParameter(PLAYBACK_RATE))}function h(a,b){n(a,b.getValue());b.addObserver(v)}
function n(a,b,c){c&&(b+=v.getParameterValue(a));0>b&&0<=G.indexOf(a)&&(b=0);c=b;m[a]&&(m[a].value||0==m[a].value?m[a].value=c:m[a].setValue(c));0<=H.indexOf(a)?(0==m[DISTANCE].value&&(m[DISTANCE].value=-.01),D.setPosition(m[PAN].value,m[HEIGHT].value,m[DISTANCE].value)):a==LOOP&&(A.loop=1==b)}function p(){t=!1;var a=b.currentTime;m[AMPLITUDE].setValueAtTime(m[AMPLITUDE].value,a);m[AMPLITUDE].linearRampToValueAtTime(0,a+.02);A.stop(a+.04)}function l(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}
function u(a,c,d){d=Math.min(a.length-c,d);for(var e=b.createBuffer(a.numberOfChannels,d,a.sampleRate),m=0;m<a.numberOfChannels;m++)for(var z=e.getChannelData(m),f=a.getChannelData(m),t=0;t<d;t++)z[t]=f[c+t];return e}function q(a,b){for(var c=.02*a.sampleRate,d=0;d<a.numberOfChannels;d++)for(var e=a.getChannelData(d),m=0;m<c;m++)e[m]*=m/c,e[b-m-1]*=m/c}function r(a,b,c,d){var e=a.lastIndexOf("/");e&&(a=a.substring(e+1));w("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+
c,function(a){d(a)})}function w(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType="arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}var v=this,t,x,m={},z=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER,PAN,HEIGHT,DISTANCE,LOOP],G=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER],H=[PAN,HEIGHT,DISTANCE],A=b.createBufferSource(),C=b.createGain();C.connect(b.destination);
var E=g(C,d);d=g(C,e);var D=b.createPanner();D.connect(C);e=b.createBiquadFilter();e.type="lowpass";e.frequency.value=2E4;e.connect(D);A.connect(e);var F=a.getSegment(),B=F[0],y=F[1];B||(B=0);!y&&c&&(y=c.duration-B);c?k():r(a.getSourcePath(),B,B+y+.3,function(d){c=d;d=a.getParameter(TIME_STRETCH_RATIO).getValue();1!=d?(c=(new AudioProcessor(b)).timeStretch(c,d),d=y/d,c=u(c,0,l(d+.02,c)),y=d):c=u(c,0,l(y+.02,c));q(c,c.length);A.buffer=c});m[AMPLITUDE]=C.gain;m[TIME_STRETCH_RATIO]={value:0};m[REVERB]=
E.gain;m[DELAY]=d.gain;m[FILTER]=e.frequency;m[PAN]={value:0};m[HEIGHT]={value:0};m[DISTANCE]={value:0};m[LOOP]={value:0};h(AMPLITUDE,a.getParameter(AMPLITUDE));h(TIME_STRETCH_RATIO,a.getParameter(TIME_STRETCH_RATIO));h(REVERB,a.getParameter(REVERB));h(DELAY,a.getParameter(DELAY));h(PAN,a.getParameter(PAN));h(HEIGHT,a.getParameter(HEIGHT));h(DISTANCE,a.getParameter(DISTANCE));h(LOOP,a.getParameter(LOOP));this.observedParameterChanged=function(a){n(a.getName(),a.getChange(),!0)};this.getDymo=function(){return a};
this.getDuration=function(){return y};this.isLoopingAndPlaying=function(){return A.loop&&t};this.getParameterValue=function(a){if(m[a])return m[a].value||0==m[a].value?m[a].value:m[a].getValue()};this.play=function(b){A.onended=function(){for(var b=0;b<z.length;b++){var c=a.getParameter(z[b]);c&&c.removeObserver(v)}A.disconnect();C.disconnect();E.disconnect();f&&f(v)};b||(b=0);A.start(b);t=!0};this.pause=function(){t?(p(),x=!0):x&&(x=!1,this.play())};this.stop=function(){t&&p()}};function SchedulerThread(a,b,c,d,e,f,g,k){function h(){w=t?t:u();n(w);x||(x=w[0].getDymo().getParameter(ONSET).getValue());var a;a=v?v-c.currentTime:SCHEDULE_AHEAD_TIME;for(var d=c.currentTime+a,e=0;e<w.length;e++)w[e].play(d);setTimeout(function(){g()},a);(t=u())?(v=l(d),a=1E3*(v-c.currentTime-SCHEDULE_AHEAD_TIME),r=setTimeout(function(){h()},a)):(v=l(d),a=1E3*(v-c.currentTime),setTimeout(function(){b.reset();k&&k()},a))}function n(a){for(var b=0;b<a.length;b++)q.set(a[b].getDymo(),a[b])}function p(a){q.delete(a.getDymo());
0==q.size&&window.clearTimeout(r);g()}function l(a){if(t){var b=t[0].getDymo().getParameter(ONSET).getValue(),c=b-x;x=b;if(!isNaN(b))return a+c}b=w[0];c=b.getDymo().getParameter(PLAYBACK_RATE).getValue();b=b.getDuration()/c;return a+b}function u(){var a=b.getNextParts();if(a){console.log(a.map(function(a){return a.getIndex()}));for(var z=[],t=0;t<a.length;t++)if(a[t].getSourcePath()){var h=d[a[t].getSourcePath()];z.push(new Source(a[t],c,h,e,f,p))}return z}}var q=new Map,r,w=[],v,t,x;b||(b=new DymoNavigator(a,
new SequentialNavigator(a)));h();this.hasDymo=function(b){if(a.getAllDymosInHierarchy().indexOf(b))return!0};this.getNavigator=function(){return b};this.pause=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=q.get(a[b]);d&&d.pause()}};this.stop=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=q.get(a[b]);d&&d.stop()}};this.getSources=function(){return q};this.getSource=function(a){return q.get(a)}};function AutoControl(a,b,c){function d(a){var b=a.getName();f[b]=a;f[b].addObserver(e)}var e=this,f={};d(new Parameter(AUTO_CONTROL_FREQUENCY,100));d(new Parameter(AUTO_CONTROL_TRIGGER,0));Control.call(this,a,AUTO_CONTROL,f);var g;this.addParameter=d;this.getParameter=function(a){return f[a]};this.startUpdate=function(){g=setInterval(b,f[AUTO_CONTROL_FREQUENCY].getValue())};this.reset=function(){clearInterval(g);g=null;c&&c()};this.observedParameterChanged=function(a){a==f[AUTO_CONTROL_FREQUENCY]?
g&&(this.reset(),this.startUpdate()):a==f[AUTO_CONTROL_TRIGGER]&&(g?this.reset():this.startUpdate())}}inheritPrototype(AutoControl,Control);function BrownianControl(a){var b=this;a||(a=.5);AutoControl.call(this,BROWNIAN,function(){var a=b.getParameter(BROWNIAN_MAX_STEP_SIZE).getValue(),a=2*a*Math.random()-a,a=b.getValue()+a,a=Math.min(Math.max(a,0),1);b.update(a)});this.addParameter(new Parameter(BROWNIAN_MAX_STEP_SIZE,.1));this.update(a);this.startUpdate()}inheritPrototype(BrownianControl,AutoControl);function RampControl(a,b){var c=this;a||(a=1E4);var d=0;b&&(d=b);var e=1!=d;AutoControl.call(this,RAMP,function(){var b=1/a*c.getParameter(AUTO_CONTROL_FREQUENCY).getValue();e||(b*=-1);d+=b;0<d&&1>d?c.update(d):1<=d?(c.update(1),c.reset()):0>=d&&(c.update(0),c.reset())},function(){e=!e});this.update(d)}inheritPrototype(RampControl,AutoControl);function RandomControl(){var a=this;AutoControl.call(this,RANDOM,function(){a.update(Math.random())});this.startUpdate()}inheritPrototype(RandomControl,AutoControl);function UIControl(a,b){var c=this;this.value=a.getValue();a.setUpdateFunction(function(a){console.log("UPDATE!!");c.value=a;b&&setTimeout(function(){b.$apply()},10)});this.getName=function(){return a.getName()};this.getType=function(){return a.getType()};this.update=function(){a.getType()==BUTTON&&(isNaN(this.value)&&(this.value=0),this.value=1-this.value);1==this.value?a.update(1):0==this.value?a.update(0):a.update(this.value)}};function OntologyLoader2(a,b,c){};function DymoLoader(a,b){function c(a,b,e,z,f){var g=new XMLHttpRequest;g.open("GET",v+a,!0);g.onload=function(){var g;a:{try{JSON.parse(this.responseText)}catch(h){g=!1;break a}g=!0}g&&(b=b?b.replace('"'+a+'"',this.responseText):this.responseText);(g=d(b))?c(g,b,e,z,f):b&&(g=f(JSON.parse(b),e),z(g))};g.error=function(a){console.log(a)};g.send()}function d(a){var b=a.indexOf(".json");if(0<=b){if(b!=a.indexOf("context.json")+7){var c=a.substring(0,b).lastIndexOf('"');return a.substring(c+1,b+5)}return d(a.substring(b+
1))}}function e(a,b){var c=f(a,b);g(a,b);return[c,b]}function f(b,c){var d=new DynamicMusicObject(b["@id"],b.ct,a);d.setBasePath(v);c[b["@id"]]=d;b.source&&d.setSourcePath(b.source);for(var e in b)0>w.indexOf(e)&&(b[e].type==FEATURE?d.setFeature(e,b[e].value):b[e].type==PARAMETER&&p(d,e,b[e].value));if(b.parts)for(e=0;e<b.parts.length;e++)d.addPart(f(b.parts[e],c));return d}function g(a,b){var c=b[a["@id"]];if(a.similars)for(var d=0;d<a.similars.length;d++)c.addSimilar(b[a.similars[d]]);if(a.mappings)for(d=
0;d<a.mappings.length;d++)c.addMapping(h(a.mappings[d],b,c));if(a.parts)for(d=0;d<a.parts.length;d++)g(a.parts[d],b)}function k(a,b){for(var c=new Rendering(b[a.dymo]),d={},e=0;e<a.mappings.length;e++)c.addMapping(h(a.mappings[e],b,void 0,d));a.navigator&&(e=Function.apply(null,a.navigator.dymos.args.concat(a.navigator.dymos.body)),c.addNavigator(a.navigator.type==SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):new SequentialNavigator(void 0),e));return[c,d]}function h(b,c,d,e){if(b.controls){for(var f=
[],g=0;g<b.controls.length;g++)f.push(e[b.controls[g]]);return n(b,c,d,f,e)}if(b.dymos){var f=[],h;if(b.dymos instanceof Array)for(g=0;g<b.dymos.length;g++)f.push(c[b.dymos[g]]);else h=Function.apply(null,b.dymos.args.concat(b.dymos.body)),g=Object.keys(c).map(function(a){return c[a]}),Array.prototype.push.apply(f,g.filter(h));return n(b,c,d,f,e,h)}return n(b,c,d,[a],e)}function n(a,b,c,d,e,f){b=a.relative;for(var g=[],h=0;h<a.domainDims.length;h++){var n=a.domainDims[h],k=n.name,l=n.type;l==FEATURE?
g.push(k):l==PARAMETER?(n=c?p(c,k,0):new Parameter(k,0),g.push(n)):(l=e&&e[k]?e[k]:u(n),"value"in n&&l.update(n.value),e&&!e[k]&&(e[k]=l),g.push(l))}return new Mapping(g,b,a["function"],d,a.parameter,f)}function p(a,b,c){var d=a.getParameter(b);d?d.update(c):(d=new Parameter(b,c),a.addParameter(d));return d}function l(a,b){for(var c=0;c<a.length;c++)if(a[c])for(var d=0;d<a[c].length;d++){var e=b["dymo"+c],f=b["dymo"+a[c][d]];e&&f&&e.addSimilar(f)}}function u(a){var c=a.type,d=a.name;if(c==ACCELEROMETER_X)return q(0);
if(c==ACCELEROMETER_Y)return q(1);if(c==ACCELEROMETER_Z)return q(2);if(c==TILT_X)return q(3);if(c==TILT_Y)return q(4);if(c==GEOLOCATION_LATITUDE)return r(0);if(c==GEOLOCATION_LONGITUDE)return r(1);if(c==GEOLOCATION_DISTANCE)return r(2);if(c==COMPASS_HEADING)return b.compassWatcher||(b.compassWatcher=new CompassWatcher),a=b.compassWatcher.headingControl,a;if(c==SLIDER||c==TOGGLE||c==BUTTON||c==CUSTOM)return new Control(d,c);if(c==RANDOM)return new RandomControl;if(c==BROWNIAN)return new BrownianControl(parseFloat(a.value));
if(c==RAMP)return c=Math.round(1E3*parseFloat(a.duration)),new RampControl(c,parseFloat(a.value))}function q(a){b.accelerometerWatcher||(b.accelerometerWatcher=new AccelerometerWatcher);if(0==a)return b.accelerometerWatcher.xControl;if(1==a)return b.accelerometerWatcher.yControl;if(2==a)return b.accelerometerWatcher.zControl;if(3==a)return b.accelerometerWatcher.tiltXControl;if(4==a)return b.accelerometerWatcher.tiltYControl}function r(a){b.geolocationWatcher||(b.geolocationWatcher=new GeolocationWatcher);
return 0==a?b.geolocationWatcher.latitudeControl:1==a?b.geolocationWatcher.longitudeControl:b.geolocationWatcher.distanceControl}var w="@id @type ct source navigator similars mappings parts".split(" "),v="";this.loadDymoFromJson=function(a,b){var d=a.lastIndexOf("/")+1;v=a.substring(0,d);a=a.substring(d);c(a,"",{},b,e)};this.parseDymoFromJson=function(a,b){b(e(a,{}))};this.loadRenderingFromJson=function(a,b,d){c(a,"",b,d,k)};this.loadGraphFromJson=function(a,b,d){c(a,"",b,d,l)}};function DymoWriter(a){function b(a){a.addTriple("_:","http://www.w3.org/1999/02/22-rdf-syntax-ns#type","dmo:DMO");a.addTriple("_:","ch:hasPart","ch:test2")}function c(a,b){var c=new XMLHttpRequest;c.send(b);c.addEventListener("save",function(){console.log("saved "+a)});c.addEventListener("error",function(){console.log("saving "+a+" failed")});c.open("POST",a);c.send()}this.writeDymoToJson=function(a,b,f){a["@context"]="http://purl.org/ontology/dymo/context.json";f||(f="dymo.json");c(b+f,a)};this.writeRenderingToJson=
function(a,b){c(b+"rendering.json",a)};this.writeDmoToRdf=function(a){a=N3.Writer({prefixes:{ch:"rdf/charm.n3#",dmo:"rdf/dmo.n3#",mb:"rdf/mobile.n3#"}});b(a);a.end(function(a,b){console.log(b)})}};function Control(a,b,c){function d(a,b){if(void 0==f||1E-6<Math.abs(a-f))f=a,e(b)}function e(a){for(var b=0;b<h.length;b++)h[b]!=a&&h[b].updateParameter(f,this)}var f,g,k,h=[],n,p,l;this.getName=function(){return a};this.getParameter=function(a){if(c)return c[a]};this.getValue=function(){return f};this.getType=function(){return b};this.getReferenceValue=function(){return g};this.setReferenceAverageCount=function(a){k=a;this.resetReferenceValue()};this.resetReferenceValue=function(){g=f=void 0;p=n=
0};this.setUpdateFunction=function(a){l=a};this.addMapping=function(a){h.push(a);a.updateParameter(f,this)};this.removeMapping=function(a){a=h.indexOf(a);-1<a&&h.splice(a,1)};this.backpropagate=function(a,b){isFinite(a)&&(d(a,b),l&&l(f))};this.update=function(a){k&&n<k?(p+=a,n++,n==k&&(g=p/=k)):isNaN(a)||(g&&(a-=g),d(a))}};function DynamicMusicObject(a,b,c){function d(a,b){b[a.getUri()]=a;for(var c=0;c<a.getParts().length;c++)d(a.getParts()[c],b)}function e(a,b,c,d){var f=a-b,g=d.getParts();if(d.getLevel()==c&&0==f)return d;if(d.getLevel()>=c-1)return f<g.length?g[f]:b+g.length;for(d=0;d<g.length;d++)if(b=e(a,b,c,g[d]),isNaN(b))return b}function f(a,b,c){c(a,b);a=a.getParts();for(var d=0,e=a.length;d<e;d++)f(a[d],b,c)}function g(a,b,c){var d=b.toFlatJson();a.nodes.push(d);c&&a.links.push(h(c,d));for(c=0;c<b.getParts().length;c++)g(a,
b.getParts()[c],d)}function k(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var f=b.getSimilars()[e];c[f.getUri()]||(c[f.getUri()]=f.toFlatJson());a.links.push(h(d,c[f.getUri()]))}for(e=0;e<b.getParts().length;e++)k(a,b.getParts()[e],c)}function h(a,b){return{source:a,target:b,value:1}}var n=this,p=null,l=[],u=[],q={},r={},w=[],v=[],t="",x;r[PLAY]=new Parameter(PLAY,0,!0);r[LOOP]=new Parameter(LOOP,0,!0);
r[ONSET]=new Parameter(ONSET,void 0);r[DURATION_RATIO]=new Parameter(DURATION_RATIO,1);r[AMPLITUDE]=new Parameter(AMPLITUDE,1);r[PLAYBACK_RATE]=new Parameter(PLAYBACK_RATE,1);r[TIME_STRETCH_RATIO]=new Parameter(TIME_STRETCH_RATIO,1);r[PAN]=new Parameter(PAN,0);r[DISTANCE]=new Parameter(DISTANCE,0);r[HEIGHT]=new Parameter(HEIGHT,0);r[REVERB]=new Parameter(REVERB,0);r[DELAY]=new Parameter(DELAY,0);r[FILTER]=new Parameter(FILTER,0);r[PART_INDEX]=new Parameter(PART_INDEX,0,!0);r[PART_COUNT]=new Parameter(PART_COUNT,
Number.POSITIVE_INFINITY,!0);r[PLAY].addObserver(n);r[PART_INDEX].addObserver(n);r[PART_COUNT].addObserver(n);this.setType=function(a){b=a};this.getType=function(){return b};this.getUri=function(){return a};this.setParent=function(a){p=a;for(var b in r)b!=PLAY&&b!=PART_COUNT&&b!=PART_INDEX&&v.push(new Mapping([a.getParameter(b)],!0,void 0,[this],b));a=p.getMappings();var c=this.getDymoMap(),c=Object.keys(c).map(function(a){return c[a]});for(b=0;b<a.length;b++){var d=a[b].getDymoConstraint();d&&(d=
a[b].getTargets().concat(c.filter(d)),a[b].setTargets(d))}};this.removeParent=function(){for(var a=0;a<v.length;a++)v[a].disconnect();v=[];for(var b=p.getMappings(),c=this,a=0;a<b.length;a++){var d=b[a].getTargets(),d=d.filter(function(a){return!a.isSubDymoOf(c)});b[a].setTargets(d)}p=null};this.getParent=function(){return p};this.isSubDymoOf=function(a){for(var b=this;b;){if(b==a)return!0;b=b.getParent()}return!1};this.getDymoMap=function(){var a={};d(this,a);return a};this.getLevel=function(){return p?
p.getLevel()+1:0};this.getIndex=function(){if(p)return p.getParts().indexOf(this)};this.addPart=function(a){l.push(a);a.setParent(this)};this.removePart=function(a){l.splice(l.indexOf(a));a.removeParent(this)};this.replacePart=function(a,b){l[a]&&l[a].removeParent(this);l[a]=b;b.setParent(this)};this.getPart=function(a){return l[a]};this.getParts=function(){return l};this.hasParts=function(){return 0<l.length};this.getNthPart=function(a,b){return e(a,0,b,this)};this.setBasePath=function(a){t=a};this.getBasePath=
function(){return p&&!t?p.getBasePath():t};this.setSourcePath=function(a){x=a};this.getSourcePath=function(){if(p&&!x)return p.getSourcePath();if(x)return this.getBasePath()+x};this.getAllSourcePaths=function(){var a=[];f(this,a,function(b,c){var d=b.getSourcePath();d&&0>a.indexOf(d)&&c.push(d)});return a};this.getAllDymosInHierarchy=function(){var a=[];f(this,a,function(a,b){b.push(a)});return a};this.addSimilar=function(a){u.push(a)};this.getSimilars=function(a){return u};this.getFeatures=function(){return q};
this.setFeature=function(a,b){q[a]=b};this.getFeature=function(a){if(a===LEVEL)return this.getLevel();if(a===INDEX)return this.getIndex();if(q.hasOwnProperty(a))return q[a];if(p)return p.getFeature(a);for(var b=[],c=0;c<l.length;c++)if(l[c]){var d=l[c].getFeatureWithoutLookingFurther(a);isNaN(d)||b.push(d)}if(0<b.length)return b};this.getFeatureWithoutLookingFurther=function(a){if(q[a])return q[a]};this.getMappings=function(){return w};this.getSegment=function(){return[this.getFeature("time"),r[DURATION_RATIO].getValue()*
this.getFeature("duration")]};this.addParameter=function(a){r[a.getName()]=a};this.getParameter=function(a){return r[a]};this.addMapping=function(a){w.push(a)};this.observedParameterChanged=function(a){a.getName()==PLAY&&c&&(0<a.getChange()?c.play(n):c.stop(n))};this.updatePartOrder=function(a){l&&l[0]&&!isNaN(l[0].getFeature(a))?l.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):l&&l.sort(function(b,c){return b.getParameter(a).getValue()-c.getParameter(a).getValue()})};this.toJsonHierarchy=
function(){for(var a=this.toFlatJson(),b=0;b<l.length;b++)a.parts.push(l[b].toJsonHierarchy());return a};this.toJsonHierarchyGraph=function(){var a={nodes:[],links:[]};g(a,this);return a};this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};k(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=b);x&&(c.source=x);for(var d in q)c[d]=this.getFeatureJson(d);if(0<w.length)for(c.mappings=[],d=0;d<w.length;d++)c.mappings.push(w[d].toJson());0<l.length&&(c.parts=
[]);if(0<u.length)for(c.similars=[],d=0;d<u.length;d++)c.similars.push(u[d].getUri());return c};this.getFeatureJson=function(a){return{value:q[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+a.slice(1)}}};function Mapping(a,b,c,d,e,f){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=f;this.init()}
...
e,f,g,k,h,n,p;k=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*k,h=e*k,f=2*e,n=f+a,p=f+d,c[p+0]=b[n+0]*h+this.pMidBuffer[f+0]*g,c[p+1]=b[n+1]*h+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
