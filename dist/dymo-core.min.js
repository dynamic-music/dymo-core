...
function DymoManager(a,b,c,d,e,k){function g(a,b){f=a;m=b[0];for(var d in b[1]){var k=b[1][d];DYMO_STORE.isSubclassOf(k.getType(),UI_CONTROL)&&(h[d]=new UIControl(k,e));DYMO_STORE.isSubclassOf(k.getType(),SENSOR_CONTROL)&&(n[d]=k)}c||(c="bower_components/dymo-core/audio/impulse_rev.wav");l.init(c)}DYMO_STORE=new DymoStore(d);var l=new Scheduler(a,k);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var f,m,h={},n={};this.loadDymoAndRenderingFromStore=function(a,b){var c=new DymoLoader(a);g(c.createDymoFromStore(),
c.createRenderingFromStore());l.loadBuffers(m.getDymoUri(),b)};this.loadDymoAndRendering=function(a,b,c,d){var h=new DymoLoader(DYMO_STORE);h.loadDymoFromJson(a,function(a){h.loadRenderingFromJson(b,function(b){g(a,b);l.loadBuffers(a,d);c&&c()})})};this.loadDymoFromJson=function(a,b){(new DymoLoader(DYMO_STORE)).loadDymoFromJson(a,b)};this.parseDymoFromJson=function(a,b){(new DymoLoader(DYMO_STORE)).parseDymoFromJson(a,b)};this.replacePartOfTopDymo=function(a,b){var c=DYMO_STORE.replacePartAt(f[0],
b,a);l.stop(c)};this.updateNavigatorPosition=function(a,b,c){l.updateNavigatorPosition(a,b,c)};this.getNavigatorPosition=function(a,b){return l.getNavigatorPosition(a,b)};this.syncNavigators=function(a,b,c){l.syncNavigators(a,b,c)};this.startPlayingUri=function(a){l.play(a)};this.stopPlayingUri=function(a){l.stop(a)};this.startPlaying=function(){for(var a=0;a<f.length;a++)DYMO_STORE.updatePartOrder(f[a],ONSET),l.play(f[a],m.getNavigator())};this.stopPlaying=function(){for(var a=0;a<f.length;a++)l.stop(f[a])};
this.getTopDymo=function(){return f[0]};this.getRendering=function(){return m};this.getUIControls=function(){return h};this.getUIControl=function(a){return h[a]};this.getSensorControls=function(){return n}};function DymoNode(a,b,c,d){this.POSITION_PARAMS=[PAN,HEIGHT,DISTANCE];this.dymoUri=a;this.parameters={};this.init(b,c,d)}
...
DymoNode.prototype.removeAndDisconnect=function(){for(var a in this.parameters)DYMO_STORE.removeParameterObserver(this.dymoUri,a,this);this.dryGain.disconnect();this.reverbGain&&this.reverbGain.disconnect();this.delayGain&&this.delayGain.disconnect()};function AudioProcessor(a){function b(a){return{extract:function(b,e,k){for(var g=[],l=0;l<a.numberOfChannels;l++)g.push(a.getChannelData(l));for(l=0;l<e;l++)for(var f=0;f<g.length;f++)b[l*g.length+f%g.length]=g[f][l+k];return Math.min(e,g[0].length-k)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var k=b(c),e=new SimpleFilter(k,e),k=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),g=[],l=0;l<c.numberOfChannels;l++)g.push(k.getChannelData(l));for(var f=
new Float32Array(2048),m=e.extract(f,1024),h=0;m;){for(l=0;l<m;l++)for(var n=0;n<g.length;n++)g[n][l+h]=f[l*g.length+n%g.length];h+=m;m=e.extract(f,1024)}return k}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var k=new SimpleFilter({extract:function(a,c,d){for(var n=[],p=0;p<b.numberOfChannels;p++)n.push(b.getChannelData(p));for(p=0;p<c;p++)for(var e=0;e<n.length;e++)a[p*n.length+e%n.length]=n[e][p+d];return Math.min(c,n[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),l=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],h=0;h<b.numberOfChannels;h++)c.push(a.outputBuffer.getChannelData(h));a=k.extract(l,1024);0==a&&d.stop();for(h=0;h<a;h++)for(var n=0;n<c.length;n++)c[n][h]=l[h*c.length+n%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b){function c(a){l.splice(l.indexOf(a),1)}function d(){for(var a=[],c=0;c<l.length;c++)for(var d=$jscomp.makeIterator(l[c].getAllSources().keys()),h=d.next();!h.done;h=d.next())for(h=h.value;null!=h;)0>a.indexOf(h)&&a.push(h),h=DYMO_STORE.findParents(h)[0];a.sort();f=a;b&&b(f)}function e(b,c){h++;var d=new XMLHttpRequest;d.open("GET",b,!0);d.responseType="arraybuffer";d.onload=function(){a.decodeAudioData(d.response,function(a){c(a,b)})};d.error=function(){};d.send()}var k=this,
g={},l=[],f=[],m,h=0;this.init=function(b){DYMO_STORE.addParameter(null,LISTENER_ORIENTATION,0,k);0<DYMO_STORE.find(null,null,REVERB).length&&(m=a.createConvolver(),m.connect(a.destination),e(b,function(a){m.buffer=a;0<h&&h--}));if(0<DYMO_STORE.find(null,null,DELAY).length){b=a.createDelay();b.delayTime.value=.5;b.connect(a.destination);var c=a.createGain();c.gain.value=.6;b.connect(c);c.connect(b)}};this.loadBuffers=function(a,b){for(var c=[],d=0,f=a.length;d<f;d++)var m=DYMO_STORE.findAllObjectsInHierarchy(a[d]),
c=c.concat(m.map(function(a){return DYMO_STORE.getSourcePath(a)}));d=0;for(f=c.length;d<f;d++)c[d]&&!g[c[d]]&&e(c[d],function(a,c){g[c]=a;0<h&&h--;0==h&&b&&b()});0==h&&b&&b()};this.getBuffer=function(a){return g[DYMO_STORE.getSourcePath(a)]};this.updateNavigatorPosition=function(a,b,c){for(var d=0,h=l.length;d<h;d++)l[d].hasDymo(a)&&l[d].getNavigator().setPosition(c,b,a)};this.getNavigatorPosition=function(a,b){for(var c=0,d=l.length;c<d;c++)if(l[c].hasDymo(a))return l[c].getNavigator().getPosition(b,
a)};this.syncNavigators=function(a,b,c){for(var d,h,f=0,e=l.length;f<e;f++)l[f].hasDymo(a)&&(d=l[f].getNavigator()),l[f].hasDymo(b)&&(h=l[f].getNavigator());h&&(b=h.getPosition(c,b),d.setPosition(b,c,a))};this.play=function(b,h){var f=new SchedulerThread(b,h,a,g,m,void 0,d,c);l.push(f)};this.pause=function(a){if(a)for(var b=0;b<l.length;b++)l[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<l.length;b++)l[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<l.length;c++){var d=l[c].getSources(a);
if(d)for(var h=0;h<d.length;h++)b=b.concat(d)}return b};this.getUrisOfPlayingDymos=function(){return f};this.observedValueChanged=function(b,c,d){c==LISTENER_ORIENTATION&&(b=d/180*Math.PI,a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0))}};function DymoSource(a,b,c,d,e,k){function g(b,d){var f=b[0],e=b[1],e=1!=d?e+.3:e+FADE_LENGTH;if(c||isNaN(f+e)){if(0!=f||e<c.duration)c=h(c,m(f,c),m(e,c));return l(c,e,d)}n(DYMO_STORE.getSourcePath(a),f,f+e,function(a){return l(a,e,d)})}function l(a,c,d){1!=d&&(a=(new AudioProcessor(b)).timeStretch(a,d),a=h(a,0,m(c/d+FADE_LENGTH,a)));c=a;d=a.length;for(var n=c.sampleRate*FADE_LENGTH,f=0;f<c.numberOfChannels;f++)for(var e=c.getChannelData(f),p=0;p<n;p++)e[p]*=p/n,e[d-p-1]*=p/n;return a}function f(){u=
!1;var a=b.currentTime;r.parameters[AMPLITUDE].setValueAtTime(r.parameters[AMPLITUDE].value,a);r.parameters[AMPLITUDE].linearRampToValueAtTime(0,a+FADE_LENGTH);q.stop(a+2*FADE_LENGTH)}function m(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function h(a,c,d){d=Math.min(a.length-c,d);for(var h=b.createBuffer(a.numberOfChannels,d,a.sampleRate),n=0;n<a.numberOfChannels;n++)for(var f=h.getChannelData(n),e=a.getChannelData(n),p=0;p<d;p++)f[p]=e[c+p];return h}function n(a,b,c,d){var h=a.lastIndexOf("/");
h&&(a=a.substring(h+1));p("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function p(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType="arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}DymoNode.call(this,a,b,d,e);var r=this,u,t,v,q=b.createBufferSource();q.connect(this.getInput());(function(){var b=[DYMO_STORE.findFeatureValue(a,TIME_FEATURE),
DYMO_STORE.findFeatureValue(a,DURATION_FEATURE)];b[0]||(b[0]=0);!b[1]&&c&&(b[1]=c.duration-b[0]);var d=DYMO_STORE.findParameterValue(a,DURATION_RATIO);d&&0<d&&1>d&&(b[1]*=d);v=b[1];d=DYMO_STORE.findParameterValue(a,TIME_STRETCH_RATIO);q.buffer=g(b,d?d:1)})();this.addParameter(PLAYBACK_RATE,q.playbackRate);this.addParameter(TIME_STRETCH_RATIO,{value:0});this.addParameter(LOOP,{value:0});this.getDuration=function(){return v};this.isLoopingAndPlaying=function(){return q.loop&&u};this.play=function(a){q.onended=
function(){q.disconnect();r.removeAndDisconnect();k&&k(r)};a||(a=0);q.start(a);u=!0};this.pause=function(){u?(f(),t=!0):t&&(t=!1,this.play())};this.stop=function(){u&&f()}}inheritPrototype(DymoSource,DymoNode);function SchedulerThread(a,b,c,d,e,k,g,l){function f(){y=q?q:r();m(y);if(!z){var a=y.keys().next().value;z=DYMO_STORE.findParameterValue(a,ONSET)}a=w?w-c.currentTime:SCHEDULE_AHEAD_TIME;for(var b=c.currentTime+a,d=$jscomp.makeIterator(y.values()),h=d.next();!h.done;h=d.next())h.value.play(b);setTimeout(function(){g()},a);(q=r())&&0<q.size?(w=p(b),a=w[1],w=w[0],DYMO_STORE.findParameterValue(a.getDymoUri(),LOOP)&&(w-=FADE_LENGTH),a=1E3*(w-c.currentTime-SCHEDULE_AHEAD_TIME),x=setTimeout(function(){f()},
a)):(w=p(b),a=1E3*(w-c.currentTime),setTimeout(function(){n()},a+100))}function m(a){for(var b=$jscomp.makeIterator(a.keys()),c=b.next();!c.done;c=b.next())c=c.value,t.get(c)||t.set(c,[]),t.get(c).push(a.get(c))}function h(a){var b=t.get(a.getDymoUri());b.splice(b.indexOf(a),1);0>=b.length&&t.delete(a.getDymoUri());g();n()}function n(){if(0==t.size&&(!q||0==q.size)){window.clearTimeout(x);b.reset();for(var c=DYMO_STORE.findAllObjectsInHierarchy(a),d=0,h=c.length;d<h;d++){var n=v.get(c[d]);n&&n.removeAndDisconnect()}l&&
l()}}function p(a){if(q){var b=q.keys().next().value,b=DYMO_STORE.findParameterValue(b,ONSET),c=b-z;z=b;if(!isNaN(b))return a+c}for(var b=0,d,c=$jscomp.makeIterator(y.values()),h=c.next();!h.done;h=c.next()){var h=h.value,n=h.getDuration();n>b&&(b=n,d=h)}return[a+b,d]}function r(){var a=b.getNextParts();if(a){u(a);for(var n=new Map,f=0;f<a.length;f++){var p=DYMO_STORE.getSourcePath(a[f]);if(p){p=new DymoSource(a[f],c,d[p],e,k,h);a:{for(var m=p,g=DYMO_STORE.findParents(p.getDymoUri())[0];g;){if(v.has(g)){m.connect(v.get(g).getInput());
break a}var r=new DymoNode(g,c,e,k);m.connect(r.getInput());v.set(g,r);g=DYMO_STORE.findParents(g)[0];m=r}m.connect(c.destination)}n.set(a[f],p)}}return n}}function u(a){console.log(a.map(function(a){a=DYMO_STORE.findPartIndex(a);return isNaN(a)?"top":a}))}var t=new Map,v=new Map,q,x,y=[],w,z;b||(b=new DymoNavigator(a,new SequentialNavigator(a)));f();this.hasDymo=function(b){if(DYMO_STORE.findAllObjectsInHierarchy(a).indexOf(b))return!0};this.getNavigator=function(){return b};this.stop=function(a){a=
DYMO_STORE.findAllObjectsInHierarchy(a);for(var b=0,c=a.length;b<c;b++){var d=t.get(a[b]);if(d)for(var h=0;h<d.length;h++)d[h].stop();q&&(q.delete(a[b]),0>=q.size&&(q=void 0))}};this.getAllSources=function(){return t};this.getSources=function(a){return t.get(a)}};function AutoControl(a,b,c,d){DYMO_STORE.addParameter(a,AUTO_CONTROL_FREQUENCY,100,this);DYMO_STORE.addParameter(a,AUTO_CONTROL_TRIGGER,0,this);Control.call(this,a,b,AUTO_CONTROL);var e;this.startUpdate=function(b){b||(b=DYMO_STORE.findParameterValue(a,AUTO_CONTROL_FREQUENCY));e=setInterval(c,b)};this.reset=function(){clearInterval(e);e=null;d&&d()};this.observedValueChanged=function(a,b,c){b==AUTO_CONTROL_FREQUENCY?e&&(this.reset(),this.startUpdate(c)):b==AUTO_CONTROL_TRIGGER&&(e?this.reset():this.startUpdate())}}
inheritPrototype(AutoControl,Control);function BrownianControl(a,b){var c=this;b||(b=.5);AutoControl.call(this,a,BROWNIAN,function(){var b=DYMO_STORE.findParameterValue(a,BROWNIAN_MAX_STEP_SIZE),b=2*b*Math.random()-b,b=c.getValue()+b,b=Math.min(Math.max(b,0),1);c.update(b)});DYMO_STORE.addParameter(a,BROWNIAN_MAX_STEP_SIZE,.1,c);this.update(b);this.startUpdate()}inheritPrototype(BrownianControl,AutoControl);function RampControl(a,b,c){var d=this;b||(b=1E4);var e=0;c&&(e=c);var k=1!=e;AutoControl.call(this,a,RAMP,function(){var c=1/b*DYMO_STORE.findParameterValue(a,AUTO_CONTROL_FREQUENCY);k||(c*=-1);e+=c;0<e&&1>e?d.update(e):1<=e?(d.update(1),d.reset()):0>=e&&(d.update(0),d.reset())},function(){k=!k});this.update(e)}inheritPrototype(RampControl,AutoControl);function RandomControl(a){var b=this;AutoControl.call(this,a,RANDOM,function(){b.update(Math.random())});this.startUpdate()}inheritPrototype(RandomControl,AutoControl);function UIControl(a,b){var c=this;this.value=a.getValue();a.setUpdateFunction(function(a){c.value=a;b&&setTimeout(function(){b.$apply()},10)});this.getName=function(){return a.getName()};this.getType=function(){return a.getType()};this.update=function(){a.getType()==BUTTON&&(isNaN(this.value)&&(this.value=0),this.value=1-this.value);1==this.value?a.update(1):0==this.value?a.update(0):a.update(this.value)}};function DymoGenerator(a,b){function c(b,c){var h=d();a.addDymo(h,b,null,c);b||(p=h);return h}function d(){var a=CONTEXT_URI+"dymo"+x;x++;return a}function e(a){var b=[];if(a&&0<a.length){for(var c=0;c<a.length;c++)a[c].value.constructor!==Array&&(a[c].value=[a[c].value]);for(var c=a[0].value.length,d=0;d<c;d++)if("string"==typeof a[0].value[d])b[d]=a[0].value[d];else if(v==SUMMARY.FIRST)b[d]=a[0].value[d];else if(v==SUMMARY.MEAN)b[d]=a.reduce(function(a,b){return a+b.value[d]},0)/a.length;else if(v==
SUMMARY.MEDIAN){a.sort(function(a,b){return a.value[d]-b.value[d]});var h=Math.floor(a.length/2);b[d]=a[h].value[d];0==a.length%2&&(b[d]+=a[h-1].value[d])}return 1==b.length?b[0]:b}return 0}function k(){0==x?p=c(void 0,q):u&&(p=c(n,q),u=!1)}function g(b,c){for(var d=p,h=a.findLevel(p);h<c;){var n=a.findParts(d);if(0<n.length){n=n.map(function(b){return[a.findFeatureValue(b,TIME_FEATURE),b]});n.sort(function(a,b){return a[0]-b[0]});for(var f=0;f<n.length;f++)if(n[f][0]<=b)d=n[f][1];else if(0==f)d=
n[f][1];else break;h++}else break}return d}function l(a,b){void 0==a.max?(a.min=b,a.max=b):(a.min=Math.min(b,a.min),a.max=Math.max(b,a.max))}function f(a,b){for(var c=0;c<t.length;c++)if(t[c].name==a||t[c].uri==b)return t[c];return m(a,b)}function m(a,c,d,h){!a&&c&&(a=URI_TO_TERM[c]);a&&!c&&(c=CONTEXT_URI+a);a=void 0!=d&&void 0!=h?{name:a,uri:c,min:d,max:h}:{name:a,uri:c,min:1E3,max:0};2>t.length?t.push(a):t.splice(t.length-2,0,a);b(a);return a}var h=this,n,p,r,u,t,v=SUMMARY.MEAN,q,x=0;this.resetDymo=
function(){p=n=void 0;u=!1;t=[];m("level",LEVEL_FEATURE);m("random",null,0,1)};this.setStore=function(b){a=b};this.getStore=function(){return a};this.addRendering=function(){var b=CONTEXT_URI+"rendering0";b++;r=b;a.addRendering(r,p)};this.addMapping=function(b,c,d,h){a.addMapping(b,c,d,h)};this.addNavigator=function(b,c,d){a.addNavigator(r,b,c,d)};this.getCurrentTopDymo=function(){return p};this.getFeatures=function(){return t};this.setSummarizingMode=function(a){v=a};this.setCurrentSourcePath=function(a){q=
a};this.setAudioFileChanged=function(){u=!0;if(p){var b=d();a.addDymo(b,null,p);p=b}};this.addDymo=function(a,b){return c(a,b)};this.addFeature=function(b,c){Benchmarker.startTask("addFeature");k();for(var d=f(b),h=a.findAllObjectsInHierarchy(p),n=0;n<h.length;n++){var m=a.findFeatureValue(h[n],TIME_FEATURE),r=a.findFeatureValue(h[n],DURATION_FEATURE),g=c;isNaN(m)||(g=g.filter(function(a){return m<=a.time&&(isNaN(r)||a.time<m+r)}));if(1>g.length)var v=c.filter(function(a){return a.time.value<=m}),
g=0<v.length?[v[g.length-1]]:[c[0]];Benchmarker.startTask("summarize");g=e(g);this.setDymoFeature(h[n],d.uri,g)}};this.addSegmentation=function(b){k();for(var d=a.findMaxLevel(),n=0;n<b.length;n++){var f=g(b[n].time,d),e=b[n].time,p;if(b[n].duration)p=b[n].duration;else if(b[n+1])p=b[n+1].time-e;else{var m=a.findFeatureValue(f,TIME_FEATURE),r=a.findFeatureValue(f,DURATION_FEATURE);m&&r&&(p=m+r-e)}if(0<p){m=c(f);this.setDymoFeature(m,TIME_FEATURE,e);this.setDymoFeature(m,DURATION_FEATURE,p);var v=
m,e=a.findFeatureValue(f,TIME_FEATURE),m=a.findFeatureValue(v,TIME_FEATURE);if(isNaN(e)||Array.isArray(e)||m<e)h.setDymoFeature(f,TIME_FEATURE,m),e=m;r=a.findFeatureValue(f,DURATION_FEATURE);v=a.findFeatureValue(v,DURATION_FEATURE);(isNaN(r)||Array.isArray(r)||e+r<m+v)&&h.setDymoFeature(f,DURATION_FEATURE,m+v-e)}}};this.setDymoFeature=function(b,c,d){a.setFeature(b,c,d);if(!isNaN(d))l(f(null,c),d);else if(d instanceof Array)for(b=0;b<d.length;b++)l(f(null,c),d[b])};this.resetDymo()};function DymoTemplates(){}DymoTemplates.createSingleSourceDymoFromFeatures=function(a,b,c,d,e){a.addDymo(void 0,b);DymoTemplates.loadMultipleFeatures(a,c,d,0,function(){e()})};
...
Number.parseInt(k[4].substring(1),10),l=Number.parseInt(k[6].substring(2),10)/1E3,f=Number.parseInt(k[7].substring(2),10)/1E3,m=Number.parseInt(k[8].substring(2),10),h=Number.parseInt(k[9].substring(2),10)/1E3,k=Number.parseInt(k[10].substring(2),10)/1E3,n=a.addDymo(c,"audio/Chopin_Op028-11_003_20100611-SMD-cut/"+b[e]);a.setDymoFeature(n,PITCH_FEATURE,g);a.setDymoFeature(n,ONSET_FEATURE,l);a.setDymoFeature(n,DURATION_FEATURE,f);a.setDymoFeature(n,"velocity",m);a.setDymoFeature(n,"onsetS",h);a.setDymoFeature(n,
"durationS",k);n.getParameter(ONSET).update(l)}DYMO_STORE.updatePartOrder(c,ONSET_FEATURE)})};
DymoTemplates.createAchBachDymo=function(a,b){var c=a.getFeature(ONSET_FEATURE),d=a.getFeature(PITCH_FEATURE),e=a.getFeature(DURATION_FEATURE),k=a.getFeature("onsetS"),g=a.getFeature("durationS"),l=a.getFeature("time"),f=a.addDymo();a.setDymoFeature(f,c,0);a.setDymoFeature(f,d,0);a.setDymoFeature(f,e,0);$http.get("audio/achachbach10/01-AchGottundHerr.txt").success(function(b){b=b.split("\n");for(var h=0;h<b.length;h++)b[h]=b[h].split("\t");b.sort(function(a,b){return a[0]-b[0]});for(var n=[],h=0;h<
b.length;h++){var p=b[h][0],r=b[h][1],u=Number.parseInt(b[h][3],10);if(n[u]){var t=n[u][0],v=r-n[u][2];b[t][4]=p-n[u][1];b[t][5]=v}n[u]=[h,p,r]}for(h=b.length-4;h<b.length;h++)b[h][4]=2700,b[h][5]=2700;for(h=0;h<b.length;h++)if(6==b[h].length){var n=b[h],p=Number.parseInt(n[2],10),r=Number.parseInt(n[0],10)/1E3,u=Number.parseInt(n[1],10)/1E3,t=Number.parseInt(n[4],10)/1E3,v=Number.parseInt(n[5],10)/1E3,q;1==n[3]?q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-violin.wav"):2==n[3]?q=a.addDymo(f,
"audio/achachbach10/01-AchGottundHerr-clarinet.wav"):3==n[3]?q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-saxphone.wav"):4==n[3]&&(q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-bassoon.wav"));a.setDymoFeature(q,d,p);a.setDymoFeature(q,l,r);a.setDymoFeature(q,c,r);a.setDymoFeature(q,e,t);a.setDymoFeature(q,k,u);a.setDymoFeature(q,g,v);q.getParameter(ONSET).update(r)}DYMO_STORE.updatePartOrder(f,c.name)});$http.get("getsourcefilesindir/",{params:{directory:"audio/achachbach10/"}}).success(function(a){a=
a.filter(function(a){return 0<=a.indexOf("wav")});for(var b=0;b<a.length;b++);})};function createRandomTriangle(){return[{0:Math.random(),1:Math.random()},{0:Math.random(),1:Math.random()},{0:Math.random(),1:Math.random()}]};function FeatureLoader(){function a(a,h,n,f){u(a,function(e){b(e,function(b){c(b,function(c){0<c.length?(l(a,h,n,c),f&&f()):d(b,function(b){var c=b.name;c||(c=a.split("_"),c=c[c.length-1].split(".")[0]);n.addFeature(c,b.values);f&&f()})})})})}function b(a,b){var c=N3.Store();N3.Parser().parse(a,function(a,d,h){d?c.addTriple(d):b(c)})}function c(a,b){for(var c=[],d=a.find(null,"http://purl.org/NET/c4dm/event.owl#time",null),h=0,n=d.length;h<n;h++){var f=e(a,d[h].object,"http://purl.org/NET/c4dm/timeline.owl#at");
...
Similarity.toVectors=function(a,b,c){for(var d=[],e=0,k=a.length;e<k;e++){for(var g=[],l=b.findAllFeatureValues(a[e]).filter(function(a){return"string"!=typeof a}),f=0,m=l.length;f<m;f++){var h=l[f];c&&1<h.length&&(h=Similarity.reduce(h));1<h.length?g=g.concat(h):(h=Number(h),g.push(h))}d[e]=g}for(var n=[],p=[],e=0;e<d[0].length;e++){b=[];for(f=0;f<a.length;f++)isNaN(d[f][e])||b.push(d[f][e]);n[e]=math.mean(b);p[e]=math.var(b)}d=d.map(function(a){return a.map(function(a,b){return(a-n[b])/p[b]})});
...
"http://tiny.cc/mobile-audio-ontology#isAverageOf",HAS_UUID="http://tiny.cc/mobile-audio-ontology#hasUuid",HAS_MAJOR="http://tiny.cc/mobile-audio-ontology#hasMajor",HAS_MINOR="http://tiny.cc/mobile-audio-ontology#hasMinor",HAS_DURATION="http://tiny.cc/mobile-audio-ontology#hasDuration",HAS_NAVIGATOR="http://tiny.cc/mobile-audio-ontology#hasNavigator",NAV_DYMOS="http://tiny.cc/mobile-audio-ontology#navDymos";function OntologyLoader2(a,b,c){};function DymoLoader(a){function b(b,d){c(b,"",function(b){a.loadData(b,!0,d)})}function c(a,b,h){loadFile(a,function(f){if(0>f.indexOf("Cannot GET")){var e;a:{try{JSON.parse(f)}catch(k){e=!1;break a}e=!0}e&&(b?(0<=a.indexOf(m)&&(a=a.replace(m,"")),b=b.replace('"'+a+'"',f)):b=f);(f=d(b))?(0>f.indexOf(m)&&(f=m+f),c(f,b,h)):b&&h(JSON.parse(b))}})}function d(a){var b=a.indexOf(".json");if(0<=b){if(b!=a.indexOf("context.json")+7){var c=a.substring(0,b).lastIndexOf('"');return a.substring(c+1,b+5)}return d(a.substring(b+
1))}}function e(b){for(var c=0;c<b.length;c++)for(var d=a.findAllObjects(b[c],HAS_DOMAIN_DIMENSION),f=0;f<d.length;f++){var e=a.findObject(d[f],TYPE),m=a.findObjectValue(d[f],NAME);m||(m=d[f]);if(a.isSubclassOf(e,MOBILE_CONTROL)&&!h[d[f]]){var k=d[f],g=e,e=void 0;g==ACCELEROMETER_X||g==ACCELEROMETER_Y||g==ACCELEROMETER_Z?e=new AccelerometerControl(g):g==TILT_X||g==TILT_Y?e=new TiltControl(g):g==GEOLOCATION_LATITUDE||g==GEOLOCATION_LONGITUDE?e=new GeolocationControl(g):g==GEOLOCATION_DISTANCE?e=new DistanceControl:
g==COMPASS_HEADING?e=new CompassControl:g==BEACON?(e=a.findObjectValue(k,HAS_UUID),m=a.findObjectValue(k,HAS_MAJOR),g=a.findObjectValue(k,HAS_MINOR),e=new BeaconControl(e,m,g)):g==SLIDER||g==TOGGLE||g==BUTTON||g==CUSTOM_CONTROL?(e=new Control(k,m,g),m=a.findObjectValue(k,HAS_INITIAL_VALUE),e.update(m)):g==RANDOM?e=new RandomControl(k):g==BROWNIAN?(m=a.findObjectValue(k,HAS_INITIAL_VALUE),e=new BrownianControl(k,m)):g==RAMP&&(e=Math.round(1E3*a.findObject(k,HAS_DURATION)),m=a.findObjectValue(k,HAS_INITIAL_VALUE),
e=new RampControl(k,e,m));a.findObjectValue(k,IS_SMOOTH)&&e.setSmooth&&e.setSmooth(!0);k=a.findObjectValue(k,IS_AVERAGE_OF);!isNaN(k)&&e.setAverageOf&&e.setAverageOf(k);h[d[f]]=e}}}function k(b,c){var d=a.findAllObjects(b,TO_TARGET);if(0<d.length){var h=[],f=a.findFunction(d[0]);if(f)d=a.findAllSubjects(TYPE,DYMO),Array.prototype.push.apply(h,d);else for(var e=0;e<d.length;e++)h.push(d[e]);return g(b,c,h,f)}return g(b,c,[])}function g(b,c,d,f){for(var e=a.findObject(b,IS_RELATIVE),m=[],k=a.findAllObjects(b,
HAS_DOMAIN_DIMENSION),g=0;g<k.length;g++){var l=a.findObject(k[g],TYPE);l==FEATURE_TYPE||a.isSubtypeOf(l,FEATURE_TYPE)?(l==FEATURE_TYPE&&(l=k[g]),m.push(l)):l==PARAMETER_TYPE||a.isSubclassOf(l,PARAMETER_TYPE)?(l==CUSTOM_PARAMETER&&(l=k[g]),l=c?a.setParameter(c,l):a.setParameter(null,l),m.push(l)):m.push(h[k[g]])}k=$jscomp.makeIterator(a.findArgsAndBody(a.findObject(b,HAS_FUNCTION)));c=k.next().value;k=k.next().value;b=a.findObject(b,HAS_RANGE);return new Mapping(m,e,{args:c,body:k},d,b,f)}function l(a){return a==
SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):a==GRAPH_NAVIGATOR?new GraphNavigator(void 0):a==ONE_SHOT_NAVIGATOR?new OneShotNavigator(void 0):a==REPEATED_NAVIGATOR?new RepeatedNavigator(void 0):new SequentialNavigator(void 0)}var f=this,m="",h={};this.loadDymoFromJson=function(a,c){var d=a.lastIndexOf("/")+1;m=a.substring(0,d);b(a,function(){c(f.createDymoFromStore())})};this.parseDymoFromJson=function(b,c){a.loadData(b,!0,function(){c(f.createDymoFromStore())})};this.loadRenderingFromJson=
function(a,c){b(a,function(){c(f.createRenderingFromStore())})};this.createDymoFromStore=function(){for(var b=a.findTopDymos(),c=0;c<b.length;c++){a.addBasePath(b[c],m);for(var d=a.findAllObjectsInHierarchy(b[c]),h=a.find(null,HAS_MAPPING),c=0;c<h.length;c++){var f=h[c].subject;if(0<=d.indexOf(f)){var g=h[c].object;e(g);k(g,f)}}}return b};this.createRenderingFromStore=function(){var b=a.findSubject(TYPE,RENDERING),c=new Rendering(a.findObject(b,HAS_DYMO)),d=a.findAllObjects(b,HAS_MAPPING);e(d);for(var f=
0;f<d.length;f++)c.addMapping(k(d[f]));b=a.findAllObjects(b,HAS_NAVIGATOR);for(f=0;f<b.length;f++)d=a.findObject(b[f],NAV_DYMOS),d=a.findFunction(d),c.addSubsetNavigator(d,l(a.findObject(b[f],TYPE)));return[c,h]}};function DymoStore(a){function b(a,c){for(var d=0;d<a.length;d++){var e=c(a[d]);if(e)return e}for(d=0;d<a.length;d++)if(e=b(g.findParents(a[d]),c))return e}function c(a,b,c){g.triplesToRdf(a,function(a){d(a,b,c)})}function d(a,b,c){a=a.split("_b").join("b");jsonld.fromRDF(a,{format:"application/nquads"},function(a,d){jsonld.frame(d,{"@id":b},function(a,b){jsonld.compact(b,DYMO_CONTEXT,function(a,b){jsonld.compact(b,DYMO_SIMPLE_CONTEXT,function(a,b){k(b);b=JSON.stringify(b,null,2);b=b.replace(/http:\/\/tiny.cc\/dymo-context\//g,
"");b=b.replace("http://tiny.cc/dymo-context-simple","http://tiny.cc/dymo-context");c(b)})})})})}function e(a,b){var d=g.findObject(a,TYPE,null),d=g.recursiveFindAllTriplesExcept(a,d);c(d,a,function(a){b(null,JSON.parse(a))})}function k(a){if(a&&a instanceof Object)for(var b in a)"@id"==b&&a[b].includes("_:b")?delete a[b]:k(a[b])}var g=this;EasyStore.call(this);var l={};(function(){g.loadFileIntoStore("../ontologies/dymo-ontology.n3",!1,function(){g.loadFileIntoStore("../ontologies/mobile-audio-ontology.n3",
...
HAS_NAVIGATOR)};this.findFunction=function(a){var b=$jscomp.makeIterator(this.findArgsAndBody(a));a=b.next().value;b=b.next().value;if(a&&b)return Function.apply(null,a.concat(b))};this.findArgsAndBody=function(a){var b=g.findAllObjectValues(a,HAS_ARGUMENT);a=g.findObjectValue(a,HAS_BODY);return[b,a]};this.findAttributeValue=function(a,b){var c=this.findParameterValue(a,b);null==c&&(c=this.findFeatureValue(a,b));return c};this.findFeatureValue=function(a,b){return b===LEVEL_FEATURE?this.findLevel(a):
...
p={},k=this.findAllSubjects(TYPE,a),l=this.find(null,b,null);async.map(k,e,function(e,v){d.nodes=v;for(var q=0;q<k.length;q++)p[k[q]]=d.nodes[q];d.edges=[];for(q=0;q<l.length;q++)if(0==g.find(l[q].object,TYPE,a).length){if(0<g.find(l[q].object,FIRST).length){var x=g.findObjectOrList(l[q].subject,b),x=x.map(function(a){return{source:p[l[q].subject],target:p[a],value:1}});d.edges=d.edges.concat(x)}}else d.edges.push({source:p[l[q].subject],target:p[l[q].object],value:1});c(d)})}}
inheritPrototype(DymoStore,EasyStore);function DymoWriter(a,b){function c(a,b){var c=new XMLHttpRequest;c.send(b);c.addEventListener("save",function(){console.log("saved "+a)});c.addEventListener("error",function(){console.log("saving "+a+" failed")});c.open("POST",a);c.send()}this.writeToJsonld=function(b,e){e||(e="dymo.json");a.toJsonld(function(a){c(b+e,a)})}};function EasyStore(){function a(a,b,c){var d=[];f[a]&&f[a][b]&&(d=d.concat(f[a][b]));b=g.findObject(a,TYPE);m[b]&&(d=d.concat(m[b]));for(var e in d)d[e].observedValueChanged&&d[e].observedValueChanged(a,b,c)}function b(a,b){var c=l.find(null,a,b);if(0<c.length)return c[0].subject}function c(a){if(a){var b=g.findObject(a,FIRST);if(b){var c=[];c.push(b);for(a=g.findObject(a,REST);a!=NIL;)c.push(g.findObject(a,FIRST)),a=g.findObject(a,REST);return c}return a}}function d(a,b){if(a){for(var c=0,d=a,e=
g.findObject(d,REST);e!=NIL&&(null==b||c<b);)d=e,e=g.findObject(d,REST),c++;if(null==b||c==b)return d}}function e(a){var b=N3.Util.getLiteralValue(a);"http://www.w3.org/2001/XMLSchema#string"!=N3.Util.getLiteralType(a)&&(b=Number(b));return b}function k(a,b,c){b?jsonld.toRDF(a,{format:"application/nquads"},function(a,b){c(b)}):c(a)}var g=this,l=N3.Store(),f={},m={};this.addValueObserver=function(a,b,c){f[a]||(f[a]={});f[a][b]||(f[a][b]=[]);f[a][b].push(c)};this.addTypeObserver=function(a,b,c){m[a]||
(m[a]=[]);m[a].push(c)};this.removeValueObserver=function(a,b,c){f[a]&&f[a][b]&&(c=f[a][b].indexOf(c),-1<c&&(f[a][b].splice(c,1),b=f,f[b]&&0==f[b][a].length&&(delete f[b][a],0===Object.keys(f[b]).length&&delete f[b])))};this.removeTypeObserver=function(a,b,c){m[a]&&(b=m[a].indexOf(c),-1<b&&(m[a].splice(b,1),0==m[a].length&&delete m[a]))};this.getValueObservers=function(a,b){return f[a]?f[a][b]?f[a][b]:flattenArray(Object.values(f[a])):null==a&&null==b?flattenArray(Object.keys(f).map(function(a){return flattenArray(Object.values(f[a]))})):
[]};this.getTypeObservers=function(a){return m[a]?m[a]:null==a?flattenArray(Object.keys(m).map(function(a){return m[a]})):[]};this.addTriple=function(a,b,c){null!=a&&null!=b&&null!=c&&l.addTriple(a,b,c)};this.removeTriple=function(a,b,c){c||(c=this.findObject(a,b));return l.removeTriple(a,b,c)};this.createBlankNode=function(){return l.createBlankNode()};this.setTriple=function(a,b,c){this.removeTriple(a,b);this.addTriple(a,b,c)};this.setValue=function(b,c,d){b&&c&&null!=d&&!Number.isNaN(d)&&(Array.isArray(d)?
...
Control.prototype.updateMappings=function(a){for(var b=0;b<this.mappings.length;b++)this.mappings[b]!=a&&this.mappings[b].updateParameter(this.value,this)};function Mapping(a,b,c,d,e,k){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=k;this.init()}
...
e=this._outputBuffer.endIndex,k=0,g=0;1>this.slopeCount;)d[e+2*g]=(1-this.slopeCount)*this.prevSampleL+this.slopeCount*b[c],d[e+2*g+1]=(1-this.slopeCount)*this.prevSampleR+this.slopeCount*b[c+1],g++,this.slopeCount+=this._rate;--this.slopeCount;if(1!=a)a:for(;;){for(;1<this.slopeCount;)if(--this.slopeCount,k++,k>=a-1)break a;var l=c+2*k;d[e+2*g]=(1-this.slopeCount)*b[l]+this.slopeCount*b[l+2];d[e+2*g+1]=(1-this.slopeCount)*b[l+1]+this.slopeCount*b[l+3];g++;this.slopeCount+=this._rate}this.prevSampleL=
...
e,k,g,l,f,m,h;l=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*l,f=e*l,k=2*e,m=k+a,h=k+d,c[h+0]=b[m+0]*f+this.pMidBuffer[k+0]*g,c[h+1]=b[m+1]*f+this.pMidBuffer[k+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
