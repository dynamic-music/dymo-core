...
var SCHEDULE_AHEAD_TIME=.1,FADE_LENGTH=.02,DYMO="Dymo",PARALLEL="parallel",SEQUENTIAL="sequential",DYMO_TYPES=[PARALLEL,SEQUENTIAL],FEATURE="Feature",PARAMETER="Parameter",ONE_SHOT_NAVIGATOR="OneShotNavigator",SEQUENTIAL_NAVIGATOR="SequentialNavigator",SIMILARITY_NAVIGATOR="SimilarityNavigator",PLAY="Play",LOOP="Loop",ONSET="Onset",DURATION_RATIO="DurationRatio",AMPLITUDE="Amplitude",PLAYBACK_RATE="PlaybackRate",TIME_STRETCH_RATIO="TimeStretchRatio",PAN="Pan",DISTANCE="Distance",HEIGHT="Height",REVERB=
"Reverb",DELAY="Delay",FILTER="Filter",PART_INDEX="PartIndex",PART_COUNT="PartCount",PART_ORDER="PartOrder",LISTENER_ORIENTATION="ListenerOrientation",AUTO_CONTROL_FREQUENCY="AutoControlFrequency",AUTO_CONTROL_TRIGGER="AutoControlTrigger",BROWNIAN_MAX_STEP_SIZE="BrownianMaxStepSize",LEAPING_PROBABILITY="LeapingProbability",CONTINUE_AFTER_LEAPING="ContinueAfterLeaping",PARAMETERS=[PLAY,LOOP,ONSET,DURATION_RATIO,AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,PAN,DISTANCE,HEIGHT,REVERB,DELAY,FILTER,PART_INDEX,
PART_COUNT,PART_ORDER,LISTENER_ORIENTATION,AUTO_CONTROL_FREQUENCY,AUTO_CONTROL_TRIGGER],LEVEL="level",INDEX="index",ONSET_FEATURE="onset",PITCH_FEATURE="pitch",DURATION_FEATURE="duration",AUTO_CONTROL="AutoControl",SLIDER="Slider",TOGGLE="Toggle",BUTTON="Button",CUSTOM="Custom",ACCELEROMETER_X="AccelerometerX",ACCELEROMETER_Y="AccelerometerY",ACCELEROMETER_Z="AccelerometerZ",TILT_X="TiltX",TILT_Y="TiltY",GEOLOCATION_LATITUDE="GeolocationLatitude",GEOLOCATION_LONGITUDE="GeolocationLongitude",GEOLOCATION_DISTANCE=
...
COMPASS_HEADING,BEACON];function DymoManager(a,b,c,d){var e=new Scheduler(a,function(){});c||(c="bower_components/dymo-core/audio/impulse_rev.wav");e.setReverbFile(c);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var f,g={},k={};this.loadDymoAndRendering=function(a,b,c,m){var q=new DymoLoader(e);q.loadDymoFromJson(a,function(a){q.loadRenderingFromJson(b,a[1],function(b){f=b[0];f.dymo=a[0];for(var h in b[1]){var w=b[1][h];0<=UI_CONTROLS.indexOf(w.getType())&&(g[h]=new UIControl(w,d));0<=SENSOR_CONTROLS.indexOf(w.getType())&&(k[h]=w)}e.loadBuffers(f.dymo,
m);c&&c()})})};this.loadDymoFromJson=function(a,b){(new DymoLoader(e)).loadDymoFromJson(a,function(a){b&&b(a[0])})};this.parseDymoFromJson=function(a,b){(new DymoLoader(e)).parseDymoFromJson(a,function(a){b(a[0])})};this.replacePartOfTopDymo=function(a,b){var c=f.dymo.getPart(a);f.dymo.replacePart(a,b);e.stop(c)};this.updateNavigatorPosition=function(a,b,c){e.updateNavigatorPosition(a,b,c)};this.getNavigatorPosition=function(a,b){return e.getNavigatorPosition(a,b)};this.syncNavigators=function(a,
b,c){e.syncNavigators(a,b,c)};this.startPlaying=function(){f.dymo.updatePartOrder(ONSET);e.play(f.dymo)};this.stopPlaying=function(){e.stop(f.dymo)};this.getTopDymo=function(){return f.dymo};this.getRendering=function(){return f};this.getUIControls=function(){return g};this.getUIControl=function(a){return g[a]};this.getSensorControls=function(){return k}};function AudioProcessor(a){function b(a){return{extract:function(b,e,f){for(var g=[],k=0;k<a.numberOfChannels;k++)g.push(a.getChannelData(k));for(k=0;k<e;k++)for(var h=0;h<g.length;h++)b[k*g.length+h%g.length]=g[h][k+f];return Math.min(e,g[0].length-f)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var f=b(c),e=new SimpleFilter(f,e),f=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),g=[],k=0;k<c.numberOfChannels;k++)g.push(f.getChannelData(k));for(var h=
new Float32Array(2048),t=e.extract(h,1024),p=0;t;){for(k=0;k<t;k++)for(var m=0;m<g.length;m++)g[m][k+p]=h[k*g.length+m%g.length];p+=t;t=e.extract(h,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var n=0;n<e.length;n++)a[f*e.length+n%e.length]=e[n][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),k=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(k,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=k[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(a){0<n&&n--;0==n&&(b&&b(n),a&&a())}function e(a){h.splice(h.indexOf(a),1)}function f(){for(var a=[],b=0;b<h.length;b++)for(var d=$jscomp.makeIterator(h[b].getAllSources().keys()),e=d.next();!e.done;e=d.next())for(e=e.value;null!=e;)0>a.indexOf(e.getUri())&&a.push(e.getUri()),e=e.getParent();a.sort();t=a;c&&c()}function g(b,c){n++;var d=new XMLHttpRequest;d.open("GET",b,!0);d.responseType="arraybuffer";d.onload=function(){a.decodeAudioData(d.response,function(a){c(a,
b)})};d.error=function(){};d.send()}var k={},h=[],t=[];this.listenerOrientation=new Parameter(LISTENER_ORIENTATION,0);this.listenerOrientation.addObserver(this);var p=a.createConvolver();p.connect(a.destination);var m=a.createDelay();m.delayTime.value=.5;m.connect(a.destination);var q=a.createGain();q.gain.value=.6;m.connect(q);q.connect(m);var n=0;this.setReverbFile=function(a){g(a,function(a){p.buffer=a;d()})};this.loadBuffers=function(a,b){for(var c=a.getAllSourcePaths(),e=0,f=c.length;e<f;e++)k[c[e]]||
...
f=h.length;l<f;l++)h[l].hasDymo(a)&&(d=h[l].getNavigator()),h[l].hasDymo(b)&&(e=h[l].getNavigator());e&&(b=e.getPosition(c,b),d.setPosition(b+1,c,a))};this.play=function(b){b=new SchedulerThread(b,void 0,a,k,p,m,f,e);h.push(b)};this.pause=function(a){if(a)for(var b=0;b<h.length;b++)h[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<h.length;b++)h[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<h.length;c++){var d=h[c].getSources(a);if(d)for(var e=0;e<d.length;e++)b=b.concat(d)}return b};
this.getUrisOfPlayingDymos=function(){return t};this.observedParameterChanged=function(b){b=this.listenerOrientation.getValue()/180*Math.PI;a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0)}};function Source(a,b,c,d,e,f){function g(a,c){var d=b.createGain();d.connect(c);d.gain.value=0;a.connect(d);return d}function k(){var d=a.getParameter(TIME_STRETCH_RATIO).getValue();if(1!=d){if(0!=z||x<c.duration)c=q(c,m(z,c),m(x+.3,c));c=(new AudioProcessor(b)).timeStretch(c,d);d=x/d;c=q(c,0,m(d+FADE_LENGTH,c));x=d}else if(0!=z||x<c.duration)c=q(c,m(z,c),m(x+FADE_LENGTH,c));n(c,c.length);A.buffer=c;l[PLAYBACK_RATE]=A.playbackRate;h(PLAYBACK_RATE,a.getParameter(PLAYBACK_RATE))}function h(a,b){t(a,
b.getValue());b.addObserver(w)}function t(a,b,c){c&&(b+=w.getParameterValue(a));0>b&&0<=G.indexOf(a)&&(b=0);l[a]&&(l[a].value||0==l[a].value?l[a].value=b:l[a].setValue(b));0<=H.indexOf(a)&&(0==l[DISTANCE].value&&(l[DISTANCE].value=-.01),E.setPosition(l[PAN].value,l[HEIGHT].value,l[DISTANCE].value))}function p(){y=!1;var a=b.currentTime;l[AMPLITUDE].setValueAtTime(l[AMPLITUDE].value,a);l[AMPLITUDE].linearRampToValueAtTime(0,a+FADE_LENGTH);A.stop(a+2*FADE_LENGTH)}function m(a,b){if(a||0==a)return Math.round(a*
b.sampleRate)}function q(a,c,d){d=Math.min(a.length-c,d);for(var e=b.createBuffer(a.numberOfChannels,d,a.sampleRate),l=0;l<a.numberOfChannels;l++)for(var f=e.getChannelData(l),D=a.getChannelData(l),n=0;n<d;n++)f[n]=D[c+n];return e}function n(a,b){for(var c=a.sampleRate*FADE_LENGTH,d=0;d<a.numberOfChannels;d++)for(var e=a.getChannelData(d),l=0;l<c;l++)e[l]*=l/c,e[b-l-1]*=l/c}function r(a,b,c,d){var e=a.lastIndexOf("/");e&&(a=a.substring(e+1));v("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+
b+"&toSecond="+c,function(a){d(a)})}function v(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType="arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}var w=this,y,u,l={},D=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER,PAN,HEIGHT,DISTANCE,LOOP],G=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,DELAY,FILTER],H=[PAN,HEIGHT,DISTANCE],A=b.createBufferSource(),B=b.createGain();
B.connect(b.destination);var F=g(B,d);d=g(B,e);var E=b.createPanner();E.connect(B);e=b.createBiquadFilter();e.type="lowpass";e.frequency.value=2E4;e.connect(E);A.connect(e);var C=a.getSegment(),z=C[0],x=C[1];z||(z=0);!x&&c&&(x=c.duration-z);C=a.getParameter(DURATION_RATIO).getValue();0<C&&1>C&&(x*=C);c?k():isNaN(z+x)||r(a.getSourcePath(),z,z+x+.3,function(d){c=d;d=a.getParameter(TIME_STRETCH_RATIO).getValue();1!=d?(c=(new AudioProcessor(b)).timeStretch(c,d),d=x/d,c=q(c,0,m(d+FADE_LENGTH,c)),x=d):
c=q(c,0,m(x+FADE_LENGTH,c));n(c,c.length);A.buffer=c});l[AMPLITUDE]=B.gain;l[TIME_STRETCH_RATIO]={value:0};l[REVERB]=F.gain;l[DELAY]=d.gain;l[FILTER]=e.frequency;l[PAN]={value:0};l[HEIGHT]={value:0};l[DISTANCE]={value:0};l[LOOP]={value:0};h(AMPLITUDE,a.getParameter(AMPLITUDE));h(TIME_STRETCH_RATIO,a.getParameter(TIME_STRETCH_RATIO));h(REVERB,a.getParameter(REVERB));h(DELAY,a.getParameter(DELAY));h(FILTER,a.getParameter(FILTER));h(PAN,a.getParameter(PAN));h(HEIGHT,a.getParameter(HEIGHT));h(DISTANCE,
a.getParameter(DISTANCE));h(LOOP,a.getParameter(LOOP));this.observedParameterChanged=function(a){t(a.getName(),a.getChange(),!0)};this.getDymo=function(){return a};this.getDuration=function(){return x};this.isLoopingAndPlaying=function(){return A.loop&&y};this.getParameterValue=function(a){if(l[a])return l[a].value||0==l[a].value?l[a].value:l[a].getValue()};this.play=function(b){A.onended=function(){for(var b=0;b<D.length;b++){var c=a.getParameter(D[b]);c&&c.removeObserver(w)}A.disconnect();B.disconnect();
F.disconnect();f&&f(w)};b||(b=0);A.start(b);y=!0};this.pause=function(){y?(p(),u=!0):u&&(u=!1,this.play())};this.stop=function(){y&&p()}};function SchedulerThread(a,b,c,d,e,f,g,k){function h(){y=v?v:n();p(y);l||(l=y.keys().next().value.getParameter(ONSET).getValue());var a;a=u?u-c.currentTime:SCHEDULE_AHEAD_TIME;for(var b=c.currentTime+a,d=$jscomp.makeIterator(y.values()),e=d.next();!e.done;e=d.next())e.value.play(b);setTimeout(function(){g()},a);(v=n())&&0<v.size?(u=q(b),a=u[1],u=u[0],a.getDymo().getParameter(LOOP).getValue()&&(u-=FADE_LENGTH),a=1E3*(u-c.currentTime-SCHEDULE_AHEAD_TIME),w=setTimeout(function(){h()},a)):(u=q(b),a=1E3*
(u-c.currentTime),setTimeout(function(){t()},a+100))}function t(){0!=r.size||v&&0!=v.size||(window.clearTimeout(w),b.reset(),k&&k())}function p(a){for(var b=$jscomp.makeIterator(a.keys()),c=b.next();!c.done;c=b.next())c=c.value,r.get(c)||r.set(c,[]),r.get(c).push(a.get(c))}function m(a){var b=r.get(a.getDymo());b.splice(b.indexOf(a),1);0>=b.length&&r.delete(a.getDymo());g();t()}function q(a){if(v){var b=v.keys().next().value.getParameter(ONSET).getValue(),c=b-l;l=b;if(!isNaN(b))return a+c}for(var b=
0,d,c=$jscomp.makeIterator(y.values()),e=c.next();!e.done;e=c.next()){var e=e.value,f;f=e;var n=f.getDymo().getParameter(PLAYBACK_RATE).getValue();f=f.getDuration()/n;f>b&&(b=f,d=e)}return[a+b,d]}function n(){var a=b.getNextParts();if(a){console.log(a.map(function(a){return isNaN(a.getIndex())?"top":a.getIndex()}));for(var l=new Map,n=0;n<a.length;n++)if(a[n].getSourcePath()){var r=d[a[n].getSourcePath()];l.set(a[n],new Source(a[n],c,r,e,f,m))}return l}}var r=new Map,v,w,y=[],u,l;b||(b=new DymoNavigator(a,
...
Control.prototype.setValue=function(a,b){if(void 0==this.value||1E-6<Math.abs(a-this.value))this.value=a,this.updateMappings(b)};Control.prototype.updateMappings=function(a){for(var b=0;b<this.mappings.length;b++)this.mappings[b]!=a&&this.mappings[b].updateParameter(this.value,this)};function DynamicMusicObject(a,b,c){function d(a,b){b[a.getUri()]=a;for(var c=0;c<a.getParts().length;c++)d(a.getParts()[c],b)}function e(a,b,c,d){var f=a-b,n=d.getParts();if(d.getLevel()==c&&0==f)return d;if(d.getLevel()>=c-1)return f<n.length?n[f]:b+n.length;for(d=0;d<n.length;d++)if(b=e(a,b,c,n[d]),isNaN(b))return b}function f(a,b,c){c(a,b);a=a.getParts();for(var d=0,e=a.length;d<e;d++)f(a[d],b,c)}function g(a,b,c){var d=b.toFlatJson();a.nodes.push(d);c&&a.links.push(h(c,d));for(c=0;c<b.getParts().length;c++)g(a,
b.getParts()[c],d)}function k(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var f=b.getSimilars()[e];c[f.getUri()]||(c[f.getUri()]=f.toFlatJson());a.links.push(h(d,c[f.getUri()]))}for(e=0;e<b.getParts().length;e++)k(a,b.getParts()[e],c)}function h(a,b){return{source:a,target:b,value:1}}var t=this,p=null,m=[],q=[],n={},r={},v=[],w=[],y="",u;r[PLAY]=new Parameter(PLAY,0,!0);r[LOOP]=new Parameter(LOOP,0,!0);
r[ONSET]=new Parameter(ONSET,void 0);r[DURATION_RATIO]=new Parameter(DURATION_RATIO,1);r[AMPLITUDE]=new Parameter(AMPLITUDE,1);r[PLAYBACK_RATE]=new Parameter(PLAYBACK_RATE,1);r[TIME_STRETCH_RATIO]=new Parameter(TIME_STRETCH_RATIO,1);r[PAN]=new Parameter(PAN,0);r[DISTANCE]=new Parameter(DISTANCE,0);r[HEIGHT]=new Parameter(HEIGHT,0);r[REVERB]=new Parameter(REVERB,0);r[DELAY]=new Parameter(DELAY,0);r[FILTER]=new Parameter(FILTER,2E4);r[PART_INDEX]=new Parameter(PART_INDEX,0,!0);r[PART_COUNT]=new Parameter(PART_COUNT,
Number.POSITIVE_INFINITY,!0);r[PLAY].addObserver(t);r[PART_INDEX].addObserver(t);r[PART_COUNT].addObserver(t);this.setType=function(a){b=a};this.getType=function(){return b};this.getUri=function(){return a};this.setParent=function(a){p=a;for(var b in r)b!=PLAY&&b!=PART_COUNT&&b!=PART_INDEX&&w.push(new Mapping([a.getParameter(b)],!0,void 0,[this],b));a=p.getMappings();var c=this.getDymoMap(),c=Object.keys(c).map(function(a){return c[a]});for(b=0;b<a.length;b++){var d=a[b].getDymoConstraint();d&&(d=
a[b].getTargets().concat(c.filter(d)),a[b].setTargets(d))}};this.removeParent=function(){for(var a=0;a<w.length;a++)w[a].disconnect();w=[];for(var b=p.getMappings(),c=this,a=0;a<b.length;a++){var d=b[a].getTargets(),d=d.filter(function(a){return!a.isSubDymoOf(c)});b[a].setTargets(d)}p=null};this.getParent=function(){return p};this.isSubDymoOf=function(a){for(var b=this;b;){if(b==a)return!0;b=b.getParent()}return!1};this.getDymoMap=function(){var a={};d(this,a);return a};this.getLevel=function(){return p?
p.getLevel()+1:0};this.getIndex=function(){if(p)return p.getParts().indexOf(this)};this.addPart=function(a){m.push(a);a.setParent(this)};this.removePart=function(a){m.splice(m.indexOf(a));a.removeParent(this)};this.replacePart=function(a,b){m[a]&&m[a].removeParent(this);m[a]=b;b.setParent(this)};this.getPart=function(a){return m[a]};this.getParts=function(){return m};this.hasParts=function(){return 0<m.length};this.getNthPart=function(a,b){return e(a,0,b,this)};this.setBasePath=function(a){y=a};this.getBasePath=
function(){return p&&!y?p.getBasePath():y};this.setSourcePath=function(a){u=a};this.getSourcePath=function(){if(p&&!u)return p.getSourcePath();if(u)return this.getBasePath()+u};this.getAllSourcePaths=function(){var a=[];f(this,a,function(b,c){var d=b.getSourcePath();d&&0>a.indexOf(d)&&c.push(d)});return a};this.getAllDymosInHierarchy=function(){var a=[];f(this,a,function(a,b){b.push(a)});return a};this.addSimilar=function(a){q.push(a)};this.getSimilars=function(a){return q};this.getFeatures=function(){return n};
this.setFeature=function(a,b){n[a]=b};this.getFeature=function(a){if(a===LEVEL)return this.getLevel();if(a===INDEX)return this.getIndex();if(n.hasOwnProperty(a))return n[a];if(p)return p.getFeature(a);for(var b=[],c=0;c<m.length;c++)if(m[c]){var d=m[c].getFeatureWithoutLookingFurther(a);isNaN(d)||b.push(d)}if(0<b.length)return b};this.getFeatureWithoutLookingFurther=function(a){if(n[a])return n[a]};this.getMappings=function(){return v};this.getSegment=function(){return[this.getFeature("time"),this.getFeature("duration")]};
this.addParameter=function(a){r[a.getName()]=a};this.getParameter=function(a){return r[a]};this.addMapping=function(a){v.push(a)};this.observedParameterChanged=function(a){a.getName()==PLAY&&c&&(0<a.getChange()?c.play(t):c.stop(t))};this.updatePartOrder=function(a){m&&m[0]&&!isNaN(m[0].getFeature(a))?m.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):m&&m.sort(function(b,c){return b.getParameter(a).getValue()-c.getParameter(a).getValue()})};this.toJsonHierarchy=function(){for(var a=this.toFlatJson(),
b=0;b<m.length;b++)a.parts.push(m[b].toJsonHierarchy());return a};this.toJsonHierarchyGraph=function(){var a={nodes:[],links:[]};g(a,this);return a};this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};k(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=b);u&&(c.source=u);for(var d in n)c[d]=this.getFeatureJson(d);if(0<v.length)for(c.mappings=[],d=0;d<v.length;d++)c.mappings.push(v[d].toJson());0<m.length&&(c.parts=[]);if(0<q.length)for(c.similars=[],d=
0;d<q.length;d++)c.similars.push(q[d].getUri());return c};this.getFeatureJson=function(a){return{value:n[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+a.slice(1)}}};function Mapping(a,b,c,d,e,f){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=f;this.init()}
...
return d}};function OntologyLoader2(a,b,c){};function DymoLoader(a){function b(a,d,e,f,h){var g=new XMLHttpRequest;g.open("GET",a,!0);g.onload=function(){if(0>this.responseText.indexOf("Cannot GET")){var g;a:{try{JSON.parse(this.responseText)}catch(t){g=!1;break a}g=!0}g&&(d?(0<=a.indexOf(q)&&(a=a.replace(q,"")),d=d.replace('"'+a+'"',this.responseText)):d=this.responseText);(g=c(d))?(0>g.indexOf(q)&&(g=q+g),b(g,d,e,f,h)):d&&(g=h(JSON.parse(d),e),f(g))}};g.error=function(a){console.log(a)};g.send()}function c(a){var b=a.indexOf(".json");if(0<=
b){if(b!=a.indexOf("context.json")+7){var d=a.substring(0,b).lastIndexOf('"');return a.substring(d+1,b+5)}return c(a.substring(b+1))}}function d(a,b){var c=e(a,b);f(a,b);return[c,b]}function e(b,c){var d=new DynamicMusicObject(b["@id"],b.cdt,a);d.setBasePath(q);c[b["@id"]]=d;b.source&&d.setSourcePath(b.source);if(b.features)for(var f=0;f<b.features.length;f++)d.setFeature(b.features[f]["@id"],b.features[f].value);if(b.parameters)for(f=0;f<b.parameters.length;f++)t(d,b.parameters[f]["@type"],b.parameters[f].value);
...
c.addNavigator(a.navigator["@type"]==SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):new SequentialNavigator(void 0),e));return[c,d]}function k(b,c,d,e){if(b.controls){for(var f=[],g=0;g<b.controls.length;g++)f.push(e[b.controls[g]]);return h(b,c,d,f,e)}if(b.dymos){var f=[],l;if(b.dymos instanceof Array)for(g=0;g<b.dymos.length;g++)f.push(c[b.dymos[g]]);else l=Function.apply(null,b.dymos.args.concat(b.dymos.body)),g=Object.keys(c).map(function(a){return c[a]}),Array.prototype.push.apply(f,g.filter(l));
return h(b,c,d,f,e,l)}return h(b,c,d,[a],e)}function h(a,b,c,d,e,f){b=a.relative;for(var g=[],h=0;h<a.domainDims.length;h++){var k=a.domainDims[h],p=k.name,q=k["@type"];q==FEATURE?g.push(p):q==PARAMETER?(k=c?t(c,p,0):new Parameter(p,0),g.push(k)):(e&&e[p]?q=e[p]:(q=m(k),k.smooth&&q.setSmooth&&q.setSmooth(!0),!isNaN(k.average)&&q.setAverageOf&&q.setAverageOf(k.average)),"value"in k&&q.update(k.value),e&&!e[p]&&(e[p]=q),g.push(q))}return new Mapping(g,b,a["function"],d,a.range,f)}function t(a,b,c){var d=
...
e,f,g,k,h,t,p;k=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*k,h=e*k,f=2*e,t=f+a,p=f+d,c[p+0]=b[t+0]*h+this.pMidBuffer[f+0]*g,c[p+1]=b[t+1]*h+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
