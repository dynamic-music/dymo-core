...
function DymoManager(a,b,c){var d=new Scheduler(a,function(){});c||(c="bower_components/dymo-core/audio/impulse_rev.wav");d.setReverbFile(c);d.setScheduleAheadTime(b);var e,f={};this.loadDymoAndRendering=function(a,b,c){var m=new DymoLoader;m.loadDymoFromJson(a,function(a){m.loadRenderingFromJson(b,a[1],function(b){e=b[0];e.dymo=a[0];for(var g in b[1]){var l=b[1][g];0<=UI_CONTROLS.indexOf(l.getType())&&(f[g]=new UIControl(l))}d.loadBuffers(e.dymo);c&&c()})})};this.loadDymoFromJson=function(a,b){(new DymoLoader(d)).loadDymoFromJson(a,
function(a){b&&b(a[0])})};this.parseDymoFromJson=function(a,b){(new DymoLoader(d)).parseDymoFromJson(a,function(a){b(a[0])})};this.startPlaying=function(){d.play(e.dymo)};this.stopPlaying=function(){d.stop(e.dymo)};this.getTopDymo=function(){return e.dymo};this.getUIControl=function(a){return f[a]}};function AudioProcessor(a){function b(a){return{extract:function(b,e,f){for(var g=[],l=0;l<a.numberOfChannels;l++)g.push(a.getChannelData(l));for(l=0;l<e;l++)for(var h=0;h<g.length;h++)b[l*g.length+h%g.length]=g[h][l+f];return Math.min(e,g[0].length-f)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var f=b(c),e=new SimpleFilter(f,e),f=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),g=[],l=0;l<c.numberOfChannels;l++)g.push(f.getChannelData(l));for(var h=
new Float32Array(2048),m=e.extract(h,1024),n=0;m;){for(l=0;l<m;l++)for(var q=0;q<g.length;q++)g[q][l+n]=h[l*g.length+q%g.length];n+=m;m=e.extract(h,1024)}return f}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var f=new SimpleFilter({extract:function(a,c,d){for(var e=[],f=0;f<b.numberOfChannels;f++)e.push(b.getChannelData(f));for(f=0;f<c;f++)for(var g=0;g<e.length;g++)a[f*e.length+g%e.length]=e[g][f+d];return Math.min(c,e[0].length-d)}},e),g=a.createScriptProcessor(1024,
2,b.numberOfChannels),l=new Float32Array(2048);g.onaudioprocess=function(a){for(var c=[],e=0;e<b.numberOfChannels;e++)c.push(a.outputBuffer.getChannelData(e));a=f.extract(l,1024);0==a&&d.stop();for(e=0;e<a;e++)for(var g=0;g<c.length;g++)c[g][e]=l[e*c.length+g%c.length]};this.start=function(a){setTimeout(function(){g.connect(c)},a)};this.stop=function(){g.disconnect()}};function Scheduler(a,b,c){function d(){0<k&&k--;0==k&&b&&b(k)}function e(b){var c=b.getUri(),d;if(u[c])for(d=u[c],k=0;k<d.length;k++)p.set(d[k].getDymo(),d[k]);else{d=l(b);for(var k=0;k<d.length;k++){var h=d[k].getDymo();p.set(h,d[k]);w[c]=h.getParameter(ONSET).getValue()}}var m;m=t[c]?t[c]-a.currentTime:n;h=a.currentTime+m;for(k=0;k<d.length;k++)d[k].play(h);setTimeout(function(){g()},m);(k=l(b))?(u[c]=k,k=k[0].getDymo().getParameter(ONSET).getValue(),m=k-w[c],-1!=k?(t[c]=h+m,w[c]=k):t[c]=h+d[0].getDuration()/
d[0].getDymo().getParameter(PLAYBACK_RATE).getValue(),t[c]&&(r=setTimeout(function(){e(b)},1E3*(t[c]-a.currentTime-n)))):(t[c]=h+d[0].getDuration()/d[0].getDymo().getParameter(PLAYBACK_RATE).getValue(),r=setTimeout(function(){f(b)},1E3*(t[c]-a.currentTime)))}function f(a){window.clearTimeout(r);var b=a.getUri();u[b]=null;t[b]=null;a.resetPartsPlayed();g()}function g(){for(var a=[],b=$jscomp.makeIterator(p.keys()),d=b.next();!d.done;d=b.next())for(d=d.value;null!=d;)0>a.indexOf(d.getUri())&&a.push(d.getUri()),
d=d.getParent();a.sort();m.urisOfPlayingDymos=a;c&&c()}function l(b){if(b=b.getNextParts()){for(var c=[],d=0;d<b.length;d++)if(b[d].getSourcePath()){var r=q[b[d].getSourcePath()];c.push(new Source(b[d],a,r,v))}return c}}function h(b,c){k++;var r=new XMLHttpRequest;r.open("GET",b,!0);r.responseType="arraybuffer";r.onload=function(){a.decodeAudioData(r.response,function(a){c?c(a):(q[b]=a,d())})};r.error=function(){};r.send()}var m=this,n=.1,q={},p=new Map,u={},t={},w={};this.urisOfPlayingDymos=[];this.listenerOrientation=
new Parameter(this,0);var v=a.createConvolver();v.connect(a.destination);var k=0,r;this.setReverbFile=function(a){h(a,function(a){v.buffer=a;d()})};this.setScheduleAheadTime=function(a){n=a};this.loadBuffers=function(a){a=a.getAllSourcePaths();for(var b=0,c=a.length;b<c;b++)q[a[b]]||h(a[b])};this.getBuffer=function(a){return q[a.getSourcePath()]};this.play=function(a){a.updatePartOrder(ONSET);e(a)};this.pause=function(a){a=a.getAllDymosInHierarchy();for(var b=0,c=a.length;b<c;b++){var d=p.get(a[b]);
d&&d.pause()}};this.stop=function(a){for(var b=a.getAllDymosInHierarchy(),c=0,d=b.length;c<d;c++){var r=p.get(b[c]);r&&(r.stop(),p.delete(b[c]))}f(a)};this.getSource=function(a){return p.get(a)};this.updateListenerOrientation=function(){var b=this.listenerOrientation.getValue()/180*Math.PI;a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0)}};function Source(a,b,c,d,e){function f(){var d=a.getParameter(TIME_STRETCH_RATIO).getValue();if(1!=d){if(0!=z||x<c.duration)c=n(c,m(z,c),m(x+.3,c));c=(new AudioProcessor(b)).timeStretch(c,d);d=x/d;c=n(c,0,m(d+.02,c));x=d}else if(0!=z||x<c.duration)c=n(c,m(z,c),m(x+.02,c));q(c,c.length);y.buffer=c;k[PLAYBACK_RATE]=y.playbackRate;g(PLAYBACK_RATE,a.getParameter(PLAYBACK_RATE))}function g(a,b){l(a,b.getValue());b.addObserver(t)}function l(a,b,c){c&&(b+=t.getParameterValue(a));0>b&&0<=A.indexOf(a)&&(b=
0);c=b;k[a]&&(k[a].value||0==k[a].value?k[a].value=c:k[a].setValue(c));0<=F.indexOf(a)?(0==k[DISTANCE].value&&(k[DISTANCE].value=-.01),D.setPosition(k[PAN].value,k[HEIGHT].value,k[DISTANCE].value)):a==LOOP&&(y.loop=1==b)}function h(){w=!1;var a=b.currentTime;k[AMPLITUDE].setValueAtTime(k[AMPLITUDE].value,a);k[AMPLITUDE].linearRampToValueAtTime(0,a+.02);y.stop(a+.04)}function m(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function n(a,c,d){d=Math.min(a.length-c,d);for(var r=b.createBuffer(a.numberOfChannels,
d,a.sampleRate),e=0;e<a.numberOfChannels;e++)for(var k=r.getChannelData(e),f=a.getChannelData(e),A=0;A<d;A++)k[A]=f[c+A];return r}function q(a,b){for(var c=.02*a.sampleRate,d=0;d<a.numberOfChannels;d++)for(var r=a.getChannelData(d),e=0;e<c;e++)r[e]*=e/c,r[b-e-1]*=e/c}function p(a,b,c,d){var r=a.lastIndexOf("/");r&&(a=a.substring(r+1));u("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function u(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType=
"arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}var t=this,w,v,k={},r=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,FILTER,PAN,HEIGHT,DISTANCE,LOOP],A=[AMPLITUDE,PLAYBACK_RATE,TIME_STRETCH_RATIO,REVERB,FILTER],F=[PAN,HEIGHT,DISTANCE],y=b.createBufferSource(),B=b.createGain();B.connect(b.destination);var C=b.createGain();C.connect(d);C.gain.value=0;B.connect(C);var D=b.createPanner();D.connect(B);
d=b.createBiquadFilter();d.type="lowpass";d.frequency.value=2E4;d.connect(D);y.connect(d);var E=a.getSegment(),z=E[0],x=E[1];z||(z=0);!x&&c&&(x=c.duration-z);c?f():p(a.getSourcePath(),z,z+x+.3,function(d){c=d;d=a.getParameter(TIME_STRETCH_RATIO).getValue();1!=d?(c=(new AudioProcessor(b)).timeStretch(c,d),d=x/d,c=n(c,0,m(d+.02,c)),x=d):c=n(c,0,m(x+.02,c));q(c,c.length);y.buffer=c});k[AMPLITUDE]=B.gain;k[TIME_STRETCH_RATIO]={value:0};k[REVERB]=C.gain;k[FILTER]=d.frequency;k[PAN]={value:0};k[HEIGHT]=
{value:0};k[DISTANCE]={value:0};k[LOOP]={value:0};g(AMPLITUDE,a.getParameter(AMPLITUDE));g(TIME_STRETCH_RATIO,a.getParameter(TIME_STRETCH_RATIO));g(REVERB,a.getParameter(REVERB));g(PAN,a.getParameter(PAN));g(HEIGHT,a.getParameter(HEIGHT));g(DISTANCE,a.getParameter(DISTANCE));g(LOOP,a.getParameter(LOOP));this.observedParameterChanged=function(a){l(a.getName(),a.getChange(),!0)};this.getDymo=function(){return a};this.getDuration=function(){return x};this.isLoopingAndPlaying=function(){return y.loop&&
w};this.getParameterValue=function(a){if(k[a])return k[a].value||0==k[a].value?k[a].value:k[a].getValue()};this.play=function(b){y.onended=function(){for(var b=0;b<r.length;b++){var c=a.getParameter(r[b]);c&&c.removeObserver(t)}y.disconnect();B.disconnect();C.disconnect();e&&e()};y.start(b);w=!0};this.pause=function(){w?(h(),v=!0):v&&(v=!1,this.play())};this.stop=function(){w&&h()}};function AutoControl(a,b,c){function d(a){var b=a.getName();f[b]=a;f[b].addObserver(e)}var e=this,f={};d(new Parameter(AUTO_CONTROL_FREQUENCY,100));d(new Parameter(AUTO_CONTROL_TRIGGER,0));Control.call(this,a,AUTO_CONTROL,f);var g;this.addParameter=d;this.getParameter=function(a){return f[a]};this.startUpdate=function(){g=setInterval(b,f[AUTO_CONTROL_FREQUENCY].getValue())};this.reset=function(){clearInterval(g);g=null;c&&c()};this.observedParameterChanged=function(a){a==f[AUTO_CONTROL_FREQUENCY]?
g&&(this.reset(),this.startUpdate()):a==f[AUTO_CONTROL_TRIGGER]&&(g?this.reset():this.startUpdate())}}inheritPrototype(AutoControl,Control);function BrownianControl(a){var b=this;a||(a=.5);AutoControl.call(this,BROWNIAN,function(){var a=b.getParameter(BROWNIAN_MAX_STEP_SIZE).getValue(),a=2*a*Math.random()-a,a=b.getValue()+a,a=Math.min(Math.max(a,0),1);b.update(a)});this.addParameter(new Parameter(BROWNIAN_MAX_STEP_SIZE,.1));this.update(a);this.startUpdate()}inheritPrototype(BrownianControl,AutoControl);function RampControl(a,b){var c=this;a||(a=1E4);var d=0;b&&(d=b);var e=1!=d;AutoControl.call(this,RAMP,function(){var b=1/a*c.getParameter(AUTO_CONTROL_FREQUENCY).getValue();e||(b*=-1);d+=b;0<d&&1>d?c.update(d):1<=d?(c.update(1),c.reset()):0>=d&&(c.update(0),c.reset())},function(){e=!e});this.update(d)}inheritPrototype(RampControl,AutoControl);function RandomControl(){var a=this;AutoControl.call(this,RANDOM,function(){a.update(Math.random())});this.startUpdate()}inheritPrototype(RandomControl,AutoControl);function UIControl(a,b){var c=this;this.value=a.getValue();a.setUpdateFunction(function(a){c.value=a;b&&setTimeout(function(){b.$apply()},10)});this.getName=function(){return a.getName()};this.getType=function(){return a.getType()};this.update=function(){a.getType()==BUTTON&&(this.value=1-this.value);1==this.value?a.update(1):0==this.value?a.update(0):a.update(this.value)}};function Control(a,b,c){function d(a,b){if(void 0==f||1E-6<Math.abs(a-f))f=a,e(b)}function e(a){for(var b=0;b<h.length;b++)h[b]!=a&&h[b].updateParameter(f,this)}var f,g,l,h=[],m,n,q;this.getName=function(){return a};this.getParameter=function(a){if(c)return c[a]};this.getValue=function(){return f};this.getType=function(){return b};this.getReferenceValue=function(){return g};this.setReferenceAverageCount=function(a){l=a;this.resetReferenceValue()};this.resetReferenceValue=function(){g=f=void 0;n=m=
0};this.setUpdateFunction=function(a){q=a};this.addMapping=function(a){h.push(a);a.updateParameter(f,this)};this.removeMapping=function(a){a=h.indexOf(a);-1<a&&h.splice(a,1)};this.backpropagate=function(a,b){isFinite(a)&&(d(a,b),q&&q(f))};this.update=function(a){l&&m<l?(n+=a,m++,m==l&&(g=n/=l)):isNaN(a)||(g&&(a-=g),d(a))}};function DynamicMusicObject(a,b){function c(a,b){b[a.getUri()]=a;for(var d=0;d<a.getParts().length;d++)c(a.getParts()[d],b)}function d(a,b,c,e){var k=a-b,f=e.getParts();if(e.getLevel()==c&&0==k)return e;if(e.getLevel()>=c-1)return k<f.length?f[k]:b+f.length;for(e=0;e<f.length;e++)if(b=d(a,b,c,f[e]),isNaN(b))return b}function e(a,b,c){c(a,b);a=a.getParts();for(var d=0,k=a.length;d<k;d++)e(a[d],b,c)}function f(a,b,c){var d=b.toFlatJson();a.nodes.push(d);c&&a.links.push(l(c,d));for(c=0;c<b.getParts().length;c++)f(a,
b.getParts()[c],d)}function g(a,b,c){c||(c={});c[b.getUri()]||(c[b.getUri()]=b.toFlatJson());var d=c[b.getUri()];a.nodes.push(d);for(var e=0;e<b.getSimilars().length;e++){var k=b.getSimilars()[e];c[k.getUri()]||(c[k.getUri()]=k.toFlatJson());a.links.push(l(d,c[k.getUri()]))}for(e=0;e<b.getParts().length;e++)g(a,b.getParts()[e],c)}function l(a,b){return{source:a,target:b,value:1}}var h=null,m=[],n=[],q={},p={},u=[],t=[],w="",v;p[PLAY]=new Parameter(PLAY,0,!0);p[LOOP]=new Parameter(LOOP,0,!0);p[ONSET]=
new Parameter(ONSET,-1);p[DURATION_RATIO]=new Parameter(DURATION_RATIO,1);p[AMPLITUDE]=new Parameter(AMPLITUDE,1);p[PLAYBACK_RATE]=new Parameter(PLAYBACK_RATE,1);p[TIME_STRETCH_RATIO]=new Parameter(TIME_STRETCH_RATIO,1);p[PAN]=new Parameter(PAN,0);p[DISTANCE]=new Parameter(DISTANCE,0);p[HEIGHT]=new Parameter(HEIGHT,0);p[REVERB]=new Parameter(REVERB,0);p[FILTER]=new Parameter(FILTER,0);p[PART_INDEX]=new Parameter(PART_INDEX,0,!0);p[PART_COUNT]=new Parameter(PART_COUNT,Number.POSITIVE_INFINITY,!0);
p[PLAY].addObserver(this);p[PART_INDEX].addObserver(this);p[PART_COUNT].addObserver(this);var k=new SequentialNavigator(this);this.setType=function(a){b=a};this.getType=function(){return b};this.getUri=function(){return a};this.setParent=function(a){h=a;for(var b in p)b!=PLAY&&b!=PART_COUNT&&b!=PART_INDEX&&t.push(new Mapping([a.getParameter(b)],!0,void 0,[this],b));a=h.getMappings();var c=this.getDymoMap(),c=Object.keys(c).map(function(a){return c[a]});for(b=0;b<a.length;b++){var d=a[b].getDymoConstraint();
d&&(d=a[b].getTargets().concat(c.filter(d)),a[b].setTargets(d))}};this.removeParent=function(){for(var a=0;a<t.length;a++)t[a].disconnect();t=[];for(var b=h.getMappings(),c=this,a=0;a<b.length;a++){var d=b[a].getTargets(),d=d.filter(function(a){return!a.isSubDymoOf(c)});b[a].setTargets(d)}h=null};this.getParent=function(){return h};this.isSubDymoOf=function(a){for(var b=this;b;){if(b==a)return!0;b=b.getParent()}return!1};this.getDymoMap=function(){var a={};c(this,a);return a};this.getLevel=function(){return h?
h.getLevel()+1:0};this.getIndex=function(){if(h)return h.getParts().indexOf(this)};this.addPart=function(a){m.push(a);a.setParent(this)};this.removePart=function(a){m.splice(m.indexOf(a));a.removeParent(this)};this.replacePart=function(a,b){m[a]&&m[a].removeParent(this);m[a]=b;b.setParent(this)};this.getPart=function(a){return m[a]};this.getParts=function(){return m};this.getNthPart=function(a,b){return d(a,0,b,this)};this.setBasePath=function(a){w=a};this.getBasePath=function(){return h&&!w?h.getBasePath():
w};this.setSourcePath=function(a){v=a};this.getSourcePath=function(){if(h&&!v)return h.getSourcePath();if(v)return this.getBasePath()+v};this.getAllSourcePaths=function(){var a=[];e(this,a,function(b,c){var d=b.getSourcePath();d&&0>a.indexOf(d)&&c.push(d)});return a};this.getAllDymosInHierarchy=function(){var a=[];e(this,a,function(a,b){b.push(a)});return a};this.addSimilar=function(a){n.push(a)};this.getSimilars=function(a){return n};this.getFeatures=function(){return q};this.setFeature=function(a,
b){q[a]=b};this.getFeature=function(a){if(a===LEVEL)return this.getLevel();if(a===INDEX)return this.getIndex();if(q.hasOwnProperty(a))return q[a];if(h)return h.getFeature(a);for(var b=[],c=0;c<m.length;c++)if(m[c]){var d=m[c].getFeatureWithoutLookingFurther(a);isNaN(d)||b.push(d)}if(0<b.length)return b};this.getFeatureWithoutLookingFurther=function(a){if(q[a])return q[a]};this.getMappings=function(){return u};this.getSegment=function(){return[this.getFeature("time"),p[DURATION_RATIO].getValue()*this.getFeature("duration")]};
this.addParameter=function(a){p[a.getName()]=a};this.getParameter=function(a){return p[a]};this.addMapping=function(a){u.push(a)};this.observedParameterChanged=function(a){a.getName()==PLAY&&a.getChange()};this.setNavigator=function(a){k=a};this.getNavigator=function(){return k};this.resetPartsPlayed=function(){k.resetPartsPlayed();for(var a=0;a<m.length;a++)m[a].resetPartsPlayed()};this.updatePartOrder=function(a){m&&m[0]&&!isNaN(m[0].getFeature(a))?m.sort(function(b,c){return b.getFeature(a)-c.getFeature(a)}):
m&&m.sort(function(b,c){return b.getParameter(a).getValue()-c.getParameter(a).getValue()})};this.getNextParts=function(){return k.getNextParts()};this.toJsonHierarchy=function(){for(var a=this.toFlatJson(),b=0;b<m.length;b++)a.parts.push(m[b].toJsonHierarchy());return a};this.toJsonHierarchyGraph=function(){var a={nodes:[],links:[]};f(a,this);return a};this.toJsonSimilarityGraph=function(){var a={nodes:[],links:[]};g(a,this);return a};this.toFlatJson=function(){var c={"@id":a,"@type":DYMO};b&&(c.ct=
b);v&&(c.source=v);k instanceof SimilarityNavigator&&(c.navigator=SIMILARITY_NAVIGATOR);for(var d in q)c[d]=this.getFeatureJson(d);if(0<u.length)for(c.mappings=[],d=0;d<u.length;d++)c.mappings.push(u[d].toJson());0<m.length&&(c.parts=[]);if(0<n.length)for(c.similars=[],d=0;d<n.length;d++)c.similars.push(n[d].getUri());return c};this.getFeatureJson=function(a){return{value:q[a],type:FEATURE,adt:a.charAt(0).toUpperCase()+a.slice(1)}}};function Mapping(a,b,c,d,e,f){this.relative=b;this.functionJson=c;this.parameterName=e;this.setTargets(d);this.domainDims=a;this.dymoConstraint=f;this.init()}
...
d[c].getNextParts())&&(!g.length||0<g.length))return g instanceof Array||(g=[g]),g;c++}c=0;b=!1;return null}if(b)return b=!1,null;b=!0;return[a]}};function SimilarityNavigator(a){function b(b){for(var d=0;d<b.length;d++)if(0<b[d].getSimilars().length&&Math.random()<c.leapingProbability.getValue()){var l=b[d].getSimilars(),h=Math.floor(Math.random()*l.length);if(c.continueAfterLeaping.getValue()){var m=a.getParts().indexOf(h);m&&(e=m+1)}b[d]=l[h]}}var c=this,d=!1,e=0;this.leapingProbability=new Parameter(LEAPING_PROBABILITY,.5);this.continueAfterLeaping=new Parameter(CONTINUE_AFTER_LEAPING,0,!0);this.resetPartsPlayed=function(){e=0};this.getNextParts=
function(){var c=a.getParts();if(0<c.length){if(a.getType()==PARALLEL){for(var g=[],l=0;l<c.length;l++){var h=c[l].getNextParts();h&&(!h.length||0<h.length)&&(g=g.concat(h))}if(0<g.length)return b(g),g}else for(d=!0;e<c.length&&e<a.getParameter(PART_COUNT).getValue();){if((h=c[e].getNextParts())&&(!h.length||0<h.length))return h instanceof Array||(h=[h]),b(h),h;e++}e=0;d=!1;return null}if(d)return d=!1,null;d=!0;return[a]}};function FunctionTools(){}FunctionTools.IDENTITY_JSON={args:["a"],body:"return a;"};FunctionTools.inverted={};FunctionTools.created={};FunctionTools.createFunction=function(a,b){FunctionTools.created[b]||(FunctionTools.created[b]=Function.apply(null,a.concat(b)));return FunctionTools.created[b]};
...
e,f,g,l,h,m,n;l=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)g=(this.overlapLength-e)*l,h=e*l,f=2*e,m=f+a,n=f+d,c[n+0]=b[m+0]*h+this.pMidBuffer[f+0]*g,c[n+1]=b[m+1]*h+this.pMidBuffer[f+1]*g},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
