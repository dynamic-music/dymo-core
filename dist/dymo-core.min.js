...
function DymoManager(a,b,c,d,e,g){function k(a,b){f=a;m=b[0];for(var d in b[1]){var g=b[1][d];DYMO_STORE.isSubclassOf(g.getType(),UI_CONTROL)&&(h[d]=new UIControl(g,e));DYMO_STORE.isSubclassOf(g.getType(),SENSOR_CONTROL)&&(n[d]=g)}c||(c="bower_components/dymo-core/audio/impulse_rev.wav");l.init(c)}DYMO_STORE=new DymoStore(d);var l=new Scheduler(a,g);isNaN(b)||(SCHEDULE_AHEAD_TIME=b);var f,m,h={},n={};this.loadDymoAndRenderingFromStore=function(a,b){var c=new DymoLoader(a);k(c.createDymoFromStore(),
c.createRenderingFromStore());l.loadBuffers(m.getDymoUri(),b)};this.loadDymoAndRendering=function(a,b,c,d){var h=new DymoLoader(DYMO_STORE);h.loadDymoFromJson(a,function(a){h.loadRenderingFromJson(b,function(b){k(a,b);l.loadBuffers(a,d);c&&c()})})};this.loadDymoFromJson=function(a,b){(new DymoLoader(DYMO_STORE)).loadDymoFromJson(a,b)};this.parseDymoFromJson=function(a,b){(new DymoLoader(DYMO_STORE)).parseDymoFromJson(a,b)};this.replacePartOfTopDymo=function(a,b){var c=DYMO_STORE.replacePartAt(f[0],
...
DymoNode.prototype.removeAndDisconnect=function(){for(var a in this.parameters)DYMO_STORE.removeParameterObserver(this.dymoUri,a,this);this.dryGain.disconnect();this.reverbGain&&this.reverbGain.disconnect();this.delayGain&&this.delayGain.disconnect()};function AudioProcessor(a){function b(a){return{extract:function(b,e,g){for(var k=[],l=0;l<a.numberOfChannels;l++)k.push(a.getChannelData(l));for(l=0;l<e;l++)for(var f=0;f<k.length;f++)b[l*k.length+f%k.length]=k[f][l+g];return Math.min(e,k[0].length-g)}}}this.timeStretch=function(c,d){var e=new SoundTouch(c.sampleRate);e.tempo=d;for(var g=b(c),e=new SimpleFilter(g,e),g=a.createBuffer(c.numberOfChannels,1/d*c.length,c.sampleRate),k=[],l=0;l<c.numberOfChannels;l++)k.push(g.getChannelData(l));for(var f=
new Float32Array(2048),m=e.extract(f,1024),h=0;m;){for(l=0;l<m;l++)for(var n=0;n<k.length;n++)k[n][l+h]=f[l*k.length+n%k.length];h+=m;m=e.extract(f,1024)}return g}};function AudioProcessorSource(a,b,c){var d=this,e=new SoundTouch(b.sampleRate);this.stretchRatio={setValue:function(a){e.tempo=a},getValue:function(){return e.tempo}};this.playbackRate={setValue:function(a){e.rate=a},getValue:function(){return e.rate}};var g=new SimpleFilter({extract:function(a,c,d){for(var n=[],p=0;p<b.numberOfChannels;p++)n.push(b.getChannelData(p));for(p=0;p<c;p++)for(var e=0;e<n.length;e++)a[p*n.length+e%n.length]=n[e][p+d];return Math.min(c,n[0].length-d)}},e),k=a.createScriptProcessor(1024,
2,b.numberOfChannels),l=new Float32Array(2048);k.onaudioprocess=function(a){for(var c=[],h=0;h<b.numberOfChannels;h++)c.push(a.outputBuffer.getChannelData(h));a=g.extract(l,1024);0==a&&d.stop();for(h=0;h<a;h++)for(var n=0;n<c.length;n++)c[n][h]=l[h*c.length+n%c.length]};this.start=function(a){setTimeout(function(){k.connect(c)},a)};this.stop=function(){k.disconnect()}};function Scheduler(a,b){function c(a){l.splice(l.indexOf(a),1)}function d(){for(var a=[],c=0;c<l.length;c++)for(var d=$jscomp.makeIterator(l[c].getAllSources().keys()),h=d.next();!h.done;h=d.next())for(h=h.value;null!=h;)0>a.indexOf(h)&&a.push(h),h=DYMO_STORE.findParents(h)[0];a.sort();f=a;b&&b(f)}function e(b,c){h++;var d=new XMLHttpRequest;d.open("GET",b,!0);d.responseType="arraybuffer";d.onload=function(){a.decodeAudioData(d.response,function(a){c(a,b)})};d.error=function(){};d.send()}var g=this,
k={},l=[],f=[],m,h=0;this.init=function(b){DYMO_STORE.addParameter(null,LISTENER_ORIENTATION,0,g);0<DYMO_STORE.find(null,null,REVERB).length&&(m=a.createConvolver(),m.connect(a.destination),e(b,function(a){m.buffer=a;0<h&&h--}));if(0<DYMO_STORE.find(null,null,DELAY).length){b=a.createDelay();b.delayTime.value=.5;b.connect(a.destination);var c=a.createGain();c.gain.value=.6;b.connect(c);c.connect(b)}};this.loadBuffers=function(a,b){for(var c=[],d=0,f=a.length;d<f;d++)var m=DYMO_STORE.findAllObjectsInHierarchy(a[d]),
c=c.concat(m.map(function(a){return DYMO_STORE.getSourcePath(a)}));d=0;for(f=c.length;d<f;d++)c[d]&&!k[c[d]]&&e(c[d],function(a,c){k[c]=a;0<h&&h--;0==h&&b&&b()});0==h&&b&&b()};this.getBuffer=function(a){return k[DYMO_STORE.getSourcePath(a)]};this.updateNavigatorPosition=function(a,b,c){for(var d=0,h=l.length;d<h;d++)l[d].hasDymo(a)&&l[d].getNavigator().setPosition(c,b,a)};this.getNavigatorPosition=function(a,b){for(var c=0,d=l.length;c<d;c++)if(l[c].hasDymo(a))return l[c].getNavigator().getPosition(b,
a)};this.syncNavigators=function(a,b,c){for(var d,h,e=0,f=l.length;e<f;e++)l[e].hasDymo(a)&&(d=l[e].getNavigator()),l[e].hasDymo(b)&&(h=l[e].getNavigator());h&&(b=h.getPosition(c,b),d.setPosition(b,c,a))};this.play=function(b,h){var e=new SchedulerThread(b,h,a,k,m,void 0,d,c);l.push(e)};this.pause=function(a){if(a)for(var b=0;b<l.length;b++)l[b].pause(a)};this.stop=function(a){if(a)for(var b=0;b<l.length;b++)l[b].stop(a)};this.getSources=function(a){for(var b=[],c=0;c<l.length;c++){var d=l[c].getSources(a);
if(d)for(var h=0;h<d.length;h++)b=b.concat(d)}return b};this.getUrisOfPlayingDymos=function(){return f};this.observedValueChanged=function(b,c,d){c==LISTENER_ORIENTATION&&(b=d/180*Math.PI,a.listener.setOrientation(Math.sin(b),0,-Math.cos(b),0,1,0))}};function DymoSource(a,b,c,d,e,g){function k(b,d){var e=b[0],f=b[1],f=1!=d?f+.3:f+FADE_LENGTH;if(c||isNaN(e+f)){if(0!=e||f<c.duration)c=h(c,m(e,c),m(f,c));return l(c,f,d)}n(DYMO_STORE.getSourcePath(a),e,e+f,function(a){return l(a,f,d)})}function l(a,c,d){1!=d&&(a=(new AudioProcessor(b)).timeStretch(a,d),a=h(a,0,m(c/d+FADE_LENGTH,a)));c=a;d=a.length;for(var e=c.sampleRate*FADE_LENGTH,f=0;f<c.numberOfChannels;f++)for(var p=c.getChannelData(f),n=0;n<e;n++)p[n]*=n/e,p[d-n-1]*=n/e;return a}function f(){v=
!1;var a=b.currentTime;r.parameters[AMPLITUDE].setValueAtTime(r.parameters[AMPLITUDE].value,a);r.parameters[AMPLITUDE].linearRampToValueAtTime(0,a+FADE_LENGTH);q.stop(a+2*FADE_LENGTH)}function m(a,b){if(a||0==a)return Math.round(a*b.sampleRate)}function h(a,c,d){d=Math.min(a.length-c,d);for(var h=b.createBuffer(a.numberOfChannels,d,a.sampleRate),e=0;e<a.numberOfChannels;e++)for(var f=h.getChannelData(e),n=a.getChannelData(e),p=0;p<d;p++)f[p]=n[c+p];return h}function n(a,b,c,d){var h=a.lastIndexOf("/");
h&&(a=a.substring(h+1));p("http://localhost:8060/getAudioChunk?filename="+a+"&fromSecond="+b+"&toSecond="+c,function(a){d(a)})}function p(a,c){var d=new XMLHttpRequest;d.open("GET",a,!0);d.responseType="arraybuffer";d.onload=function(){b.decodeAudioData(d.response,function(a){c(a)},function(a){console.log("audio from server is faulty")})};d.send()}DymoNode.call(this,a,b,d,e);var r=this,v,t,u,q=b.createBufferSource();q.connect(this.getInput());(function(){var b=[DYMO_STORE.findFeatureValue(a,TIME_FEATURE),
DYMO_STORE.findFeatureValue(a,DURATION_FEATURE)];b[0]||(b[0]=0);!b[1]&&c&&(b[1]=c.duration-b[0]);var d=DYMO_STORE.findParameterValue(a,DURATION_RATIO);d&&0<d&&1>d&&(b[1]*=d);u=b[1];d=DYMO_STORE.findParameterValue(a,TIME_STRETCH_RATIO);q.buffer=k(b,d?d:1)})();this.addParameter(PLAYBACK_RATE,q.playbackRate);this.addParameter(TIME_STRETCH_RATIO,{value:0});this.addParameter(LOOP,{value:0});this.getDuration=function(){return u};this.isLoopingAndPlaying=function(){return q.loop&&v};this.play=function(a){q.onended=
function(){q.disconnect();r.removeAndDisconnect();g&&g(r)};a||(a=0);q.start(a);v=!0};this.pause=function(){v?(f(),t=!0):t&&(t=!1,this.play())};this.stop=function(){v&&f()}}inheritPrototype(DymoSource,DymoNode);function SchedulerThread(a,b,c,d,e,g,k,l){function f(){w=q?q:r();m(w);if(!z){var a=w.keys().next().value;z=DYMO_STORE.findParameterValue(a,ONSET)}a=x?x-c.currentTime:SCHEDULE_AHEAD_TIME;for(var b=c.currentTime+a,d=$jscomp.makeIterator(w.values()),h=d.next();!h.done;h=d.next())h.value.play(b);setTimeout(function(){k()},a);(q=r())&&0<q.size?(x=p(b),a=x[1],x=x[0],DYMO_STORE.findParameterValue(a.getDymoUri(),LOOP)&&(x-=FADE_LENGTH),a=1E3*(x-c.currentTime-SCHEDULE_AHEAD_TIME),y=setTimeout(function(){f()},
a)):(x=p(b),a=1E3*(x-c.currentTime),setTimeout(function(){n()},a+100))}function m(a){for(var b=$jscomp.makeIterator(a.keys()),c=b.next();!c.done;c=b.next())c=c.value,t.get(c)||t.set(c,[]),t.get(c).push(a.get(c))}function h(a){var b=t.get(a.getDymoUri());b.splice(b.indexOf(a),1);0>=b.length&&t.delete(a.getDymoUri());k();n()}function n(){if(0==t.size&&(!q||0==q.size)){window.clearTimeout(y);b.reset();for(var c=DYMO_STORE.findAllObjectsInHierarchy(a),d=0,h=c.length;d<h;d++){var e=u.get(c[d]);e&&e.removeAndDisconnect()}l&&
l()}}function p(a){if(q){var b=q.keys().next().value,b=DYMO_STORE.findParameterValue(b,ONSET),c=b-z;z=b;if(!isNaN(b))return a+c}for(var b=0,d,c=$jscomp.makeIterator(w.values()),h=c.next();!h.done;h=c.next()){var h=h.value,e=h.getDuration();e>b&&(b=e,d=h)}return[a+b,d]}function r(){var a=b.getNextParts();if(a){v(a);for(var f=new Map,p=0;p<a.length;p++){var n=DYMO_STORE.getSourcePath(a[p]);if(n){n=new DymoSource(a[p],c,d[n],e,g,h);a:{for(var m=n,w=DYMO_STORE.findParents(n.getDymoUri())[0];w;){if(u.has(w)){m.connect(u.get(w).getInput());
break a}var r=new DymoNode(w,c,e,g);m.connect(r.getInput());u.set(w,r);w=DYMO_STORE.findParents(w)[0];m=r}m.connect(c.destination)}f.set(a[p],n)}}return f}}function v(a){console.log(a.map(function(a){a=DYMO_STORE.findPartIndex(a);return isNaN(a)?"top":a}))}var t=new Map,u=new Map,q,y,w=[],x,z;b||(b=new DymoNavigator(a,new SequentialNavigator(a)));f();this.hasDymo=function(b){if(DYMO_STORE.findAllObjectsInHierarchy(a).indexOf(b))return!0};this.getNavigator=function(){return b};this.stop=function(a){a=
DYMO_STORE.findAllObjectsInHierarchy(a);for(var b=0,c=a.length;b<c;b++){var d=t.get(a[b]);if(d)for(var h=0;h<d.length;h++)d[h].stop();q&&(q.delete(a[b]),0>=q.size&&(q=void 0))}};this.getAllSources=function(){return t};this.getSources=function(a){return t.get(a)}};function DymoGenerator(a,b){function c(b,c){var h=d();a.addDymo(h,b,null,c);b||(p=h);return h}function d(){var a=CONTEXT_URI+"dymo"+y;y++;return a}function e(a){var b=[];if(a&&0<a.length){for(var c=0;c<a.length;c++)a[c].value.constructor!==Array&&(a[c].value=[a[c].value]);for(var c=a[0].value.length,d=0;d<c;d++)if("string"==typeof a[0].value[d])b[d]=a[0].value[d];else if(u==SUMMARY.FIRST)b[d]=a[0].value[d];else if(u==SUMMARY.MEAN)b[d]=a.reduce(function(a,b){return a+b.value[d]},0)/a.length;else if(u==
SUMMARY.MEDIAN){a.sort(function(a,b){return a.value[d]-b.value[d]});var h=Math.floor(a.length/2);b[d]=a[h].value[d];0==a.length%2&&(b[d]+=a[h-1].value[d])}return 1==b.length?b[0]:b}return 0}function g(){0==y?p=c(void 0,q):v&&(p=c(n,q),v=!1)}function k(b,c){for(var d=p,h=a.findLevel(p);h<c;){var e=a.findParts(d);if(0<e.length){e=e.map(function(b){return[a.findFeatureValue(b,TIME_FEATURE),b]});e.sort(function(a,b){return a[0]-b[0]});for(var f=0;f<e.length;f++)if(e[f][0]<=b)d=e[f][1];else if(0==f)d=
e[f][1];else break;h++}else break}return d}function l(a,b){void 0==a.max?(a.min=b,a.max=b):(a.min=Math.min(b,a.min),a.max=Math.max(b,a.max))}function f(a,b){for(var c=0;c<t.length;c++)if(t[c].name==a||t[c].uri==b)return t[c];return m(a,b)}function m(a,c,d,h){!a&&c&&(a=URI_TO_TERM[c]);a&&!c&&(c=CONTEXT_URI+a);a=void 0!=d&&void 0!=h?{name:a,uri:c,min:d,max:h}:{name:a,uri:c,min:1E3,max:0};2>t.length?t.push(a):t.splice(t.length-2,0,a);b(a);return a}var h=this,n,p,r,v,t,u=SUMMARY.MEAN,q,y=0;this.resetDymo=
function(){p=n=void 0;v=!1;t=[];m("level",LEVEL_FEATURE);m("random",null,0,1)};this.setStore=function(b){a=b};this.getStore=function(){return a};this.addRendering=function(){var b=CONTEXT_URI+"rendering0";b++;r=b;a.addRendering(r,p)};this.addMapping=function(b,c,d,h){a.addMapping(b,c,d,h)};this.addNavigator=function(b,c,d){a.addNavigator(r,b,c,d)};this.getCurrentTopDymo=function(){return p};this.getFeatures=function(){return t};this.setSummarizingMode=function(a){u=a};this.setCurrentSourcePath=function(a){q=
a};this.setAudioFileChanged=function(){v=!0;if(p){var b=d();a.addDymo(b,null,p);p=b}};this.addDymo=function(a,b){return c(a,b)};this.addFeature=function(b,c){Benchmarker.startTask("addFeature");g();for(var d=f(b),h=a.findAllObjectsInHierarchy(p),n=0;n<h.length;n++){var m=a.findFeatureValue(h[n],TIME_FEATURE),r=a.findFeatureValue(h[n],DURATION_FEATURE),u=c;isNaN(m)||(u=u.filter(function(a){return m<=a.time&&(isNaN(r)||a.time<m+r)}));if(1>u.length)var k=c.filter(function(a){return a.time.value<=m}),
u=0<k.length?[k[u.length-1]]:[c[0]];Benchmarker.startTask("summarize");u=e(u);this.setDymoFeature(h[n],d.uri,u)}};this.addSegmentation=function(b){g();for(var d=a.findMaxLevel(),e=0;e<b.length;e++){var f=k(b[e].time,d),n=b[e].time,p;if(b[e].duration)p=b[e].duration;else if(b[e+1])p=b[e+1].time-n;else{var m=a.findFeatureValue(f,TIME_FEATURE),r=a.findFeatureValue(f,DURATION_FEATURE);m&&r&&(p=m+r-n)}if(0<p){m=c(f);this.setDymoFeature(m,TIME_FEATURE,n);this.setDymoFeature(m,DURATION_FEATURE,p);var u=
m,n=a.findFeatureValue(f,TIME_FEATURE),m=a.findFeatureValue(u,TIME_FEATURE);if(isNaN(n)||Array.isArray(n)||m<n)h.setDymoFeature(f,TIME_FEATURE,m),n=m;r=a.findFeatureValue(f,DURATION_FEATURE);u=a.findFeatureValue(u,DURATION_FEATURE);(isNaN(r)||Array.isArray(r)||n+r<m+u)&&h.setDymoFeature(f,DURATION_FEATURE,m+u-n)}}};this.setDymoFeature=function(b,c,d){a.setFeature(b,c,d);if(!isNaN(d))l(f(null,c),d);else if(d instanceof Array)for(b=0;b<d.length;b++)l(f(null,c),d[b])};this.resetDymo()};function DymoTemplates(){}DymoTemplates.createSingleSourceDymoFromFeatures=function(a,b,c,d,e){a.addDymo(void 0,b);DymoTemplates.loadMultipleFeatures(a,c,d,0,function(){e()})};
...
h=a.addDymo(g,"audio/Chopin_Op028-04_003_20100611-SMD/"+b[c]);a.setDymoFeature(h,d,m);a.setDymoFeature(h,e,f)}DYMO_STORE.updatePartOrder(g,d.name)})};
...
b[k]);a.setDymoFeature(m,c,f);a.setDymoFeature(m,d,l)}DYMO_STORE.updatePartOrder(e,c.name)})};
...
Number.parseInt(g[4].substring(1),10),l=Number.parseInt(g[6].substring(2),10)/1E3,f=Number.parseInt(g[7].substring(2),10)/1E3,m=Number.parseInt(g[8].substring(2),10),h=Number.parseInt(g[9].substring(2),10)/1E3,g=Number.parseInt(g[10].substring(2),10)/1E3,n=a.addDymo(c,"audio/Chopin_Op028-11_003_20100611-SMD-cut/"+b[e]);a.setDymoFeature(n,PITCH_FEATURE,k);a.setDymoFeature(n,ONSET_FEATURE,l);a.setDymoFeature(n,DURATION_FEATURE,f);a.setDymoFeature(n,"velocity",m);a.setDymoFeature(n,"onsetS",h);a.setDymoFeature(n,
...
b.length;h++){var p=b[h][0],r=b[h][1],v=Number.parseInt(b[h][3],10);if(n[v]){var t=n[v][0],u=r-n[v][2];b[t][4]=p-n[v][1];b[t][5]=u}n[v]=[h,p,r]}for(h=b.length-4;h<b.length;h++)b[h][4]=2700,b[h][5]=2700;for(h=0;h<b.length;h++)if(6==b[h].length){var n=b[h],p=Number.parseInt(n[2],10),r=Number.parseInt(n[0],10)/1E3,v=Number.parseInt(n[1],10)/1E3,t=Number.parseInt(n[4],10)/1E3,u=Number.parseInt(n[5],10)/1E3,q;1==n[3]?q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-violin.wav"):2==n[3]?q=a.addDymo(f,
"audio/achachbach10/01-AchGottundHerr-clarinet.wav"):3==n[3]?q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-saxphone.wav"):4==n[3]&&(q=a.addDymo(f,"audio/achachbach10/01-AchGottundHerr-bassoon.wav"));a.setDymoFeature(q,d,p);a.setDymoFeature(q,l,r);a.setDymoFeature(q,c,r);a.setDymoFeature(q,e,t);a.setDymoFeature(q,g,v);a.setDymoFeature(q,k,u);q.getParameter(ONSET).update(r)}DYMO_STORE.updatePartOrder(f,c.name)});$http.get("getsourcefilesindir/",{params:{directory:"audio/achachbach10/"}}).success(function(a){a=
a.filter(function(a){return 0<=a.indexOf("wav")});for(var b=0;b<a.length;b++);})};function createRandomTriangle(){return[{0:Math.random(),1:Math.random()},{0:Math.random(),1:Math.random()},{0:Math.random(),1:Math.random()}]};function FeatureLoader(){function a(a,h,e,f){v(a,function(n){b(n,function(b){c(b,function(c){0<c.length?(l(a,h,e,c),f&&f()):d(b,function(b){var c=b.name;c||(c=a.split("_"),c=c[c.length-1].split(".")[0]);e.addFeature(c,b.values);f&&f()})})})})}function b(a,b){var c=N3.Store();N3.Parser().parse(a,function(a,d,h){d?c.addTriple(d):b(c)})}function c(a,b){for(var c=[],d=a.find(null,"http://purl.org/NET/c4dm/event.owl#time",null),h=0,f=d.length;h<f;h++){var n=e(a,d[h].object,"http://purl.org/NET/c4dm/timeline.owl#at");
n||(n=e(a,d[h].object,"http://purl.org/NET/c4dm/timeline.owl#beginsAt"));var p=e(a,d[h].object,"http://purl.org/NET/c4dm/timeline.owl#duration"),n={time:k(n),label:g(e(a,d[h].subject,"http://www.w3.org/2000/01/rdf-schema#label"))};p&&(n.duration=k(p));c.push(n)}b(c)}function d(a,b){for(var c=g(e(a,null,"http://purl.org/dc/elements/1.1/title")),d=g(e(a,null,"http://purl.org/ontology/af/value")),d=d.split(" ").map(function(a){return parseFloat(a)}),h=g(e(a,null,"http://purl.org/ontology/af/dimensions")),
h=h.split(" ").map(function(a){return Number.parseInt(a,10)}),f=e(a,null,"http://purl.org/ontology/vamp/computed_by"),n=k(e(a,f,"http://purl.org/ontology/vamp/step_size")),f=k(e(a,f,"http://purl.org/ontology/vamp/sample_rate")),p=[],m=0;m<d.length;){for(var r=[],v=0;v<h[0];v++)r[v]=d[m+v];p.push({time:{value:m*n/f},value:r});m+=h[0]}b({name:c,values:p})}function e(a,b,c){a=a.find(b,c,null);if(0<a.length)return a[0].object}function g(a){if(a)return N3.Util.getLiteralValue(a)}function k(a){a=N3.Util.getLiteralValue(a);
"P"==a.charAt(0)&&(a=a.substring(2,a.length-1));return Number(a)}function l(a,b,c,d){t[a]=d.sort(function(a,b){return a.time.value-b.time.value});d=t[a];b&&t[a][0].label&&(d=d.filter(function(a){return a.label.value==b}));c.addSegmentation(d)}function f(a,b,c,d){v(a,function(a){m(JSON.parse(a),b,c,d)})}function m(a,b,c,d){"file_metadata"==Object.keys(a)[0]?h(a,b,c,d):n(a,b,c,d)}function h(a,b,c,d){a=a[Object.keys(a)[1]][0];var h=a.annotation_metadata.annotator.output_id;"beats"==h||"onsets"==h||"segmentation"==
...
Similarity.toVectors=function(a,b,c){for(var d=[],e=0,g=a.length;e<g;e++){for(var k=[],l=b.findAllFeatureValues(a[e]).filter(function(a){return"string"!=typeof a}),f=0,m=l.length;f<m;f++){var h=l[f];c&&1<h.length&&(h=Similarity.reduce(h));1<h.length?k=k.concat(h):(h=Number(h),k.push(h))}d[e]=k}for(var n=[],p=[],e=0;e<d[0].length;e++){b=[];for(f=0;f<a.length;f++)isNaN(d[f][e])||b.push(d[f][e]);n[e]=math.mean(b);p[e]=math.var(b)}d=d.map(function(a){return a.map(function(a,b){return(a-n[b])/p[b]})});
...
"http://tiny.cc/mobile-audio-ontology#isAverageOf",HAS_UUID="http://tiny.cc/mobile-audio-ontology#hasUuid",HAS_MAJOR="http://tiny.cc/mobile-audio-ontology#hasMajor",HAS_MINOR="http://tiny.cc/mobile-audio-ontology#hasMinor",HAS_DURATION="http://tiny.cc/mobile-audio-ontology#hasDuration",HAS_NAVIGATOR="http://tiny.cc/mobile-audio-ontology#hasNavigator",NAV_DYMOS="http://tiny.cc/mobile-audio-ontology#navDymos";function OntologyLoader2(a,b,c){};function DymoLoader(a){function b(b,d){c(b,"",function(b){a.loadData(b,!0,d)})}function c(a,b,h){loadFile(a,function(e){if(0>e.indexOf("Cannot GET")){var f;a:{try{JSON.parse(e)}catch(n){f=!1;break a}f=!0}f&&(b?(0<=a.indexOf(m)&&(a=a.replace(m,"")),b=b.replace('"'+a+'"',e)):b=e);(e=d(b))?(0>e.indexOf(m)&&(e=m+e),c(e,b,h)):b&&h(JSON.parse(b))}})}function d(a){var b=a.indexOf(".json");if(0<=b){if(b!=a.indexOf("context.json")+7){var c=a.substring(0,b).lastIndexOf('"');return a.substring(c+1,b+5)}return d(a.substring(b+
1))}}function e(b){for(var c=0;c<b.length;c++)for(var d=a.findAllObjects(b[c],HAS_DOMAIN_DIMENSION),e=0;e<d.length;e++){var f=a.findObject(d[e],TYPE),n=a.findObjectValue(d[e],NAME);n||(n=d[e]);if(a.isSubclassOf(f,MOBILE_CONTROL)&&!h[d[e]]){var m=d[e],g=f,f=void 0;g==ACCELEROMETER_X||g==ACCELEROMETER_Y||g==ACCELEROMETER_Z?f=new AccelerometerControl(g):g==TILT_X||g==TILT_Y?f=new TiltControl(g):g==GEOLOCATION_LATITUDE||g==GEOLOCATION_LONGITUDE?f=new GeolocationControl(g):g==GEOLOCATION_DISTANCE?f=new DistanceControl:
g==COMPASS_HEADING?f=new CompassControl:g==BEACON?(f=a.findObjectValue(m,HAS_UUID),n=a.findObjectValue(m,HAS_MAJOR),g=a.findObjectValue(m,HAS_MINOR),f=new BeaconControl(f,n,g)):g==SLIDER||g==TOGGLE||g==BUTTON||g==CUSTOM_CONTROL?(f=new Control(m,n,g),n=a.findObjectValue(m,HAS_INITIAL_VALUE),f.update(n)):g==RANDOM?f=new RandomControl(m):g==BROWNIAN?(n=a.findObjectValue(m,HAS_INITIAL_VALUE),f=new BrownianControl(m,n)):g==RAMP&&(f=Math.round(1E3*a.findObject(m,HAS_DURATION)),n=a.findObjectValue(m,HAS_INITIAL_VALUE),
f=new RampControl(m,f,n));a.findObjectValue(m,IS_SMOOTH)&&f.setSmooth&&f.setSmooth(!0);m=a.findObjectValue(m,IS_AVERAGE_OF);!isNaN(m)&&f.setAverageOf&&f.setAverageOf(m);h[d[e]]=f}}}function g(b,c){var d=a.findAllObjects(b,TO_TARGET);if(0<d.length){var h=[],e=a.findFunction(d[0]);if(e)d=a.findAllSubjects(TYPE,DYMO),Array.prototype.push.apply(h,d);else for(var f=0;f<d.length;f++)h.push(d[f]);return k(b,c,h,e)}return k(b,c,[])}function k(b,c,d,e){for(var f=a.findObject(b,IS_RELATIVE),n=[],m=a.findAllObjects(b,
HAS_DOMAIN_DIMENSION),g=0;g<m.length;g++){var k=a.findObject(m[g],TYPE);k==FEATURE_TYPE||a.isSubtypeOf(k,FEATURE_TYPE)?(k==FEATURE_TYPE&&(k=m[g]),n.push(k)):k==PARAMETER_TYPE||a.isSubclassOf(k,PARAMETER_TYPE)?(k==CUSTOM_PARAMETER&&(k=m[g]),k=c?a.setParameter(c,k):a.setParameter(null,k),n.push(k)):n.push(h[m[g]])}m=$jscomp.makeIterator(a.findArgsAndBody(a.findObject(b,HAS_FUNCTION)));c=m.next().value;m=m.next().value;b=a.findObject(b,HAS_RANGE);return new Mapping(n,f,{args:c,body:m},d,b,e)}function l(a){return a==
SIMILARITY_NAVIGATOR?new SimilarityNavigator(void 0):a==GRAPH_NAVIGATOR?new GraphNavigator(void 0):a==ONE_SHOT_NAVIGATOR?new OneShotNavigator(void 0):a==REPEATED_NAVIGATOR?new RepeatedNavigator(void 0):new SequentialNavigator(void 0)}var f=this,m="",h={},n={};this.getMappings=function(){return n};this.loadDymoFromJson=function(a,c){var d=a.lastIndexOf("/")+1;m=a.substring(0,d);b(a,function(){c(f.createDymoFromStore())})};this.parseDymoFromJson=function(b,c){a.loadData(b,!0,function(){c(f.createDymoFromStore())})};
this.loadRenderingFromJson=function(a,c){b(a,function(){c(f.createRenderingFromStore())})};this.parseDymoFromTurtle=function(b,c){a.loadData(b,!1,function(){c(f.createDymoFromStore())})};this.createDymoFromStore=function(){for(var b=a.findTopDymos(),c=0;c<b.length;c++){a.addBasePath(b[c],m);a.findAllObjectsInHierarchy(b[c]);for(var d=a.findAllSubjects(TYPE,MAPPING),h=0;h<d.length;h++){var f=d[h],k=DYMO_STORE.findSubject(HAS_MAPPING,f);e(f);n[f]=g(f,k)}}return b};this.createRenderingFromStore=function(){var b=
a.findSubject(TYPE,RENDERING),c=new Rendering(a.findObject(b,HAS_DYMO)),d=a.findAllObjects(b,HAS_MAPPING);e(d);for(var f=0;f<d.length;f++)c.addMapping(g(d[f]));b=a.findAllObjects(b,HAS_NAVIGATOR);for(f=0;f<b.length;f++)d=a.findObject(b[f],NAV_DYMOS),d=a.findFunction(d),c.addSubsetNavigator(d,l(a.findObject(b[f],TYPE)));return[c,h]}};function DymoStore(a){function b(a,c){for(var d=0;d<a.length;d++){var e=c(a[d]);if(e)return e}for(d=0;d<a.length;d++)if(e=b(k.findParents(a[d]),c))return e}function c(a,b,c){k.triplesToRdf(a,function(a){d(a,b,c)})}function d(a,b,c){a=a.split("_b").join("b");jsonld.fromRDF(a,{format:"application/nquads"},function(a,d){jsonld.frame(d,{"@id":b},function(a,b){jsonld.compact(b,DYMO_CONTEXT,function(a,b){jsonld.compact(b,DYMO_SIMPLE_CONTEXT,function(a,b){g(b);b=JSON.stringify(b,null,2);b=b.replace(/http:\/\/tiny.cc\/dymo-context\//g,
"");b=b.replace("http://tiny.cc/dymo-context-simple","http://tiny.cc/dymo-context");c(b)})})})})}function e(a,b){var d=k.findObject(a,TYPE,null),d=k.recursiveFindAllTriplesExcept(a,d);c(d,a,function(a){b(null,JSON.parse(a))})}function g(a){if(a&&a instanceof Object)for(var b in a)"@id"==b&&a[b].includes("_:b")?delete a[b]:g(a[b])}var k=this;EasyStore.call(this);var l={};(function(){k.loadFileIntoStore("../ontologies/dymo-ontology.n3",!1,function(){k.loadFileIntoStore("../ontologies/mobile-audio-ontology.n3",
...
d)};this.setParameter=function(a,b,c){this.findParameterUri(a,b)||null!=c&&!isNaN(c)||(c=this.findObjectValue(b,HAS_STANDARD_VALUE,null));return this.setObjectValue(a,HAS_PARAMETER,b,VALUE,c)};this.addControl=function(a,b){a||(a=this.createBlankNode());this.addTriple(a,TYPE,b);return a};this.addRendering=function(a,b){this.addTriple(a,TYPE,RENDERING);this.addTriple(a,HAS_DYMO,b)};this.addMapping=function(a,b,c,d,e,g){var k=this.createBlankNode();this.addTriple(a,HAS_MAPPING,k);for(a=0;a<b.length;a++){var l=
this.createBlankNode();this.addTriple(k,HAS_DOMAIN_DIMENSION,l);b[a].name&&this.addTriple(l,NAME,N3.Util.createLiteral(b[a].name));b[a].type&&this.addTriple(l,TYPE,b[a].type)}b=this.addFunction(c[0],c[1]);this.addTriple(k,HAS_FUNCTION,b);if(d)for(a=0;a<d.length;a++)this.addTriple(k,TO_TARGET,d[a]);e&&(b=this.addFunction(e[0],e[1]),this.addTriple(k,TO_TARGET,b));this.addTriple(k,HAS_RANGE,g);return k};this.addNavigator=function(a,b,c,d){var e=this.createBlankNode();this.addTriple(a,HAS_NAVIGATOR,e);
this.addTriple(e,TYPE,b);a=this.addFunction(c,d);this.addTriple(e,NAV_DYMOS,a);return e};this.addFunction=function(a,b){var c=this.createBlankNode();this.addTriple(c,HAS_ARGUMENT,N3.Util.createLiteral(a));this.addTriple(c,HAS_BODY,N3.Util.createLiteral(b));return c};this.updatePartOrder=function(a,b){var c=this.findParts(a);0<c.length&&(c.sort(function(a,c){return k.findAttributeValue(a,b)-k.findAttributeValue(c,b)}),this.setParts(a,c))};this.findTopDymos=function(){var a=this.findAllSubjects(TYPE,
...
HAS_NAVIGATOR)};this.findFunction=function(a){var b=$jscomp.makeIterator(this.findArgsAndBody(a));a=b.next().value;b=b.next().value;if(a&&b)return Function.apply(null,a.concat(b))};this.findArgsAndBody=function(a){var b=k.findAllObjectValues(a,HAS_ARGUMENT);a=k.findObjectValue(a,HAS_BODY);return[b,a]};this.findAttributeValue=function(a,b){var c=this.findParameterValue(a,b);null==c&&(c=this.findFeatureValue(a,b));return c};this.findFeatureValue=function(a,b){return b===LEVEL_FEATURE?this.findLevel(a):
...
p={},g=this.findAllSubjects(TYPE,a),l=this.find(null,b,null);async.map(g,e,function(e,u){d.nodes=u;for(var q=0;q<g.length;q++)p[g[q]]=d.nodes[q];d.edges=[];for(q=0;q<l.length;q++)if(0==k.find(l[q].object,TYPE,a).length){if(0<k.find(l[q].object,FIRST).length){var y=k.findObjectOrList(l[q].subject,b),y=y.map(function(a){return{source:p[l[q].subject],target:p[a],value:1}});d.edges=d.edges.concat(y)}}else d.edges.push({source:p[l[q].subject],target:p[l[q].object],value:1});c(d)})}}
inheritPrototype(DymoStore,EasyStore);function DymoWriter(a,b){function c(a,b){var c=new XMLHttpRequest;c.send(b);c.addEventListener("save",function(){console.log("saved "+a)});c.addEventListener("error",function(){console.log("saving "+a+" failed")});c.open("POST",a);c.send()}this.writeToJsonld=function(b,e){e||(e="dymo.json");a.toJsonld(function(a){c(b+e,a)})}};function EasyReasoner(){function a(a,c,d){var e=new XMLHttpRequest;e.open("POST",a);e.setRequestHeader("Content-Type","application/x-www-form-urlencoded");e.onload=function(){d(e.responseText)};e.error=function(a){console.log(a)};e.send(c)}this.queryAll=function(b,c){for(var d="",e=0;e<b.length;e++)d+=encodeURIComponent("data="+b[e])+"&";d+=encodeURIComponent("query={ ?a ?b ?c. } => { ?a ?b ?c. }.");a("http://eye.restdesc.org/",d,function(a){c(a)})}};function EasyStore(){function a(a,b,c){var d=[];f[a]&&f[a][b]&&(d=d.concat(f[a][b]));b=k.findObject(a,TYPE);m[b]&&(d=d.concat(m[b]));for(var e=0;e<d.length;e++)d[e].observedValueChanged&&d[e].observedValueChanged(a,b,c)}function b(a,b){var c=l.find(null,a,b);if(0<c.length)return c[0].subject}function c(a){if(a){var b=k.findObject(a,FIRST);if(b){var c=[];c.push(b);for(a=k.findObject(a,REST);a!=NIL;)c.push(k.findObject(a,FIRST)),a=k.findObject(a,REST);return c}return a}}function d(a,b){if(a){for(var c=
0,d=a,e=k.findObject(d,REST);e!=NIL&&(null==b||c<b);)d=e,e=k.findObject(d,REST),c++;if(null==b||c==b)return d}}function e(a){var b=N3.Util.getLiteralValue(a);"http://www.w3.org/2001/XMLSchema#string"!=N3.Util.getLiteralType(a)&&(b=Number(b));return b}function g(a,b,c){b?jsonld.toRDF(a,{format:"application/nquads"},function(a,b){c(b)}):c(a)}var k=this,l=N3.Store(),f={},m={};this.addValueObserver=function(a,b,c){f[a]||(f[a]={});f[a][b]||(f[a][b]=[]);f[a][b].push(c)};this.addTypeObserver=function(a,
b,c){m[a]||(m[a]=[]);m[a].push(c)};this.removeValueObserver=function(a,b,c){f[a]&&f[a][b]&&(c=f[a][b].indexOf(c),-1<c&&(f[a][b].splice(c,1),f[a]&&0==f[a][b].length&&(delete f[a][b],0===Object.keys(f[a]).length&&delete f[a])))};this.removeTypeObserver=function(a,b,c){m[a]&&(b=m[a].indexOf(c),-1<b&&(m[a].splice(b,1),0==m[a].length&&delete m[a]))};this.getValueObservers=function(a,b){return f[a]?f[a][b]?f[a][b]:flattenArray(Object.values(f[a])):null==a&&null==b?flattenArray(Object.keys(f).map(function(a){return flattenArray(Object.values(f[a]))})):
...
b)return!0;c=this.findObject(c,RDFS_URI+"subClassOf")}return!1};this.isSubtypeOf=function(a,b){for(var c=this.findObject(a,TYPE);c;){if(c==b)return!0;c=this.findObject(c,TYPE)}return!1};this.loadData=function(a,b,c){g(a,b,function(a){N3.Parser().parse(a,function(a,b,d){b?l.addTriple(b):c&&c()})})};this.loadFileIntoStore=function(a,b,c){loadFile(a,function(a){k.loadData(a,b,c)})};this.toRdf=function(a){var b=l.find(null,null,null);this.triplesToRdf(b,a)};this.triplesToRdf=function(a,b){for(var c=N3.Writer({format:"application/nquads"}),
...
e,g,k,l,f,m,h;l=1/this.overlapLength;for(e=0;e<this.overlapLength;e++)k=(this.overlapLength-e)*l,f=e*l,g=2*e,m=g+a,h=g+d,c[h+0]=b[m+0]*f+this.pMidBuffer[g+0]*k,c[h+1]=b[m+1]*f+this.pMidBuffer[g+1]*k},process:function(){var a,b;if(null==this.pMidBuffer){if(this._inputBuffer.frameCount<this.overlapLength)return;this.pMidBuffer=new Float32Array(2*this.overlapLength);this._inputBuffer.receiveSamples(this.pMidBuffer,this.overlapLength)}for(;this._inputBuffer.frameCount>=this.sampleReq;)a=this.seekBestOverlapPosition(),
