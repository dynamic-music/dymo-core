...
describe("a control", function() {
	...
	beforeAll(function(done) {
		...
		//GlobalVars.DYMO_STORE.loadOntologies().then(() => done());
	});
	...
	it("can be a brownian control", function(done) {
		var brownianControl = new BrownianControl("brownian");
		GlobalVars.DYMO_STORE.setParameter("brownian", AUTO_CONTROL_FREQUENCY, 50);
	...
	it("can be a ramp control", function(done) {
		var rampControl = new RampControl("ramp", 200);
		GlobalVars.DYMO_STORE.setParameter("ramp", AUTO_CONTROL_FREQUENCY, 20);
		expect(rampControl.getValue()).toBe(0);
		//turn on
		GlobalVars.DYMO_STORE.setParameter("ramp", AUTO_CONTROL_TRIGGER, 1);
		setTimeout(function() {
			...
			setTimeout(function() {
				...
				GlobalVars.DYMO_STORE.setParameter("ramp", AUTO_CONTROL_TRIGGER, 0);
				setTimeout(function() {
